...
public class Pattern {
    ...
    private static final Map<String, Pattern> allPatterns;
    private static final Map<String, Pattern> defaultPatterns;
    ...
    private final Map<String, String> SVGStringMap;
    ...
    public Pattern(String id, String name, Path xmlFile) {
        ...
        if (!name.equals("none")) {
            try {
                String fileName = xmlFile.getParent().toString() + xmlFile.getFileSystem().getSeparator() + name + ".svg";
                Path patternFile = FS.newPath(fileName);
        ...
        for (Entry<String, Map<String, Path>> entry : moddedFilesMap.entrySet()) {
            for (Entry<String, Path> innerEntry : entry.getValue().entrySet()) {
                try {
                    ...
                    System.getLogger("").log(System.Logger.Level.ERROR, "Loading modded pattern failed at 'Pattern'. File path: " + innerEntry.getValue());
...
//					System.out.println("P: "+innerEntry.getKey());
                    ...
                    System.getLogger("").log(System.Logger.Level.ERROR, "Loading pattern failed at 'Pattern'. File path: " + innerEntry.getValue());
                    System.getLogger("").log(System.Logger.Level.ERROR, "Actual exception: ");
                    ex.printStackTrace(System.err);
                }
            }
