...
//		weaponAttributeList.add(TFModifier.DAMAGE_WEAPON);
    ...
    public String getSVGString() {
        if (SVGString == null) {
            ...
            try {
                InputStream is = FS.newResourceAsStream("/com/lilithslegacy/res/crafting/" + path + ".svg");
                if (is == null) {
                    System.getLogger("").log(System.Logger.Level.ERROR, "Error! TFModifier icon file does not exist (Trying to read from '" + path + "')! (Code 2)");
                }
                ...
                if (path.contains("flavour")) {
                    ...
                    is = FS.newResourceAsStream("/com/lilithslegacy/res/crafting/flavours/background.svg");
                    if (is == null) {
                        System.getLogger("").log(System.Logger.Level.ERROR, "Error! Subspecies background icon file does not exist (Trying to read from 'flavours/background')!");
                    }
