...
import com.lilithslegacy.game.inventory.clothing.ClothingAccess;
...
import com.lilithslegacy.utils.comparators.ClothingRarityComparator;
import com.lilithslegacy.utils.comparators.ClothingZLayerComparator;
import com.lilithslegacy.utils.comparators.InventoryClothingComparator;
import com.lilithslegacy.utils.comparators.InventoryItemComparator;
import com.lilithslegacy.utils.comparators.InventoryWeaponComparator;
import com.lilithslegacy.utils.comparators.ReverseClothingZLayerComparator;
...
public class CharacterInventory implements XMLSaving {
    ...
    public CharacterInventory(CharacterInventory inventoryToCopy) {
        ...
        mainWeapon = new AbstractWeapon[Arm.MAXIMUM_ROWS];
        System.arraycopy(inventoryToCopy.mainWeapon, 0, mainWeapon, 0, mainWeapon.length);
        offhandWeapon = new AbstractWeapon[Arm.MAXIMUM_ROWS];
        System.arraycopy(inventoryToCopy.offhandWeapon, 0, offhandWeapon, 0, offhandWeapon.length);
...
//								&& bp.displacementType!=DisplacementType.REMOVE_OR_EQUIP
    ...
    private void transformationIncompatible(GameCharacter character, AbstractClothing c, List<AbstractClothing> clothingRemovalList, String description) {
        ...
        tempSB.append("<br/><span style='color:").append(PresetColour.GENERIC_BAD.toWebHexString()).append(";'>").append(UtilText.parse(character, description)).append("</span>");
    ...
    private void transformationIncompatibleWeapon(GameCharacter character, AbstractWeapon w, String description) {
        ...
        tempSB.append("<br/><span style='color:").append(PresetColour.GENERIC_BAD.toWebHexString()).append(";'>").append(UtilText.parse(character, "Due to the loss of [npc.her] extra pair of arms, [npc.name] can no longer hold the " + w.getName() + "!")).append("</span>");

        tempSB.append("<br/>").append(description);
    }
    ...
    public boolean isAbleToEquip(AbstractClothing newClothing, InventorySlot slotToEquipInto, boolean equipIfAble, boolean automaticClothingManagement, GameCharacter characterClothingOwner, GameCharacter characterClothingEquipper) {
        ...
        if (block != null && Collections.disjoint(block.getRequiredTags(), newClothing.getItemTags())) {
            equipTextSB.append("[style.colourBad(").append(UtilText.parse(characterClothingOwner, block.getDescription())).append(")]");
            return false;
        }

        if (!newClothing.isAbleToBeEquipped(characterClothingOwner, slotToEquipInto).getKey()) {
            equipTextSB.append("[style.colourBad(").append(newClothing.isAbleToBeEquipped(characterClothingOwner, slotToEquipInto).getValue()).append(")]");
            return false;
        }
        ...
        if (getClothingInSlot(slotToEquipInto) != null) {
            if (getClothingInSlot(slotToEquipInto).isSealed()) {
                if (characterClothingOwner.isPlayer()) {
                    equipTextSB.append("You can't equip the ").append(newClothing.getName()).append(", as your <b style='color:").append(PresetColour.SEALED.toWebHexString()).append(";'>sealed</b> ").append(getClothingInSlot(slotToEquipInto).getName()).append(" can't be removed!");
                } else {
...
//					tempClothingList.add(getClothingInSlot(newClothing.getClothingType().getSlot()));
                ...
                for (AbstractClothing c : tempClothingList) {
                    ...
                    equipTextSB.append(equipTextSB.isEmpty() ? "" : "<br/>").append(!clothingToRemove.containsKey(c) || clothingToRemove.get(c) == DisplacementType.REMOVE_OR_EQUIP
                ...
                for (AbstractClothing c : incompatibleRemovableClothing) {
                    if (!characterClothingOwner.isInventoryFull() || characterClothingOwner.hasClothing(c)) {
                        equipTextSB.append("<br/>").append(characterClothingOwner.addedItemToInventoryText(c));
                    } else {
                        equipTextSB.append("<br/>").append(characterClothingOwner.droppedItemText(c));
                    }
                ...
                if (getClothingInSlot(slotToEquipInto) != null) {
                    if (getClothingInSlot(slotToEquipInto).isDiscardedOnUnequip(slotToEquipInto)) {
                        ...
                        if ((!characterClothingOwner.isInventoryFull() || characterClothingOwner.hasClothing(getClothingInSlot(slotToEquipInto))) && Main.game.isInNewWorld()) {
                            equipTextSB.append("<br/>").append(characterClothingOwner.addedItemToInventoryText(getClothingInSlot(slotToEquipInto)));
                        } else {
                            equipTextSB.append("<br/>").append(characterClothingOwner.droppedItemText(getClothingInSlot(slotToEquipInto)));
                        }
                        String oldEquipText = equipTextSB.toString();// this is a hack to fix the string builder being overwritten
                        if (Main.game.isInNewWorld()) {
                ...
                equipTextSB.append(equipTextSB.isEmpty() ? "" : "<br/>").append(newClothing.onEquipApplyEffects(characterClothingOwner, characterClothingEquipper, (Main.game.isInSex() && Main.sex.getSexPace(characterClothingEquipper) == SexPace.DOM_ROUGH)));
            ...
            if (clothingToRemove.isEmpty()) {
                equipTextSB.append(Util.capitaliseSentence(newClothing.getName(true))).append(" ").append(newClothing.getClothingType().isPlural() ? "are" : "is").append(" able to be equipped.");
            } else {
                equipTextSB.append(Util.capitaliseSentence("Before" + newClothing.getClothingType().getDeterminer())).append(" ").append(newClothing.getName()).append(" is able to be equipped, ").append(Util.clothesToStringList(clothingToRemove.keySet(), false)).append(" need").append(clothingToRemove.size() > 1 ? "" : "s").append(" to be removed.");
            }
        }
...
//			Set<AbstractClothing> blockingClothingSet = clothingToRemove.keySet().stream().filter(c -> c != clothing).collect(Collectors.toSet());
        ...
        if (unequipIfAble) {
            ...
            List<AbstractClothing> tempClothingList = new ArrayList<>(clothingToRemove.keySet());
            tempClothingList.sort(new ClothingZLayerComparator());
            ...
            List<AbstractClothing> clothingToBeReplaced = new ArrayList<>(clothingToRemove.keySet());
...
//			System.getLogger("").log(System.Logger.Level.ERROR,"Error: "+clothing.getName()+" and "+(displacementClothingCheck!=null?displacementClothingCheck.getName():"(unknown clothing)")+" are blocking one another's displacement!!!");
//			return true;
//		}
        ...
        if (!displacementTypeFound) {
            IllegalArgumentException e = new IllegalArgumentException("The supplied clothing (" + clothing.getClothingType().getId() + ") cannot be displaced in this manner (" + dt.getDescription() + ")!");
        ...
        if (displaceIfAble) {
            ...
            // Description of each clothing item that is removed/displaced:
            for (AbstractClothing c : tempClothingList) {
                unableToDisplaceText.append(unableToDisplaceText.isEmpty()
                        ? ""
                        : "<br/>").append(clothingToRemove.get(c) == DisplacementType.REMOVE_OR_EQUIP
                        ? c.onUnequipText(characterClothingOwner, characterRemovingClothing, (Main.game.isInSex() && Main.sex.getSexPace(characterRemovingClothing) == SexPace.DOM_ROUGH))
            ...
            unableToDisplaceText.append(unableToDisplaceText.isEmpty()
                    ? ""
                    : "<br/><span style='color:" + PresetColour.GENERIC_ARCANE.toWebHexString() + ";'>").append(clothing.getClothingType().displaceText(characterClothingOwner, characterRemovingClothing, clothing.getSlotEquippedTo(), dt, (Main.game.isInSex() && Main.sex.getSexPace(characterRemovingClothing) == SexPace.DOM_ROUGH))).append("</span>");

            List<AbstractClothing> replaceClothingList = new ArrayList<>(clothingToRemove.keySet());
            ...
            if (!replaceClothingList.isEmpty()) {
                unableToDisplaceText.append(UtilText.parse(characterClothingOwner, "<br/>You replace [npc.namePos] ")).append(Util.clothesToStringList(replaceClothingList, false)).append(".");
            }
    ...
    public boolean isAbleToBeReplaced(AbstractClothing clothing, DisplacementType dt, boolean replaceIfAble, boolean automaticClothingManagement, GameCharacter characterClothingOwner, GameCharacter characterRemovingClothing) {
        ...
        if (replaceIfAble) {
            ...
            // Description of each clothing item that is removed/displaced:
            for (AbstractClothing c : tempClothingList)
                unableToReplaceText.append(unableToReplaceText.isEmpty() ? "" : "<br/>").append(clothingToRemove.get(c) == DisplacementType.REMOVE_OR_EQUIP
                        ? (c == clothing
            ...
            unableToReplaceText.append(unableToReplaceText.isEmpty()
                    ? ""
                    : "<br/><span style='color:" + PresetColour.GENERIC_GOOD.toWebHexString() + ";'>").append(clothing.getClothingType().replaceText(characterClothingOwner, characterRemovingClothing, clothing.getSlotEquippedTo(), dt, (Main.game.isInSex() && Main.sex.getSexPace(characterRemovingClothing) == SexPace.DOM_ROUGH))).append("</span>");

            List<AbstractClothing> replaceClothingList = new ArrayList<>(clothingToRemove.keySet());
            replaceClothingList.sort(new ReverseClothingZLayerComparator());
            if (!replaceClothingList.isEmpty()) {
                unableToReplaceText.append(UtilText.parse(characterClothingOwner, "<br/>You replace [npc.namePos] ")).append(Util.clothesToStringList(replaceClothingList, false)).append(".");
            }
    ...
    public List<AbstractClothing> getBlockingCoverableAreaClothingList(GameCharacter character, CoverableArea area, boolean byRemovingClothing) {
        ...
        for (AbstractClothing clothing : clothingCurrentlyEquipped) {
            ...
            if (blockedPartsMap == null) {
                System.getLogger("").log(System.Logger.Level.ERROR, "Clothing error in getBlockingCoverableAreaClothingList(): blockedPartsMap is returning null!");
                continue;
            }
    ...
    public SimpleEntry<AbstractClothing, DisplacementType> getNextClothingToRemoveForCoverableAreaAccess(GameCharacter character, CoverableArea coverableArea) {
        ...
        //System.getLogger("").log(System.Logger.Level.ERROR,"There is no clothing covering this part!");
        return null;
    }
...
//									System.out.println(":3 "+clothing.getName()+" | "+clothingToRemove.getName()+" | "+ca);
                                    if (previousDisplacements.containsKey(clothing) && previousDisplacements.get(clothing).equals(bpIterated.displacementType)) {
                                        System.getLogger("").log(System.Logger.Level.ERROR, "findNextClothingDisplacement() error: " + clothing.getName() + " is interfering with " + clothingToRemove.getName());
