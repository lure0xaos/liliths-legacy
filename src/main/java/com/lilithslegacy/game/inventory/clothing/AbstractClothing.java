...
public abstract class AbstractClothing extends AbstractCoreItem implements XMLSaving {
    ...
    protected final List<ItemEffect> effects;
    ...
    public static AbstractClothing loadFromXML(Element parentElement, Document doc) {
        ...
        try {
            ...
            System.getLogger("").log(System.Logger.Level.ERROR, "Warning: An instance of AbstractClothing was unable to be imported. (" + parentElement.getAttribute("id") + ")");
            return null;
        }
            ...
            if (displacementElement != null) {
                ...
                for (int i = 0; i < displacementTypeElements.getLength(); i++) {
                    ...
                    if (displacementTypeFound) {
                        ...
                        System.getLogger("").log(System.Logger.Level.ERROR, "Warning: Invalid displacement");
                    }
                }
            }
        } catch (Exception ex) {
    ...
    public String getDescription() {
        ...
        descriptionSB.append("<p>").append(getTypeDescription()).append("<br/>").append(enchantmentKnown
                ? (getClothingType().isPlural() ? "They have" : "It has") + " a value of: " + UtilText.formatAsMoney(getValue())
                : (getClothingType().isPlural() ? "They have" : "It has") + " an <b>unknown value</b>!").append("</p>");

        // Physical resistance
        if (getClothingType().getPhysicalResistance() > 0) {
            descriptionSB.append("<p>").append(getClothingType().isPlural()
                    ? "They are armoured, and provide "
                    : "It is armoured, and provides ").append(" <b>").append(getClothingType().getPhysicalResistance()).append("</b> [style.colourResPhysical(").append(Attribute.RESISTANCE_PHYSICAL.getName()).append(")].").append("</p>");
        }
        if (enchantmentKnown) {
            if (!this.getEffects().isEmpty()) {
        ...
        if (getClothingType().getClothingSet() != null) {
            descriptionSB.append("<p>").append(getClothingType().isPlural() ? "They are" : "It is").append(" part of the <b style='color:").append(PresetColour.RARITY_EPIC.toWebHexString()).append(";'>").append(getClothingType().getClothingSet().getName()).append("</b> set.").append("</p>");
        }
    ...
    public String getColourName() {
        try {
            ...
            System.getLogger("").log(System.Logger.Level.ERROR, "Warning: AbstractClothing.getColourName() returning null!");
            return "";
        }
    }

    /**
     * @param withDeterminer True if you want the determiner to prefix the name
...
//			System.out.println(this.name+ " | "+this.getClothingType().getName());
    ...
    public String onEquipApplyEffects(GameCharacter clothingOwner, GameCharacter clothingEquipper, boolean rough) {
        ...
        if (!enchantmentKnown) {
            ...
            if (this.isBadEnchantment()) {
                sb.append("<p style='text-align:center;'>" + "<b style='color:").append(PresetColour.GENERIC_BAD.toWebHexString()).append(";'>Negative Enchantment Revealed:</b><br/>").append(getDisplayName(true));

            } else {
                sb.append("<p style='text-align:center;'>" + "<b style='color:").append(PresetColour.GENERIC_GOOD.toWebHexString()).append(";'>Enchantment Revealed:</b><br/>").append(getDisplayName(true));
            }

            for (Entry<AbstractAttribute, Integer> att : getAttributeModifiers().entrySet()) {
                sb.append("<br/>").append(att.getKey().getFormattedValue(att.getValue()));
            }
    ...
    public String getDisplacementBlockingDescriptions(GameCharacter equippedToCharacter) {
        ...
        for (BlockedParts bp : getBlockedPartsMap(equippedToCharacter, this.getSlotEquippedTo())) {
            descriptionSB.append("<br/><b>").append(Util.capitaliseSentence(bp.displacementType.getDescription())).append(":</b> ");
            if (bp.displacementType == DisplacementType.REMOVE_OR_EQUIP) {
                if (equippedToCharacter.isAbleToUnequip(this, false, equippedToCharacter)) {
                    descriptionSB.append("<b style='color:").append(PresetColour.GENERIC_GOOD.toWebHexString()).append(";'>Available</b>");
                } else {
                    descriptionSB.append("<b style='color:").append(PresetColour.GENERIC_BAD.toWebHexString()).append(";'>Blocked</b> by ").append(equippedToCharacter.getBlockingClothing().getName());
                }
            } else {
    ...
    public boolean isSealed() {
        ...
        for (ItemEffect effect : this.getEffects()) {
            if (effect != null && effect.getSecondaryModifier() == TFModifier.CLOTHING_SEALING) {
                ...
                System.getLogger("").log(System.Logger.Level.ERROR, "AbstractClothing.isSealed() for " + this.getName() + " is encountering a null ItemEffect!");
            }
        }
        return false;
    }
    ...
    public TFPotency getVibratorIntensity() {
        for (ItemEffect effect : this.getEffects()) {
            if (effect != null && effect.getSecondaryModifier() == TFModifier.CLOTHING_VIBRATION) {
                ...
                System.getLogger("").log(System.Logger.Level.ERROR, "AbstractClothing.getVibratorIntensity() for " + this.getName() + " is encountering a null ItemEffect!");
            }
        }
        return null;
    }
...
//		}
    ...
    public String setEnchantmentKnown(GameCharacter owner, boolean enchantmentKnown) {
        ...
        if (enchantmentKnown && !getAttributeModifiers().isEmpty()) {
            if (isBadEnchantment()) {
                sb.append("<p style='text-align:center;'>" + "<b style='color:").append(PresetColour.GENERIC_BAD.toWebHexString()).append(";'>Negative Enchantment Revealed:</b><br/>").append("<b>").append(Util.capitaliseSentence(getDisplayName(true))).append("</b>");

            } else {
                sb.append("<p style='text-align:center;'>" + "<b style='color:").append(PresetColour.GENERIC_GOOD.toWebHexString()).append(";'>Enchantment Revealed:</b><br/>").append("<b>").append(Util.capitaliseSentence(getDisplayName(true))).append("</b>");
            }
