...
public abstract class AbstractClothingType extends AbstractCoreType {
    ...
    static final Map<AbstractSetBonus, List<AbstractClothingType>> clothingSetMap = new HashMap<>();
    /**
    ...
    public AbstractClothingType(Path clothingXMLFile, String author) throws XMLLoadException { // Be sure to catch this exception correctly - if it's thrown mod is invalid and should not be continued to load
        ...
        try {
            ...
            try {
                ...
                for (Element blockedPartsElement : coreAttributes.getAllOf("blockedPartsList")) {
                    ...
                    List<BlockedParts> blockedPartsList = blockedPartsElement
                            ...
                            .map(e -> BlockedParts.loadFromXML(e.getInnerElement(), e.getDocument(), clothingXMLFile.toString()))
                            .collect(Collectors.toList());
            ...
            Path path2 = clothingXMLFile.getParent();
            this.pathNamePrefix = path2 + "/";
...
//						return ColourListPresets.getColourListFromId("ALL_WITH_METALS");
                    ...
                    System.getLogger("").log(System.Logger.Level.ERROR, "AAAAAAAAAAAAA");
            ...
            if (coreAttributes.getOptionalFirstOf("customColours").isPresent()) {
                ...
                for (Element e : customColoursElement.getAllOf("customColour")) {
                    try {
                        ...
                        System.getLogger("").log(System.Logger.Level.ERROR, "Error in loading customColours from clothing: " + this.getName());
                    }
                }
            }

            if (this.colourReplacements.isEmpty()) {
                throw new Exception("AbstractClothingType (" + clothingXMLFile.getFileName().toString() + "): colourReplacements is empty!");
            }
            ...
            if (coreAttributes.getOptionalFirstOf("defaultPatterns").isPresent()) {
                ...
                patternChance = !patternChanceStr.isEmpty() ? Float.valueOf(patternChanceStr) : 0;
                ...
                for (Element categoryElement : stickersElement.getAllOf("category")) {
                    ...
                    if (!categoryId.isEmpty()) { // Need an id or else saving/loading won't work
                        ...
                        for (Element stickerElement : categoryElement.getAllOf("sticker")) {
                            ...
                            Map<InventorySlot, Map<Integer, Path>> stickerSvgPaths = new HashMap<>();
                            ...
                            for (Element svgPathElement : stickerElement.getAllOf("imageName")) {
                                ...
                                if (!svgPathElement.getTextContent().isEmpty()) {
                                    Path path1 = clothingXMLFile.getParent();
                                    path = path1.resolve(svgPathElement.getTextContent().trim());
                                    svgImageFound = true;
                                }
                                InventorySlot slot = null;
...
//									System.out.println(zLayer);
        ...
        if (exMessage.startsWith("No ColourListPreset constant")) {
            for (Entry<Class, Set<String>> possibleMatch : possibleEnumValues.entrySet()) {
                if (exMessage.contains(possibleMatch.getKey().getCanonicalName())) {
                    ...
                    System.getLogger("").log(System.Logger.Level.ERROR, "Possible values for " + possibleMatch.getKey().getSimpleName() + " are " + valueLister);
                }
            }
        }
    }
    ...
    private void populateEmptyPatternColours() {
        if (patternColourReplacements.isEmpty()) {
...
//			}
        ...
        if (displacementDescriptions != null && displacementDescriptions.get(slotToEquipInto) != null && !displacementDescriptions.get(slotToEquipInto).isEmpty()) {
            ...
            try {
                ...
                System.getLogger("").log(System.Logger.Level.ERROR, this.getName() + " failed to generate equip text.");
...
//	}
    ...
    private String getSVGImage(GameCharacter character,
        ...
        if (this.equals(ClothingType.HIPS_CONDOMS)) {
            if (!colours.isEmpty()) {
                try {
                    InputStream is = FS.newResourceAsStream("/com/lilithslegacy/res/clothing/belt_used_condoms_base_back.svg");
                    ...
                    if (!equippedVariant) {
                        is = FS.newResourceAsStream("/com/lilithslegacy/res/clothing/belt_used_condoms_base_front.svg");
                            ...
                            if (item.getItemType().equals(ItemType.CONDOM_USED)) {
                                for (int count = 0; count < character.getItemCount(item); count++) {
                                    if (condomColours.size() < 8) {
                                        ...
                                        is = FS.newResourceAsStream("/com/lilithslegacy/res/clothing/belt_used_condoms_" + condomColours.size() + "_back.svg");
                                        s += "<div style='width:100%;height:100%;position:absolute;left:0;bottom:0;padding:0;margin:0'>" + Util.inputStreamToString(is) + "</div>";
                                        s = getSVGWithHandledPattern(s, pattern, patternColours);
                        ...
                        for (Colour c : condomColours) {
                            is = FS.newResourceAsStream("/com/lilithslegacy/res/clothing/belt_used_condoms_" + i + "_front.svg");
                            s += "<div style='width:100%;height:100%;position:absolute;left:0;bottom:0;padding:0;margin:0'>" + Util.inputStreamToString(is) + "</div>";
                            s = getSVGWithHandledPattern(s, pattern, patternColours);
                            s = SvgUtil.colourReplacement(this.getId(),
                ...
                if (stringFromMap != null && !this.equals(ClothingType.WRIST_WOMENS_WATCH) && !this.equals(ClothingType.WRIST_MENS_WATCH)) {
                    ...
                    if (!colours.isEmpty()) {
                        try {
                            ...
                            String s;
                            if (isMod) {
                                List<String> lines = Files.readAllLines(FS.newPath(getPathNameEquipped(character, slotEquippedTo)));
                                ...
                                is = FS.newResourceAsStream("/com/lilithslegacy/res/" + getPathNameEquipped(character, slotEquippedTo) + ".svg");
                    ...
                    if (!colours.isEmpty()) {
                        try {
                            ...
                            if (isMod) {
                                ...
                                is = FS.newResourceAsStream("/com/lilithslegacy/res/" + pathName + ".svg");
                                s = Util.inputStreamToString(is);
                                is.close();
                            }
...
//							s = SvgUtil.colourReplacement(this.getId(), colours, this.getColourReplacements(), s);
    ...
    private String getSVGWithHandledStickers(InventorySlot slotEquippedTo, String s, Map<String, String> handledStickers) {
        List<Value<Integer, Path>> svgRendering = new ArrayList<>();
            ...
            if (handledStickers.containsKey(typeStickers.getKey().getId())) {
                ...
                for (Sticker stickerFromType : typeStickers.getValue()) {
                    if (stickerFromType.getId().equals(stickerId)) {
                        for (Entry<Integer, Path> stickerSvgEntry : stickerFromType.getSvgPaths().get(slotEquippedTo).entrySet()) {
        ...
        for (Value<Integer, Path> entry : svgRendering) {
//			System.out.println(entry.getKey());
            if (entry.getKey() >= 0) {
                break;
            }
            if (entry.getValue().getNameCount()!=0) {
                try {
                    List<String> lines = Files.readAllLines(entry.getValue());
                    StringBuilder sb = new StringBuilder();
                    for (String line : lines) {
                        sb.append(line);
                    }
                    finalSvg.append("<div style='width:100%;height:100%;position:absolute;left:0;top:0;'>").append(sb).append("</div>");
                } catch (IOException ex) {
                    System.getLogger("").log(System.Logger.Level.ERROR, "Error: getSVGWithHandledStickers() Code 2");
                    System.getLogger("").log(System.Logger.Level.ERROR, ex.getMessage(), ex);
                }
            }
        }
        ...
        finalSvg = new StringBuilder();
        for (Value<Integer, Path> entry : svgRendering) {
//			System.out.println(entry.getKey());
            if (entry.getKey() > 0 && entry.getValue().getNameCount()!=0) {
                try {
                    List<String> lines = Files.readAllLines(entry.getValue());
                    StringBuilder sb = new StringBuilder();
                    for (String line : lines) {
