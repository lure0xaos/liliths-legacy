...
public abstract class AbstractItemType extends AbstractCoreType {
    ...
    public AbstractItemType(Path itemXMLFile, String author, boolean mod) throws XMLLoadException { // Be sure to catch this exception correctly - if it's thrown mod is invalid and should not be continued to load
        boolean debug = false;
        ...
        try {
            ...
            for (Element imagePathElement : coreAttributes.getAllOf("imageName")) {
                ...
                if (rawPath.contains("/res/")) {
                    ...
                    Path path = itemXMLFile.getParent();
                    pathName = path.resolve(imagePathElement.getTextContent()).toString();
                }
                int zLayer = 1;
                int imageSize = 100;
                int imageRotation = 0;
...
//							lines = Files.readAllLines(FS.newPath(backgroundPathName));
...
//						SVGString = SvgUtil.colourReplacement(this.getId(), colourShades, null, SVGString);
                        ...
                        InputStream is = FS.newResourceAsStream("/com/lilithslegacy/res/items/" + svgPathInformation.get(0).getPathName() + ".svg");
                        if (is == null) {
                            System.getLogger("").log(System.Logger.Level.ERROR, "Error! AbstractItemType icon file does not exist (Trying to read from '" + svgPathInformation.get(0).getPathName() + "')!");
                        }
