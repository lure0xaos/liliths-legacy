            ...
            if (index == 1) {
                return new Response("Knocking", "Rose said she'd be back in about half an hour, so that must be her knocking at your door.", INTRO_NEW_WORLD_9) {
                    ...
                    public void effects() {
                        ...
                        Main.game.getTextEndStringBuilder().append("<p style='text-align:center;'>[style.boldExcellent(").append(spellBook.getName()).append(")] added to your room's storage!</p>");
        ...
        public Response getResponse(int responseTab, int index) {
            if (index == 1) {
                return new Response("Blinded", "The pink flash was so bright that you're left temporarily blinded!", INTRO_NEW_WORLD_5) {
                    ...
                    public void effects() {
                        // Remove clothing:
                        List<AbstractClothing> tempList = new ArrayList<>(Main.game.getPlayer().getClothingCurrentlyEquipped());
