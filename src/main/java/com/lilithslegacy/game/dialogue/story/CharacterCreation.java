package com.lilithslegacy.game.dialogue.story;
...
//			}
            ...
            if (index == 1) {
                return new ResponseEffectsOnly("Continue", "Use this name and continue to the next stage of the character creation screen.") {
                    ...
                    public void effects() {
                        ...
                        if (Main.mainController.getWebEngine().getDocument() != null) {
                            unsuitableSurname = !Main.mainController.getWebEngine().getDocument().getElementById("hiddenFieldSurname").getTextContent().isEmpty()
    ...
    public static String getCheckingClothingDescription() {
        ...
        sb.append("<div class='container-full-width' style='background:transparent;'>" + "<p>" + "There doesn't seem to be any sign of activity on the main stage, so, afforded a few more minutes, you decide to smarten up your clothes a little." + " After all, this is a big evening for Lily, and you want her to see that you've put some effort into your appearance." + "</p>" + "<p>" + "Turning this way and that to get a better look at yourself in the mirror, you begin to notice just how ").append(Main.game.getPlayer().isFeminine() ? "hot" : "handsome").append(" you're looking tonight...").append("</p>").append("<p>").append("[pc.thought(Why am I feeling so horny all of a sudden?)]").append("</p>").append("<div class='container-full-width' style='text-align:center;'>").append("<i>Choose what you decided to wear to the museum.</i><br/>").append("<i>You'll need to be wearing some kind of footwear, as well as clothing that conceals your genitals and chest, before being able to proceed.</i>").append("</div>").append("</div>");
    ...
    public static final DialogueNode CHOOSE_ADVANCED_APPEARANCE = new DialogueNode("In the Museum", "", true) {
        ...
        public String getHeaderContent() {
            return "<p>"
                    + "[pc.speech(It's " + (!Main.game.getPlayer().getSurname().isEmpty() ? "[pc.surname], [pc.name] [pc.surname]" : "[pc.name]") + ",)]"
                    ...
                    + " [npcMale.speech(Have a good evening, " + (!Main.game.getPlayer().getSurname().isEmpty()
    ...
    public static final DialogueNode CHOOSE_BACKGROUND = new DialogueNode("In the Museum", "-", true) {
        ...
        public String getContent() {
            ...
            UtilText.nodeContentSB.append("<p>" + "Satisfied with your appearance, you turn away from the mirror and begin to walk towards the main stage." + " With each step you take, you inexplicably find yourself getting more and more turned on, and by the time you've barely covered half the distance to the bustling crowd of visitors,").append(Main.game.getPlayer().hasPenis()
                    ? " you're struggling to keep yourself from getting an erection."
                    : " you can feel your pussy getting wet from arousal.").append("</p>").append("<p>").append("Ducking behind a nearby pillar, you shake your head to try and dislodge the dirty thoughts that are starting to seep into your mind.").append(" As you lean back against the cold stone and take a deep breath, a voice suddenly interrupts your thoughts,");

            if (!femalePrologueNPC()) {
                UtilText.nodeContentSB.append(" [prologueMale.speech(Taking a break from the crowds as well?)]" + "</p>" + "<p>" + "Turning around, you see a tall, handsome-looking man, who must be only a couple of years older than you, giving you the most charming smile you've ever seen." + " Before you know what you're doing, your eyes are travelling up and down every [unit.size] of his manly, muscular body, and you only just manage to stop yourself from letting out a desperate little whine." + "</p>" + "<p>" + "[pc.thought(Focus, [pc.name], focus!)] you think, trying to act as casual as possible as you smile back at the stranger before you." + "</p>" + "<p>" + "[pc.speech(Actually,)] you say, [pc.speech(I've only just arrived. I thought I was going to be late, but it looks like nothing's started yet.)]" + "</p>" + "<p>" + "[prologueMale.speech(Ah, you must have just missed the announcement,)] he replies, [prologueMale.speech(the opening speech is being delayed by half an hour." + " I tried hanging around in that crowd back there, but I'm no historian, and most of the conversation is pretty dry...)]" + "</p>" + "<p>" + "[pc.speech(Haha,)]" + " you laugh, desperately trying not to imagine how he looks naked," + " [pc.speech(I know <i>exactly</i> what you mean. My aunt is the lady giving the opening speech, and every time I meet her friends from the museum, I can never follow their conversations." + " Well, apart from Arthur that is. He's closer to our age, and is really easy-going and fun to talk to.)]" + "</p>" + "<p>" + "[prologueMale.speech(Hah! You know Arthur? I'm here by his invitation. He and I go way back,)]" + " the man cheerily replies, his smile causing your heart to race." + " [prologueMale.speech(I'm [prologueMale.name] by the way, pleased to meet you ").append(Main.game.getPlayer().isFeminine() ? "Ms. ...?" : "Mr. ...?").append(")]").append("</p>").append("<p>").append("[pc.speech(Likewise,)] you respond, shaking his offered hand while trying not to think of how powerful and dominant his grip is. [pc.speech(I'm [pc.Name].)]").append("</p>").append("<p>").append("You and [prologueMale.name] continue talking with one another as you wait for the presentation to start.").append(" Before long, the subject shifts to work, and you find out that he's an airline pilot, based out of the airport on the city's outskirts.").append(" Conversation then moves on to what it is you do, and you end up talking about that for a little while...").append("</p>");

            } else {
                UtilText.nodeContentSB.append(" [prologueFemale.speech(Taking a break from the crowds as well?)]" + "</p>" + "<p>" + "Turning around, you see a beautiful woman, who looks to be about the same age as you, giving you the most stunning smile you've ever seen." + " Before you know what you're doing, your eyes are travelling up and down every [unit.size] of her curvy, womanly body, and you only just manage to stop yourself from letting out a hungry groan." + "</p>" + "<p>" + "[pc.thought(Focus [pc.name], focus!)] you think, trying to act as casual as possible as you smile back at the stranger before you." + "</p>" + "<p>" + "[pc.speech(Actually,)] you say, [pc.speech(I've only just arrived. I thought I was going to be late, but it looks like nothing's started yet.)]" + "</p>" + "<p>" + "[prologueFemale.speech(Ah, you must have just missed the announcement,)] she replies, [prologueFemale.speech(the opening speech is being delayed by half an hour." + " I tried hanging around in that crowd back there, but I'm no historian, and most of the conversation is pretty dry...)]" + "</p>" + "<p>" + "[pc.speech(Haha,)]" + " you laugh, desperately trying not to imagine how she looks naked," + " [pc.speech(I know <i>exactly</i> what you mean. My aunt is the lady giving the opening speech, and every time I meet her friends from the museum, I can never follow their conversations." + " Well, apart from Arthur that is. He's closer to our age, and is really easy-going and fun to talk to.)]" + "</p>" + "<p>" + "[prologueFemale.speech(Oh! You know Arthur? I'm here by his invitation, actually. He and I go way back,)]" + " the woman cheerily replies, her smile causing your heart to race." + " [prologueFemale.speech(I'm [prologueFemale.name] by the way, pleased to meet you ").append(Main.game.getPlayer().isFeminine() ? "Ms. ...?" : "Mr. ...?").append(")]").append("</p>").append("<p>").append("[pc.speech(Likewise,)] you respond, shaking her offered hand while trying not to think of how soft and delicate her skin is. [pc.speech(I'm [pc.Name].)]").append("</p>").append("<p>").append("You and [prologueFemale.name] continue talking with one another as you wait for the presentation to start.").append(" Before long, the subject shifts to work, and you find out that she's training to become a doctor, and is studying here at the city's university.").append(" Conversation then moves on to what it is you do, and you end up talking about that for a little while...").append("</p>");
            }
    ...
    public static final DialogueNode BACKGROUND_SELECTION_MENU = new DialogueNode("In the Museum", "-", true) {
        ...
        public String getContent() {
            ...
            for (Occupation history : Occupation.getAvailableHistories(Main.game.getPlayer())) {
                UtilText.nodeContentSB.append("<div class='container-full-width'>" + "<div class='container-full-width' style='margin:0;padding:0;'>" + "<h6 style='color:").append(history.getAssociatedPerk().getColour().toWebHexString()).append(";'>").append(Util.capitaliseSentence(history.getName(Main.game.getPlayer()))).append("</h6>").append("</div>").append("<div class='container-full-width' style='margin:0 8px; width: calc(10% - 16px);'>").append("<div id='OCCUPATION_").append(history).append("' class='fetish-icon full").append(Main.game.getPlayer().getHistory() == history
                        ? " owned' style='border:2px solid " + PresetColour.GENERIC_GOOD.toWebHexString() + ";'>"
                        : " unlocked' style='border:2px solid " + PresetColour.TEXT_GREY.toWebHexString() + ";" + "'>").append("<div class='fetish-icon-content'>").append(history.getAssociatedPerk().getSVGString(Main.game.getPlayer())).append("</div>").append("</div>").append("</div>").append("<div class='container-full-width' style='margin:0 8px; width: calc(90% - 16px);'>").append("<p>").append(history.getDescription(Main.game.getPlayer())).append("</p>").append("</div>").append("</div>");
            }
    ...
    public static final DialogueNode CHOOSE_SEX_EXPERIENCE = new DialogueNode("Start", "", true) {
        ...
        public String getContent() {
            ...
            UtilText.nodeContentSB.append("<div class='container-full-width' style='text-align:center;'>" + "<i>More sexual experience will result in gaining more corruption. (You can see your corruption, along with your other attributes, in the character panel to the left of the screen.)" + "<br/>" + "Selecting '<span style='color:").append(FetishDesire.FOUR_LOVE.getColour().toWebHexString()).append(";'>").append(FetishDesire.FOUR_LOVE.getName()).append("</span>'").append(" for any fetish desire will result in your character starting the game with that fetish, while the other four desires simply determine your character's attitude towards that fetish.</i>").append("</div>").append(CharacterModificationUtils.getSexualExperienceDiv()).append(CharacterModificationUtils.getFetishChoiceDiv());
    ...
    public static final DialogueNode FINAL_CHECK = new DialogueNode("Start", "", true) {
        ...
        public String getContent() {
            UtilText.nodeContentSB.setLength(0);
            UtilText.nodeContentSB.append("<div class='container-full-width' style='text-align:center;'>" + "<i>Once you're happy with your appearance, press the 'Start Game' button to begin!<br/>" + "[style.colourGood(This is the end of character creation, so only proceed once you're happy with your choices!)]</i>" + "</div>" + "<br/>" + "<div class='container-full-width'>" + "<h5 style='text-align:center;'>Final Appearance</h5>").append(Main.game.getPlayer().getBodyDescription()).append("</div>");
...
//					+ "(If it doesn't work, please let me know as a comment on my blog, and I'll get it fixed ASAP!)"
            ...
            for (Path f : Main.getCharactersForImport()) {
                importSB.append(getImportRow(i, f.getFileName().toString()));
                i++;
            }
