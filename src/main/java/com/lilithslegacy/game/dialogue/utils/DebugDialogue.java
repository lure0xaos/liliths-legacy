...
import com.lilithslegacy.game.character.npc.fields.ElisAlleywayAttacker;
...
import com.lilithslegacy.game.character.npc.submission.SubmissionAttacker;
...
public class DebugDialogue {
    ...
    public static final List<AbstractClothingType> clothingTotal = new ArrayList<>();
                    ...
                    return new Response("Test colours", "Test text for readability", COLOURS) {
                        ...
                        public void effects() {
                            ...
                            for (Colour c : PresetColour.getAllPresetColours()) {
                                coloursSB.append(c.getId()).append(": <span style='color:").append(c.toWebHexString()).append(";'>Test text for readability.</span><br/>");
                            }
                            coloursSB.append("<br/><br/>");
                            for (BaseColour bc : BaseColour.values()) {
                                coloursSB.append(bc.toString()).append(": <span style='color:").append(bc.toWebHexString()).append(";'>Test text for readability.</span><br/>");
                            }
                            coloursSB.append("</p>");
...
//				}
                    ...
                    return new Response("+1 Epona stamp", "Gives you one stamp you can get by playing the breeder roulette.", DEBUG_MENU) {
                        ...
                        public void effects() {
                            ...
                            Main.game.getTextEndStringBuilder().append("Added 1 stamp, you now have ").append(Main.game.getDialogueFlags().eponaStamps).append(" stamp(s)");
                        }
                    };
    ...
    public static final List<AbstractItemType> itemsTotal = new ArrayList<>();
    public static final DialogueNode OFFSPRING = new DialogueNode("", "", false) {
        ...
        public String getContent() {
            ...
            for (NPC npc : Main.game.getOffspring()) {
                ...
                UtilText.nodeContentSB.append(isBorn ? "" : "(Not born yet) ").append("<span style='color:").append(npc.getFemininity().getColour().toWebHexString()).append(";'>").append(npc.getName(true)).append(" ").append(npc.getSurname()).append("</span>").append(" (").append(npc.getSubspecies().getName(npc.getBody())).append(" | ").append(npc.getHalfDemonSubspecies().getName(npc.getBody())).append(")").append(" (").append(npc.getCovering(npc.getBody().getTorsoType().getBodyCoveringType(npc.getBody())).getPrimaryColour().getName()).append(")" // Primary covering colour
                ).append(" M:").append(npc.getMother() != null ? npc.getMother().getName(true) : "Deleted NPC").append(" F:").append(npc.getFather() != null ? npc.getFather().getName(true) : "Deleted NPC").append("<br/>");
            }
            for (OffspringSeed os : Main.game.getOffspringNotSpawned(os -> true, true)) {
                UtilText.nodeContentSB.append("Not yet").append(os.isBorn() ? " met " : " born ").append("<span style='color:").append(os.getFemininity().getColour().toWebHexString()).append(";'>").append(os.getName()).append(" ").append(os.getSurname()).append("</span>").append(" (").append(os.getSubspecies().getName(os.getBody())).append(" | ").append(os.getHalfDemonSubspecies().getName(os.getBody())).append(")").append(" (").append(os.getBody().getCovering(os.getBody().getTorsoType().getBodyCoveringType(os.getBody()), true).getPrimaryColour().getName()).append(")" // Primary covering colour
                ).append(" M:").append(os.getMother() != null ? os.getMother().getName(true) : "Deleted NPC").append(" F:").append(os.getFather() != null ? os.getFather().getName(true) : "Deleted NPC").append("<br/>");
            }
            if (activeOffspring != null) {
                for (AbstractFetish f : activeOffspring.getFetishes(true)) {
                    UtilText.nodeContentSB.append("<br/>[style.boldSex(Fetish:)] ").append(f.getName(activeOffspring));
                }
                UtilText.nodeContentSB.append("<br/>").append(activeOffspring.getDescription()).append("<br/>").append(activeOffspring.getBodyDescription());
            }
    ...
    public static final List<AbstractWeaponType> weaponsTotal = new ArrayList<>();
    public static final List<AbstractTattooType> tattoosTotal = new ArrayList<>();
    ...
    public static final DialogueNode SPAWN_MENU = new DialogueNode("Spawn Menu", "Access the spawn menu.", false) {
        ...
        public String getHeaderContent() {
            ...
            inventorySB.append("<p style='width:100%; text-align:center; padding:0 margin:0;'>").append(itemTag == ItemTag.CHEAT_ITEM
                    ...
                    : "<b style='color:" + PresetColour.BASE_YELLOW_LIGHT.toWebHexString() + ";'>Spawn Clothing</b> (" + Util.capitaliseSentence(activeSlot.getName()) + ")"))).append("</p>");
            ...
            if (itemTag == ItemTag.CHEAT_ITEM) {
                for (AbstractClothingType c : ClothingType.getAllClothing()) {
                    if (c.getDefaultItemTags().contains(ItemTag.CHEAT_ITEM)) {
                        inventorySB.append("<div class='inventory-item-slot unequipped' style='background-color:").append(c.getRarity().getBackgroundColour().toWebHexString()).append(";'>").append("<div class='inventory-icon-content'>").append(c.getSVGImage()).append("</div>").append("<div class='overlay' id='").append(c.getId()).append("_SPAWN'></div>").append("</div>");
                    }
                }
                for (AbstractWeaponType weaponType : WeaponType.getAllWeapons()) {
                    if (weaponType.getItemTags().contains(ItemTag.CHEAT_ITEM)) {
                        inventorySB.append("<div class='inventory-item-slot unequipped' style='background-color:").append(weaponType.getRarity().getBackgroundColour().toWebHexString()).append(";'>").append("<div class='inventory-icon-content'>").append(weaponType.getSVGImage()).append("</div>").append("<div class='overlay' id='").append(weaponType.getId()).append("_SPAWN'></div>").append("</div>");
                    }
                }
                for (AbstractItemType itemType : ItemType.getAllItems()) {
                    if (itemType.getItemTags().contains(ItemTag.CHEAT_ITEM)) {
                        inventorySB.append("<div class='inventory-item-slot unequipped' style='background-color:").append(itemType.getRarity().getBackgroundColour().toWebHexString()).append(";'>").append("<div class='inventory-icon-content'>").append(itemType.getSVGString()).append("</div>").append("<div class='overlay' id='").append(itemType.getId()).append("_SPAWN'></div>").append("</div>");
                    }
                }
                ...
                for (AbstractWeaponType weaponType : weaponsTotal) {
                    if ((weaponType.isMelee() && activeSlot == InventorySlot.WEAPON_MAIN_1)
                        ...
                        inventorySB.append("<div class='inventory-item-slot unequipped' style='background-color:").append(rarity.getBackgroundColour().toWebHexString()).append(";'>").append("<div class='inventory-icon-content'>").append(weaponType.getSVGImage()).append("</div>").append("<div class='overlay' id='").append(weaponType.getId()).append("_SPAWN'></div>").append("</div>");
                        count++;
                    }
                }
                ...
                for (AbstractClothingType clothingType : clothingTotal) {
                    if (clothingType.getEquipSlots().contains(activeSlot)) {
                        inventorySB.append("<div class='inventory-item-slot unequipped' style='background-color:").append(clothingType.getRarity().getBackgroundColour().toWebHexString()).append(";'>").append("<div class='inventory-icon-content'>").append(clothingType.getSVGImage()).append("</div>").append("<div class='overlay' id='").append(clothingType.getId()).append("_SPAWN'></div>").append("</div>");
                        count++;
                    }
                }
            }
            ...
            for (InventorySlot slot : InventorySlot.values()) {
                if (slot != InventorySlot.WEAPON_MAIN_2
                    ...
                    inventorySB.append("<div class='normal-button' id='").append(slot).append("_SPAWN_SELECT' style='width:18%; margin:1%; padding:2px; font-size:0.9em; color:").append(slot.isWeapon() ? PresetColour.BASE_RED_LIGHT.toWebHexString() : PresetColour.BASE_YELLOW_LIGHT.toWebHexString()).append(";'>").append(slot == InventorySlot.WEAPON_MAIN_1
                            ? "Melee"
                            : (slot == InventorySlot.WEAPON_OFFHAND_1
                            ? "Ranged"
                            : Util.capitaliseSentence(slot.getName()))).append("</div>");
                }
            }
            inventorySB.append("<div class='normal-button' id='ITEM_SPAWN_SELECT' style='width:18%; margin:1%; padding:2px; font-size:0.9em; color:").append(PresetColour.BASE_BLUE_LIGHT.toWebHexString()).append(";'>Items</div>");
            inventorySB.append("<div class='normal-button' id='ESSENCE_SPAWN_SELECT' style='width:18%; margin:1%; padding:2px; font-size:0.9em; color:").append(PresetColour.GENERIC_ARCANE.toWebHexString()).append(";'>Essences</div>");
            inventorySB.append("<div class='normal-button' id='BOOK_SPAWN_SELECT' style='width:18%; margin:1%; padding:2px; font-size:0.9em; color:").append(PresetColour.BASE_ORANGE.toWebHexString()).append(";'>Books</div>");
            inventorySB.append("<div class='normal-button' id='SPELL_SPAWN_SELECT' style='width:18%; margin:1%; padding:2px; font-size:0.9em; color:").append(PresetColour.DAMAGE_TYPE_SPELL.toWebHexString()).append(";'>Spells</div>");
            inventorySB.append("<div class='normal-button' id='HIDDEN_SPAWN_SELECT' style='width:18%; margin:1%; padding:2px; font-size:0.9em; opacity:0; cursor:default; color:").append(PresetColour.BASE_GREEN_DARK.toWebHexString()).append(";'>Cheats</div>");
    ...
    public static final DialogueNode SPAWN_MENU_SET = new DialogueNode("Set spawns", "", false) {
        ...
        public String getHeaderContent() {
            ...
            for (AbstractSetBonus sb : bonuses) {
                inventorySB.append("<div class='normal-button' id='SET_BONUS_").append(SetBonus.getIdFromSetBonus(sb)).append("' style='text-align:center;width:23%; margin:1%; padding:2px; font-size:0.9em;'>");
                inventorySB.append("<b style='color:").append(sb.getAssociatedStatusEffect().getColour().toWebHexString()).append(";'>#</b>");
    ...
    public static final DialogueNode ITEM_VIEWER = new DialogueNode("", "", false) {
        ...
        public String getContent() {
            ...
            if (!viewAllClothing && !viewAllTattoos && (viewAll || itemViewSlot == null)) {
                inventorySB.append("<div class='inventory-not-equipped' style='-webkit-user-select:auto;'>" + "<h5>Total items: ").append(itemsTotal.size()).append("</h5>");
                for (AbstractItemType itemType : itemsTotal) {
                    if ((itemTag == null
                            && (!itemType.getItemTags().contains(ItemTag.BOOK)
                            && !itemType.getItemTags().contains(ItemTag.ESSENCE)
                            ...
                            && (itemType.getItemTags().contains(itemTag)
                            || (itemTag == ItemTag.SPELL_BOOK && itemType.getItemTags().contains(ItemTag.SPELL_SCROLL))))) {
                        inventorySB.append("<div class='container-full-width' style='width:").append(width).append("%; white-space: nowrap; word-wrap: break-word; font-size:0.75em; -webkit-user-select:auto; padding:0; margin:0;'>").append("<div class='inventory-item-slot unequipped' style='width:").append(imgWidth).append("%; box-sizing: border-box; padding:0; margin:0; background-color:").append(itemType.getRarity().getBackgroundColour().toWebHexString()).append(";'>").append("<div class='inventory-icon-content'>").append(itemType.getSVGString()).append("</div>").append("<div class='overlay' id='").append(itemType.getId()).append("_SPAWN'></div>").append("</div>").append(ItemType.getItemToIdMap().get(itemType)).append("</div>");
                    }
                }
                inventorySB.append("</div>");
            }
            ...
            if (viewAll || (itemViewSlot != null && itemViewSlot.isWeapon())) {
                inventorySB.append("<div class='inventory-not-equipped' style='-webkit-user-select:auto;'>" + "<h5>Total weapons: ").append(weaponsTotal.size()).append("</h5>");
                for (AbstractWeaponType weaponType : weaponsTotal) {
                    inventorySB.append("<div class='container-full-width' style='width:").append(width).append("%; white-space: nowrap; word-wrap: break-word; font-size:0.75em; -webkit-user-select:auto; padding:0; margin:0;'>").append("<div class='inventory-item-slot unequipped' style='width:").append(imgWidth).append("%; box-sizing: border-box; padding:0; margin:0; background-color:").append(weaponType.getRarity().getBackgroundColour().toWebHexString()).append(";'>").append("<div class='inventory-icon-content'>").append(weaponType.getSVGImage()).append("</div>").append("<div class='overlay' id='").append(weaponType.getId()).append("_SPAWN'></div>").append("</div>").append(WeaponType.getIdFromWeaponType(weaponType)).append("</div>");
                }
                inventorySB.append("</div>");
            }

            if (viewAll || viewAllClothing) {
                inventorySB.append("<div class='inventory-not-equipped' style='-webkit-user-select:auto;'>" + "<h5>Total clothing: ").append(clothingTotal.size()).append("</h5>");
                for (AbstractClothingType clothingType : clothingTotal) {
                    inventorySB.append("<div class='container-full-width' style='width:").append(width).append("%; white-space: nowrap; word-wrap: break-word; font-size:0.75em; -webkit-user-select:auto; padding:0; margin:0;'>").append("<div class='inventory-item-slot unequipped' style='width:").append(imgWidth).append("%; box-sizing: border-box; padding:0; margin:0; background-color:").append(clothingType.getRarity().getBackgroundColour().toWebHexString()).append(";'>").append("<div class='inventory-icon-content'>").append(clothingType.getSVGImage()).append("</div>").append("<div class='overlay' id='").append(clothingType.getId()).append("_SPAWN'></div>").append("</div>").append(clothingType.getPhysicalResistance() > 0
                            ...
                            : ClothingType.getIdFromClothingType(clothingType)).append("</div>");
                }
                inventorySB.append("</div>");

            } else if (viewAllTattoos) {
                inventorySB.append("<div class='inventory-not-equipped' style='-webkit-user-select:auto;'>" + "<h5>Total tattoos: ").append(tattoosTotal.size()).append("</h5>");
                for (AbstractTattooType tattooType : tattoosTotal) {
                    inventorySB.append("<div class='container-full-width' style='width:").append(width).append("%; white-space: nowrap; word-wrap: break-word; font-size:0.75em; -webkit-user-select:auto; padding:0; margin:0;'>").append("<div class='inventory-item-slot unequipped' style='width:").append(imgWidth).append("%; box-sizing: border-box; padding:0; margin:0; background-color:").append(tattooType.getRarity().getBackgroundColour().toWebHexString()).append(";'>").append("<div class='inventory-icon-content'>").append(tattooType.getSVGImage(Main.game.getPlayer())).append("</div>").append("<div class='overlay' id='").append(tattooType.getId()).append("_SPAWN'></div>").append("</div>").append(TattooType.getIdFromTattooType(tattooType)).append("</div>");
                }
                inventorySB.append("</div>");

            } else if (itemViewSlot != null && !itemViewSlot.isWeapon()) {
                List<AbstractClothingType> clothingToDisplay = clothingTotal.stream().filter((c) -> c.getEquipSlots().get(0) == itemViewSlot).toList();
                inventorySB.append("<div class='inventory-not-equipped' style='-webkit-user-select:auto;'>" + "<h5>Total '").append(itemViewSlot.getName()).append("' slot clothing: ").append(clothingToDisplay.size()).append("</h5>");
                for (AbstractClothingType clothingType : clothingToDisplay) {
                    inventorySB.append("<div class='container-full-width' style='width:").append(width).append("%; white-space: nowrap; word-wrap: break-word; font-size:0.75em; -webkit-user-select:auto; padding:0; margin:0;'>").append("<div class='inventory-item-slot unequipped' style='width:").append(imgWidth).append("%; box-sizing: border-box; padding:0; margin:0; background-color:").append(clothingType.getRarity().getBackgroundColour().toWebHexString()).append(";'>").append("<div class='inventory-icon-content'>").append(clothingType.getSVGImage()).append("</div>").append("<div class='overlay' id='").append(clothingType.getId()).append("_SPAWN'></div>").append("</div>").append(ClothingType.getIdFromClothingType(clothingType)).append("</div>");
                }
                inventorySB.append("</div>");
            }
    ...
    public static final DialogueNode BODY_PART_RACE_RESET = new DialogueNode("Reset body", "Set race.", false) {
        ...
        public String getContent() {
            ...
            for (AbstractSubspecies sub : Subspecies.getAllSubspecies()) {
                sb.append("<span style='color:").append(sub.getColour(Main.game.getPlayer()).toWebHexString()).append(";'>").append(Util.capitaliseSentence(sub.getName(Main.game.getPlayer().getBody()))).append("</span>: ").append(Subspecies.getIdFromSubspecies(sub));
                sb.append("</br>");
            }
...
//							System.out.println("Subspecies override: "+Main.game.getPlayer().getSubspeciesOverride());
                        ...
                        Main.game.getTextEndStringBuilder().append("<p>" + "[style.boldTfGeneric(Transformed:)] You are now ").append(UtilText.generateSingularDeterminer(name)).append(" ").append(name).append("!").append("</p>");
                    }
                };
    ...
    public static final DialogueNode CLOTHING_COLLAGE = new DialogueNode("Clothing collage", "Clothing collage.", false) {
        ...
        public String getContent() {
            ...
            inventorySB.append("<div class='inventory-not-equipped' style='-webkit-user-select:auto;'>" + "<h5>Total items: ").append(itemsTotal.size()).append("</h5>");
            for (AbstractItemType itemType : itemsTotal) {
                inventorySB.append("<div class='container-full-width' style='width:").append(width).append("%; white-space: nowrap; word-wrap: break-word; font-size:0.75em; -webkit-user-select:auto; padding:1px; margin:0;'>").append("<div class='inventory-item-slot unequipped' style='width:").append(imgWidth).append("%; box-sizing: border-box; padding:0; margin:0; background-color:").append(itemType.getRarity().getBackgroundColour().toWebHexString()).append(";'>").append("<div class='inventory-icon-content'>").append(itemType.getSVGString()).append("</div>").append("<div class='overlay' id='").append(itemType.getId()).append("_SPAWN'></div>").append("</div>").append("</div>");
            }
            inventorySB.append("</div>");
            ...
            inventorySB.append("<div class='inventory-not-equipped' style='-webkit-user-select:auto;'>" + "<h5>Total clothing: ").append(clothingTotal.size()).append("</h5>");
            for (AbstractClothingType clothingType : clothingTotal) {
                inventorySB.append("<div class='container-full-width' style='width:").append(width).append("%; white-space: nowrap; word-wrap: break-word; font-size:0.75em; -webkit-user-select:auto; padding:1px; margin:0;'>").append("<div class='inventory-item-slot unequipped' style='width:").append(imgWidth).append("%; box-sizing: border-box; padding:0; margin:0; background-color:").append(clothingType.getRarity().getBackgroundColour().toWebHexString()).append(";'>").append("<div class='inventory-icon-content'>").append(clothingType.getSVGImageRandomColour(true, false, false)).append("</div>").append("<div class='overlay' id='").append(clothingType.getId()).append("_SPAWN'></div>").append("</div>").append("</div>");
            }
            inventorySB.append("</div>");
    ...
    public static final DialogueNode PARSER_TARGETS = new DialogueNode("Parser", "", true) {
        ...
        public String getHeaderContent() {
            ...
            UtilText.nodeContentSB.append("<p>Here are a list of accepted <i style='color:").append(PresetColour.CLOTHING_BLUE_LIGHT.toWebHexString()).append(";'>targets</i>, for use in the parsing syntax:<br/>").append("[<i style='color:").append(PresetColour.CLOTHING_BLUE_LIGHT.toWebHexString()).append(";'>target</i>.<i style='color:").append(PresetColour.CLOTHING_PINK_LIGHT.toWebHexString()).append(";'>command</i>").append("<i style='color:").append(PresetColour.CLOTHING_YELLOW.toWebHexString()).append(";'>(arguments)</i>]</p>");
            ...
            for (AbstractParserTarget character : ParserTarget.getAllParserTargets()) {
                ...
                for (String s : character.getTags()) {
                    UtilText.nodeContentSB.append(first ? "" : " | ").append("<i style='color:").append(PresetColour.CLOTHING_BLUE_LIGHT.toWebHexString()).append(";'>").append(s).append("</i>");
                    first = false;
                }

                UtilText.nodeContentSB.append("<br/>").append(character.getDescription()).append("</p>");
            }
    ...
    public static final DialogueNode PARSER_COMMANDS = new DialogueNode("Parser", "", true) {
        ...
        public String getHeaderContent() {
            ...
            UtilText.nodeContentSB.append("<p>Here are a list of accepted <i style='color:").append(PresetColour.CLOTHING_PINK_LIGHT.toWebHexString()).append(";'>commands</i>, for use in the parsing syntax:<br/>").append("[<i style='color:").append(PresetColour.CLOTHING_BLUE_LIGHT.toWebHexString()).append(";'>target</i>.<i style='color:").append(PresetColour.CLOTHING_PINK_LIGHT.toWebHexString()).append(";'>command</i>").append("<i style='color:").append(PresetColour.CLOTHING_YELLOW.toWebHexString()).append(";'>(arguments)</i>]</p>").append("<p>").append("<b>Please don't be intimidated by the number of commands!</b>").append(" The <i>vast</i> majority of them are automatically generated 'standard' variations for body parts, following a (hopefully) intuitive naming system.").append("</p>");

            int count = 1;
            for (ParserCommand command : UtilText.commandsList) {
                UtilText.nodeContentSB.append("<hr/>" + "<p>" + "<b style='color:").append(PresetColour.TEXT_GREY.toWebHexString()).append(";'>").append(String.format("%03d.", count)).append("</b> <i style='color:").append(PresetColour.CLOTHING_PINK_LIGHT.toWebHexString()).append(";'>").append(command.getTags().get(0)).append("</i>");

                if (command.getTags().size() > 1) {
                    for (int i = 1; i < command.getTags().size(); i++)
                        UtilText.nodeContentSB.append(" | <i style='color:").append(PresetColour.CLOTHING_PINK_LIGHT.toWebHexString()).append(";'>").append(command.getTags().get(i)).append("</i>");
                }
    ...
    public static final DialogueNode PARSER_COMMANDS_NEAT = new DialogueNode("Parser", "", true) {
        ...
        public String getHeaderContent() {
            ...
            for (BodyPartType bpt : BodyPartType.values()) {
                UtilText.nodeContentSB.append("<details>" + "<summary style='cursor:pointer;'><b style='color:").append(PresetColour.TEXT_GREY.toWebHexString()).append(";'>(").append(String.format("%03d", count)).append(" - ").append(String.format("%03d", count + UtilText.commandsMap.get(bpt).size() - 1)).append(")</b> ").append(Util.capitaliseSentence(bpt.getName())).append("</summary>");
                for (ParserCommand command : UtilText.commandsMap.get(bpt)) {
                    UtilText.nodeContentSB.append("<p>" + "<hr/>" + "<b style='color:").append(PresetColour.TEXT_GREY.toWebHexString()).append(";'>").append(String.format("%03d.", count)).append("</b> <i style='color:").append(PresetColour.CLOTHING_PINK_LIGHT.toWebHexString()).append(";'>").append(command.getTags().get(0)).append("</i>");
                    ...
                    UtilText.nodeContentSB.append("<br/>").append(command.getArguments() == "" ? "<i style='color:" + PresetColour.TEXT_GREY.toWebHexString() + ";'>No arguments</i>" : "<i style='color:" + PresetColour.CLOTHING_YELLOW.toWebHexString() + ";'>" + command.getArguments() + "</i>").append("<br/>").append(command.isAllowsCapitalisation() ? "<i style='color:" + PresetColour.GENERIC_GOOD.toWebHexString() + ";'>Capitalisation</i>" : "<i style='color:" + PresetColour.TEXT_GREY.toWebHexString() + ";'>Capitalisation</i>").append(" | ").append(command.isAllowsPronoun() ? "<i style='color:" + PresetColour.GENERIC_GOOD.toWebHexString() + ";'>Pronouns</i>" : "<i style='color:" + PresetColour.TEXT_GREY.toWebHexString() + ";'>Pronouns</i>").append("<br/>").append(command.getDescription()).append("<br/>").append("Examples:<br/>").append(command.getExampleBeforeParse("lilaya", (command.getArguments() == "" ? "" : command.getArgumentExample()))).append(" -> ").append(UtilText.parse("[lilaya." + command.getTags().get(0) + (command.getArguments() == "" ? "" : "(" + command.getArgumentExample() + ")") + "]")).append("<br/>").append(command.getExampleBeforeParse("brax", (command.getArguments() == "" ? "" : command.getArgumentExample()))).append(" -> ").append(UtilText.parse("[brax." + command.getTags().get(0) + (command.getArguments() == "" ? "" : "(" + command.getArgumentExample() + ")") + "]")).append("<br/>").append(command.getExampleBeforeParse("kate", (command.getArguments() == "" ? "" : command.getArgumentExample()))).append(" -> ").append(UtilText.parse("[kate." + command.getTags().get(0) + (command.getArguments() == "" ? "" : "(" + command.getArgumentExample() + ")") + "]")).append("</p>");
