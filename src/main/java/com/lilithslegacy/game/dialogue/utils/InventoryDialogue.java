...
import com.lilithslegacy.game.inventory.clothing.BlockedParts;
import com.lilithslegacy.game.inventory.clothing.DisplacementType;
import com.lilithslegacy.game.inventory.clothing.Sticker;
import com.lilithslegacy.game.inventory.clothing.StickerCategory;
...
import com.lilithslegacy.game.sex.sexActions.SexActionUtility;
...
import com.lilithslegacy.utils.comparators.ClothingZLayerComparator;
    ...
    private static void equipAll(GameCharacter character) {
        ...
        for (AbstractClothing c : zlayerClothing) {
            if (!slotsTaken.contains(c.getClothingType().getEquipSlots().get(0))) {
                Main.game.getTextEndStringBuilder().append("<p style='text-align:center;'>").append(character.equipClothingFromInventory(c, true, character, character)).append("</p>");
...
//		}
        ...
        for (AbstractClothing c : zlayerClothing) {
            if ((!Main.game.isInSex() || (!c.getSlotEquippedTo().isJewellery() && !c.isCondom())) && !c.isMilkingEquipment()) {
                ...
                sb.append("<p style='text-align:center;'>").append(character.getUnequipDescription()).append("</p>");
            }
        }
    ...
    private static String getClothingDyeUI() {
        ...
        if (!clothing.getClothingType().getStickers().isEmpty()) { //TODO test stickers
            ...
            for (StickerCategory cat : orderedCategories) {
                ...
                stickerSB.append(Util.capitaliseSentence(cat.getName())).append(":"); // Category name
                ...
                for (Sticker sticker : orderedStickers) {
                    ...
                    if (requirements.isEmpty() || sticker.isShowDisabledButton()) {
                        ...
                        if (!requirements.isEmpty()) {
                            stickerSB.append("<div id='").append(id).append("' class='cosmetics-button disabled'>").append("<b style='color:").append(PresetColour.TEXT_GREY.toWebHexString()).append(";'>").append(Util.capitaliseSentence(sticker.getName())).append(specialSticker ? "*" : "").append("</b>").append("</div>");

                        } else if (dyePreviewStickers.get(cat) == sticker) {
                            stickerSB.append("<div id='").append(id).append("' class='cosmetics-button active'>").append("<b style='color:").append(sticker.getColourSelected().toWebHexString()).append(";'>").append(Util.capitaliseSentence(sticker.getName())).append(specialSticker ? "*" : "").append("</b>").append("</div>");
                        } else {
                            stickerSB.append("<div id='").append(id).append("' class='cosmetics-button'>").append("<span style='color:").append(sticker.getColourDisabled().toWebHexString()).append(";'>").append(Util.capitaliseSentence(sticker.getName())).append(specialSticker ? "*" : "").append("</span>").append("</div>");
                        }
                    }
                }
                stickerSB.append("</div>");
        ...
        for (int i = 0; i < clothing.getClothingType().getColourReplacements().size(); i++) {
            ...
            if (!cr.getAllColours().isEmpty()) {
                inventorySB.append("<div class='container-quarter-width' style='width:calc(75% - 16px);'>").append(Util.capitaliseSentence(Util.intToPrimarySequence(i + 1))).append(" Colour").append(cr.isRecolouringAllowed() ? "" : " ([style.italicsBad(cannot be changed)])").append(":<br/>");

                for (Colour c : cr.getAllColours()) {
                    inventorySB.append("<div class='normal-button").append(dyePreviews.size() > i && dyePreviews.get(i) == c ? " selected" : "").append("' id='DYE_CLOTHING_").append(i).append("_").append(c.getId()).append("'").append(" style='width:auto; margin-right:4px; border-width:1px;").append(cr.getDefaultColours().contains(c)
                            ? "border-color:" + PresetColour.TEXT_GREY.toWebHexString() + ";"
                            : "").append(dyePreviews.size() > i && dyePreviews.get(i) == c
                            ? " background-color:" + PresetColour.BASE_GREEN.getShades()[4] + ";"
                            : "").append("'>").append("<div class='phone-item-colour' style='").append(c.isMetallic()
                            ? "background: repeating-linear-gradient(135deg, " + c.toWebHexString() + ", " + c.getShades()[4] + " 10px);"
                            : "background-color:" + c.toWebHexString() + ";").append("'></div>").append("</div>");
                }
        ...
        if (clothing.getClothingType().isPatternAvailable()) {
            ...
            for (Pattern pattern : Pattern.getAllPatterns()) {
                if (dyePreviewPattern.equals(pattern.getId())) {
                    inventorySB.append("<div class='cosmetics-button active'>" + "<b style='color:").append(PresetColour.GENERIC_GOOD.toWebHexString()).append(";'>").append(Util.capitaliseSentence(pattern.getNiceName())).append("</b>").append("</div>");
                } else {
                    inventorySB.append("<div id='ITEM_PATTERN_").append(pattern.getId()).append("' class='cosmetics-button'>").append("<span style='color:").append(PresetColour.TRANSFORMATION_GENERIC.getShades()[0]).append(";'>").append(Util.capitaliseSentence(pattern.getNiceName())).append("</span>").append("</div>");
                }
            }
            inventorySB.append("</div>");
            ...
            for (int i = 0; i < clothing.getClothingType().getPatternColourReplacements().size(); i++) {
                ...
                if (!cr.getAllColours().isEmpty() && Pattern.getPattern(dyePreviewPattern).isRecolourAvailable(cr)) {
                    inventorySB.append("<div class='container-quarter-width' style='width:calc(75% - 16px);'>" + "Pattern ").append(Util.capitaliseSentence(Util.intToPrimarySequence(i + 1))).append(" Colour:<br/>");

                    for (Colour c : cr.getAllColours()) {
                        inventorySB.append("<div class='normal-button").append(dyePreviewPatterns.size() > i && dyePreviewPatterns.get(i) == c ? " selected" : "").append("' id='DYE_CLOTHING_PATTERN_").append(i).append("_").append(c.getId()).append("'").append(" style='width:auto; margin-right:4px;").append(dyePreviewPatterns.size() > i && dyePreviewPatterns.get(i) == c ? " background-color:" + PresetColour.BASE_GREEN.getShades()[4] + ";" : "").append("'>").append("<div class='phone-item-colour' style='").append(c.isMetallic()
                                ? "background: repeating-linear-gradient(135deg, " + c.toWebHexString() + ", " + c.getShades()[4] + " 10px);"
                                : "background-color:" + c.toWebHexString() + ";").append("'></div>").append("</div>");
                    }
                    inventorySB.append("</div>");
                                    ...
                                    for (AbstractClothing c : Main.game.getPlayer().getClothingCurrentlyEquipped()) {
                                        for (BlockedParts bp : c.getBlockedPartsMap(Main.game.getPlayer(), c.getSlotEquippedTo())) {
                                            if (bp.displacementType != DisplacementType.REMOVE_OR_EQUIP) {
                                                ...
                                                Main.game.getTextEndStringBuilder().append("<p style='text-align:center;'>").append(Main.game.getPlayer().getDisplaceDescription()).append("</p>");
                                            }
                                        }
                                    }
                                }
                                    ...
                                    for (AbstractClothing c : zlayerClothing) {
                                        for (BlockedParts bp : c.getBlockedPartsMap(Main.game.getPlayer(), c.getSlotEquippedTo())) {
                                            if (bp.displacementType != DisplacementType.REMOVE_OR_EQUIP) {
                                                ...
                                                Main.game.getTextEndStringBuilder().append("<p style='text-align:center;'>").append(Main.game.getPlayer().getReplaceDescription()).append("</p>");
                                            }
                                        }
                                    }
                                }
                                    ...
                                    for (AbstractClothing c : inventoryNPC.getClothingCurrentlyEquipped()) {
                                        for (BlockedParts bp : c.getBlockedPartsMap(inventoryNPC, c.getSlotEquippedTo())) {
                                            if (bp.displacementType != DisplacementType.REMOVE_OR_EQUIP) {
                                                ...
                                                Main.game.getTextEndStringBuilder().append("<p style='text-align:center;'>").append(inventoryNPC.getDisplaceDescription()).append("</p>");
                                            }
                                        }
                                    }
                                }
                                    ...
                                    for (AbstractClothing c : zlayerClothing) {
                                        for (BlockedParts bp : c.getBlockedPartsMap(inventoryNPC, c.getSlotEquippedTo())) {
                                            if (bp.displacementType != DisplacementType.REMOVE_OR_EQUIP) {
                                                ...
                                                Main.game.getTextEndStringBuilder().append("<p style='text-align:center;'>").append(inventoryNPC.getReplaceDescription()).append("</p>");
                                            }
                                        }
                                    }
                                }
                                ...
                                public void effects() {
                                    ...
                                    for (AbstractClothing c : Main.game.getPlayer().getClothingCurrentlyEquipped()) {
                                        for (BlockedParts bp : c.getBlockedPartsMap(Main.game.getPlayer(), c.getSlotEquippedTo())) {
                                            if (bp.displacementType != DisplacementType.REMOVE_OR_EQUIP) {
                                                Main.game.getPlayer().isAbleToBeDisplaced(c, bp.displacementType, true, true, Main.game.getPlayer());
                                                responseSB.append("<p style='text-align:center;'>").append(Main.game.getPlayer().getDisplaceDescription()).append("</p>");
                                            }
                                        }
                                    }
                                ...
                                public void effects() {
                                    ...
                                    for (AbstractClothing c : inventoryNPC.getClothingCurrentlyEquipped()) {
                                        for (BlockedParts bp : c.getBlockedPartsMap(inventoryNPC, c.getSlotEquippedTo())) {
                                            if (bp.displacementType != DisplacementType.REMOVE_OR_EQUIP) {
                                                inventoryNPC.isAbleToBeDisplaced(c, bp.displacementType, true, true, Main.game.getPlayer());
                                                responseSB.append("<p style='text-align:center;'>").append(inventoryNPC.getDisplaceDescription()).append("</p>");
                                            }
                                        }
                                    }
    ...
    private static String getWeaponDyeUI() {
        ...
        for (DamageType dt : weapon.getWeaponType().getAvailableDamageTypes()) {
            inventorySB.append("<div class='normal-button").append(damageTypePreview == dt ? " selected" : "").append("' id='DAMAGE_TYPE_").append(dt.toString()).append("'").append("style='width:20%; margin:0 2.5%; color:").append(damageTypePreview == dt ? dt.getColour().toWebHexString() : dt.getColour().getShades(8)[0]).append(";'>").append(Util.capitaliseSentence(dt.getName())).append("</div>");
        }
        inventorySB.append("</div>");
        ...
        for (int i = 0; i < weapon.getWeaponType().getColourReplacements(false).size(); i++) {
            ...
            if (!cr.getAllColours().isEmpty()) {
                ...
                for (Colour c : cr.getAllColours()) {
                    inventorySB.append("<div class='normal-button").append(dyePreviews.size() > i && dyePreviews.get(i) == c ? " selected" : "").append("' id='DYE_WEAPON_").append(i).append("_").append(c.getId()).append("'").append(" style='width:auto; margin-right:4px; border-width:1px;").append(cr.getDefaultColours().contains(c)
                            ? "border-color:" + PresetColour.TEXT_GREY.toWebHexString() + ";"
                            : "").append(dyePreviews.size() > i && dyePreviews.get(i) == c
                            ? " background-color:" + PresetColour.BASE_GREEN.getShades()[4] + ";"
                            : "").append("'>").append("<div class='phone-item-colour' style='").append(c.isMetallic()
                ...
                for (Entry<String, List<InventorySlot>> entry : Main.game.getPlayer().getUnlockKeyMap().entrySet()) {
                    try {
                        ...
                        if (!slots.isEmpty()) {
                            ...
                            int i = 0;
                            for (String s : slots) {
                                UtilText.nodeContentSB.append(i > 0 ? ", " : "").append(s);
                                i++;
                            }
                        }
                    } catch (Exception e) {
                            ...
                            if (index == 1) {
                                ...
                                if (!item.isAbleToBeUsedFromInventory()) {
                                    ...
                                    return new Response(
                                        ...
                                        @Override
                                        public void effects() {
                                            Main.game.getTextEndStringBuilder().append("<p style='text-align:center;'>").append(Main.game.getPlayer().useItem(item, Main.game.getPlayer(), false)).append("</p>");
                                            resetPostAction();
                                        }
                                    };
                                }
                            ...
                            if (index == 1) {
                                ...
                                if (!item.isAbleToBeUsedFromInventory()) {
                                    ...
                                    return new Response(
                                        ...
                                        @Override
                                        public void effects() {
                                            Main.game.getTextEndStringBuilder().append("<p style='text-align:center;'>").append(Main.game.getPlayer().useItem(item, Main.game.getPlayer(), false, false, false)).append("</p>");
                            ...
                            if (index == 1) {
                                ...
                                if (weapon.isCanBeEquipped(Main.game.getPlayer(), slot)) {
                                    return new Response("Equip Main (Self)", "Equip the " + weapon.getName() + " as your main weapon.", INVENTORY_MENU) {
                                        @Override
                                        public void effects() {
                                            Main.game.getTextEndStringBuilder().append("<p style='text-align:center;'>").append(Main.game.getPlayer().equipMainWeaponFromInventory(weapon, Main.game.getPlayer())).append("</p>");
                                            resetPostAction();
                                        }
                                    };
                                } else {
                                ...
                                if (weapon.isCanBeEquipped(Main.game.getPlayer(), slot)) {
                                    return new Response("Equip Offhand (Self)", "Equip the " + weapon.getName() + " as your offhand weapon.", INVENTORY_MENU) {
                                        @Override
                                        public void effects() {
                                            Main.game.getTextEndStringBuilder().append("<p style='text-align:center;'>").append(Main.game.getPlayer().equipOffhandWeaponFromInventory(weapon, Main.game.getPlayer())).append("</p>");
                                            resetPostAction();
                                        }
                                    };
                                } else {
...
//								}
                                ...
                                if (weapon.isCanBeEquipped(inventoryNPC, slot)) {
                                    return new Response(UtilText.parse(inventoryNPC, "Equip Main ([npc.HerHim])"), UtilText.parse(inventoryNPC, "Make [npc.name] equip the " + weapon.getName() + " as [npc.her] main weapon."), INVENTORY_MENU) {
                                        @Override
                                        public void effects() {
                                            Main.game.getTextEndStringBuilder().append("<p style='text-align:center;'>").append(inventoryNPC.equipMainWeaponFromInventory(weapon, Main.game.getPlayer())).append("</p>");
                                            resetPostAction();
                                        }
                                    };
                                } else {
...
//								}
                                ...
                                if (weapon.isCanBeEquipped(inventoryNPC, slot)) {
                                    return new Response(UtilText.parse(inventoryNPC, "Equip Offhand ([npc.HerHim])"), UtilText.parse(inventoryNPC, "Make [npc.name] equip the " + weapon.getName() + " as [npc.her] offhand weapon."), INVENTORY_MENU) {
                                        @Override
                                        public void effects() {
                                            Main.game.getTextEndStringBuilder().append("<p style='text-align:center;'>").append(inventoryNPC.equipOffhandWeaponFromInventory(weapon, Main.game.getPlayer())).append("</p>");
                                            resetPostAction();
                                        }
                                    };
                                } else {
                            ...
                            if (index == 1) {
                                ...
                                if (weapon.isCanBeEquipped(Main.game.getPlayer(), slot)) {
                                    return new Response("Equip Main (Self)", "Equip the " + weapon.getName() + " as your main weapon.", INVENTORY_MENU) {
                                        @Override
                                        public void effects() {
                                            Main.game.getTextEndStringBuilder().append("<p style='text-align:center;'>").append(Main.game.getPlayer().equipMainWeaponFromInventory(weapon, inventoryNPC)).append("</p>");
                                            resetPostAction();
                                        }
                                    };
                                } else {
                                ...
                                if (weapon.isCanBeEquipped(Main.game.getPlayer(), slot)) {
                                    return new Response("Equip Offhand (Self)", "Equip the " + weapon.getName() + " as your offhand weapon.", INVENTORY_MENU) {
                                        @Override
                                        public void effects() {
                                            Main.game.getTextEndStringBuilder().append("<p style='text-align:center;'>").append(Main.game.getPlayer().equipOffhandWeaponFromInventory(weapon, inventoryNPC)).append("</p>");
                                            resetPostAction();
                                        }
                                    };
                                } else {
                                ...
                                if (weapon.isCanBeEquipped(inventoryNPC, slot)) {
                                    return new Response(UtilText.parse(inventoryNPC, "Equip Main ([npc.HerHim])"), UtilText.parse(inventoryNPC, "Get [npc.name] to equip the " + weapon.getName() + " as [npc.her] main weapon."), INVENTORY_MENU) {
                                        @Override
                                        public void effects() {
                                            Main.game.getTextEndStringBuilder().append("<p style='text-align:center;'>").append(inventoryNPC.equipMainWeaponFromInventory(weapon, inventoryNPC)).append("</p>");
                                            resetPostAction();
                                        }
                                    };
                                } else {
                                ...
                                if (weapon.isCanBeEquipped(inventoryNPC, slot)) {
                                    return new Response(UtilText.parse(inventoryNPC, "Equip Offhand ([npc.HerHim])"), UtilText.parse(inventoryNPC, "Get [npc.name] to equip the " + weapon.getName() + " as [npc.her] offhand weapon."), INVENTORY_MENU) {
                                        @Override
                                        public void effects() {
                                            Main.game.getTextEndStringBuilder().append("<p style='text-align:center;'>").append(inventoryNPC.equipOffhandWeaponFromInventory(weapon, inventoryNPC)).append("</p>");
                                            resetPostAction();
                                        }
                                    };
                                } else {
        ...
        public String getContent() {
            ...
            for (InventorySlot is : clothing.getClothingType().getEquipSlots()) {
                ...
                if (!descriptions.isEmpty()) {
                    sb.append("<i>When equipped into the '").append(is.getName()).append("' slot:</i><br/>");
                            ...
                            if (index == 1) {
                                        ...
                                        if (Main.game.getPlayer().getEssenceCount() >= IDENTIFICATION_ESSENCE_PRICE) {
                                            return new Response("Identify ([style.italicsArcane(" + IDENTIFICATION_ESSENCE_PRICE + " Essences)])",
                                                ...
                                                public void effects() {
                                                    ...
                                                    Main.game.getTextEndStringBuilder().append("<p>" + "You channel the power of ").append(Util.intToString(IDENTIFICATION_ESSENCE_PRICE)).append(" of your arcane essences into the ").append(clothing.getName()).append(", and as it emits a faint purple glow, you find yourself able to detect what sort of enchantment it has!").append("</p>").append(enchantmentRemovedString).append("<p style='text-align:center;'>").append("Identifying the ").append(clothing.getName()).append(" has cost you [style.boldBad(").append(Util.intToString(IDENTIFICATION_ESSENCE_PRICE)).append(")] [style.boldArcane(Arcane Essences)]!").append("</p>");
                                                    RenderingEngine.setPage(Main.game.getPlayer(), clothing);
                                                }
                                            };
                                        } else {
                                ...
                                if (clothing.isCanBeEquipped(Main.game.getPlayer(), slot)) {
                                    return new Response("Equip: " + Util.capitaliseSentence(slot.getName()), "Equip the " + clothing.getName() + ".", INVENTORY_MENU) {
                                        @Override
                                        public void effects() {
                                            Main.game.getTextEndStringBuilder().append("<p style='text-align:center;'>").append(equipClothingFromInventory(Main.game.getPlayer(), slot, Main.game.getPlayer(), clothing)).append("</p>");
                                        }
                                    };
                                } else {
                                    return new Response("Equip: " + Util.capitaliseSentence(slot.getName()), clothing.getCannotBeEquippedText(Main.game.getPlayer(), slot), null);
...
//												enslavementTargets.removeIf((npc) -> Main.game.getPlayer().getFriendlyOccupants().contains(npc.getId()));
                                                ...
                                                if (inventoryNPC.getEnslavementDialogue(clothing) == null) {
                                                    Main.game.getTextEndStringBuilder().append("<p style='text-align:center;'>").append(equipClothingFromInventory(inventoryNPC, slot, Main.game.getPlayer(), clothing)).append("</p>");
                                ...
                                if (!inventoryNPC.willBuy(clothing)) {
                                    ...
                                    return new Response("Identify (" + UtilText.formatAsMoney(IDENTIFICATION_PRICE, "span") + ")",
                                        ...
                                        public void effects() {
                                            ...
                                            Main.game.getTextEndStringBuilder().append("<p style='text-align:center;'>").append(UtilText.parse(inventoryNPC,
                                                    "You hand over " + UtilText.formatAsMoney(IDENTIFICATION_PRICE) + " to [npc.name],"
                                                            + " who promptly feeds several bottles of arcane essence into a specialist identification device, before using it to reveal the enchantment on your " + clothing.getName() + ".")).append("</p>").append(enchantmentRemovedString);
                            ...
                            if (index == 1) {
                                ...
                                if (clothing.isCanBeEquipped(Main.game.getPlayer(), slot)) {
                                    return new Response("Equip: " + Util.capitaliseSentence(slot.getName()), "Equip the " + clothing.getName() + ".", INVENTORY_MENU) {
                                        @Override
                                        public void effects() {
                                            Main.game.getTextEndStringBuilder().append("<p style='text-align:center;'>").append(equipClothingFromGround(Main.game.getPlayer(), slot, Main.game.getPlayer(), clothing)).append("</p>");
                                        }
                                    };
                                } else {
                                    return new Response("Equip: " + Util.capitaliseSentence(slot.getName()), clothing.getCannotBeEquippedText(Main.game.getPlayer(), slot), null);
                    ...
                    case CHARACTER_CREATION:
                        if (index == 1) {
                            ...
                            if (Main.game.getPlayer().getLocationPlace().isItemsDisappear()) {
                                if (!weapon.getWeaponType().isAbleToBeDropped()) {
                                    ...
                                    return new Response("Drop", "Drop your " + weapon.getName() + ".", INVENTORY_MENU) {
                                        @Override
                                        public void effects() {
                                            Main.game.getTextEndStringBuilder().append("<p style='text-align:center;'>").append(Main.game.getPlayer().unequipWeapon(weaponSlot, weapon, true, true)).append("</p>");
                                            resetPostAction();
                                        }
                                    };
                                }
                            ...
                            return new Response("Unequip", "Unequip the " + weapon.getName() + ".", INVENTORY_MENU) {
                                @Override
                                public void effects() {
                                    Main.game.getTextEndStringBuilder().append("<p style='text-align:center;'>").append(Main.game.getPlayer().unequipWeapon(weaponSlot, weapon, false, true)).append("</p>");
                                    resetPostAction();
                                }
                            };
                    ...
                    case CHARACTER_CREATION:
                        ...
                        if (index == 1) {
                            ...
                            if (Main.game.getPlayer().getLocationPlace().isItemsDisappear()) {
                                if (!weapon.getWeaponType().isAbleToBeDropped()) {
                                    ...
                                    return new Response("Drop", UtilText.parse(inventoryNPC, "Get [npc.name] to drop [npc.her] " + weapon.getName() + "."), INVENTORY_MENU) {
                                        @Override
                                        public void effects() {
                                            Main.game.getTextEndStringBuilder().append("<p style='text-align:center;'>").append(inventoryNPC.unequipWeapon(weaponSlot, weapon, true, false)).append("</p>");
                                            resetPostAction();
                                        }
                                    };
                                }
                            ...
                            if (inventoryFull) {
                                ...
                                return new Response("Take",
                                    ...
                                    public void effects() {
                                        ...
                                        Main.game.getTextEndStringBuilder().append("<p style='text-align:center;'>").append(Main.game.getPlayer().addWeapon(weapon, false)).append("</p>");
                                        resetPostAction();
                                    }
                                };
                            }
                            ...
                            return new Response("Unequip", UtilText.parse(inventoryNPC, "Get [npc.name] to unequip [npc.her] " + weapon.getName() + "."), INVENTORY_MENU) {
                                @Override
                                public void effects() {
                                    Main.game.getTextEndStringBuilder().append("<p style='text-align:center;'>").append(inventoryNPC.unequipWeapon(weaponSlot, weapon, false, false)).append("</p>");
                                    resetPostAction();
                                }
                            };
                        ...
                        if (index == 1) {
                            ...
                            if (Main.game.getPlayer().getLocationPlace().isItemsDisappear()) {
                                if (!clothing.getClothingType().isAbleToBeDropped()) {
                                    ...
                                    return new Response((clothing.isDiscardedOnUnequip(slotEquippedTo) ? "Discard" : "Drop"),
                                        ...
                                        @Override
                                        public void effects() {
                                            Main.game.getTextEndStringBuilder().append("<p style='text-align:center;'>").append(unequipClothingToFloor(Main.game.getPlayer(), clothing)).append("</p>");
                                        }
                                    };
                                }
                            ...
                            if (owner.isAbleToUnequip(clothing, true, Main.game.getPlayer())) {
                                return new Response("Unequip", "Unequip the " + clothing.getName() + ".", INVENTORY_MENU) {
                                    @Override
                                    public void effects() {
                                        Main.game.getTextEndStringBuilder().append("<p style='text-align:center;'>").append(unequipClothingToInventory(Main.game.getPlayer(), clothing)).append("</p>");
                                    }
                                };
                        ...
                        if (index == 1) {
                            ...
                            if (inventoryFull) {
                                ...
                                return new Response("Take",
                                    ...
                                    @Override
                                    public void effects() {
                                        Main.game.getTextEndStringBuilder().append("<p style='text-align:center;'>").append(unequipClothingToUnequippersInventory(Main.game.getPlayer(), clothing)).append("</p>");
                                    }
                                };
                            }
                            ...
                            if (clothing.getDisplacedList().contains(clothing.getBlockedPartsKeysAsListWithoutNONE(inventoryNPC, clothing.getSlotEquippedTo()).get(index - 11))) {
                                ...
                                return new Response(Util.capitaliseSentence(clothing.getBlockedPartsKeysAsListWithoutNONE(inventoryNPC, clothing.getSlotEquippedTo()).get(index - 11).getDescription()),
                                    ...
                                    public void effects() {
                                        ...
                                        Main.game.getTextEndStringBuilder().append("<p style='text-align:center;'>").append(owner.getDisplaceDescription()).append("</p>");
                                    }
                                };
                            }
                ...
                return new Response("Dye",
                    ...
                    public void effects() {
                        if (!Main.game.getPlayer().isSpellSchoolSpecialAbilityUnlocked(SpellSchool.EARTH)) {
                            ...
                            Main.game.getTextEndStringBuilder().append("<p style='text-align:center;'>").append(getDyeBrushEffects(clothing, dyePreviews.get(0))).append("</p>").append("<p>").append("<b>The ").append(clothing.getName()).append(" ").append(clothing.getClothingType().isPlural() ? "have been" : "has been").append(" dyed</b>!").append("</p>").append("<p>").append(Main.game.getPlayer().hasItemType(ItemType.DYE_BRUSH) || Main.game.getPlayer().isSpellSchoolSpecialAbilityUnlocked(SpellSchool.EARTH)
                                    ...
                                    : "You have <b>0</b> dye-brushes left!").append("</p>");

                        } else {
                            Main.game.getTextEndStringBuilder().append("<p>" + "Thanks to your proficiency with [style.boldEarth(Earth spells)], you are able to dye the ").append(clothing.getName()).append(" without needing to use a dye-brush!").append("</p>");
                        }
                ...
                return new Response("Dye all (stack)",
                    ...
                    public void effects() {
                        if (!Main.game.getPlayer().isSpellSchoolSpecialAbilityUnlocked(SpellSchool.EARTH)) {
                            ...
                            Main.game.getTextEndStringBuilder().append("<p style='text-align:center;'>").append(getDyeBrushEffects(clothing, dyePreviews.get(0))).append("</p>").append("<p>").append("<b>The ").append(clothing.getName()).append(clothing.getClothingType().isPlural() ? "have" : "has").append(" been dyed</b>!").append("</p>");

                            Main.game.getTextEndStringBuilder().append("<p>You then repeat this for the other ").append(Util.intToString(finalCount - 1)).append(" ").append(clothing.getNamePlural()).append("...</p>");

                            Main.game.getTextEndStringBuilder().append("<p>").append(Main.game.getPlayer().hasItemType(ItemType.DYE_BRUSH) || Main.game.getPlayer().isSpellSchoolSpecialAbilityUnlocked(SpellSchool.EARTH)
                                    ? "You have <b>" + Main.game.getPlayer().getAllItemsInInventory().get(Main.game.getItemGen().generateItem(ItemType.DYE_BRUSH))
                                    + "</b> dye-brush" + (Main.game.getPlayer().getAllItemsInInventory().get(Main.game.getItemGen().generateItem(ItemType.DYE_BRUSH)) == 1 ? "" : "es") + " left!"
                                    : "You have <b>0</b> dye-brushes left!").append("</p>");

                        } else {
                            Main.game.getTextEndStringBuilder().append("<p>" + "Thanks to your proficiency with [style.boldEarth(Earth spells)], you are able to dye ").append(finalCount == 2
                                    ? "both"
                                    : "all " + Util.intToString(finalCount)).append(" of the ").append(clothing.getNamePlural()).append(" without needing to use a single dye-brush!").append("</p>");
                        }
            ...
            if (index == 0) {
                ...
                return new Response("Dye",
                    ...
                    public void effects() {
                        if (!Main.game.getPlayer().isSpellSchoolSpecialAbilityUnlocked(SpellSchool.EARTH)) {
                            Main.game.getPlayer().useItem(Main.game.getItemGen().generateItem(ItemType.DYE_BRUSH), owner, false);
                            Main.game.getTextEndStringBuilder().append("<p style='text-align:center;'>").append(getDyeBrushEffects(weapon, dyePreviews.get(0))).append("</p>").append("<p>").append("<b>The ").append(weapon.getName()).append(" ").append(weapon.getWeaponType().isPlural() ? "have been" : "has been").append(" dyed</b>!").append("</p>").append("<p>").append(Main.game.getPlayer().hasItemType(ItemType.DYE_BRUSH) || Main.game.getPlayer().isSpellSchoolSpecialAbilityUnlocked(SpellSchool.EARTH)
                                    ? "You have <b>" + Main.game.getPlayer().getAllItemsInInventory().get(Main.game.getItemGen().generateItem(ItemType.DYE_BRUSH))
                                    + "</b> dye-brush" + (Main.game.getPlayer().getAllItemsInInventory().get(Main.game.getItemGen().generateItem(ItemType.DYE_BRUSH)) == 1 ? "" : "es") + " left!"
                                    : "You have <b>0</b> dye-brushes left!").append("</p>");

                        } else {
                            Main.game.getTextEndStringBuilder().append("<p>" + "Thanks to your proficiency with [style.boldEarth(Earth spells)], you are able to dye the ").append(weapon.getName()).append(" without needing to use a dye-brush!").append("</p>");
                        }
                ...
                return new Response("Reforge",
                    ...
                    public void effects() {
                        if (!Main.game.getPlayer().isSpellSchoolSpecialAbilityUnlocked(SpellSchool.EARTH)) {
                            ...
                            Main.game.getTextEndStringBuilder().append("<p style='text-align:center;'>").append(getReforgeHammerEffects(weapon, damageTypePreview)).append("</p>").append("<p>").append("<b>The ").append(weapon.getName()).append(" ").append(weapon.getWeaponType().isPlural() ? "have been" : "has been").append(" reforged</b>!").append("</p>").append("<p>").append(Main.game.getPlayer().hasItemType(ItemType.REFORGE_HAMMER) || Main.game.getPlayer().isSpellSchoolSpecialAbilityUnlocked(SpellSchool.EARTH)
                                    ...
                                    : "You have <b>0</b> reforging hammers left!").append("</p>");

                        } else {
                            Main.game.getTextEndStringBuilder().append("<p>" + "Thanks to your proficiency with [style.boldEarth(Earth spells)], you are able to reforge the ").append(weapon.getName()).append(" without needing to use a reforging hammer!").append("</p>");
                        }
                ...
                return new Response("Dye & reforge",
                    ...
                    public void effects() {
                        if (!Main.game.getPlayer().isSpellSchoolSpecialAbilityUnlocked(SpellSchool.EARTH)) {
                            Main.game.getPlayer().useItem(Main.game.getItemGen().generateItem(ItemType.DYE_BRUSH), owner, false);
                            Main.game.getPlayer().useItem(Main.game.getItemGen().generateItem(ItemType.REFORGE_HAMMER), owner, false);
                            Main.game.getTextEndStringBuilder().append("<p style='text-align:center;'>").append(getDyeBrushEffects(weapon, dyePreviews.get(0))).append("</p>").append("<p style='text-align:center;'>").append(getReforgeHammerEffects(weapon, damageTypePreview)).append("</p>").append("<p>").append("<b>The ").append(weapon.getName()).append(" ").append(weapon.getWeaponType().isPlural() ? "have been" : "has been").append(" dyed and reforged</b>!").append("</p>").append("<p>").append(Main.game.getPlayer().hasItemType(ItemType.DYE_BRUSH) || Main.game.getPlayer().isSpellSchoolSpecialAbilityUnlocked(SpellSchool.EARTH)
                                    ? "You have <b>" + Main.game.getPlayer().getAllItemsInInventory().get(Main.game.getItemGen().generateItem(ItemType.DYE_BRUSH))
                                    + "</b> dye-brush" + (Main.game.getPlayer().getAllItemsInInventory().get(Main.game.getItemGen().generateItem(ItemType.DYE_BRUSH)) == 1 ? "" : "es") + " left!"
                                    : "You have <b>0</b> dye-brushes left!").append("<br/>").append(Main.game.getPlayer().hasItemType(ItemType.REFORGE_HAMMER) || Main.game.getPlayer().isSpellSchoolSpecialAbilityUnlocked(SpellSchool.EARTH)
                                    ? "You have <b>" + Main.game.getPlayer().getAllItemsInInventory().get(Main.game.getItemGen().generateItem(ItemType.REFORGE_HAMMER))
                                    + "</b> reforging " + (Main.game.getPlayer().getAllItemsInInventory().get(Main.game.getItemGen().generateItem(ItemType.REFORGE_HAMMER)) == 1 ? "hammer" : "hammers") + " left!"
                                    : "You have <b>0</b> reforging hammers left!").append("</p>");

                        } else {
                            Main.game.getTextEndStringBuilder().append("<p>" + "Thanks to your proficiency with [style.boldEarth(Earth spells)], you are able to dye and reforge the ").append(weapon.getName()).append(" without needing to use a dye-brush or reforging hammer!").append("</p>");
                        }
                ...
                return new Response("Dye all (stack)",
                    ...
                    public void effects() {
                        if (!Main.game.getPlayer().isSpellSchoolSpecialAbilityUnlocked(SpellSchool.EARTH)) {
                            Main.game.getPlayer().removeItem(Main.game.getItemGen().generateItem(ItemType.DYE_BRUSH), finalCount);
                            Main.game.getTextEndStringBuilder().append("<p style='text-align:center;'>").append(getDyeBrushEffects(weapon, dyePreviews.get(0))).append("</p>").append("<p>").append("<b>The ").append(weapon.getName()).append(weapon.getWeaponType().isPlural() ? "have" : "has").append(" been dyed</b>!").append("</p>");

                            Main.game.getTextEndStringBuilder().append("<p>You then repeat this for the other ").append(Util.intToString(finalCount - 1)).append(" ").append(weapon.getNamePlural()).append("...</p>");
                            ...
                            Main.game.getTextEndStringBuilder().append("<p>" + "Thanks to your proficiency with [style.boldEarth(Earth spells)], you are able to dye ").append(finalCount == 2
                                    ? "both"
                                    : "all " + Util.intToString(finalCount)).append(" of the ").append(weapon.getNamePlural()).append(" without needing to use a single dye-brush!").append("</p>");
                        }
                ...
                return new Response("Reforge all (stack)",
                    ...
                    public void effects() {
                        if (!Main.game.getPlayer().isSpellSchoolSpecialAbilityUnlocked(SpellSchool.EARTH)) {
                            ...
                            Main.game.getTextEndStringBuilder().append("<p style='text-align:center;'>").append(getReforgeHammerEffects(weapon, damageTypePreview)).append("</p>").append("<p>").append("<b>The ").append(weapon.getName()).append(weapon.getWeaponType().isPlural() ? "have" : "has").append(" been reforged</b>!").append("</p>");

                            Main.game.getTextEndStringBuilder().append("<p>You then repeat this for the other ").append(Util.intToString(finalCount - 1)).append(" ").append(weapon.getNamePlural()).append("...</p>");

                            Main.game.getTextEndStringBuilder().append("<p>").append(Main.game.getPlayer().hasItemType(ItemType.REFORGE_HAMMER) || Main.game.getPlayer().isSpellSchoolSpecialAbilityUnlocked(SpellSchool.EARTH)
                                    ? "You have <b>" + Main.game.getPlayer().getAllItemsInInventory().get(Main.game.getItemGen().generateItem(ItemType.REFORGE_HAMMER))
                            ...
                            Main.game.getTextEndStringBuilder().append("<p>" + "Thanks to your proficiency with [style.boldEarth(Earth spells)], you are able to reforge ").append(finalCount == 2
                                    ? "both"
                                    : "all " + Util.intToString(finalCount)).append(" of the ").append(weapon.getNamePlural()).append(" without needing to use a single reforging hammer!").append("</p>");
                        }
                ...
                return new Response("Dye & reforge all (stack)",
                    ...
                    public void effects() {
                        if (!Main.game.getPlayer().isSpellSchoolSpecialAbilityUnlocked(SpellSchool.EARTH)) {
                            Main.game.getPlayer().removeItem(Main.game.getItemGen().generateItem(ItemType.DYE_BRUSH), finalCount);
                            Main.game.getPlayer().removeItem(Main.game.getItemGen().generateItem(ItemType.REFORGE_HAMMER), finalCount);
                            Main.game.getTextEndStringBuilder().append("<p style='text-align:center;'>").append(getDyeBrushEffects(weapon, dyePreviews.get(0))).append("</p>").append("<p style='text-align:center;'>").append(getReforgeHammerEffects(weapon, damageTypePreview)).append("</p>").append("<p>").append("<b>The ").append(weapon.getName()).append(weapon.getWeaponType().isPlural() ? "have" : "has").append(" been dyed and reforged</b>!").append("</p>");

                            Main.game.getTextEndStringBuilder().append("<p>You then repeat this for the other ").append(Util.intToString(finalCount - 1)).append(" ").append(weapon.getNamePlural()).append("...</p>");

                            Main.game.getTextEndStringBuilder().append("<p>").append(Main.game.getPlayer().hasItemType(ItemType.DYE_BRUSH)
                                    ? "You have <b>" + Main.game.getPlayer().getAllItemsInInventory().get(Main.game.getItemGen().generateItem(ItemType.DYE_BRUSH))
                                    + "</b> dye-brush" + (Main.game.getPlayer().getAllItemsInInventory().get(Main.game.getItemGen().generateItem(ItemType.DYE_BRUSH)) == 1 ? "" : "es") + " left!"
                                    : "You have <b>0</b> dye-brushes left!").append("</p>");

                            Main.game.getTextEndStringBuilder().append("<p>").append(Main.game.getPlayer().hasItemType(ItemType.REFORGE_HAMMER)
                                    ? "You have <b>" + Main.game.getPlayer().getAllItemsInInventory().get(Main.game.getItemGen().generateItem(ItemType.REFORGE_HAMMER))
                                    + "</b> reforging hammer" + (Main.game.getPlayer().getAllItemsInInventory().get(Main.game.getItemGen().generateItem(ItemType.REFORGE_HAMMER)) == 1 ? "" : "s") + " left!"
                                    : "You have <b>0</b> reforging hammers left!").append("</p>");
                            ...
                            Main.game.getTextEndStringBuilder().append("<p>" + "Thanks to your proficiency with [style.boldEarth(Earth spells)], you are able to dye ").append(finalCount == 2
                                    ? "both"
                                    : "all " + Util.intToString(finalCount)).append(" of the ").append(weapon.getNamePlural()).append(" without needing to use a single dye-brush or reforging hammer!").append("</p>");
                        }
                ...
                return new Response("Dye & reforge all",
                    ...
                    public void effects() {
                        if (!Main.game.getPlayer().isSpellSchoolSpecialAbilityUnlocked(SpellSchool.EARTH)) {
                            ...
                            Main.game.getTextEndStringBuilder().append("<p>" + "Thanks to your proficiency with [style.boldEarth(Earth spells)], you are able to dye and reforge ").append(finalCount == 2
                                    ? "both"
                                    : "all " + Util.intToString(finalCount)).append(" of the ").append(weapon.getNamePlural()).append(" without needing to use a single dye-brush or reforging hammer!").append("</p>");
                        }
                ...
                return new Response("Dye & reforge",
                    ...
                    public void effects() {
                        if (!Main.game.getPlayer().isSpellSchoolSpecialAbilityUnlocked(SpellSchool.EARTH)) {
                            Main.game.getPlayer().useItem(Main.game.getItemGen().generateItem(ItemType.DYE_BRUSH), owner, false);
                            Main.game.getPlayer().useItem(Main.game.getItemGen().generateItem(ItemType.REFORGE_HAMMER), owner, false);
                            Main.game.getTextEndStringBuilder().append("<p style='text-align:center;'>").append(getDyeBrushEffects(weapon, dyePreviews.get(0))).append("</p>").append("<p style='text-align:center;'>").append(getReforgeHammerEffects(weapon, damageTypePreview)).append("</p>").append("<p>").append("<b>The ").append(weapon.getName()).append(" ").append(weapon.getWeaponType().isPlural() ? "have been" : "has been").append(" reforged</b>!").append("</p>").append("<p>").append(Main.game.getPlayer().hasItemType(ItemType.DYE_BRUSH) || Main.game.getPlayer().isSpellSchoolSpecialAbilityUnlocked(SpellSchool.EARTH)
                                    ? "You have <b>" + Main.game.getPlayer().getAllItemsInInventory().get(Main.game.getItemGen().generateItem(ItemType.DYE_BRUSH))
                                    + "</b> dye-brush" + (Main.game.getPlayer().getAllItemsInInventory().get(Main.game.getItemGen().generateItem(ItemType.DYE_BRUSH)) == 1 ? "" : "es") + " left!"
                                    : "You have <b>0</b> dye-brushes left!").append("<br/>").append(Main.game.getPlayer().hasItemType(ItemType.REFORGE_HAMMER) || Main.game.getPlayer().isSpellSchoolSpecialAbilityUnlocked(SpellSchool.EARTH)
                                    ? "You have <b>" + Main.game.getPlayer().getAllItemsInInventory().get(Main.game.getItemGen().generateItem(ItemType.REFORGE_HAMMER))
...
//						weapon.setSecondaryColour(dyePreviewSecondary);
    ...
    private static void sellClothing(GameCharacter from, GameCharacter to, AbstractClothing clothing, int count, int itemPrice) {
        ...
        if (!to.isPlayer() || !to.isInventoryFull() || to.hasClothing(clothing) || clothing.getRarity() == Rarity.QUEST) {
            ...
            if (buyback && to.isPlayer()) {
                Main.game.getPlayer().addClothing(new AbstractClothing(clothing) {
                }, count, false, true);
                Main.game.getPlayer().getBuybackStack().get(buyBackIndex).incrementCount(-count);
                if (Main.game.getPlayer().getBuybackStack().get(buyBackIndex).getCount() <= 0) {
                    Main.game.getPlayer().getBuybackStack().remove(buyBackIndex);
                ...
                if (from.isPlayer()) {
                    ...
                    to.addClothing(new AbstractClothing(clothing) {
                    }, count, false, true);
                }
                from.removeClothing(clothing, count);
            }
