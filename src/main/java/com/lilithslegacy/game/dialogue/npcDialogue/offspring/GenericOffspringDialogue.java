package com.lilithslegacy.game.dialogue.npcDialogue.offspring;
...
public class GenericOffspringDialogue {
    ...
    private static String getStatus() {
        ...
        if (tokens >= 1) {
            sb.append("[npc.Name] only has time for [style.italicsMinorBad(").append(Util.intToString(tokens)).append(" more thing").append(tokens > 1 ? "s" : "").append(")] to discuss.");
        } else {
    ...
    public static final DialogueNode OFFSPRING_ENCOUNTER_CHOOSE_NAME = new DialogueNode("", "", true) {
        ...
        public String getContent() {
            ...
            UtilText.nodeContentSB.append("<p>" + "You decide to ask [npc.name] to call you by a different name." + " At the moment, [npc.sheIs] calling you '[npc.pcName]'." + "</p>"

                    // TODO align this properly

                    + "<div class='container-full-width' style='text-align:center;'>" + "<div style='position:relative; display: inline-block; padding:0 auto; margin:0 auto;vertical-align:middle;width:100%;'>" + "<p style='float:left; padding:0; margin:0; height:32px; line-height:32px;'>[npc.Name] will call you: </p>" + "<form style='float:left; padding:auto 0 auto 0;'><input type='text' id='offspringPetNameInput' value='").append(UtilText.parseForHTMLDisplay(offspring().getPetName(Main.game.getPlayer()))).append("'></form>").append(" <div class='SM-button' id='").append(offspring().getId()).append("_PET_NAME' style='float:left; width:auto; height:28px;'>").append("Rename").append("</div>").append("</div>").append("<p>").append("<i>The names 'Mom'/'Dad' and 'Mommy'/'Daddy' are special, and will automatically switch to the appropriate femininity of your character.</i>").append("</p>").append("</div>").append("<p id='hiddenFieldName' style='display:none;'></p>");
