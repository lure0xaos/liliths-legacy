...
public class ResponseSex extends Response {
    ...
    public String getTooltipText() {
        if (sexManager != null && sexManager.getStartingSexPaceModifier(Main.game.getPlayer()) != null) {
            ...
            sb.append("<br/><i>Starts sex in the '<span style='color:").append(pace.getColour().toWebHexString()).append(";'>").append(pace.getName()).append("</span>' pace.</i>");
            return sb.toString();
        ...
        if (isFromExternalFile) {
            if (isUsingExternalManager) {
                ...
                if (sexStartFolderPath != null && !sexStartFolderPath.isEmpty()) {
                    this.sexStartDescription = UtilText.parseFromXMLFile(new ArrayList<>(), FS.newPath("/res"), sexStartFolderPath, sexStartDialogueTag, new ArrayList<>());
                }
