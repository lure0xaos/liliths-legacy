...
import com.lilithslegacy.game.character.npc.dominion.EnforcerWarehouseGuard;
...
import com.lilithslegacy.game.sex.managers.dominion.SMClaireWarehouse;
        ...
        public String getContent() {
            ...
            if (isMainEntrance) {
                ...
                if (!dangerousDirections.isEmpty()) {
                    sb.append("<p style='test-align:center; color:").append(PresetColour.GENERIC_BAD.toWebHexString()).append(";'><i>");
                    if (dangerousDirections.size() > 1) {
                        sb.append("There are manned guard posts to the ").append(Util.stringsToStringList(dangerousDirections, false)).append("! Entering these areas will surely result in a fight!");
                    } else {
                        sb.append("There is a manned guard post to the ").append(Util.stringsToStringList(dangerousDirections, false)).append("! Entering that area will surely result in a fight!");
                    }
                    sb.append("</i></p>");
                }
            }
            ...
            if (index == 1) {
                return new Response("Learn spell", "Skim through the teleport spell book and try to learn it as quickly as possible.", ENCLOSURE_SHELVING_DRAG_TELEPORT_LEARNED) {
                    ...
                    public void effects() {
                        ...
                        Main.game.getTextStartStringBuilder().append("<span style='border:0; padding:0; text-align:center;'><i>").append(Main.game.getPlayer().useItem(Main.game.getItemGen().generateItem(ItemType.getSpellBookType(Spell.TELEPORT)), Main.game.getPlayer(), true)).append("</i></span>");
            ...
            if (index == 0) {
                ...
                return new Response("Lightning globe",
                    ...
                    public void effects() {
                        ...
                        Main.game.getTextEndStringBuilder().append("<p style='text-align:center;'><i>You [style.colourBad(lose)] <b>").append(Main.game.getPlayer().getMana()).append("</b> [style.colourAura(aura)]!</i></p>");
