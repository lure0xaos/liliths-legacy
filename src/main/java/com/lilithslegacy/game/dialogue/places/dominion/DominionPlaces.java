...
import com.lilithslegacy.game.dialogue.companions.OccupantDialogue;
import com.lilithslegacy.game.dialogue.npcDialogue.dominion.CultistDialogue;
import com.lilithslegacy.game.dialogue.npcDialogue.dominion.ReindeerOverseerDialogue;
import com.lilithslegacy.game.dialogue.npcDialogue.dominion.RentalMommyDialogue;
import com.lilithslegacy.game.dialogue.places.dominion.helenaHotel.HelenaHotel;
import com.lilithslegacy.game.dialogue.places.dominion.nyansApartment.NyanDateFinalRepeat;
import com.lilithslegacy.game.dialogue.places.dominion.nyansApartment.NyanFirstDate;
import com.lilithslegacy.game.dialogue.places.dominion.nyansApartment.NyanFirstDoubleDate;
import com.lilithslegacy.game.dialogue.places.dominion.nyansApartment.NyanRepeatDate;
...
public class DominionPlaces {
    ...
    public static final DialogueNode BACK_ALLEYS = new DialogueNode("", "", false) {
        ...
        public String getContent() {
            ...
            for (NPC npc : Main.game.getNonCompanionCharactersPresent()) {
                UtilText.nodeContentSB.append(npc.getPresentInTileDescription());
            }
    ...
    public static final DialogueNode DARK_ALLEYS = new DialogueNode("Dark Alleyways", "", false) {
        ...
        public String getContent() {
            ...
            for (NPC npc : Main.game.getNonCompanionCharactersPresent()) {
                UtilText.nodeContentSB.append(npc.getPresentInTileDescription(false));
            }
                ...
                if (Main.game.getCurrentWeather() == Weather.MAGIC_STORM) {
                    ...
                    return new Response(
                        ...
                        public void effects() {
                            ...
                            Main.game.getTextEndStringBuilder().append("<p>").append(UtilText.returnStringAtRandom(
                                    ...
                                    Util.capitaliseSentence(UtilText.generateSingularDeterminer(randomFemalePerson)) + " " + randomFemalePerson + " is currently reading out a list of advertisements for shops in the local area."
                                            + " There's really nothing of interest to be heard, and you soon find yourself turning away and moving on.")).append("</p>");
                        }
                    };
                }
            } else {
    ...
    private static String getExtraStreetFeatures() {
        ...
        if (Main.game.getPlayerCell().getPlace().getPlaceType() == PlaceType.DOMINION_NYAN_APARTMENT) {
            mommySB.append("<p>" + "[style.boldPinkLight(Nyan's Apartment:)]<br/>").append(Main.game.getNpc(Nyan.class).getWorldLocation() == WorldType.NYANS_APARTMENT
                    ? "Nyan lives in this area, and so if you wanted to, you could head over to her apartment building and pay her a visit..."
                    : "Nyan lives in this area, although you know that she'll be at work at this hour, so there's not much point in heading over to her apartment building...").append("</p>");
        }
        ...
        for (NPC npc : characters) {
            ...
            if (npc instanceof Cultist) {
                cultistSB.append("<p>" + "<b style='color:").append(PresetColour.GENERIC_ARCANE.toWebHexString()).append(";'>Cultist's Chapel:</b><br/>").append(UtilText.parse(npc, "You remember that [npc.namePos] chapel is near here, and if you were so inclined, you could easily find it again...")).append("</p>");
                break;
            }

            if (npc instanceof ReindeerOverseer) {
                reindeerSB.append("<p>" + "<b style='color:").append(PresetColour.RACE_REINDEER_MORPH.toWebHexString()).append(";'>Reindeer Workers:</b><br/>").append(Main.game.getCurrentWeather() == Weather.MAGIC_STORM
                        ...
                        + " Although the workers look to be far too busy to stop and talk, you'd probably be able to catch a word with the overseer if you wanted to.")).append("</p>");
                break;
            }
        }
