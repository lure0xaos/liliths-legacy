...
public class LilayaSpa {
    ...
    private static String getReceptionSlavesDescription(List<GameCharacter> slavesReception) {
        ...
        sb.append("<p>");
        if (soloSlave) {
            sb.append("Having been instructed to work as a receptionist at your private spa, your slave, ").append(Util.stringsToStringList(names, false)).append(", is standing behind the counter.");
        } else {
            sb.append("Having been instructed to work as receptionists at your private spa, your slaves, ").append(Util.stringsToStringList(names, false)).append(", are standing behind the counter.");
        }
        sb.append("</p>");
        ...
        List<GameCharacter> nice = slavesReception.stream().filter(npc -> npc.getObedienceBasic() == ObedienceLevelBasic.OBEDIENT || npc.getAffectionLevelBasic(Main.game.getPlayer()) == AffectionLevelBasic.LIKE).toList();
    ...
    private static String getShowerSlavesDescription(List<GameCharacter> slavesWashing) {
        ...
        List<GameCharacter> nice = slavesWashing.stream().filter(npc -> npc.getObedienceBasic() == ObedienceLevelBasic.OBEDIENT || npc.getAffectionLevelBasic(Main.game.getPlayer()) == AffectionLevelBasic.LIKE).toList();
        for (GameCharacter npc : nice) {
            SlavePermissionSetting behaviour = npc.getSlavePermissionSettings().get(SlavePermission.BEHAVIOUR).iterator().next();
            boolean mute = npc.isMute() || npc.hasSlavePermissionSetting(SlavePermissionSetting.GENERAL_SILENCE);
            // Populate greeting and speech maps if empty:
    ...
    public static final DialogueNode SPA_CORE = new DialogueNode("", "", false) {
        ...
        public void applyPreParsingEffects() {
            try {
                ...
                System.getLogger("").log(System.Logger.Level.ERROR, "Clothes cleaning in SPA_CORE failed!");
            }
        }
        ...
        public Response getResponse(int responseTab, int index) {
            ...
            if (isGuestPresent()) {
                ...
                if (index == 1) {
                    ...
                    if (slaves.stream().noneMatch(npc -> npc.hasSlaveJobSetting(SlaveJob.SPA, SlaveJobSetting.SPA_MASSAGE))) {
        ...
        if (soloSlave) {
            ...
            if (!bathingNames.isEmpty()) {
                ...
                if (bathingNames.size() == 1) {
                    ...
                    sb.append("As you have instructed them to join you whenever you bathe in the spa, you can be sure that ").append(Util.stringsToStringList(bathingNames, false)).append(" will accompany you if you were to slip into one of the pools.");
                }
            }
            ...
            if (!massageNames.isEmpty()) {
                ...
                if (massageNames.size() == 1) {
                    ...
                    sb.append(Util.stringsToStringList(bathingNames, false)).append(" have been given some training in how to perform a massage,").append(" so if you wanted one of these slaves to give you one, all you'd need to do is lie down on one of the loungers and call one of them over.");
                }
            }
        }
        sb.append("</p>");
...
//				UtilText.addSpecialParsingString(String.valueOf(slavesSexNoNulls.size()), true);
                ...
                if (slavesSexNoNulls.stream().noneMatch(s -> s.isAttractedTo(Main.game.getPlayer()))) {
                    return new Response("Submissive sex",
                            slavesSexNoNulls.size() == 1
