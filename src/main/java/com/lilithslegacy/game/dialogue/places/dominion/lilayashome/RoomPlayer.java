...
import com.lilithslegacy.game.sex.managers.universal.SMBath;
import com.lilithslegacy.game.sex.managers.universal.SMGeneric;
import com.lilithslegacy.game.sex.managers.universal.SMShower;
...
//						companions.removeIf((c) -> !c.isPlayer() && (!c.isSlave() || !c.getOwner().isPlayer()));
    ...
    public static final DialogueNode ROOM = new DialogueNode("Your Room", "", false) {
        ...
        public String getContent() {
            ...
            if (!charactersPresent.isEmpty()) {
                ...
                sb.append("<p>" + "Assigned to your room ").append(soloSlave ? "is your slave" : "are your slaves").append("; ").append(Util.stringsToStringList(names, false)).append(".");

                List<NPC> greetings = charactersPresent.stream().filter(npc -> npc.hasSlaveJobSetting(SlaveJob.BEDROOM, SlaveJobSetting.BEDROOM_GREETING)).toList();
                names = new ArrayList<>();
                ...
                if (!greetings.isEmpty()) {
                    sb.append(" Having been instructed to greet you upon your arrival, ").append(soloSlave
                            ? UtilText.parse(charactersPresent.get(0), "[npc.she] steps forwards and welcomes you back.")
                            : Util.stringsToStringList(names, false) + " step forwards and welcome you back.").append("</p>");

                    List<NPC> greetingsNice = greetings.stream().filter(npc -> npc.getObedienceBasic() == ObedienceLevelBasic.OBEDIENT || npc.getAffectionLevelBasic(Main.game.getPlayer()) == AffectionLevelBasic.LIKE).toList();
    ...
    private static String getShowerSlavesDescription(List<GameCharacter> slavesWashing) {
        ...
        if (soloSlave) {
            ...
            sb.append("Having been instructed to assist you in washing yourself, your slaves, ").append(Util.stringsToStringList(names, false)).append(", similarly leave their clothes by the door before following you into the bathroom.");
            sb.append(UtilText.parse(slavesWashing,
        ...
        List<GameCharacter> washingNice = slavesWashing.stream().filter(npc -> npc.getObedienceBasic() == ObedienceLevelBasic.OBEDIENT || npc.getAffectionLevelBasic(Main.game.getPlayer()) == AffectionLevelBasic.LIKE).toList();
    ...
    private static String getCalendarImageDescription(Month month) {
        ...
        sb.append("<p>" + "Flicking through the calendar until you're looking at the page for ").append(month.getDisplayName(TextStyle.FULL, Locale.ENGLISH)).append(", you see that this month's image is now of ");
        ...
        if (Util.random.nextInt() < 15) {
            ...
            switch (month) {
                case JANUARY:
                    if (Main.game.getPlayer().getSexualOrientation() == SexualOrientation.ANDROPHILIC) {
                        sb.append("a toned ").append(Subspecies.RAT_MORPH.getSingularMaleName(null)).append(", who's grinning mischievously at you while stroking his fat, erect cock.");
                    } else {
                        sb.append("a horny ").append(Subspecies.RAT_MORPH.getSingularFemaleName(null)).append(", who's bent over a table in order to present her dripping pussy to you.");
                    }
                    break;
                case FEBRUARY:
                    if (Main.game.getPlayer().getSexualOrientation() == SexualOrientation.ANDROPHILIC) {
                        sb.append("a topless ").append(Subspecies.COW_MORPH.getSingularMaleName(null)).append(".").append(" His huge muscles are flexing as he carries a felled tree over one shoulder, while between his legs, you can't help but notice that he's got a massive bulge pressing out against the fabric of his shorts.");
                    } else {
                        sb.append("a black-and-white ").append(Subspecies.COW_MORPH.getSingularFemaleName(null)).append(", who's sitting on a small milking stool.").append(" With a happy smile on her face, she's busily pinching and tugging at her engorged nipples, causing a stream of milk to flow out into a metal bucket.");
                    }
                    break;
                case MARCH:
                    if (Main.game.getPlayer().getSexualOrientation() == SexualOrientation.ANDROPHILIC) {
                        sb.append("a fierce-looking ").append(Subspecies.getSubspeciesFromId("innoxia_panther_subspecies_tiger").getSingularMaleName(null)).append(".").append(" Striking a dominant pose, he's flashing you a toothy grin, clearly excited by the fact that his huge feline cock is fully on display.");
                    } else {
                        sb.append("a fierce-looking ").append(Subspecies.getSubspeciesFromId("innoxia_panther_subspecies_tiger").getSingularFemaleName(null)).append(".").append(" Striking a dominant pose, she's flashing you a toothy grin, clearly excited by the fact that her large breasts and tight pussy are fully on display.");
                    }
                    break;
                case APRIL:
                    if (Main.game.getPlayer().getSexualOrientation() == SexualOrientation.ANDROPHILIC) {
                        sb.append("a handsome ").append(Subspecies.RABBIT_MORPH.getSingularMaleName(null)).append(", who's holding his massive cock in one hand while giving you a suggestive wink.");
                    } else {
                        sb.append("three blushing ").append(Subspecies.RABBIT_MORPH.getPluralFemaleName(null)).append(", who are down on all fours, side-by-side, presenting their pussies to you.");
                    }
                    break;
                case MAY:
                    if (Main.game.getPlayer().getSexualOrientation() == SexualOrientation.ANDROPHILIC) {
                ...
                case JULY:
                    if (Main.game.getPlayer().getSexualOrientation() == SexualOrientation.ANDROPHILIC) {
                        sb.append("an impressively-endowed ").append(Subspecies.HORSE_MORPH.getSingularMaleName(null)).append(", who's flexing his muscles as he presents his fully-erect flared cock to you.");
                    } else {
                        sb.append("a fit ").append(Subspecies.HORSE_MORPH.getSingularFemaleName(null)).append(", who's leaning against a fence, flicking her tail to one side in order to present her animalistic-pussy to you.");
                    }
                    break;
                case AUGUST:
                    if (Main.game.getPlayer().getSexualOrientation() == SexualOrientation.ANDROPHILIC) {
                ...
                case OCTOBER:
                    if (Main.game.getPlayer().getSexualOrientation() == SexualOrientation.ANDROPHILIC) {
                        sb.append("a fit, handsome ").append(Subspecies.DEMON.getSingularMaleName(null)).append(", who's suggestively winking at you as he runs his fingers over his huge, erect cock.");
                    } else {
                        sb.append("a fit, beautiful ").append(Subspecies.DEMON.getSingularFemaleName(null)).append(", wearing nothing but a witch's hat, who's suggestively winking at you as she runs her fingers over her wet pussy and huge breasts.");
                    }
                    break;
                case NOVEMBER:
                    if (Main.game.getPlayer().getSexualOrientation() == SexualOrientation.ANDROPHILIC) {
                        sb.append("an energetic-looking ").append(Subspecies.DOG_MORPH.getSingularMaleName(null)).append(", who's smiling at you as he strokes his erect, knotted dog-cock.");
                    } else {
                        sb.append("an excited-looking ").append(Subspecies.DOG_MORPH.getSingularFemaleName(null)).append(", who's down on all fours, raising her hips in order to present you with her wet pussy.");
                    }
                    break;
                case DECEMBER:
                    if (Main.game.getPlayer().getSexualOrientation() == SexualOrientation.ANDROPHILIC) {
    ...
    public static final DialogueNode AUNT_HOME_PLAYERS_ROOM_SLEEP = new DialogueNode("Your Room", "", false) {
        ...
        public String getContent() {
            ...
            if (!slavesPresentWhenGoingToSleep.isEmpty()) {
                ...
                List<String> names = new ArrayList<>();
                slavesPresentWhenGoingToSleep.forEach((npc) -> names.add(npc.getName()));

                sb.append("<p>" + "Feeling tired and in need of some rest, you head over to your bed and collapse down onto the mattress, before pulling back the covers and slipping beneath them." + " Realising that you want to go to sleep, ").append(Util.stringsToStringList(names, false)).append(soloSlave ? " quickly draws the curtains" : " quickly draw the curtains").append(", before stepping back over towards you.").append("</p>");

                // Sleeping arrangements:
                List<GameCharacter> floorSlaves = slavesPresentWhenGoingToSleep.stream().filter((npc) -> npc.hasSlaveJobSetting(SlaveJob.BEDROOM, SlaveJobSetting.BEDROOM_SLEEP_FLOOR)).toList();
                List<GameCharacter> onBedSlaves = slavesPresentWhenGoingToSleep.stream().filter((npc) -> npc.hasSlaveJobSetting(SlaveJob.BEDROOM, SlaveJobSetting.BEDROOM_SLEEP_ON_BED)).toList();
                if (!floorSlaves.isEmpty() || !onBedSlaves.isEmpty()) {
                    sb.append("<p>");
                    boolean soloFloor = floorSlaves.size() == 1;
                    List<String> floorNames = new ArrayList<>();
                    floorSlaves.forEach((npc) -> floorNames.add(npc.getName()));
                    ...
                    if (!onBedSlaves.isEmpty()) {
                        ...
                        onBedSlaves.forEach((npc) -> onBedNames.add(npc.getName()));
                        if (!floorSlaves.isEmpty()) {
                            sb.append(soloOnBed
                ...
                List<GameCharacter> inBedSlaves = slavesPresentWhenGoingToSleep.stream().filter((npc) -> npc.hasSlaveJobSetting(SlaveJob.BEDROOM, SlaveJobSetting.BEDROOM_SLEEP_IN_BED)).toList();
                if (!inBedSlaves.isEmpty()) {
                    sb.append("<p>");
                    boolean soloInBed = inBedSlaves.size() == 1;
                    List<String> inBedNames = new ArrayList<>();
                    inBedSlaves.forEach((npc) -> inBedNames.add(npc.getName()));
                    sb.append(soloInBed
                    ...
                    List<GameCharacter> niceSlaves = inBedSlaves.stream().filter(npc -> npc.getObedienceBasic() == ObedienceLevelBasic.OBEDIENT || npc.getAffectionLevelBasic(Main.game.getPlayer()) == AffectionLevelBasic.LIKE).toList();
...
//				boolean soloSlave = slavesToWakePlayer.size()==1;
                ...
                if (!slavesToWakePlayer.isEmpty()) {
                    List<String> names = new ArrayList<>();
                    slavesToWakePlayer.forEach((npc) -> names.add(npc.getName()));
...
//							+ "</p>");
                    ...
                    if (hornySlaves.isEmpty()) {
                        List<String> names = new ArrayList<>();
                        slavesPresentWhenWaking.forEach((npc) -> names.add(npc.getName()));
                        sb.append("<p>" + "<i>Beep-beep... beep-beep... bee-</i>" + "</p>" + "<p>" + "Rolling over, you fumble for your phone, turning off the alarm before sinking back onto your bed with a sigh." + " As comfortable as you are, you decide that you'd better get out of bed, and so, after a minute of lying beneath the covers, you do just that." + " With a satisfied yawn and stretch of your [pc.arms], you allow ").append(Util.stringsToStringList(names, false)).append(" to assist you as you gather your things, and with one final stretch, you get ready to set out once more...").append("</p>");
                    }
                }
                ...
                if (!hornySlaves.isEmpty()) {
                    ...
                    hornySlaves.forEach((npc) -> hornyNames.add(npc.getName()));
        ...
        public Response getResponse(int responseTab, int index) {
            ...
            if (!hornySlaves.isEmpty()) {
                if (index == 1) {
                    ...
                    List<String> names = new ArrayList<>();
                    hornySlaves.forEach((npc) -> names.add(npc.getName()));
    ...
    public static final DialogueNode AUNT_HOME_PLAYERS_ROOM_QUICK_SHOWER = new DialogueNode("Your Room", "", true) {
        ...
        public Response getResponse(int responseTab, int index) {
            ...
            if (index == 1) {
                ...
                if (slavesWashing.stream().noneMatch(s -> s.isAttractedTo(Main.game.getPlayer()))) {
    ...
    public static final DialogueNode AUNT_HOME_PLAYERS_ROOM_CALENDAR = new DialogueNode("Calendar", "", true) {
        ...
        public void applyPreParsingEffects() {
            ...
            sb.append(" and see that it's the <b style='color:").append(PresetColour.BASE_BLUE_LIGHT.toWebHexString()).append(";'>").append(Units.date(Main.game.getDateNow(), Units.DateType.LONG)).append("</b>. From a quick calculation ").append(Main.game.getPlayer().getAttributeValue(Attribute.MAJOR_ARCANE) < IntelligenceLevel.ONE_AVERAGE.getMaximumValue() ? "(with some help from your phone's calculator)" : "").append(", you figure out that it's been <b style='color:").append(PresetColour.GENERIC_EXCELLENT.toWebHexString()).append(";'>").append(Main.game.getDayNumber()).append(" day").append(Main.game.getDayNumber() > 1 ? "s" : "").append("</b> since you appeared in this world.").append("</p>");

            if (!Main.game.getDialogueFlags().values.contains(DialogueFlagValue.knowsDate)) {
                sb.append("<p>" + "[pc.thought(Wait... ").append(Main.game.getDateNow().format(DateTimeFormatter.ofPattern("yyyy", Locale.ENGLISH))).append("?! I need to check in with Lilaya about that...)]").append("</p>");
            }
...
//			}
    ...
    public static final DialogueNode AUNT_HOME_PLAYERS_ROOM_CALENDAR_MAY = new DialogueNode("Calendar", "", true) {
        ...
        public String getContent() {
            ...
            UtilText.nodeContentSB.append("<h4 style='text-align:center;'>" + "<span style='color:").append(PresetColour.BASE_BLUE_LIGHT.toWebHexString()).append(";'>May</span>").append("</h4>").append("<h6 style='text-align:center;'>").append("<span style='color:").append(PresetColour.BASE_PINK_LIGHT.toWebHexString()).append(";'>Mother's Week</span>").append("<br/>").append("8th-14th May").append("</h6>").append("<p><i>").append("The second week of May is a time in which to celebrate mothers, motherhood, and the nature of the maternal bond between mother and child.").append(" During this time, fertility-enhancing consumables are generously provided free of charge for all residents of Dominion, and are handed out by volunteers down the main boulevards.").append(" In this way, Lilith shows her love for mothers, and ensures that many more will be made!").append("</i></p>");
    ...
    public static final DialogueNode AUNT_HOME_PLAYERS_ROOM_CALENDAR_JUNE = new DialogueNode("Calendar", "", true) {
        ...
        public String getContent() {
            ...
            UtilText.nodeContentSB.append("<h4 style='text-align:center;'>" + "<span style='color:").append(PresetColour.BASE_BLUE_LIGHT.toWebHexString()).append(";'>June</span>").append("</h4>").append("<h6 style='text-align:center;'>").append("<span style='color:").append(PresetColour.BASE_BLUE.toWebHexString()).append(";'>Father's Week</span>").append("<br/>").append("15th-21st June").append("</h6>").append("<p><i>").append("The third week of June is a time in which to celebrate fathers, fatherhood, and the nature of the paternal bond between father and child.").append(" During this time, fertility-enhancing consumables are generously provided free of charge for all residents of Dominion, and are handed out by volunteers down the main boulevards.").append(" In this way, Lilith shows her love for fathers, and ensures that many more will be made!").append("</i></p>");
    ...
    public static final DialogueNode AUNT_HOME_PLAYERS_ROOM_CALENDAR_OCTOBER = new DialogueNode("Calendar", "", true) {
        ...
        public String getContent() {
            ...
            UtilText.nodeContentSB.append("<h4 style='text-align:center;'>" + "<span style='color:").append(PresetColour.BASE_BLUE_LIGHT.toWebHexString()).append(";'>October</span>").append("</h4>").append("<h6 style='text-align:center;'>").append("<span style='color:").append(PresetColour.GENERIC_ARCANE.toWebHexString()).append(";'>Lilith's Month</span>").append("<br/>").append("All Month").append("</h6>").append("<p><i>").append("October was chosen by Lilith herself to be the month in which all of Dominion shows their devotion towards their glorious queen!").append(" Banners and ribbons, typically in Lilith's traditional colours of orange, purple, and black, are proudly flown from every building, in order to show our queen just how devoted her subjects are!").append(" While all citizens are expected to celebrate Lilith's rule, the most devout of her followers dress up in traditional demonic costumes in order to prove their loyalty.").append("</p>").append("<p>").append("The officially sanctioned 'Cult of Lilith' is the most fanatical group of our queen's supporters, and are very easy to spot during October,").append(" as they refuse to wear anything but traditional witch's outfits, of the sort worn by Lilith herself in centuries past.").append(" While content to carry out their acts of devotion in private for the rest of the year, these cultists can get quite zealous during October,").append(" and will sometimes even go so far as to approach members of the public and demand a display of loyalty from them!").append("</i></p>");
    ...
    public static final DialogueNode AUNT_HOME_PLAYERS_ROOM_CALENDAR_DECEMBER = new DialogueNode("Calendar", "", true) {
        ...
        public String getContent() {
            ...
            UtilText.nodeContentSB.append("<h4 style='text-align:center;'>" + "<span style='color:").append(PresetColour.BASE_BLUE_LIGHT.toWebHexString()).append(";'>December</span>").append("</h4>").append("<h6 style='text-align:center;'>").append("<span style='color:").append(PresetColour.BASE_GOLD.toWebHexString()).append(";'>Yuletide</span>").append("<br/>").append("All Month").append("</h6>").append("<i>").append("<p>").append("The celebration of Yuletide is held throughout the month of December, and sometimes even drags on through January and February!").append(" Giving gifts, holding feasts, and throwing parties are the ways in which Yuletide is celebrated.").append(" As this celebration coincides with the arrival of the reindeer-morphs in Dominion, it has become tradition for the gifts given during Yuletide to be items purchased from these reindeer-morphs.").append("</p>").append("<p>").append("The figure associated with this season is the Lilin 'J&oacute;lnir' (meaning 'the Yule one', or 'Yule figure').").append(" Not much is known about this Lilin, other than the obvious fact that their name breaks with the tradition of all Lilin's names beginning with an 'L', and that they are the leader of the 'Wild Hunt'.").append("</p>").append("<p>").append("Consisting of a wandering horde of summoned arcane elementals, the 'Wild Hunt' was driven away from Dominion many years ago, and is now only found during Yuletide out in the Foloi Fields and the forests nearby.").append("</p>").append("</i>");
