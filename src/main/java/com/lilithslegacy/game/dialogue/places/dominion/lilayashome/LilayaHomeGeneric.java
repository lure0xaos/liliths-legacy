...
import com.lilithslegacy.game.sex.managers.dominion.SMRoseHands;
...
public class LilayaHomeGeneric {
    ...
    public static final DialogueNode OUTSIDE = new DialogueNode("", "", false) {
        ...
        public Response getResponse(int responseTab, int index) {
            ...
            characterAnsweringDoor = guardAtEntrance.orElseGet(() -> Main.game.getNpc(Rose.class));
        ...
        return new Response(UtilText.parse(slave, "[npc.Name]"), UtilText.parse(slave, "Interact with [npc.name]."), slave.isSlave() ? SlaveDialogue.SLAVE_START : OccupantDialogue.OCCUPANT_START) {
            ...
            public void effects() {
                ...
                if (slave.isSleepingAtHour(Main.game.getHourOfDay())) {
                    Main.game.appendToTextEndStringBuilder(UtilText.parse(slave, "<p style='text-align:center;'>[style.italicsMinorBad([npc.Name] doesn't appreciate being woken up...)]</p>"));
    ...
    public static String getRoomCharactersPresentDescription() {
        ...
        if (!charactersHome.isEmpty()) {
            ...
            for (NPC npc : charactersHome) {
                ...
                sb.append("<span style='color:").append(npc.getFemininity().getColour().toWebHexString()).append(";'>");
                    ...
                    if (!Main.game.getCharactersPresent().contains(npc)) {
                        sb.append(UtilText.parse(npc,
                                "[style.colourMinorBad(is not here)] at the moment, and as you briefly scan the room for any sign of [npc.herHim], you see a little note has been left on [npc.her] bedside cabinet."
                                        ...
                                        + "Hi, [pc.name]!<br/>"));
                        if (npc.hasJob()) {
                            sb.append("I'm out at work at the moment, my hours are from ").append(npc.getHistory().getWorkHourStart()).append(":00 to ").append(npc.getHistory().getWorkHourEnd()).append(":00, ").append(npc.getHistory().getStartDay().getDisplayName(TextStyle.FULL, Locale.ENGLISH)).append("-").append(npc.getHistory().getEndDay().getDisplayName(TextStyle.FULL, Locale.ENGLISH)).append("<br/>");
                        } else {
