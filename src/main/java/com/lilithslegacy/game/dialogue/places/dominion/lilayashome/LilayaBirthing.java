            ...
            for (String id : Main.game.getPlayer().getLastLitterBirthed().getOffspring()) {
                try {
                    if (id.contains("NPCOffspring")) { // If the offspring is from the pre-offspring seed PR, handle them in the old way:
                        ...
                        UtilText.nodeContentSB.append("<br/>").append(Util.capitaliseSentence(UtilText.generateSingularDeterminer(descriptor))).append(" ").append(descriptor).append(" <i style='color:").append(offspring.getGender().getColour().toWebHexString()).append(";'>").append(offspring.getGender().getName()).append("</i>").append(offspring.isFeral() ? " <i style='color:" + RaceStage.FERAL.getColour().toWebHexString() + ";'>" + RaceStage.FERAL.getName() + "</i>" : "").append(" <i style='color:").append(offspring.getSubspecies().getColour(offspring).toWebHexString()).append(";'>").append(UtilText.parse(offspring, "[npc.race]")).append("</i>");

                    } else {
                        OffspringSeed offspring = Main.game.getOffspringSeedById(id);
                        String descriptor = getOffspringDescriptor(offspring);
                        UtilText.nodeContentSB.append("<br/>").append(Util.capitaliseSentence(UtilText.generateSingularDeterminer(descriptor))).append(" ").append(descriptor).append(offspring.isFeral() ? " <i style='color:" + RaceStage.FERAL.getColour().toWebHexString() + ";'>" + RaceStage.FERAL.getName() + "</i>" : "").append(" <i style='color:").append(offspring.getSubspecies().getColour(null).toWebHexString()).append(";'>").append(offspring.getSubspecies().getName(offspring.getBody())).append("</i>").append(" <i style='color:").append(offspring.getGender().getColour().toWebHexString()).append(";'>").append(offspring.getGenderName()).append("</i>");
                    }
                } catch (Exception ex) {
                }
            }
            ...
            try {
                if (offspringId.contains("NPCOffspring")) { // If the offspring is from the pre-offspring seed PR, handle them in the old way:
                    ...
                    if (offspring.isFeminine()) {
                        UtilText.nodeContentSB.append("<br/><br/>Some time later, you imagine seeing a strangely familiar ").append(offspring.getSubspecies().getSingularFemaleName(offspring.getBody())).append(" bending down over you, who gives you a loving hug and a kiss on your cheek before departing...");
                    } else {
                        UtilText.nodeContentSB.append("<br/><br/>Some time later, you imagine seeing a strangely familiar ").append(offspring.getSubspecies().getSingularMaleName(offspring.getBody())).append(" bending down over you, who plants a kiss on your cheek and mutters something in your ear before walking out the door...");
                    }
