...
import com.lilithslegacy.game.sex.managers.dominion.SexManagerRalphDiscount;
        ...
        public String getContent() {
            ...
            if (!Main.game.getDialogueFlags().values.contains(DialogueFlagValue.ralphIntroduced)) {
                ...
                UtilText.nodeContentSB.append("<p>" + "You make your way over, once again, to the only place in the shopping arcade that sells food to go." + " From the outside, it looks like an old-fashioned sweet shop, with large glass windows displaying all manner of exotic-looking food and drink." + " The words 'Ralph's Snacks' are painted in gold cursive letters above the entrance, and as you push the door open and step inside, a little bell rings to announce your arrival." + "</p>" + "<p>").append(Main.game.getDialogueFlags().getSavedLong(Ralph.RALPH_DISCOUNT_TIMER_ID) > 0
                        ...
                        : "[ralph.speech(Hello again! If you need any help, just ask!)] the familiar horse-boy shouts to you from behind the counter.").append("</p>").append("<p>").append("You thank Ralph, and start to have a look around his shop.").append(" Most of the goods aren't anything special, and are just the typical sorts of food and drink that you could pick up anywhere.").append(" What sets this shop apart, however, is a special display of arcane-imbued consumables.").append(" The prices aren't listed, and instead, a little label reads 'Please ask Ralph for assistance with these items'.").append("</p>");

                if (((Ralph) Main.game.getNpc(Ralph.class)).isDiscountActive()) {
                    UtilText.nodeContentSB.append("<p>" + "<b>Ralph is giving you a</b> <b style='color:").append(PresetColour.GENERIC_GOOD.toWebHexString()).append(";'>").append(Main.game.getDialogueFlags().ralphDiscount).append("%</b> <b>discount!</b>").append("</p>");
                }
