...
import com.lilithslegacy.game.sex.managers.submission.SMBreedingStall;
...
public class PregnancyRoulette {
    ...
    private static final List<GenericSexualPartner> breeders = new ArrayList<>();
            ...
            if (!Main.game.getDialogueFlags().hasFlag(DialogueFlagValue.eponaIntroduced)) {
                ...
                if (Main.game.getDialogueFlags().hasFlag(DialogueFlagValue.playedPregnancyRouletteAsBreeder) || Main.game.getDialogueFlags().hasFlag(DialogueFlagValue.playedPregnancyRouletteAsMother)) {
                    ...
                    if (Main.game.getDialogueFlags().eponaStamps >= 6) {
                        ...
                        UtilText.nodeContentSB.append(" You've got ").append(Util.intToString(6 - Main.game.getDialogueFlags().eponaStamps)).append(" stamps to go until you get to take me for a ride!)]").append("</p>");
                    }
                } else {
                    UtilText.nodeContentSB.append(
                            "<p>"
    ...
    public static final DialogueNode PREGNANCY_ROULETTE_BREEDER = new DialogueNode("", "", true) {
        ...
        public String getContent() {
            ...
            UtilText.nodeContentSB.append("<p>" + "Epona rolls the dice, and you watch as it clatters to a halt on the number ").append(Util.intToString(roll)).append(".").append(" [epona.speech(Alright, [pc.name], that means you're going ").append(Util.intToPosition(roll)).append("! Now I'll keep rolling for the rest of you!)]").append("</p>").append("<p>").append("You all then have to wait while Epona rolls for each of the other breeders.").append(" Every time she rolls an already-rolled number, she has to re-roll the dice, which results in you all having to wait for far longer than if she'd just used a more efficient method of deciding who goes first.").append("</p>").append("<p>").append("Eventually, however, Epona has finished, and you move forwards to take your place in the line...").append("</p>").append("<p style='text-align:center;'>").append("<i>You are going <b>").append(Util.intToPosition(roll)).append("</b>!</i>").append("</p>");
    ...
    private static String getBreederPanel(NPC breeder) {
        ...
        String sb = "<div class='container-half-width'>"
        ...
        return UtilText.parse(breeder, sb);
    }
