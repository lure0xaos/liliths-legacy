...
import com.lilithslegacy.game.sex.managers.dominion.vanessa.SMVanessaSex;
...
public class CityHallDemographics {
                    ...
                    return new Response("Foot massage", "Offer [vanessa.name] a foot massage.", FOOT_MASSAGE) {
                        ...
                        public void effects() {
                            ...
                            try {
                                ...
                                System.getLogger("").log(System.Logger.Level.ERROR, "Vanessa is not wearing any shoes!");
                            }
                    ...
                    return new Response("Shoulder massage", "Offer [vanessa.name] a shoulder massage.", SHOULDER_MASSAGE) {
                        ...
                        public void effects() {
                            ...
                            try {
                                ...
                                System.getLogger("").log(System.Logger.Level.ERROR, "Vanessa is not wearing a cardigan!");
                            }
    ...
    private static void applyNameChange(boolean applyOffspringSurnames) {
        ...
        if (applyOffspringSurnames && Main.mainController.getWebEngine().getDocument().getElementById("hiddenFieldSurname").getTextContent().isEmpty()) {
        ...
        if (unsuitableName || unsuitableSurname) {
            ...
            if (applyOffspringSurnames) {
                Main.game.getTextEndStringBuilder().append("<p style='text-align:center;'>" + "You fill out the forms and hand over the required fee, officially changing your name to:<br/>" + "<b>[pc.Name]").append(!Main.game.getPlayer().getSurname().isEmpty() ? " [pc.Surname]" : "").append("</b><br/>").append("All of your offspring, all the way down your family tree, have had their surnames legally changed to:<br/>").append("<b>[pc.Surname]</b><br/>").append("</p>").append(Main.game.getPlayer().incrementMoney(-5000));
            } else {
                Main.game.getTextEndStringBuilder().append("<p style='text-align:center;'>" + "You fill out the forms and hand over the required fee, officially changing your name to:<br/>" + "<b>[pc.Name]").append(!Main.game.getPlayer().getSurname().isEmpty() ? " [pc.Surname]" : "").append("</b>").append("</p>").append(Main.game.getPlayer().incrementMoney(-100));
            }
            Main.game.setContent(new Response("", "", NAME_CHANGE));
        }
    }
                ...
                return new Response("Bare feet", "Pull off [vanessa.namePos] pantyhose so that you can massage her bare feet.", BARE_FOOT_MASSAGE) {
                    ...
                    public void effects() {
                        ...
                        try {
                            ...
                            System.getLogger("").log(System.Logger.Level.ERROR, "Vanessa is not wearing pantyhose!");
                        }
    ...
    public static final DialogueNode NAME_CHANGE = new DialogueNode("Bureau of Demographics", "-", true) {
        ...
        public String getContent() {
            ...
            UtilText.nodeContentSB.append("<br/>" + "<div class='container-full-width' style='text-align:center;'>" + "<div style='position:relative; display:inline-block; padding-bottom:0; margin 0 auto; vertical-align:middle; width:100%; text-align:center;'>" + "<i>" + "Your first name can be set as three values; your masculine name, androgynous name, and feminine name." + " Your name will automatically switch to the one which corresponds to your body femininity." + "</i>" + "<br/>" + "<p style='display:inline-block; padding:0; margin:0; height:32px; line-height:32px; width:100px;'>First name: </p>" + "</form style='display:inline-block; padding:0; margin:0; text-align:center;'>" + "<input type='text' id='nameMasculineInput' style=' color:").append(PresetColour.MASCULINE.toWebHexString()).append(";' value='").append(UtilText.parseForHTMLDisplay(Main.game.getPlayer().getNameTriplet().getMasculine())).append("'>").append("</form style='display:inline-block; padding:0; margin:0; text-align:center;'>").append("<input type='text' id='nameAndrogynousInput' style=' color:").append(PresetColour.ANDROGYNOUS.toWebHexString()).append(";' value='").append(UtilText.parseForHTMLDisplay(Main.game.getPlayer().getNameTriplet().getAndrogynous())).append("'>").append("</form style='display:inline-block; padding:0; margin:0; text-align:center;'>").append("<input type='text' id='nameFeminineInput' style=' color:").append(PresetColour.FEMININE.toWebHexString()).append(";' value='").append(UtilText.parseForHTMLDisplay(Main.game.getPlayer().getNameTriplet().getFeminine())).append("'>").append("<br/>").append("<p style='display:inline-block; padding:0; margin:0; height:32px; line-height:32px; width:100px;'>Surname: </p>").append("<form style='display:inline-block; padding:0; margin:0; text-align:center;'><input type='text' id='surnameInput' value='").append(UtilText.parseForHTMLDisplay(Main.game.getPlayer().getSurname())).append("'></form>").append("</div>").append("<br/>").append("<i>Your name must be between 2 and 32 characters long. You cannot use the square bracket characters or full stops. (Surname may be left blank.)</i>").append(unsuitableName ? "<p style='text-align:center;padding-top:0;'><b style=' color:" + PresetColour.GENERIC_BAD.toWebHexString() + ";'>Invalid name.</b></p>" : "").append(unsuitableSurname ? "<p style='text-align:center;padding-top:0;'><b style=' color:" + PresetColour.GENERIC_BAD.toWebHexString() + ";'>Invalid Surname.</b></p>" : "").append("</div>").append("<p id='hiddenFieldName' style='display:none;'></p>").append("<p id='hiddenFieldSurname' style='display:none;'></p>");
