                ...
                if (!Main.game.getPlayer().isQuestProgressGreaterThan(QuestLine.RELATIONSHIP_NYAN_HELP, Quest.RELATIONSHIP_NYAN_3_STOCK_ISSUES_DOBERMANNS)) {
                    ...
                    return new Response("Surplus", "Help yourself to today's surplus stock.", STORAGE_AREA_SEARCHING) {
                        ...
                        public void effects() {
                            ...
                            Main.game.getTextEndStringBuilder().append(Main.game.getPlayer().addClothing(Main.game.getItemGen().generateClothing(Util.randomItemFrom(clothingToGenerate), false), false)).append(Main.game.getPlayer().addClothing(Main.game.getItemGen().generateClothing(Util.randomItemFrom(clothingToGenerate), true), false)).append(Math.random() > 0.5 ? Main.game.getPlayer().addClothing(Main.game.getItemGen().generateClothing(Util.randomItemFrom(clothingToGenerate), true), false) : "").append(Math.random() > 0.5 ? Main.game.getPlayer().addClothing(Main.game.getItemGen().generateClothing(Util.randomItemFrom(clothingToGenerate), true), false) : "").append(UtilText.parseFromXMLFile("places/dominion/warehouseDistrict/kaysTextiles", "STORAGE_AREA_SEARCHED"));
                        }
                    };
                }
            }
    ...
    public static final DialogueNode KAY_OFFICE_DOMINATE_MAKEUP = new DialogueNode("", "", true) {
        ...
        public String getHeaderContent() {
            ...
            String sb = UtilText.parseFromXMLFile("places/dominion/warehouseDistrict/kaysTextiles", "KAY_OFFICE_DOMINATE_MAKEUP") +
                    CharacterModificationUtils.getSelfDivHairStyles("Hair Style", UtilText.parse(BodyChanging.getTarget(), "Change [npc.namePos] hair style.")) + CharacterModificationUtils.getKatesDivCoveringsNew(
