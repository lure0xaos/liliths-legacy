...
//		gambler.setPlayerKnowsName(true);
    ...
    private static String getGamblingFormat(String turnText) {
        ...
        for (int i = 0; i < playerDice.size(); i++) {
            UtilText.nodeContentSB.append("<div class='modifier-icon' style='width:18%; margin:0 1%; border:3px solid ").append(diceToReroll.contains(playerDice.get(i)) ? PresetColour.GENERIC_MINOR_GOOD.toWebHexString() : "").append(";'>").append(Hand.getDiceInHand(playerDice).contains(playerDice.get(i))
                    ...
                    + "</div>"
                    : "").append("<div class='modifier-icon-content'>").append(playerDice.get(i).getFace().getSVGString()).append("</div>").append("<div class='overlay' id='DICE_PLAYER_").append(i).append("'></div>").append("</div>");
        }
        Hand playerHand = Hand.getHand(playerDice);
        UtilText.nodeContentSB.append("<div class='container-full-width' style='text-align:center;'>" + "<b>").append(playerHand.getRanking()).append(". ").append(playerHand.getName()).append("</b> | [style.colourDisabled(Value: ").append(Hand.getValue(playerDice)).append(")]<br/>").append(comparingHands == 0
                ? "[style.colourDisabled(You " + (progress == 3 ? "have drawn" : "are drawing") + "...)]"
                : (comparingHands < 0
                ? "[style.colourTerrible(You " + (progress == 3 ? "have lost" : "are losing") + "!)]"
                : "[style.colourExcellent(You " + (progress == 3 ? "have won" : "are winning") + "!)]")).append("</div>");
        UtilText.nodeContentSB.append("</div>");

        UtilText.nodeContentSB.append("<div class='container-half-width'>");
        for (int i = 0; i < gamblerDice.size(); i++) {
            UtilText.nodeContentSB.append("<div class='modifier-icon' style='width:18%; margin:0 1%; border:3px solid ").append(diceToReroll.contains(gamblerDice.get(i)) ? PresetColour.GENERIC_MINOR_GOOD.toWebHexString() : "").append(";'>").append(Hand.getDiceInHand(gamblerDice).contains(gamblerDice.get(i))
                    ? "<div style='width:100%;height:100%;position:absolute;left:0;bottom:0;padding:0;margin:0'>"
                    + SVGImages.SVG_IMAGE_PROVIDER.getDiceGlow()
                    + "</div>"
                    : "").append("<div class='modifier-icon-content'>").append(gamblerDice.get(i).getFace().getSVGString()).append("</div>").append("</div>");
        }
        Hand gamblerHand = Hand.getHand(gamblerDice);
        UtilText.nodeContentSB.append("<div class='container-full-width' style='text-align:center;'>" + "<b>").append(gamblerHand.getRanking()).append(". ").append(gamblerHand.getName()).append("</b> | [style.colourDisabled(Value: ").append(Hand.getValue(gamblerDice)).append(")]<br/>").append(UtilText.parse(gambler, (comparingHands == 0
                ? "[style.colourDisabled([npc.Name] " + (progress == 3 ? "has drawn" : "is drawing") + "...)]"
                : (comparingHands > 0
                ? "[style.colourTerrible([npc.Name] " + (progress == 3 ? "has lost" : "is losing") + "!)]"
                : "[style.colourExcellent([npc.Name] " + (progress == 3 ? "has won" : "is winning") + "!)]")))).append("</div>");
        UtilText.nodeContentSB.append("</div>");
        ...
        for (int i = 0; i < progressDescriptions.length; i++) {
            UtilText.nodeContentSB.append(i != 0
                    ? "<div class='container-quarter-width inner' style='width:5%; margin:0; text-align:center;'>&gt;</div>"
                    : "").append("<div class='container-quarter-width inner' style='width:13%; margin:0 2%; text-align:center;'>").append(progress == i
                    ? "[style.boldGood(" + progressDescriptions[i] + ")]"
                    : progress > i
                    ? progressDescriptions[i]
                    : "[style.colourDisabled(" + progressDescriptions[i] + ")]").append("</div>");
        }
        UtilText.nodeContentSB.append("<div class='container-full-width' style='text-align:center;'>" + "Total pot: ").append(UtilText.formatAsMoney(moneyPool, "span")).append("</div>");
        UtilText.nodeContentSB.append("</div>");
