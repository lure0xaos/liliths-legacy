...
import com.lilithslegacy.game.dialogue.places.dominion.helenaHotel.HelenaConversationTopic;
...
public class DialogueFlags implements XMLSaving {
    ...
    public final Set<AbstractDialogueFlagValue> values;
    public int ralphDiscount;
    public int scarlettPrice;
    public int eponaStamps;
    // Timers:
    public final Map<String, Long> savedLongs = new HashMap<>();
    ...
    public final Set<String> warehouseDefeatedIDs = new HashSet<>();
...
//		}
    ...
    public String incrementNatalyaPoints(int increment) {
        ...
        if (increment > 0) {
            sb.append("You [style.colourGood(gained)] [style.boldPink(").append(increment).append(")] [style.colourPinkLight(filly point").append(plural ? "s" : "").append(")]!");
        } else {
            sb.append("You [style.colourBad(lost)] [style.boldPink(").append(-increment).append(")] [style.colourPinkLight(filly point").append(plural ? "s" : "").append(")]!");
        }
        sb.append("<br/>You now have [style.boldPink(").append(getNatalyaPoints()).append(")] [style.colourPinkLight(filly points)]!");
        sb.append("</p>");
        return sb.toString();
    }
