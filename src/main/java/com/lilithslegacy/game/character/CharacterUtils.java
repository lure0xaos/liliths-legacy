...
import com.lilithslegacy.game.character.body.Ear;
...
//			System.out.println(output);
            ...
            Path path1 = FS.newPath("data/characters/" + character.getName(false) + "_day" + Main.game.getDayNumber() + ".xml");
            if (Files.exists(path1))
                saveLocation = "data/characters/" + character.getName(false) + "_day" + Main.game.getDayNumber() + "(" + saveNumber + ").xml";

            while (true) {
                Path path = FS.newPath("data/characters/" + character.getName(false) + "_day" + Main.game.getDayNumber() + "(" + saveNumber + ").xml");
    ...
    public PlayerCharacter loadCharacterFromXML(Path xmlFile, PlayerCharacter importedCharacter, CharacterImportSetting... settings) {
    ...
    public Body generateBody(GameCharacter linkedCharacter, Gender startingGender, GameCharacter mother, GameCharacter father) {
        ...
        if (body == null) {
            ...
            if ((fatherHuman && !motherHuman) || (Math.random() <= 0.5 && (fatherHuman == motherHuman))) {
                if (mother.isTaur()) {
...
//			System.out.println("height adjusted");
            body.setHeight(getSizeFromGenetics(
                    body.getHeightValue(),
                    (body.isFeminine() == mother.isFeminine()),
                    mother.getHeightValue(),
                    (body.isFeminine() == father.isFeminine()),
...
//		System.out.println(species+", "+stage);
    ...
    public void applyTaurConversion(GameCharacter character) {
        ...
        switch (raceStage) {
            case FERAL:
                System.getLogger("").log(System.Logger.Level.ERROR, "Warning: applyTaurConversion() was called on the feral character '" + character.getName() + "'!");
                break;
            case HUMAN:
                if (!halfDemon) {
...
//			character.setSkinCovering(BodyCoveringType.HUMAN, Util.randomItemFrom(BodyCoveringType.HUMAN.getNaturalColoursPrimary()), true);
        ...
        if (character.hasFetish(Fetish.FETISH_ORAL_GIVING) || character.getHistory() == Occupation.NPC_PROSTITUTE) {
            ...
            character.setFaceVirgin(!(Math.random() < 0.85f));
        }
        if (character.isFeminine()) {
        ...
        if (character.hasPenis() || character.getSubspeciesOverrideRace() == Race.DEMON) {
            character.setPenisVirgin(!(Math.random() < 0.95f)
...
//				character.setPenisGirth(Math.round(character.getPenisGirth().getValue()*sizeAlteration));
    ...
    public void generateAndApplySexCounts(GameCharacter character) {
        ...
        if (character.getFetishDesire(Fetish.FETISH_NON_CON_SUB).isPositive()) {
            ...
            character.setSexConsensualCount(null, character.getSexAsSubCount(null));
        }
    ...
    public void equipClothingFromOutfit(GameCharacter character, AbstractOutfit outfit, List<EquipClothingSetting> settings) {
        if (outfit != null) {
            try {
                ...
                System.getLogger("").log(System.Logger.Level.ERROR, "Outfit '" + outfit.getName() + "' could not be applied in CharacterUtils equipClothing(). Proceeding to randomly generate outfit...");
                System.getLogger("").log(System.Logger.Level.ERROR, e.getMessage(), e);
            }
        }
