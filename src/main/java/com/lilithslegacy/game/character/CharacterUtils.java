...
//			System.out.println(output);
            ...
            Path self1 = FS.newPath("data/characters/" + character.getName(false) + "_day" + Main.game.getDayNumber() + ".xml");
            if (Files.exists(self1))
                saveLocation = "data/characters/" + character.getName(false) + "_day" + Main.game.getDayNumber() + "(" + saveNumber + ").xml";

            while (true) {
                Path self = FS.newPath("data/characters/" + character.getName(false) + "_day" + Main.game.getDayNumber() + "(" + saveNumber + ").xml");
...
//		System.out.println(species+", "+stage);
    ...
    public void applyTaurConversion(GameCharacter character) {
        ...
        switch (raceStage) {
            case FERAL:
                String message = "Warning: applyTaurConversion() was called on the feral character '" + character.getName() + "'!";
    ...
    public void equipClothingFromOutfit(GameCharacter character, AbstractOutfit outfit, List<EquipClothingSetting> settings) {
        if (outfit != null) {
            try {
                ...
                String message = "Outfit '" + outfit.getName() + "' could not be applied in CharacterUtils equipClothing(). Proceeding to randomly generate outfit...";
                System.getLogger("").log(System.Logger.Level.ERROR, message);
                System.getLogger("").log(System.Logger.Level.ERROR, e);
            }
        }
