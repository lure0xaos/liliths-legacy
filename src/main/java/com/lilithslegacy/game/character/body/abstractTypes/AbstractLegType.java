...
import com.lilithslegacy.game.character.body.Ass;
import com.lilithslegacy.game.character.body.Body;
import com.lilithslegacy.game.character.body.BreastCrotch;
import com.lilithslegacy.game.character.body.LegConfigurationAffinity;
...
import com.lilithslegacy.game.character.body.Vagina;
...
public abstract class AbstractLegType implements BodyPartTypeInterface {
    ...
    public AbstractLegType(Path XMLFile, String author, boolean mod) {
        if (Files.exists(XMLFile)) {
            try {
                ...
                System.getLogger("").log(System.Logger.Level.ERROR, "AbstractLegType was unable to be loaded from file! (" + XMLFile.getFileName().toString() + ")\n" + ex);
            }
        }
    }
    ...
    public String applyLegConfigurationTransformation(GameCharacter character, LegConfiguration legConfiguration, boolean applyEffects, boolean applyFullEffects) {
        ...
        if (character.hasWings()) {
            if (legConfiguration.isWingsOnLegConfiguration()) {
                feralStringBuilder.append("[style.italicsFeral([npc.Her] [npc.wingSize], [npc.wings+] are now located on the sides of [npc.her] ").append(legConfiguration.getName()).append(" body!)]");
            } else {
            ...
            if (legConfiguration.isWingsOnLegConfiguration()) {
                feralStringBuilder.append("[style.italicsFeral(If [npc.she] [npc.verb(grow)] any wings, they will be located on the sides of [npc.her] ").append(legConfiguration.getName()).append(" body!)]");
            } else {
        ...
        feralStringBuilder.append("<p>").append(character.postTransformationCalculation()).append("</p>");
    ...
    private String handleLegConfigurationChanges(Body body, LegConfiguration legConfiguration, boolean applyEffects, boolean applyFullEffects) {
        ...
        switch (legConfiguration) {
            case BIPEDAL:
                ...
                feralStringBuilder.append("<p>" + "[npc.NamePos] lower body transforms back into a regular, bipedal configuration, with [npc.her] genitals shifting back to their normal position between [npc.her] [npc.legs].<br/>" + "[npc.Name] now [npc.has] [style.boldTfGeneric(bipedal)] <b style='color:").append(raceColorString).append(";'>").append(this.getTransformName()).append(" legs</b>, which are covered in [npc.legFullDescription].").append("</p>");
                break;
            case ARACHNID:
                if (applyEffects) {
                ...
                feralStringBuilder.append("<p>" + "Without warning, [npc.name] suddenly [npc.verb(lose)] all of the strength in [npc.her] [npc.legs], and [npc.she] [npc.verb(collapse)] to the ground with a startled cry." + " Before [npc.sheIs] able to react to this alarming development, [npc.her] lower body starts to rapidly transform..." + "</p>" + "<p>" + "Looking down, [npc.name] [npc.verb(watch)] in disbelief as [npc.her] [npc.legs] split and transform into eight long, segmented legs." + " The changes don't stop there, however, as [npc.her] lower body continues to rapidly morph into that of a huge, eight-legged ").append(feralRaceName).append(".").append(" A horny [npc.moan] bursts out of [npc.her] mouth as [npc.her] genitals shift to be on the underside of [npc.her] massive arachnid body,").append(" while [npc.her] anus ").append(body.getLeg().getType().hasSpinneret() ? "and spinneret are" : "is").append(" positioned near the tip of [npc.her] abdomen.").append(" [style.italicsSex(As [npc.her] genitals are only visible from below, [npc.she] [npc.do]n't feel embarrassed to have no clothing covering [npc.her] arachnid body.)]<br/>").append("[npc.Name] now [npc.has] the [style.boldTfGeneric(arachnid body)] of <b style='color:").append(raceColorString).append(";'>").append(feralRaceNameWithDeterminer).append("</b>, which is covered in [npc.legFullDescription].").append("</p>");
                break;
            case AVIAN:
                if (applyEffects) {
                    applyExtraLegConfigurationTransformations(body, legConfiguration, legConfiguration.isLargeGenitals(), applyFullEffects);
                ...
                feralStringBuilder.append("<p>" + "Without warning, [npc.name] suddenly [npc.verb(lose)] all of the strength in [npc.her] [npc.legs], and [npc.she] [npc.verb(collapse)] to the ground with a startled cry." + " Before [npc.sheIs] able to react to this alarming development, [npc.her] lower body starts to rapidly transform..." + "</p>" + "<p>" + "Looking down, [npc.name] [npc.verb(watch)] in disbelief as the entire of [npc.her] lower body rapidly morphs into that of a huge ").append(feralRaceName).append(".").append(" A horny [npc.moan] bursts out of [npc.her] mouth as [npc.her] genitals and asshole shift to be located within a cloaca that's found on the rear-facing underside of [npc.her] massive avian body.").append(" [style.italicsSex(As [npc.her] genitals are only visible from below, [npc.she] [npc.do]n't feel embarrassed to have no clothing covering [npc.her] avian body.)]<br/>").append("[npc.Name] now [npc.has] the [style.boldTfGeneric(avian body)] of <b style='color:").append(raceColorString).append(";'>").append(feralRaceNameWithDeterminer).append("</b>, which is covered in [npc.legFullDescription].").append("</p>");
                break;
            case CEPHALOPOD:
                if (applyEffects) {
                    applyExtraLegConfigurationTransformations(body, legConfiguration, legConfiguration.isLargeGenitals(), applyFullEffects);
                ...
                feralStringBuilder.append("<p>" + "Without warning, [npc.name] suddenly [npc.verb(lose)] all of the strength in [npc.her] [npc.legs], and [npc.she] [npc.verb(collapse)] to the ground with a startled cry." + " Before [npc.sheIs] able to react to this alarming development, [npc.her] lower body starts to rapidly transform..." + "</p>" + "<p>" + "Looking down, [npc.name] [npc.verb(watch)] in disbelief as [npc.her] [npc.legs] split and transform into eight long, strong tentacles." + " The changes don't stop there, however, as [npc.her] lower body continues to rapidly morph into that of a huge, eight-legged ").append(feralRaceName).append(".").append(" A horny [npc.moan] bursts out of [npc.her] mouth as [npc.her] genitals and asshole shift to sit within a cloaca located in the central underside of [npc.her] new tentacles.").append(" [style.italicsSex(As [npc.her] cloaca is only visible from below, [npc.she] [npc.do]n't feel embarrassed to have no clothing covering [npc.her] tentacled body.)]<br/>").append("[npc.Name] now [npc.has] the [style.boldTfGeneric(tentacled body)] of <b style='color:").append(raceColorString).append(";'>").append(feralRaceNameWithDeterminer).append("</b>, which is covered in [npc.legFullDescription].").append("</p>");
                break;
            case TAIL:
                if (applyEffects) {
                    applyExtraLegConfigurationTransformations(body, legConfiguration, legConfiguration.isLargeGenitals(), applyFullEffects);
                ...
                feralStringBuilder.append("<p>" + "[npc.NamePos] lower body transforms into that of ").append(feralRaceNameWithDeterminer).append("'s tail, with [npc.her] genitals and asshole shifting to sit within a front-facing cloaca,").append(" located in the equivalent place to where [npc.her] regular intercrural genitalia would be.<br/>").append("[npc.Name] now [npc.has] the [style.boldTfGeneric(tail)] of <b style='color:").append(raceColorString).append(";'>").append(feralRaceNameWithDeterminer).append("</b>, which is covered in [npc.legFullDescription].").append("</p>");
                break;
            case TAIL_LONG:
                if (applyEffects) {
                    applyExtraLegConfigurationTransformations(body, legConfiguration, legConfiguration.isLargeGenitals(), applyFullEffects);
                ...
                feralStringBuilder.append("<p>" + "[npc.NamePos] lower body transforms into that of a huge ").append(feralRaceName).append("'s tail, with [npc.her] genitals and asshole shifting to sit within a front-facing cloaca,").append(" located in the equivalent place to where [npc.her] regular intercrural genitalia would be.<br/>").append("[npc.Name] now [npc.has] the [style.boldTfGeneric(huge tail)] of <b style='color:").append(raceColorString).append(";'>").append(feralRaceNameWithDeterminer).append("</b>, which is covered in [npc.legFullDescription].").append("</p>");
                break;
            case WINGED_BIPED:
                if (applyEffects) {
                    applyExtraLegConfigurationTransformations(body, legConfiguration, legConfiguration.isLargeGenitals(), applyFullEffects);
                ...
                feralStringBuilder.append("<p>" + "[npc.NamePos] lower body transforms back into a bipedal configuration, with [npc.her] genitals shifting back to their normal position between [npc.her] [npc.legs]." + " Letting out a surprised cry, [npc.name] [npc.verb(bend)] down and [npc.verb(stoop)] over as [npc.her] spine rapidly reshapes itself." + " The transformation is over within a matter of moments, leaving [npc.name] to naturally use [npc.her] [npc.arms] in place of forelegs so as to support [npc.her] newly-shaped body.<br/>" + "[npc.Name] now [npc.has] [style.boldTfGeneric(bipedal)] <b style='color:").append(raceColorString).append(";'>").append(this.getTransformName()).append(" legs</b>, which are covered in [npc.legFullDescription],").append(" and [style.boldTfGeneric([npc.verb(use)] [npc.her] [npc.arms] as forelegs)].").append("</p>");
                break;
            case QUADRUPEDAL:
                ...
                feralStringBuilder.append(" and [npc.she] [npc.verb(let)] out an alarmed cry as [npc.she] [npc.verb(lose)] [npc.her] balance and [npc.verb(tumble)] to the floor." + " Right before [npc.her] [npc.eyes], [npc.her] lower body shifts and transforms, with [npc.her] limbs pulling back into an intermediary mass of [npc.bodyMaterial]," + " before almost immediately pushing back out in a quadrupedal configuration." + "</p>" + "<p>" + "As this is happening, [npc.her] [npc.ass+] rapidly expands and pushes out, quickly forming the body to which [npc.her] four new legs are anchored." + " Although it could be described as nothing less than extreme, the transformation is surprisingly pain-free, and the gasps and [npc.moans] that burst out from [npc.namePos] mouth are purely from shock and exertion." + "</p>" + "<p>" + "After just a few moments more, [npc.namePos] lower body has completely transformed into that of ").append(feralRaceNameWithDeterminer).append(", with [npc.her] genitals and asshole shifting to sit in the same place as that of a feral ").append(feralRaceName).append(".<br/>").append("[npc.Name] now [npc.has] the [style.boldTfGeneric(lower body)] of <b style='color:").append(raceColorString).append(";'>").append(feralRaceNameWithDeterminer).append("</b>, which is covered in [npc.legFullDescription(true)].").append("</p>");
                break;
        }
        ...
        if (applyEffects) {
            if (!body.getLegConfiguration().isTall() && legConfiguration.isTall()) {
                ...
                feralStringBuilder.append("<p>The size of [npc.namePos] new lower body has resulted in [npc.herHim] getting taller, so now when standing at full height [npc.she] [npc.verb(measure)] ").append(colouredHeightValue).append(".</p>");
                ...
                body.setHeight(newHeight);
                String colouredHeightValue = "<span style='color:" + body.getHeight().getColour().toWebHexString() + ";'>[npc.heightValue]</span>";
                feralStringBuilder.append("<p>The reduced size of [npc.namePos] new lower body has resulted in [npc.herHim] getting shorter, so now when standing at full height [npc.she] [npc.verb(measure)] ").append(colouredHeightValue).append(".</p>");
            }
        }
...
//						+ "</p>");
        ...
        feralStringBuilder.append("<p><i>" + "[style.boldTfGeneric(Every part)] of [npc.her] lower body has transformed into that of a ").append(feral ? "feral " + feralRaceName : "regular " + this.getRace().getName(false)).append(", meaning that [npc.she] now [npc.has]");
        ...
        feralStringBuilder.append(Util.stringsToStringList(partsList, false)).append(".");

        Penis penis = body.getPenis();
        if (penis.getType() != PenisType.NONE) {
            if (feral) {
                feralStringBuilder.append(" [npc.Her] cock has similarly transformed into that of a feral ").append(feralRaceName).append("'s, and not only produces musky, animal-like cum, but is also an impressive ").append(Units.size(penis.getRawLengthValue())).append(" long.");
            } else {
                feralStringBuilder.append(" [npc.Her] cock has similarly transformed into that of a regular ").append(this.getRace().getName(false)).append(", and is ").append(Units.size(penis.getRawLengthValue())).append(" long.");
            }
        }
        ...
        if (vagina.getType() != VaginaType.NONE) {
            if (feral) {
                feralStringBuilder.append(" [npc.Her] now-musky pussy is now also identical to that of ").append(feralRaceNameWithDeterminer).append(", and has shifted into being ").append(vagina.getOrificeVagina().getCapacity().getDescriptor()).append(" in order to accommodate a matching feral cock.");
            } else {
                feralStringBuilder.append(" [npc.Her] now-").append(vagina.getOrificeVagina().getCapacity().getDescriptor()).append(" pussy has similarly transformed into that of a regular.");
            }
        }
        feralStringBuilder.append("</i></p>");

        if (feral) {
            feralStringBuilder.append("<p><i>Feeling as though it's only natural, [npc.she] [style.colourGood(no longer [npc.verb(get)] embarrassed)] about having [npc.her] [style.italicsFeral(animalistic genitals").append(legConfiguration.isBipedalPositionedCrotchBoobs() ? (body.getBreastCrotch().getShape() == BreastShape.UDDERS ? " or udders" : " or crotch-boobs") : "").append(")] on display!</i></p>");
        }
        ...
        if (landSpeed > 0) {
            feralStringBuilder.append("<p style='text-align:center;'><i>" + "[npc.Her] new lower body is not as well-adapted to movement on land as a usual biped," + " and as a result, [style.colourTerrible([npc.she] [npc.verb(move)] slower than usual)] [style.colourEarth(while on land)]!" + "<br/>" + "[style.colourTerrible(+").append(landSpeed).append("%)] travel time while on land!").append("</i></p>");
        } else if (landSpeed < 0) {
            feralStringBuilder.append("<p style='text-align:center;'><i>" + "[npc.Her] new lower body is capable of speeds greater than that attainable by a usual biped," + " and as a result, [style.colourExcellent([npc.she] [npc.verb(move)] faster than usual)] [style.colourEarth(while on land)]!" + "<br/>" + "[style.colourExcellent(").append(landSpeed).append("%)] travel time while on land!").append("</i></p>");
        }

        int waterSpeed = body.getLeg().getType().getWaterSpeedModifier() + legConfiguration.getWaterSpeedModifier();
        if (waterSpeed > 0) {
            feralStringBuilder.append("<p style='text-align:center;'><i>" + "[npc.SheIsFull] now a lot less capable at moving in water than a usual biped," + " and as a result, [style.colourTerrible([npc.she] [npc.verb(move)] slower than usual)] [style.colourWater(while in water)]!" + "<br/>" + "[style.colourTerrible(+").append(waterSpeed).append("%)] travel time while in water!").append("</i></p>");
        } else if (waterSpeed < 0) {
            feralStringBuilder.append("<p style='text-align:center;'><i>" + "[npc.SheIsFull] now in possession of a body that's well-suited to moving in water," + " and as a result, [style.colourExcellent([npc.she] [npc.verb(move)] faster than usual)] [style.colourWater(while in water)]!" + "<br/>" + "[style.colourExcellent(").append(waterSpeed).append("%)] travel time while in water!").append("</i></p>");
        }
