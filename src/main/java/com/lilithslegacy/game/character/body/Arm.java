...
public class Arm implements BodyPartInterface {
    ...
    public String setType(GameCharacter owner, AbstractArmType type) {
        ...
        sb.append(type.getTransformationDescription(owner)).append("</p>");
    ...
    public String setArmRows(GameCharacter owner, int armRows) {
        ...
        if (armRows < currentArmRows) {
            ...
            if (owner.isPlayer()) {
                sb.append("<p>" + "You feel a strange pressure building up around the base of ").append(losesTwoPairs
                        ? "your two extra pairs"
                        : (armRows == 2
                        ? "the lowest of your extra pairs"
                        : "your extra pair")).append(" of [pc.arms], and before you can react, they rapidly shrink away into the [pc.skin] of your torso.<br/>").append("You now have [style.boldTfLesser(").append(Util.intToString(armRows)).append(" pair").append(armRows > 1 ? "s" : "").append(" of [pc.arms])], [pc.materialDescriptor] [pc.armFullDescriptionColour].").append("</p>");
            } else {
            ...
            if (owner.isPlayer()) {
                sb.append("<p>" + "You feel a strange pressure building up down the sides of your torso, and before you have time to react, ").append(gainsTwoPairs
                        ? "two extra pairs"
                        : "an extra pair").append(" of [pc.arms] rapidly grow out of the [pc.skin] of your lower torso.<br/>").append("You now have [style.boldTfLesser(").append(Util.intToString(armRows)).append(" pair").append(armRows > 1 ? "s" : "").append(" of [pc.arms])], [pc.materialDescriptor] [pc.armFullDescriptionColour].").append("</p>");
            } else {
                sb.append(UtilText.parse(owner,
                        "<p>"
