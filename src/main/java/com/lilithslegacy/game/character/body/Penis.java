...
public class Penis implements BodyPartInterface {
    ...
    protected final Set<PenetrationModifier> penisModifiers;

    protected final Testicle testicle;
    protected final OrificePenisUrethra orificeUrethra;
    ...
    public String setType(GameCharacter owner, AbstractPenisType type) {
        ...
        if (!owner.hasPenisIgnoreDildo()) {
            ...
            sb.append("[npc.Name] [npc.verb(feel)] an intense heat building up in [npc.her] groin, and [npc.she] [npc.verb(let)] out [npc.a_moan+] as [npc.she] [npc.verb(feel)] the [npc.skin] ").append(owner.hasVagina()
                    ? (!owner.isTaur()
                    ? "above [npc.her] pussy"
                    : "beneath [npc.her] pussy")
                    : "between [npc.her] legs").append(" tighten up and start to press outwards.").append(" Within moments, a large bump has formed ").append(owner.hasVagina()
                    ? (!owner.isTaur()
                    ? "above [npc.her] feminine slit"
                    : "beneath [npc.her] feminine slit")
                    : "in the middle of [npc.her] groin,").append(" and with a sudden splitting sensation, the bump pushes out and forms into a penis.");

            if (owner.isInternalTesticles()) {
                sb.append(" As [npc.her] new cock flops down ").append(owner.hasVagina()
                        ? (!owner.isTaur()
                        ...
                        : "between [npc.her] legs, [npc.she] [npc.verb(feel)] [npc.a_balls] growing within [npc.her] groin,").append(" and [npc.she] [npc.verb(let)] out an unwitting [npc.moan] as [npc.her] new sexual organ finishes growing.<br/>");
            } else {
                sb.append(" As [npc.her] new cock flops down ").append(owner.hasVagina()
                        ? (!owner.isTaur()
                        ...
                        : "between [npc.her] legs, [npc.she] [npc.verb(feel)] [npc.a_balls] push out underneath the base of [npc.her] new shaft,").append(" and [npc.she] [npc.verb(let)] out an unwitting [npc.moan] as [npc.her] new sexual organ finishes growing.<br/>");
            }
        ...
        if (!penisModifiers.isEmpty()) {
            ...
            for (PenetrationModifier pm : penisModifiers) {
                sb.append("<br/>[style.boldGrow(").append(Util.capitaliseSentence(pm.getName())).append(")]");
            }
        }
        sb.append("</p>");
