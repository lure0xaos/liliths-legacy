...
import com.lilithslegacy.game.character.body.abstractTypes.AbstractTentacleType;
...
import com.lilithslegacy.game.character.body.coverings.BodyCoveringSkinToneColorHelper;
...
import com.lilithslegacy.game.character.body.types.TentacleType;
...
import com.lilithslegacy.game.character.body.valueEnums.BodyShape;
...
import com.lilithslegacy.game.character.body.valueEnums.StartingSkinTone;
...
import com.lilithslegacy.game.character.pregnancy.Litter;
import com.lilithslegacy.game.character.pregnancy.PregnancyPossibility;
import com.lilithslegacy.game.character.race.AbstractRace;
import com.lilithslegacy.game.character.race.AbstractRacialBody;
import com.lilithslegacy.game.character.race.AbstractSubspecies;
import com.lilithslegacy.game.character.race.FeralAttributes;
...
import com.lilithslegacy.game.dialogue.utils.CharacterModificationUtils;
...
import com.lilithslegacy.utils.Builder;
...
//		importedBreast.nipples.areolaeShape = (AreolaeShape.valueOf(nipples.getAttribute("areolaeShape")));
        ...
        if (Main.isVersionOlderThan(version, "0.3.10")) {
            ...
            if (tail.getAttribute("length") != null && !tail.getAttribute("length").isEmpty()) {
                importedTail.lengthAsPercentageOfHeight = Float.valueOf(tail.getAttribute("length"));
            }
        }
        ...
        if (tentacle != null) {
            ...
            if (tentacle.getAttribute("girth") != null && !tentacle.getAttribute("girth").isEmpty()) {
                importedTentacle.girth = Integer.valueOf(tentacle.getAttribute("girth"));
            }

            if (tentacle.getAttribute("length") != null && !tentacle.getAttribute("length").isEmpty()) {
                importedTentacle.lengthAsPercentageOfHeight = Float.valueOf(tentacle.getAttribute("length"));
            } else {
...
//						+Units.size(owner.getHeightValue()+owner.getLegTailLength(false), Units.ValueType.NUMERIC, Units.UnitType.LONG);
        ...
        if (owner.isPlayer()) {
            sb.append("You are [pc.name], ").append(owner.getRace() == Race.HUMAN
                    ...
                    : "[pc.a_fullRace(true)] [pc.gender(true)]. ").append(owner.getAppearsAsGenderDescription(true)).append(heightDescription).append(".");
        } else {
            if (owner.isAreaKnownByCharacter(CoverableArea.PENIS, Main.game.getPlayer()) && owner.isAreaKnownByCharacter(CoverableArea.VAGINA, Main.game.getPlayer())) {
                sb.append("[npc.Name] is ").append(owner.getRace() == Race.HUMAN
                        ...
                        : "[npc.a_fullRace(true)] [npc.gender(true)]. ").append(owner.getAppearsAsGenderDescription(true)).append(heightDescription);
            } else {
                if (observant) {
                    sb.append("Thanks to your observant perk, you can detect that [npc.name] is <span style='color:").append(getGender().getColour().toWebHexString()).append(";'>[npc.a_gender]</span> [npc.raceStage] [npc.race]. ").append(owner.getAppearsAsGenderDescription(true)).append(heightDescription);
                } else {
                    sb.append("[npc.Name] is a [npc.a_fullRace(true)]. ").append(owner.getAppearsAsGenderDescription(true)).append(heightDescription);
                }
            }
            if (owner.isSizeDifferenceTallerThan(Main.game.getPlayer())) {
                ...
                sb.append(", making [npc.herHim] <span style='color:").append(PresetColour.BODY_SIZE_FOUR.toWebHexString()).append(";'>significantly ").append(descriptor).append("</span> than you.");
            } else if (owner.isSizeDifferenceShorterThan(Main.game.getPlayer())) {
                sb.append(", making [npc.herHim] <span style='color:").append(PresetColour.BODY_SIZE_ZERO.toWebHexString()).append(";'>significantly shorter</span> than you.");
            } else {
                sb.append(".");
            }
        }
        ...
        switch (owner.getLegConfiguration()) {
            ...
            case TAIL_LONG:
                if (owner.isFeral()) {
                    ...
                    sb.append(" [style.colourFeral([npc.Her] entire lower body, from the waist down, has transformed into the long tail of [npc.a_legRace]," + " which measures ").append(Units.size(owner.getLegTailLength(false))).append(" in length.").append(" [npc.Her] ass and genitals are completely feral in nature, and, in a manner identical to that of [npc.a_assRace], have shifted to be located within a front-facing cloaca.)]");
                }
                break;
            case QUADRUPEDAL:
                if (owner.isFeral()) {
        ...
        if (Main.getProperties().hasValue(PropertyValue.ageContent)) {
            sb.append(" [npc.She] [npc.verb(appear)] to be ").append(owner.getAppearsAsAge() == AgeCategory.SIXTIES_PLUS ? "" : "in [npc.her] ").append("<span style='color:").append(owner.getAppearsAsAge().getColour().toWebHexString()).append(";'>").append(owner.getAppearsAsAge().getName()).append("</span>.");
        }
        sb.append("</p>");
        ...
        switch (this.getBodyMaterial()) {
            ...
            case WATER:
            case FIRE:
                sb.append("<p>" + "[npc.NamePos] entire body, save for a small obsidian sphere in the place where [npc.her] heart should be, is made out of" + " <b style='color:").append(this.getBodyMaterial().getColour().toWebHexString()).append(";'>").append(this.getBodyMaterial().getName()).append("</b>!").append("</p>");
                break;
        }
        ...
        if (owner.getBlusher().getPrimaryColour() != PresetColour.COVERING_NONE) {
            sb.append(" [npc.SheIsFull] wearing ").append(owner.getBlusher().getColourDescriptor(owner, true, false)).append(" blusher.");
        }
        ...
        if (hair.getRawLengthValue() == 0) {
            ...
            switch (hair.getStyle()) {
                case NONE:
                    sb.append(hair.getType().isDefaultPlural(owner) ? "are" : "is").append(" unstyled.");
                    break;
                case BRAIDED:
                    sb.append(hair.getType().isDefaultPlural(owner) ? "have" : "has").append(" been woven into a long braid.");
                    break;
                case CURLY:
                    sb.append(hair.getType().isDefaultPlural(owner) ? "have" : "has").append(" been curled and left loose.");
                    break;
                case LOOSE:
                    sb.append(hair.getType().isDefaultPlural(owner) ? "are" : "is").append(" left loose and unstyled.");
                    break;
                case PONYTAIL:
                    sb.append(hair.getType().isDefaultPlural(owner) ? "have" : "has").append(" been styled into a ponytail.");
                    break;
                case STRAIGHT:
                    sb.append(hair.getType().isDefaultPlural(owner) ? "have" : "has").append(" been straightened and left loose.");
                    break;
                case TWIN_TAILS:
                    sb.append(hair.getType().isDefaultPlural(owner) ? "have" : "has").append(" been styled into twin tails.");
                    break;
                case WAVY:
                    sb.append(hair.getType().isDefaultPlural(owner) ? "have" : "has").append(" been styled into waves and left loose.");
                    break;
                case MOHAWK:
                    sb.append(hair.getType().isDefaultPlural(owner) ? "have" : "has").append(" been styled into a mohawk.");
                    break;
                case AFRO:
                    sb.append(hair.getType().isDefaultPlural(owner) ? "have" : "has").append(" been styled into an afro.");
                    break;
                case SIDECUT:
                    sb.append(hair.getType().isDefaultPlural(owner) ? "have" : "has").append(" been styled into a sidecut.");
                    break;
                case BOB_CUT:
                    sb.append(hair.getType().isDefaultPlural(owner) ? "have" : "has").append(" been styled into a bob cut.");
                    break;
                case PIXIE:
                    sb.append(hair.getType().isDefaultPlural(owner) ? "have" : "has").append(" been styled into a pixie-cut.");
                    break;
                case SLICKED_BACK:
                    sb.append(hair.getType().isDefaultPlural(owner) ? "have" : "has").append(" been slicked back.");
                    break;
                case MESSY:
                    sb.append(hair.getType().isDefaultPlural(owner) ? "are" : "is").append(" unstyled and very messy.");
                    break;
                case HIME_CUT:
                    sb.append(hair.getType().isDefaultPlural(owner) ? "have" : "has").append(" been straightened and styled into a hime cut.");
                    break;
                case CHONMAGE:
                    sb.append(hair.getType().isDefaultPlural(owner) ? "have" : "has").append(" been straightened, oiled and styled into a chonmage topknot.");
                    break;
                case TOPKNOT:
                    sb.append(hair.getType().isDefaultPlural(owner) ? "have" : "has").append(" been styled into a topknot.");
                    break;
                case DREADLOCKS:
                    sb.append(hair.getType().isDefaultPlural(owner) ? "have" : "has").append(" been styled into dreadlocks.");
                    break;
                case BIRD_CAGE:
                    sb.append(hair.getType().isDefaultPlural(owner) ? "have" : "has").append(" been styled into an elaborate bird cage").append(UtilText.returnStringAtRandom(".", ", birds not included."));
                    break;
                case TWIN_BRAIDS:
                    sb.append(hair.getType().isDefaultPlural(owner) ? "have" : "has").append(" been woven into long twin braids.");
                    break;
                case DRILLS:
                    sb.append(hair.getType().isDefaultPlural(owner) ? "have" : "has").append(" been styled into ojou ringlets.");
                    break;
                case LOW_PONYTAIL:
                    sb.append(hair.getType().isDefaultPlural(owner) ? "have" : "has").append(" been styled into a low ponytail.");
                    break;
                case CROWN_BRAID:
                    sb.append(hair.getType().isDefaultPlural(owner) ? "have" : "has").append(" been woven into a ").append(UtilText.returnStringAtRandom("crown of braids.", "braided crown."));
                    break;
                case BUN:
                    sb.append(hair.getType().isDefaultPlural(owner) ? "have" : "has").append(" been styled into a bun.");
                    break;
                case CHIGNON:
                    sb.append(hair.getType().isDefaultPlural(owner) ? "have" : "has").append(" been tied up into a chignon.");
                    break;
                case SIDE_BRAIDS:
                    sb.append(hair.getType().isDefaultPlural(owner) ? "have" : "has").append(" been woven into braids that hang down on either side of [npc.her] face.");
                    break;
            }
        }
        if (hair.isNeckFluff()) {
            sb.append(" A large amount of [npc.hair(true)] ").append(hair.getType().isDefaultPlural(owner) ? "have" : "has").append(" grown around [npc.her] neck and upper chest.");
        }

        // Horns:

        if (owner.hasHorns()) {
            sb.append(" ").append(owner.getHornType().getBodyDescription(owner));
        }

        // Antenna:

        if (owner.hasAntennae()) {
            sb.append(" ").append(owner.getAntennaType().getBodyDescription(owner));
        }
        ...
        if (owner.isFeral()) {
            ...
            sb.append(" ").append(ear.getType().getBodyDescription(owner));
            ...
            if (Main.game.isFacialHairEnabled()) {
                if (owner.getFacialHairType().getType() == BodyCoveringType.BODY_HAIR_SCALES_ALLIGATOR) {
                    switch (owner.getFacialHair()) {
                        case ZERO_NONE:
                            if (!owner.isFeminine()) {
                                sb.append(" [npc.She] [npc.do]n't have any trace of rough, stubbly ").append(owner.getFacialHairType().getName(owner)).append(".");
                            }
                            break;
                        case ONE_STUBBLE:
                            sb.append(" [npc.SheHasFull] a stubbly patch of ").append(owner.getFacialHairType().getFullDescription(owner, true)).append(" growing on [npc.her] [npc.face].");
                            break;
                        case TWO_MANICURED:
                            sb.append(" [npc.SheHasFull] a small, rough patch of ").append(owner.getFacialHairType().getFullDescription(owner, true)).append(" growing on [npc.her] [npc.face], which resembles a beard.");
                            break;
                        case THREE_TRIMMED:
                            sb.append(" [npc.SheHasFull] a rough patch of ").append(owner.getFacialHairType().getFullDescription(owner, true)).append(" growing on [npc.her] [npc.face], which resembles a beard.");
                            break;
                        case FOUR_NATURAL:
                            sb.append(" [npc.SheHasFull] a natural-looking rough patch of ").append(owner.getFacialHairType().getFullDescription(owner, true)).append(" growing on [npc.her] [npc.face], which resembles a beard.");
                            break;
                        case FIVE_UNKEMPT:
                            sb.append(" [npc.SheHasFull] an unkempt, rough patch of ").append(owner.getFacialHairType().getFullDescription(owner, true)).append(" growing on [npc.her] [npc.face], which resembles a beard.");
                            break;
                        case SIX_BUSHY:
                            sb.append(" [npc.SheHasFull] a thick, rough patch of ").append(owner.getFacialHairType().getFullDescription(owner, true)).append(" growing on [npc.her] [npc.face], which resembles a beard.");
                            break;
                        case SEVEN_WILD:
                            sb.append(" [npc.SheHasFull] a wild, rough patch of ").append(owner.getFacialHairType().getFullDescription(owner, true)).append(" growing on [npc.her] [npc.face], which resembles a beard.");
                            break;
                    }
                } else {
                    switch (owner.getFacialHair()) {
                        case ZERO_NONE:
                            if (!owner.isFeminine()) {
                                sb.append(" [npc.She] [npc.do]n't have any trace of facial ").append(owner.getFacialHairType().getName(owner)).append(".");
                            }
                            break;
                        case ONE_STUBBLE:
                            sb.append(" [npc.SheHasFull] a stubbly patch of ").append(owner.getFacialHairType().getFullDescription(owner, true)).append(" growing on [npc.her] [npc.face].");
                            break;
                        case TWO_MANICURED:
                            sb.append(" [npc.SheHasFull] a small amount of ").append(owner.getFacialHairType().getFullDescription(owner, true)).append(" growing on [npc.her] [npc.face].");
                            break;
                        case THREE_TRIMMED:
                            sb.append(" [npc.SheHasFull] a well-trimmed beard of ").append(owner.getFacialHairType().getFullDescription(owner, true)).append(" growing on [npc.her] [npc.face].");
                            break;
                        case FOUR_NATURAL:
                            sb.append(" [npc.SheHasFull] a natural-looking beard of ").append(owner.getFacialHairType().getFullDescription(owner, true)).append(" growing on [npc.her] [npc.face].");
                            break;
                        case FIVE_UNKEMPT:
                            sb.append(" [npc.SheHasFull] an unkempt, bushy beard of ").append(owner.getFacialHairType().getFullDescription(owner, true)).append(" growing on [npc.her] [npc.face].");
                            break;
                        case SIX_BUSHY:
                            sb.append(" [npc.SheHasFull] a thick, bushy beard of ").append(owner.getFacialHairType().getFullDescription(owner, true)).append(" growing on [npc.her] [npc.face].");
                            break;
                        case SEVEN_WILD:
                            sb.append(" [npc.SheHasFull] a wild, bushy beard of ").append(owner.getFacialHairType().getFullDescription(owner, true)).append(" growing on [npc.her] [npc.face].");
                            break;
                    }
                }
            }
        ...
        if (owner.isFeral()) {
            ...
            sb.append(" ").append(face.getTongue().getType().getBodyDescription(owner));
        }
        ...
        if (owner.isPlayer() || owner.isAreaKnownByCharacter(CoverableArea.MOUTH, Main.game.getPlayer())) {
            if (face.getMouth().getOrificeMouth().isVirgin()) { //TODO
                ...
                for (SexAreaPenetration pt : SexAreaPenetration.values()) {
                    if (pt.isTakesVirginity()) {
                        if (owner.getVirginityLoss(new SexType(SexParticipantType.NORMAL, SexAreaOrifice.MOUTH, pt)) != null) {
                            sb.append(" <span style='color:").append(PresetColour.GENERIC_ARCANE.toWebHexString()).append(";'>").append(owner.getVirginityLossDescription(new SexType(SexParticipantType.NORMAL, SexAreaOrifice.MOUTH, pt))).append("</span>");
                ...
                if (!virginityLossFound) {
                    sb.append(" <span style='color:").append(PresetColour.GENERIC_ARCANE.toWebHexString()).append(";'>[npc.Name] [npc.has] lost [npc.her] oral virginity.</span>");
                }

                sb.append(" It is ").append(Capacity.getCapacityFromValue(face.getMouth().getOrificeMouth().getStretchedCapacity()).getDescriptor(true)).append(", and can comfortably accommodate objects of up to").append(" [style.colourSex(").append(Units.size(Capacity.getMaximumComfortableDiameter(face.getMouth().getOrificeMouth().getElasticity(), face.getMouth().getOrificeMouth().getRawCapacityValue(), true))).append(")] in diameter.");

                if (Main.game.isPenetrationLimitationsEnabled()) {
                    if (Objects.requireNonNull(owner.getFaceDepth()) == OrificeDepth.TWO_AVERAGE) {
                    ...
                    if (owner.getBodyMaterial().isOrificesLimitedDepth()) {
                        if (owner.hasFetish(Fetish.FETISH_SIZE_QUEEN)) {
                            sb.append(" and as [npc.sheIsFull] a ").append(Fetish.FETISH_SIZE_QUEEN.getName(owner)).append(", [npc.she] can be pushed to [style.colourMinorGood(comfortably)] accommodate objects of a maximum length of ").append(" [style.colourSex(").append(Units.size(owner.getFaceMaximumPenetrationDepthUncomfortable())).append(")].");
                        } else {
                            sb.append(" allowing [npc.herHim] to [style.colourMinorGood(comfortably)] accommodate [style.colourSex(").append(Units.size(owner.getFaceMaximumPenetrationDepthComfortable())).append(")] of a penetrative object,").append(" and [style.colourMinorBad(uncomfortably)] accommodate [style.colourSex(").append(Units.size(owner.getFaceMaximumPenetrationDepthUncomfortable())).append(")].");
                        }

                    } else {
                        if (owner.hasFetish(Fetish.FETISH_SIZE_QUEEN)) {
                            sb.append(" and as [npc.sheIsFull] a ").append(Fetish.FETISH_SIZE_QUEEN.getName(owner)).append(", combined with the fact that [npc.her] body is made out of [npc.bodyMaterial],").append(" [npc.she] can be pushed to [style.colourMinorGood(comfortably)] accommodate objects of [style.colourSex(any length)].");
                        } else {
                            sb.append(" allowing [npc.herHim] to [style.colourMinorGood(comfortably)] accommodate [style.colourSex(").append(Units.size(owner.getFaceMaximumPenetrationDepthComfortable())).append(")] of a penetrative object,").append(" and as [npc.her] body is made out of [npc.bodyMaterial], [npc.she] can be pushed to [style.colourMinorBad(uncomfortably)] accommodate objects of [style.colourSex(any length)].");
                        }
                    }
                }
                ...
                sb.append(" and after being used, it ").append(face.getMouth().getOrificeMouth().getPlasticity().getDescription()).append(".");
            }
        ...
        sb.append(" [npc.SheHasFull] <span style='color:").append(BodySize.valueOf(getBodySize()).getColour().toWebHexString()).append(";'>").append(BodySize.valueOf(getBodySize()).getName(true)).append("</span>, ").append("<span style='color:").append(Muscle.valueOf(getMuscle()).getColour().toWebHexString()).append(";'>").append(Muscle.valueOf(getMuscle()).getName(false)).append("</span>").append(" body, giving [npc.herHim] <span style='color:").append(owner.getBodyShape().toWebHexStringColour()).append(";'>[npc.a_bodyShape]</span> body shape.");
        ...
        if (owner.isVisiblyPregnant()) {
            ...
            if (owner.hasStatusEffect(StatusEffect.PREGNANT_1)) {
                sb.append(" is slightly swollen, and it's clear to anyone who takes a closer look that <span style='color:").append(PresetColour.GENERIC_ARCANE.toWebHexString()).append(";'>[npc.sheIs] pregnant</span>.");

            } else if (owner.hasStatusEffect(StatusEffect.PREGNANT_2)) {
                sb.append(" is heavily swollen, and it's clear to anyone who glances [npc.her] way that <span style='color:").append(PresetColour.GENERIC_ARCANE.toWebHexString()).append(";'>[npc.sheIs] pregnant</span>.");

            } else if (owner.hasStatusEffect(StatusEffect.PREGNANT_3)) {
                sb.append(" is massively swollen, and it's blatantly obvious to anyone who glances [npc.her] way that" + " <span style='color:").append(PresetColour.GENERIC_ARCANE.toWebHexString()).append(";'>[npc.sheIs] expecting to give birth very soon</span>.");
            }
        }
        sb.append("</p>");
        ...
        if (owner.isFeral() && !owner.getFeralAttributes().isBreastsPresent()) {
            ...
            if (Main.game.getPlayer().hasIngestedPsychoactiveFluidType(FluidTypeBase.MILK)) {
                ...
                sb.append(" <i style='color:").append(PresetColour.PSYCHOACTIVE.toWebHexString()).append(";'>The psychoactive milk you recently ingested is causing your view of ").append(owner.isPlayer() ? "your" : "[npc.namePos]").append(" breasts to be distorted!</i>");
            }
            if (viewedBreast.getRawSizeValue() > 0) {
                sb.append(" [npc.SheHasFull] ").append(Util.intToString(owner.getBreastRows())).append(" pair").append(owner.getBreastRows() == 1 ? "" : "s").append(" of ");
                if (Main.game.isVestigialMultiBreastsEnabled() && owner.getBreastRows() > 1) {
                    sb.append("[npc.breasts]");
                } else {
                    sb.append(viewedBreast.getSize().getDescriptor()).append(" [npc.breasts]");
                }
                ...
                if (owner.getBreastRows() == 1) {
                    if (viewedBreast.getSize().isTrainingBraSize()) {
                        ...
                        sb.append(", which fit comfortably into ").append(UtilText.generateSingularDeterminer(viewedBreast.getSize().getCupSizeName())).append(" ").append(viewedBreast.getSize().getCupSizeName()).append("-cup bra.");
                    }

                } else if (owner.getBreastRows() == 2) {
                    if (viewedBreast.getSize().isTrainingBraSize()) {
                        sb.append(", with [npc.her] top pair fitting comfortably into a training bra,").append(Main.game.isVestigialMultiBreastsEnabled()
                                ? " and the pair below being vestigial in size."
                                : " and the pair below being slightly smaller.");
                    } else {
                        sb.append(", with [npc.her] top pair fitting comfortably into ").append(UtilText.generateSingularDeterminer(viewedBreast.getSize().getCupSizeName())).append(" ").append(viewedBreast.getSize().getCupSizeName()).append("-cup bra,").append(Main.game.isVestigialMultiBreastsEnabled()
                                ? " and the pair below being vestigial in size."
                                : " and the pair below being slightly smaller.");
                    }
                ...
                if (owner.getBreastRows() == 1) {
                    ...
                    sb.append(", with ").append(Util.intToString(owner.getBreastRows())).append(" pairs of pecs.");
                }
            }

            sb.append(" ").append(getBreastDescription(owner, viewedBreast));
        }
        sb.append("</p>");
        ...
        if (owner.isFeral() && !owner.getFeralAttributes().isArmsOrWingsPresent()) {
            ...
            if (owner.getHandNailPolish().getPrimaryColour() != PresetColour.COVERING_NONE) {
                if (owner.getArmType().equals(ArmType.HARPY)) {
                    sb.append(" The little claw on [npc.her] thumb has been painted in ").append(owner.getCovering(BodyCoveringType.MAKEUP_NAIL_POLISH_HANDS).getFullDescription(owner, true)).append(".");
                } else {
                    sb.append(" [npc.Her] fingernails have been painted in ").append(owner.getCovering(BodyCoveringType.MAKEUP_NAIL_POLISH_HANDS).getFullDescription(owner, true)).append(".");
                }
            }
            ...
            if (Main.game.isBodyHairEnabled()) {
                if (owner.getUnderarmHairType().getType() == BodyCoveringType.BODY_HAIR_SCALES_ALLIGATOR) {
                    switch (owner.getUnderarmHair()) {
                        case ZERO_NONE:
                            sb.append(" There is no trace of any rough ").append(owner.getUnderarmHairType().getName(owner)).append(" in [npc.her] armpits.");
                            break;
                        case ONE_STUBBLE:
                            sb.append(" [npc.SheHasFull] a rough patch of ").append(owner.getUnderarmHairType().getFullDescription(owner, true)).append(" in each of [npc.her] armpits.");
                            break;
                        case TWO_MANICURED:
                            sb.append(" [npc.SheHasFull] a rough patch of ").append(owner.getUnderarmHairType().getFullDescription(owner, true)).append(" in each of [npc.her] armpits.");
                            break;
                        case THREE_TRIMMED:
                            ...
                            break;
                        case FOUR_NATURAL:
                            sb.append(" [npc.SheHasFull] a natural amount of rough ").append(owner.getUnderarmHairType().getFullDescription(owner, true)).append(" in each of [npc.her] armpits.");
                            break;
                        case FIVE_UNKEMPT:
                            sb.append(" [npc.SheHasFull] an unkempt mass of rough ").append(owner.getUnderarmHairType().getFullDescription(owner, true)).append(" in each of [npc.her] armpits.");
                            break;
                        case SIX_BUSHY:
                            sb.append(" [npc.SheHasFull] a thick, rough mass of ").append(owner.getUnderarmHairType().getFullDescription(owner, true)).append(" in each of [npc.her] armpits.");
                            break;
                        case SEVEN_WILD:
                            sb.append(" [npc.SheHasFull] a wild, rough mass of ").append(owner.getUnderarmHairType().getFullDescription(owner, true)).append(" in each of [npc.her] armpits.");
                            break;
                    }
                } else {
                    switch (owner.getUnderarmHair()) {
                        case ZERO_NONE:
                            sb.append(" There is no trace of any ").append(owner.getUnderarmHairType().getName(owner)).append(" in [npc.her] armpits.");
                            break;
                        case ONE_STUBBLE:
                            sb.append(" [npc.SheHasFull] a stubbly patch of ").append(owner.getUnderarmHairType().getFullDescription(owner, true)).append(" in each of [npc.her] armpits.");
                            break;
                        case TWO_MANICURED:
                            sb.append(" [npc.SheHasFull] a well-manicured patch of ").append(owner.getUnderarmHairType().getFullDescription(owner, true)).append(" in each of [npc.her] armpits.");
                            break;
                        case THREE_TRIMMED:
                            sb.append(" [npc.SheHasFull] a trimmed patch of ").append(owner.getUnderarmHairType().getFullDescription(owner, true)).append(" in each of [npc.her] armpits.");
                            break;
                        case FOUR_NATURAL:
                            sb.append(" [npc.SheHasFull] a natural amount of ").append(owner.getUnderarmHairType().getFullDescription(owner, true)).append(" in each of [npc.her] armpits.");
                            break;
                        case FIVE_UNKEMPT:
                            sb.append(" [npc.SheHasFull] an unkempt mass of ").append(owner.getUnderarmHairType().getFullDescription(owner, true)).append(" in each of [npc.her] armpits.");
                            break;
                        case SIX_BUSHY:
                            sb.append(" [npc.SheHasFull] a thick, bushy mass of ").append(owner.getUnderarmHairType().getFullDescription(owner, true)).append(" in each of [npc.her] armpits.");
                            break;
                        case SEVEN_WILD:
                            sb.append(" [npc.SheHasFull] a wild, bushy mass of ").append(owner.getUnderarmHairType().getFullDescription(owner, true)).append(" in each of [npc.her] armpits.");
                            break;
                    }
                }
            }
            ...
            if (!(owner.isFeral() && owner.getArmType().allowsFlight())) {
                sb.append(" [npc.Her] [npc.arms] are ").append(Util.randomItemFrom(owner.getBodyShape().getLimbDescriptors())).append(", and are <span style='color:").append(owner.getFemininity().getColour().toWebHexString()).append(";'>").append(owner.getFemininity().getName(false)).append(" in appearance.");
            }
            ...
            switch (owner.getLegConfiguration()) {
                ...
                case TAIL_LONG:
                    ...
                    if (!owner.isFeral()) {
                        ...
                        sb.append(" Its [npc.tailLength]-length contributes to [npc.her] body's total length of ").append(Units.size(owner.getHeightValue() + owner.getLegTailLength(false), Units.ValueType.NUMERIC, Units.UnitType.LONG)).append(".");
        ...
        if (owner.getLegConfiguration().getNumberOfLegs() > 0) {
            switch (owner.getFootStructure()) {
                ...
                case TENTACLED:
                    break;
                case DIGITIGRADE:
                    sb.append(" [npc.Her] [npc.legs] and [npc.feet] are [style.colourTFGeneric(").append(owner.getFootStructure().getName()).append(")], meaning that [npc.she] naturally [npc.verb(walk)] on [npc.her] toes.");
                    break;
                case PLANTIGRADE:
                    sb.append(" [npc.Her] [npc.legs] and [npc.feet] are [style.colourTFGeneric(").append(owner.getFootStructure().getName()).append(")], meaning that [npc.she] naturally [npc.verb(walk)] with [npc.her] feet flat on the ground.");
                    break;
                case UNGULIGRADE:
                    sb.append(" [npc.Her] [npc.legs] and [npc.feet] are [style.colourTFGeneric(").append(owner.getFootStructure().getName()).append(")], meaning that [npc.she] naturally [npc.verb(walk)] on [npc.her] hoofs.");
                    break;
                case ARACHNOID:
                    sb.append(" [npc.Her] [npc.legs] and [npc.feet] are [style.colourTFGeneric(").append(owner.getFootStructure().getName()).append(")], meaning that [npc.she] [npc.verb(walk)] on the ends of [npc.her] segmented arachnoid legs.");
                    break;
            }
        }
        ...
        if (owner.getLegCount() > 0) {
            sb.append(" [npc.Her] [npc.legs] are ").append(Util.randomItemFrom(owner.getBodyShape().getLimbDescriptors())).append(", and ");
            ...
            sb.append(" [npc.Her] [npc.leg(true)] is ").append(Util.randomItemFrom(owner.getBodyShape().getLimbDescriptors())).append(", and ");
            if (femininity <= Femininity.MASCULINE_STRONG.getMaximumFemininity()) {
        ...
        if (owner.isPlayer()
            ...
            switch (owner.getGenitalArrangement()) {
                ...
                case CLOACA_BEHIND:
                    ...
                    sb.append("[style.colourFeral(").append(owner.getGenitalArrangement().getDescription()).append(")]");
                    sb.append("</p>");
                    break;
            }
        }
        ...
        if (owner.isAreaKnownByCharacter(CoverableArea.ANUS, Main.game.getPlayer())) {
            sb.append(" ").append(getAssDescription(owner, false));
        } else {
        ...
        if (owner.isAreaKnownByCharacter(CoverableArea.PENIS, Main.game.getPlayer())) {
            ...
            if (observant) {
                if (owner.hasPenis()) {
                    sb.append("[style.colourDisabled(Thanks to your '").append(Perk.OBSERVANT.getName(Main.game.getPlayer())).append("' trait, you can tell that [npc.sheHasFull] a cock,").append(" but as you haven't seen [npc.her] naked groin before, you don't know what it looks like.)]");
                } else {
                    sb.append("[style.colourDisabled(You haven't seen [npc.her] naked groin before, but thanks to your '").append(Perk.OBSERVANT.getName(Main.game.getPlayer())).append("' trait, you can tell that [npc.she] doesn't have a cock.)]");
                }
        ...
        if (owner.isAreaKnownByCharacter(CoverableArea.VAGINA, Main.game.getPlayer())) {
            ...
            if (observant) {
                if (vagina.getType() != VaginaType.NONE) {
                    sb.append("[style.colourDisabled(Thanks to your '").append(Perk.OBSERVANT.getName(Main.game.getPlayer())).append("' trait, you can tell that [npc.sheHasFull] a pussy,").append(" but as you haven't seen [npc.her] naked groin before, you don't know what it looks like.)]");

                } else {
                    sb.append("[style.colourDisabled(You haven't seen [npc.her] naked groin before, but thanks to your '").append(Perk.OBSERVANT.getName(Main.game.getPlayer())).append("' trait, you can tell that [npc.she] doesn't have a pussy.)]");
                }
        ...
        for (Entry<InventorySlot, Tattoo> tattoEntry : owner.getTattoos().entrySet()) {
            ...
            if (knowsArea) {
                ...
                if (tattoo.getBodyOverviewDescription().isEmpty()) { // Old version support
                    tattooSB.append("<span style='color:").append(tattoo.getPrimaryColour().toWebHexString()).append(";'>").append(Util.capitaliseSentence(tattooSlot.getTattooSlotName())).append(":</span> ");
                    tattooSB.append(tattoo.getDescription());
                    tattooSB.append(".");

                } else {
                    tattooSB.append("On [npc.her] [style.boldBlueSteel(").append(tattooSlot.getTattooSlotName()).append(")], [npc.sheHasFull] ");
                    ...
                    if (tattoo.getType() != TattooType.getTattooTypeFromId("innoxia_misc_none")) {
                        ...
                        tattooSB.append("<span style='color:").append(tattoo.getPrimaryColour().toWebHexString()).append(";'>").append(tattoo.getPrimaryColour().getName()).append("</span>");
                    }
                    tattooSB.append(".");
                }
                if (tattoo.getWriting() != null && tattoo.getWriting().getText() != null && !tattoo.getWriting().getText().isEmpty()) {
                    tattooSB.append(" It bears the words: '").append(tattoo.getFormattedWritingOutput()).append("'");
                }
                if (tattoo.getCounter() != null && tattoo.getCounter().getType() != TattooCounterType.NONE) {
                    ...
                    tattooSB.append(UtilText.generateSingularDeterminer(counterName)).append(" '").append(counterName).append("' count, which reads: '").append(tattoo.getFormattedCounterOutput(owner)).append("'.");
...
//		addRaceWeight(raceWeightMap, vagina.getType().getRace(), 1);
    ...
    public String getEyeDescription(GameCharacter owner) {
        ...
        if (owner.isFeral()) {
            ...
            sb.append(" ").append(eye.getType().getBodyDescription(owner));
        }

        // Eye makeup:
        if (owner.getEyeLiner().getPrimaryColour() != PresetColour.COVERING_NONE) {
            sb.append(" Around [npc.her] [npc.eyes], [npc.sheHas] got a layer of ").append(owner.getEyeLiner().getColourDescriptor(owner, true, false)).append(" eye liner.");
        }
        if (owner.getEyeShadow().getPrimaryColour() != PresetColour.COVERING_NONE) {
            sb.append(" [npc.SheIs] wearing a tasteful amount of ").append(owner.getEyeShadow().getFullDescription(owner, true)).append(".");
        }
    ...
    public String getAssDescription(GameCharacter owner, boolean locationSpecific) {
        ...
        descriptionSB.append(" It is ").append(Capacity.getCapacityFromValue(ass.getAnus().getOrificeAnus().getStretchedCapacity()).getDescriptor(true)).append(", and when lubricated can comfortably accommodate objects of up to").append(" [style.colourSex(").append(Units.size(Capacity.getMaximumComfortableDiameter(ass.getAnus().getOrificeAnus().getElasticity(), ass.getAnus().getOrificeAnus().getRawCapacityValue(), true))).append(")] in diameter.");

        if (Main.game.isPenetrationLimitationsEnabled()) {
            if (Objects.requireNonNull(owner.getAssDepth()) == OrificeDepth.TWO_AVERAGE) {
            ...
            if (owner.getBodyMaterial().isOrificesLimitedDepth()) {
                if (owner.hasFetish(Fetish.FETISH_SIZE_QUEEN)) {
                    descriptionSB.append(" and as [npc.sheIsFull] a ").append(Fetish.FETISH_SIZE_QUEEN.getName(owner)).append(", [npc.she] can be pushed to [style.colourMinorGood(comfortably)] accommodate objects of a maximum length of ").append(" [style.colourSex(").append(Units.size(owner.getAssMaximumPenetrationDepthUncomfortable())).append(")].");
                } else {
                    descriptionSB.append(" allowing [npc.herHim] to [style.colourMinorGood(comfortably)] accommodate [style.colourSex(").append(Units.size(owner.getAssMaximumPenetrationDepthComfortable())).append(")] of a penetrative object,").append(" and [style.colourMinorBad(uncomfortably)] accommodate [style.colourSex(").append(Units.size(owner.getAssMaximumPenetrationDepthUncomfortable())).append(")].");
                }

            } else {
                if (owner.hasFetish(Fetish.FETISH_SIZE_QUEEN)) {
                    descriptionSB.append(" and as [npc.sheIsFull] a ").append(Fetish.FETISH_SIZE_QUEEN.getName(owner)).append(", combined with the fact that [npc.her] body is made out of [npc.bodyMaterial],").append(" [npc.she] can be pushed to [style.colourMinorGood(comfortably)] accommodate objects of [style.colourSex(any length)].");
                } else {
                    descriptionSB.append(" allowing [npc.herHim] to [style.colourMinorGood(comfortably)] accommodate [style.colourSex(").append(Units.size(owner.getAssMaximumPenetrationDepthComfortable())).append(")] of a penetrative object,").append(" and as [npc.her] body is made out of [npc.bodyMaterial], [npc.she] can be pushed to [style.colourMinorBad(uncomfortably)] accommodate objects of [style.colourSex(any length)].");
                }
            }
        ...
        if (ass.getAnus().getOrificeAnus().isVirgin()) {
            ...
            for (SexAreaPenetration pt : SexAreaPenetration.values()) {
                if (pt.isTakesVirginity()) {
                    if (owner.getVirginityLoss(new SexType(SexParticipantType.NORMAL, SexAreaOrifice.ANUS, pt)) != null) {
                        descriptionSB.append(" <span style='color:").append(PresetColour.GENERIC_ARCANE.toWebHexString()).append(";'>").append(owner.getVirginityLossDescription(new SexType(SexParticipantType.NORMAL, SexAreaOrifice.ANUS, pt))).append("</span>");
                        virginityLossFound = true;
                        break;
                    }
                }
            ...
            if (!virginityLossFound) {
                descriptionSB.append(" <span style='color:").append(PresetColour.GENERIC_ARCANE.toWebHexString()).append(";'>[npc.Name] [npc.has] lost [npc.her] anal virginity.</span>");
            }
        }
        ...
        descriptionSB.append(" and after being used, it ").append(ass.getAnus().getOrificeAnus().getPlasticity().getDescription()).append(".");

        if (Main.game.isAssHairEnabled()) {
            switch (ass.getAnus().getAssHair()) {
                case ZERO_NONE:
                    descriptionSB.append(" There is no trace of any ").append(owner.getAssHairType().getName(owner)).append(" around [npc.her] asshole.");
                    break;
                case ONE_STUBBLE:
                    descriptionSB.append(" [npc.She] [npc.has] a few strands of ").append(owner.getAssHairType().getFullDescription(owner, true)).append(" around [npc.her] asshole.");
                    break;
                case TWO_MANICURED:
                    descriptionSB.append(" [npc.She] [npc.has] a very small amount of ").append(owner.getAssHairType().getFullDescription(owner, true)).append(" around [npc.her] asshole.");
                    break;
                case THREE_TRIMMED:
                    descriptionSB.append(" [npc.She] [npc.has] a small amount of ").append(owner.getAssHairType().getFullDescription(owner, true)).append(" around [npc.her] asshole.");
                    break;
                case FOUR_NATURAL:
                    descriptionSB.append(" [npc.She] [npc.has] a natural amount of ").append(owner.getAssHairType().getFullDescription(owner, true)).append(" around [npc.her] asshole.");
                    break;
                case FIVE_UNKEMPT:
                    descriptionSB.append(" [npc.She] [npc.has] an unkempt mass of ").append(owner.getAssHairType().getFullDescription(owner, true)).append(" around [npc.her] asshole.");
                    break;
                case SIX_BUSHY:
                    descriptionSB.append(" [npc.She] [npc.has] a thick, bushy mass of ").append(owner.getAssHairType().getFullDescription(owner, true)).append(" around [npc.her] asshole.");
                    break;
                case SEVEN_WILD:
                    descriptionSB.append(" [npc.She] [npc.has] a wild, bushy mass of ").append(owner.getAssHairType().getFullDescription(owner, true)).append(" around [npc.her] asshole.");
                    break;
            }
        }
    ...
    public String getBreastDescription(GameCharacter owner, Breast viewedBreast) {
        ...
        if (!owner.isPlayer() && !playerKnowledgeOfBreasts) {
            ...
            descriptionSB.append("On each of [npc.her] ").append(owner.hasBreasts() ? owner.getBreastShape().getDescriptor() + " breasts" : "pecs").append(", [npc.she] [npc.has] ").append(Util.intToString(owner.getNippleCountPerBreast())).append(" [npc.nippleSize], ");
            ...
            if (owner.getNippleCapacity() != Capacity.ZERO_IMPENETRABLE && Main.game.isNipplePenEnabled()) {
                if (viewedBreast.isFuckable()) {
                    descriptionSB.append("<br/>[npc.Her] [npc.breasts] have internal, [npc.nippleSecondaryColour(true)] channels, allowing [npc.her] [npc.breastCapacity] [npc.nipples] to be comfortably penetrated by objects of" + " [style.colourSex(").append(Units.size(Capacity.getCapacityFromValue(viewedBreast.getNipples().getOrificeNipples().getStretchedCapacity()).getMaximumValue(true))).append(")] in diameter when lubricated.");

                    if (Main.game.isPenetrationLimitationsEnabled()) {
                        if (Objects.requireNonNull(owner.getNippleDepth()) == OrificeDepth.TWO_AVERAGE) {
                        ...
                        if (owner.getBodyMaterial().isOrificesLimitedDepth()) {
                            if (owner.hasFetish(Fetish.FETISH_SIZE_QUEEN)) {
                                descriptionSB.append(" and as [npc.sheIsFull] a ").append(Fetish.FETISH_SIZE_QUEEN.getName(owner)).append(", [npc.she] can be pushed to [style.colourMinorGood(comfortably)] accommodate objects of a maximum length of ").append(" [style.colourSex(").append(Units.size(owner.getNippleMaximumPenetrationDepthUncomfortable())).append(")].");
                            } else {
                                descriptionSB.append(" allowing [npc.herHim] to [style.colourMinorGood(comfortably)] accommodate [style.colourSex(").append(Units.size(owner.getNippleMaximumPenetrationDepthComfortable())).append(")] of a penetrative object,").append(" and [style.colourMinorBad(uncomfortably)] accommodate [style.colourSex(").append(Units.size(owner.getNippleMaximumPenetrationDepthUncomfortable())).append(")].");
                            }
                            ...
                            if (owner.hasFetish(Fetish.FETISH_SIZE_QUEEN)) {
                                ...
                                descriptionSB.append(" allowing [npc.herHim] to [style.colourMinorGood(comfortably)] accommodate [style.colourSex(").append(Units.size(owner.getNippleMaximumPenetrationDepthComfortable())).append(")] of a penetrative object,").append(" and as [npc.her] body is made out of [npc.bodyMaterial], [npc.she] can be pushed to [style.colourMinorBad(uncomfortably)] accommodate objects of [style.colourSex(any length)].");
                            }
                        }
                    }

                } else {
                    descriptionSB.append("<br/>[npc.Her] [npc.breasts] have internal, [npc.nippleSecondaryColour(true)] channels," + " but [style.colourBad(they need to be at least C-cups)] before [npc.her] ").append(Capacity.getCapacityFromValue(viewedBreast.getNipples().getOrificeNipples().getStretchedCapacity()).getDescriptor(true)).append(" [npc.nipples] could be penetrated.");
                }
                ...
                descriptionSB.append(" and after being used, they ").append(viewedBreast.getNipples().getOrificeNipples().getPlasticity().getDescriptionPlural()).append(".");
                ...
                if (!viewedBreast.getNipples().getOrificeNipples().isVirgin()) {
                    ...
                    for (SexAreaPenetration pt : SexAreaPenetration.values()) {
                        if (pt.isTakesVirginity()) {
                            if (owner.getVirginityLoss(new SexType(SexParticipantType.NORMAL, SexAreaOrifice.NIPPLE, pt)) != null) {
                                descriptionSB.append(" <span style='color:").append(PresetColour.GENERIC_ARCANE.toWebHexString()).append(";'>").append(owner.getVirginityLossDescription(new SexType(SexParticipantType.NORMAL, SexAreaOrifice.NIPPLE, pt))).append("</span>");
                                virginityLossFound = true;
                                break;
                            }
                        }
                    ...
                    if (!virginityLossFound) {
                        descriptionSB.append(" <span style='color:").append(PresetColour.GENERIC_ARCANE.toWebHexString()).append(";'>[npc.Name] [npc.has] lost [npc.her] nipple virginity.</span>");
                    }
            ...
            if (viewedBreast.getRawMilkStorageValue() > 0) {
                descriptionSB.append("<br/>[npc.SheIsFull] currently producing ").append(Units.fluid(viewedBreast.getRawMilkStorageValue(), Units.UnitType.LONG)).append(" of [npc.milkPrimaryColour(true)] [npc.milk]").append(" (").append(Units.fluid(viewedBreast.getRawStoredMilkValue())).append(" currently stored) at [npc.a_milkRegen] rate.");
                ...
                switch (viewedBreast.getMilk().getFlavour()) {
                    ...
                    case FLAVOURLESS:
                        ...
                        break;
                    default:
                        descriptionSB.append(" [npc.Her] [npc.milkColour(true)] [npc.milk] tastes exactly like ").append(viewedBreast.getMilk().getFlavour().getName()).append(".");
                        break;
                }

                for (FluidModifier fm : FluidModifier.values()) {
                    if (owner.hasMilkModifier(fm)) {
                        descriptionSB.append(" ").append(fm.getBriefDescription());
                    }
                }
...
//		if(owner.isPlayer() || playerKnowledgeOfUdders) {
        ...
        if (breastCrotch.getShape() == BreastShape.UDDERS) {
            ...
            if (breastCrotch.getRows() > 0) {
                descriptionSB.append(breastCrotch.getRows() > 1 ? " pairs" : " pair").append(" of udders.");
            ...
            if (leg.getLegConfiguration().isBipedalPositionedCrotchBoobs() && breastCrotch.getRows() > 1) {
                descriptionSB.append(", with the upper pair").append(breastCrotch.getRows() > 2 ? "s" : "").append(" being positioned higher up on [npc.her] stomach.");

            } else if (breastCrotch.getRows() > 1) {
                descriptionSB.append(", with the extra pair").append(breastCrotch.getRows() > 2 ? "s" : "").append(" being positioned further towards [npc.her] underbelly.");
        ...
        if (owner.getNippleCrotchCapacity() != Capacity.ZERO_IMPENETRABLE && Main.game.isNipplePenEnabled()) {
            if (viewedBreastCrotch.isFuckable()) {
                descriptionSB.append("<br/>[npc.Her] [npc.crotchBoobs] have internal, [npc.crotchNippleSecondaryColour(true)] channels, allowing [npc.her] [npc.crotchBoobsCapacity] [npc.crotchNipples] to be comfortably penetrated by objects of" + " [style.colourSex(").append(Units.size(Capacity.getCapacityFromValue(viewedBreastCrotch.getNipples().getOrificeNipples().getStretchedCapacity()).getMaximumValue(true))).append(")] in diameter when lubricated.");

                if (Main.game.isPenetrationLimitationsEnabled()) {
                    if (Objects.requireNonNull(owner.getNippleCrotchDepth()) == OrificeDepth.TWO_AVERAGE) {
                    ...
                    if (owner.getBodyMaterial().isOrificesLimitedDepth()) {
                        if (owner.hasFetish(Fetish.FETISH_SIZE_QUEEN)) {
                            descriptionSB.append(" and as [npc.sheIsFull] a ").append(Fetish.FETISH_SIZE_QUEEN.getName(owner)).append(", [npc.she] can be pushed to [style.colourMinorGood(comfortably)] accommodate objects of a maximum length of ").append(" [style.colourSex(").append(Units.size(owner.getNippleCrotchMaximumPenetrationDepthUncomfortable())).append(")].");
                        } else {
                            descriptionSB.append(" allowing [npc.herHim] to [style.colourMinorGood(comfortably)] accommodate [style.colourSex(").append(Units.size(owner.getNippleCrotchMaximumPenetrationDepthComfortable())).append(")] of a penetrative object,").append(" and [style.colourMinorBad(uncomfortably)] accommodate [style.colourSex(").append(Units.size(owner.getNippleCrotchMaximumPenetrationDepthUncomfortable())).append(")].");
                        }
                        ...
                        if (owner.hasFetish(Fetish.FETISH_SIZE_QUEEN)) {
                            ...
                            descriptionSB.append(" allowing [npc.herHim] to [style.colourMinorGood(comfortably)] accommodate [style.colourSex(").append(Units.size(owner.getNippleCrotchMaximumPenetrationDepthComfortable())).append(")] of a penetrative object,").append(" and as [npc.her] body is made out of [npc.bodyMaterial], [npc.she] can be pushed to [style.colourMinorBad(uncomfortably)] accommodate objects of [style.colourSex(any length)].");
                        }
                    }
                }

            } else {
                descriptionSB.append("<br/>[npc.Her] [npc.crotchBoobs] have internal, [npc.crotchNippleSecondaryColour(true)] channels," + " but [style.colourBad(they need to be at least C-cups)] before [npc.her] ").append(Capacity.getCapacityFromValue(viewedBreastCrotch.getNipples().getOrificeNipples().getStretchedCapacity()).getDescriptor(true)).append(" [npc.crotchNipples] could be penetrated.");
            }
            ...
            descriptionSB.append(" and after being used, they ").append(viewedBreastCrotch.getNipples().getOrificeNipples().getPlasticity().getDescriptionPlural()).append(".");
            ...
            if (!viewedBreastCrotch.getNipples().getOrificeNipples().isVirgin()) {
                ...
                for (SexAreaPenetration pt : SexAreaPenetration.values()) {
                    if (pt.isTakesVirginity()) {
                        if (owner.getVirginityLoss(new SexType(SexParticipantType.NORMAL, SexAreaOrifice.NIPPLE_CROTCH, pt)) != null) {
                            descriptionSB.append(" <span style='color:").append(PresetColour.GENERIC_ARCANE.toWebHexString()).append(";'>").append(owner.getVirginityLossDescription(new SexType(SexParticipantType.NORMAL, SexAreaOrifice.NIPPLE_CROTCH, pt))).append("</span>");
                            virginityLossFound = true;
                            break;
                        }
                    }
                ...
                if (!virginityLossFound) {
                    descriptionSB.append(" <span style='color:").append(PresetColour.GENERIC_ARCANE.toWebHexString()).append(";'>[npc.Name] [npc.has] lost [npc.her] [npc.crotchNipple] virginity.</span>");
                }
        ...
        if (viewedBreastCrotch.getRawMilkStorageValue() > 0) {
            descriptionSB.append("<br/>[npc.SheIsFull] currently producing ").append(Units.fluid(viewedBreastCrotch.getRawMilkStorageValue(), Units.UnitType.LONG)).append(" of [npc.crotchMilkPrimaryColour(true)] [npc.crotchMilk] (").append(Units.fluid(viewedBreastCrotch.getRawStoredMilkValue(), Units.UnitType.LONG)).append(" currently stored) at [npc.a_crotchMilkRegen] rate.");
            ...
            switch (viewedBreastCrotch.getMilk().getFlavour()) {
                ...
                case FLAVOURLESS:
                    ...
                    break;
                default:
                    descriptionSB.append(" [npc.Her] [npc.crotchMilkColour(true)] [npc.crotchMilk] tastes exactly like ").append(viewedBreastCrotch.getMilk().getFlavour().getName()).append(".");
                    break;
            }
...
//		}
    ...
    public String getPenisDescription(GameCharacter owner) {
        ...
        if (Main.game.getPlayer().hasIngestedPsychoactiveFluidType(FluidTypeBase.CUM)) {
            ...
            descriptionSB.append("<i style='color:").append(PresetColour.PSYCHOACTIVE.toWebHexString()).append(";'>The psychoactive cum you recently ingested is causing your view of ").append(owner.isPlayer() ? "your" : "[npc.namePos]").append(" cock to be distorted!</i> ");
        }
        ...
        if (Main.game.getPlayer().hasIngestedPsychoactiveFluidType(FluidTypeBase.CUM)) {
            ...
            descriptionSB.append(" It measures ").append(Units.size(viewedPenis.getDiameter(), Units.UnitType.LONG)).append(" in diameter (").append(Units.size(viewedPenis.getDiameter() * Math.PI, Units.UnitType.LONG)).append(" in circumference).");
        ...
        if (owner.getCurrentPenis().getType() != PenisType.DILDO) {
            if (!owner.getCurrentPenis().isVirgin()) {
                ...
                for (SexAreaOrifice ot : SexAreaOrifice.values()) {
                    if (ot.isInternalOrifice()) {
                        if (owner.getVirginityLoss(new SexType(SexParticipantType.NORMAL, SexAreaPenetration.PENIS, ot)) != null) {
                            descriptionSB.append(" <span style='color:").append(PresetColour.GENERIC_ARCANE.toWebHexString()).append(";'>").append(owner.getVirginityLossDescription(new SexType(SexParticipantType.NORMAL, SexAreaPenetration.PENIS, ot))).append("</span>");
                            virginityLossFound = true;
                            break;
                        }
                    }
                ...
                if (!virginityLossFound) {
                    descriptionSB.append(" <span style='color:").append(PresetColour.GENERIC_ARCANE.toWebHexString()).append(";'>[npc.Name] [npc.has] lost [npc.her] penile virginity.</span>");
                }
        ...
        if (Capacity.getCapacityFromValue(viewedPenis.getOrificeUrethra().getStretchedCapacity()) != Capacity.ZERO_IMPENETRABLE) {
            descriptionSB.append("<br/>[npc.Her] cock's urethra has been loosened enough that it presents a ready orifice for penetration, and can be comfortably penetrated by objects of" + " [style.colourSex(").append(Units.size(Capacity.getCapacityFromValue(viewedPenis.getOrificeUrethra().getStretchedCapacity()).getMaximumValue(true))).append(")] in diameter when lubricated.");

            if (Main.game.isPenetrationLimitationsEnabled()) {
                if (Objects.requireNonNull(owner.getUrethraDepth()) == OrificeDepth.TWO_AVERAGE) {
                ...
                if (owner.getBodyMaterial().isOrificesLimitedDepth()) {
                    if (owner.hasFetish(Fetish.FETISH_SIZE_QUEEN)) {
                        descriptionSB.append(" and as [npc.sheIsFull] a ").append(Fetish.FETISH_SIZE_QUEEN.getName(owner)).append(", [npc.she] can be pushed to [style.colourMinorGood(comfortably)] accommodate objects of a maximum length of ").append(" [style.colourSex(").append(Units.size(owner.getUrethraMaximumPenetrationDepthUncomfortable())).append(")].");
                    } else {
                        descriptionSB.append(" allowing [npc.herHim] to [style.colourMinorGood(comfortably)] accommodate [style.colourSex(").append(Units.size(owner.getUrethraMaximumPenetrationDepthComfortable())).append(")] of a penetrative object,").append(" and [style.colourMinorBad(uncomfortably)] accommodate [style.colourSex(").append(Units.size(owner.getUrethraMaximumPenetrationDepthUncomfortable())).append(")].");
                    }
                    ...
                    if (owner.hasFetish(Fetish.FETISH_SIZE_QUEEN)) {
                        ...
                        descriptionSB.append(" allowing [npc.herHim] to [style.colourMinorGood(comfortably)] accommodate [style.colourSex(").append(Units.size(owner.getUrethraMaximumPenetrationDepthComfortable())).append(")] of a penetrative object,").append(" and as [npc.her] body is made out of [npc.bodyMaterial], [npc.she] can be pushed to [style.colourMinorBad(uncomfortably)] accommodate objects of [style.colourSex(any length)].");
                    }
                }
            }
            ...
            descriptionSB.append(" and after being used, it ").append(viewedPenis.getOrificeUrethra().getPlasticity().getDescription()).append(".");
            ...
            if (isPlayer && !owner.isUrethraVirgin()) {
                for (SexAreaPenetration pt : SexAreaPenetration.values()) {
                    if (owner.getVirginityLoss(new SexType(SexParticipantType.NORMAL, SexAreaOrifice.URETHRA_PENIS, pt)) != null) {
                        descriptionSB.append(" <span style='color:").append(PresetColour.GENERIC_ARCANE.toWebHexString()).append(";'>").append(owner.getVirginityLossDescription(new SexType(SexParticipantType.NORMAL, SexAreaOrifice.URETHRA_PENIS, pt))).append("</span>");
                        break;
                    }
                }
            }
            ...
            if (Main.game.isPubicHairEnabled()) {
                if (owner.getPubicHairType().getType() == BodyCoveringType.BODY_HAIR_SCALES_ALLIGATOR) {
                    switch (owner.getPubicHair()) {
                        case ZERO_NONE:
                            descriptionSB.append(" There's no trace of any rough  ").append(owner.getPubicHairType().getName(owner)).append(" around the base of [npc.her] cock.");
                            break;
                        case ONE_STUBBLE:
                            descriptionSB.append(" [npc.She] [npc.has] a small amount of rough ").append(owner.getPubicHairType().getFullDescription(owner, true)).append(" around the base of [npc.her] cock.");
                            break;
                        case TWO_MANICURED:
                            descriptionSB.append(" [npc.She] [npc.has] a rough patch of ").append(owner.getPubicHairType().getFullDescription(owner, true)).append(" around the base of [npc.her] cock.");
                            break;
                        case THREE_TRIMMED:
                            descriptionSB.append(" [npc.She] [npc.has] a rough patch of ").append(owner.getPubicHairType().getFullDescription(owner, true)).append(" around the base of [npc.her] cock.");
                            break;
                        case FOUR_NATURAL:
                            descriptionSB.append(" [npc.She] [npc.has] a natural amount of rough ").append(owner.getPubicHairType().getFullDescription(owner, true)).append(" around the base of [npc.her] cock.");
                            break;
                        case FIVE_UNKEMPT:
                            descriptionSB.append(" [npc.She] [npc.has] an unkempt mass of rough ").append(owner.getPubicHairType().getFullDescription(owner, true)).append(" around the base of [npc.her] cock.");
                            break;
                        case SIX_BUSHY:
                            descriptionSB.append(" [npc.She] [npc.has] a thick, rough mass of ").append(owner.getPubicHairType().getFullDescription(owner, true)).append(" around the base of [npc.her] cock.");
                            break;
                        case SEVEN_WILD:
                            descriptionSB.append(" [npc.She] [npc.has] a wild, rough mass of ").append(owner.getPubicHairType().getFullDescription(owner, true)).append(" around the base of [npc.her] cock.");
                            break;
                    }
                } else {
                    switch (owner.getPubicHair()) {
                        case ZERO_NONE:
                            descriptionSB.append(" There is no trace of any ").append(owner.getPubicHairType().getName(owner)).append(" around the base of [npc.her] cock.");
                            break;
                        case ONE_STUBBLE:
                            descriptionSB.append(" [npc.She] [npc.has] a stubbly patch of ").append(owner.getPubicHairType().getFullDescription(owner, true)).append(" around the base of [npc.her] cock.");
                            break;
                        case TWO_MANICURED:
                            descriptionSB.append(" [npc.She] [npc.has] a neat, manicured patch of ").append(owner.getPubicHairType().getFullDescription(owner, true)).append(" around the base of [npc.her] cock.");
                            break;
                        case THREE_TRIMMED:
                            descriptionSB.append(" [npc.She] [npc.has] a trimmed patch of ").append(owner.getPubicHairType().getFullDescription(owner, true)).append(" around the base of [npc.her] cock.");
                            break;
                        case FOUR_NATURAL:
                            descriptionSB.append(" [npc.She] [npc.has] a natural bush of ").append(owner.getPubicHairType().getFullDescription(owner, true)).append(" around the base of [npc.her] cock.");
                            break;
                        case FIVE_UNKEMPT:
                            descriptionSB.append(" [npc.She] [npc.has] an unkempt bush of ").append(owner.getPubicHairType().getFullDescription(owner, true)).append(" around the base of [npc.her] cock.");
                            break;
                        case SIX_BUSHY:
                            descriptionSB.append(" [npc.She] [npc.has] a thick, bushy mass of ").append(owner.getPubicHairType().getFullDescription(owner, true)).append(" around the base of [npc.her] cock.");
                            break;
                        case SEVEN_WILD:
                            descriptionSB.append(" [npc.She] [npc.has] a wild, bushy mass of ").append(owner.getPubicHairType().getFullDescription(owner, true)).append(" around the base of [npc.her] cock.");
                            break;
                    }
                }
            }
        ...
        if (viewedPenis.getType() != PenisType.DILDO) {
            ...
            switch (viewedPenis.getTesticle().getCum().getFlavour()) {
                ...
                case FLAVOURLESS:
                    ...
                    break;
                default:
                    descriptionSB.append(" tastes exactly like ").append(viewedPenis.getTesticle().getCum().getFlavour().getName()).append(".");
                    break;
            }
    ...
    public String getVaginaDescription(GameCharacter owner) {
        ...
        if (Main.game.getPlayer().hasIngestedPsychoactiveFluidType(FluidTypeBase.GIRLCUM)) {
            ...
            descriptionSB.append("<i style='color:").append(PresetColour.PSYCHOACTIVE.toWebHexString()).append(";'>The psychoactive girlcum which you recently ingested is causing your view of ").append(owner.isPlayer() ? "your" : "[npc.namePos]").append(" pussy to be distorted!</i> ");
            hallucinating = true;
        }
        ...
        // Clit:
        descriptionSB.append("<br/>[npc.She] [npc.has] [npc.a_clitSize]").append(owner.getClitorisGirth() == PenetrationGirth.THREE_AVERAGE ? "" : ", [npc.clitGirth]").append(" clit, which measures [npc.clitSizeValue] in length and [npc.clitDiameter(true)] in diameter ([npc.clitCircumference(true)] in circumference).");
        ...
        if (viewedVagina.getOrificeVagina().isVirgin()) {
            if (isPlayer || !hallucinating) {
                if (viewedVagina.getOrificeVagina().hasHymen()) {
                    descriptionSB.append(" Within [npc.her] ").append(Capacity.getCapacityFromValue(viewedVagina.getOrificeVagina().getStretchedCapacity()).getDescriptor(true)).append(" [npc.pussy], [style.colourMinorGood([npc.her] hymen is still intact)], and [style.colourExcellent([npc.she] [npc.has] retained [npc.her] vaginal virginity)].");
                    ...
                    if (owner.isDoll()) {
                        ...
                        descriptionSB.append(" Within [npc.her] ").append(Capacity.getCapacityFromValue(viewedVagina.getOrificeVagina().getStretchedCapacity()).getDescriptor(true)).append(" [npc.pussy], [style.colourMinorBad([npc.her] hymen has been torn)], but despite this, [style.colourExcellent([npc.she] [npc.has] retained [npc.her] vaginal virginity)].");
                    }
                }
            }
            ...
            for (SexAreaPenetration pt : SexAreaPenetration.values()) {
                if (pt.isTakesVirginity()) {
                    if (owner.getVirginityLoss(new SexType(SexParticipantType.NORMAL, SexAreaOrifice.VAGINA, pt)) != null) {
                        descriptionSB.append(" [style.colourArcane(").append(owner.getVirginityLossDescription(new SexType(SexParticipantType.NORMAL, SexAreaOrifice.VAGINA, pt))).append(")]");
                        virginityLossFound = true;
                        break;
                    }
                }
        ...
        if (isPlayer || !hallucinating) {
            descriptionSB.append(" [npc.Her] pussy is ").append(Capacity.getCapacityFromValue(viewedVagina.getOrificeVagina().getStretchedCapacity()).getDescriptor(true)).append(" and when lubricated can comfortably accommodate objects of up to").append(" [style.colourSex(").append(Units.size(Capacity.getMaximumComfortableDiameter(viewedVagina.orificeVagina.getElasticity(), viewedVagina.getOrificeVagina().getRawCapacityValue(), true))).append(")] in diameter.");
        }

        if (Main.game.isPenetrationLimitationsEnabled()) {
            if (Objects.requireNonNull(owner.getVaginaDepth()) == OrificeDepth.TWO_AVERAGE) {
            ...
            if (owner.getBodyMaterial().isOrificesLimitedDepth()) {
                if (owner.hasFetish(Fetish.FETISH_SIZE_QUEEN)) {
                    descriptionSB.append(" and as [npc.sheIsFull] a ").append(Fetish.FETISH_SIZE_QUEEN.getName(owner)).append(", [npc.she] can be pushed to [style.colourMinorGood(comfortably)] accommodate objects of a maximum length of ").append(" [style.colourSex(").append(Units.size(owner.getVaginaMaximumPenetrationDepthUncomfortable())).append(")].");
                } else {
                    descriptionSB.append(" allowing [npc.herHim] to [style.colourMinorGood(comfortably)] accommodate [style.colourSex(").append(Units.size(owner.getVaginaMaximumPenetrationDepthComfortable())).append(")] of a penetrative object,").append(" and [style.colourMinorBad(uncomfortably)] accommodate [style.colourSex(").append(Units.size(owner.getVaginaMaximumPenetrationDepthUncomfortable())).append(")].");
                }
                ...
                if (owner.hasFetish(Fetish.FETISH_SIZE_QUEEN)) {
                    ...
                    descriptionSB.append(" allowing [npc.herHim] to [style.colourMinorGood(comfortably)] accommodate [style.colourSex(").append(Units.size(owner.getVaginaMaximumPenetrationDepthComfortable())).append(")] of a penetrative object,").append(" and as [npc.her] body is made out of [npc.bodyMaterial], [npc.she] can be pushed to [style.colourMinorBad(uncomfortably)] accommodate objects of [style.colourSex(any length)].");
                }
            }
        }
        ...
        descriptionSB.append(" and after being used, it ").append(viewedVagina.getOrificeVagina().getPlasticity().getDescription()).append(".");
        ...
        switch (viewedVagina.getGirlcum().getFlavour()) {
            ...
            case FLAVOURLESS:
                ...
                break;
            default:
                descriptionSB.append(" tastes exactly like ").append(viewedVagina.getGirlcum().getFlavour().getName()).append(".");
                break;
        }
        ...
        if (Capacity.getCapacityFromValue(viewedVagina.getOrificeUrethra().getStretchedCapacity()) != Capacity.ZERO_IMPENETRABLE) {
            descriptionSB.append("[npc.Her] vagina's urethra has been loosened enough that it presents a ready orifice for penetration, and when lubricated can comfortably accommodate objects of up to" + " [style.colourSex(").append(Units.size(Capacity.getMaximumComfortableDiameter(viewedVagina.getOrificeUrethra().getElasticity(), viewedVagina.getOrificeUrethra().getRawCapacityValue(), true))).append(")] in diameter.");

            if (Main.game.isPenetrationLimitationsEnabled()) {
                if (Objects.requireNonNull(owner.getVaginaUrethraDepth()) == OrificeDepth.TWO_AVERAGE) {
                ...
                if (owner.getBodyMaterial().isOrificesLimitedDepth()) {
                    if (owner.hasFetish(Fetish.FETISH_SIZE_QUEEN)) {
                        descriptionSB.append(" and as [npc.sheIsFull] a ").append(Fetish.FETISH_SIZE_QUEEN.getName(owner)).append(", [npc.she] can be pushed to [style.colourMinorGood(comfortably)] accommodate objects of a maximum length of ").append(" [style.colourSex(").append(Units.size(owner.getVaginaUrethraMaximumPenetrationDepthUncomfortable())).append(")].");
                    } else {
                        descriptionSB.append(" allowing [npc.herHim] to [style.colourMinorGood(comfortably)] accommodate [style.colourSex(").append(Units.size(owner.getVaginaUrethraMaximumPenetrationDepthComfortable())).append(")] of a penetrative object,").append(" and [style.colourMinorBad(uncomfortably)] accommodate [style.colourSex(").append(Units.size(owner.getVaginaUrethraMaximumPenetrationDepthUncomfortable())).append(")].");
                    }
                    ...
                    if (owner.hasFetish(Fetish.FETISH_SIZE_QUEEN)) {
                        ...
                        descriptionSB.append(" allowing [npc.herHim] to [style.colourMinorGood(comfortably)] accommodate [style.colourSex(").append(Units.size(owner.getVaginaUrethraMaximumPenetrationDepthComfortable())).append(")] of a penetrative object,").append(" and as [npc.her] body is made out of [npc.bodyMaterial], [npc.she] can be pushed to [style.colourMinorBad(uncomfortably)] accommodate objects of [style.colourSex(any length)].");
                    }
                }
            }
            ...
            descriptionSB.append(" and after being used, it ").append(viewedVagina.getOrificeUrethra().getPlasticity().getDescription()).append(".");
        ...
        if (Main.game.isPubicHairEnabled()) {
            if (owner.getPubicHairType().getType() == BodyCoveringType.BODY_HAIR_SCALES_ALLIGATOR) {
                switch (owner.getPubicHair()) {
                    case ZERO_NONE:
                        descriptionSB.append(" There's no trace of any rough  ").append(owner.getPubicHairType().getName(owner)).append(" around [npc.her] [npc.pussy].");
                        break;
                    case ONE_STUBBLE:
                        descriptionSB.append(" [npc.SheHasFull] a small amount of rough ").append(owner.getPubicHairType().getFullDescription(owner, true)).append(" around [npc.her] [npc.pussy].");
                        break;
                    case TWO_MANICURED:
                        descriptionSB.append(" [npc.SheHasFull] a rough patch of ").append(owner.getPubicHairType().getFullDescription(owner, true)).append(" around [npc.her] [npc.pussy].");
                        break;
                    case THREE_TRIMMED:
                        descriptionSB.append(" [npc.SheHasFull] a rough patch of ").append(owner.getPubicHairType().getFullDescription(owner, true)).append(" around [npc.her] [npc.pussy].");
                        break;
                    case FOUR_NATURAL:
                        descriptionSB.append(" [npc.SheHasFull] a natural amount of rough ").append(owner.getPubicHairType().getFullDescription(owner, true)).append(" around [npc.her] [npc.pussy].");
                        break;
                    case FIVE_UNKEMPT:
                        descriptionSB.append(" [npc.SheHasFull] an unkempt mass of rough ").append(owner.getPubicHairType().getFullDescription(owner, true)).append(" around [npc.her] [npc.pussy].");
                        break;
                    case SIX_BUSHY:
                        descriptionSB.append(" [npc.SheHasFull] a thick, rough mass of ").append(owner.getPubicHairType().getFullDescription(owner, true)).append(" around [npc.her] [npc.pussy].");
                        break;
                    case SEVEN_WILD:
                        descriptionSB.append(" [npc.SheHasFull] a wild, rough mass of ").append(owner.getPubicHairType().getFullDescription(owner, true)).append(" around [npc.her] [npc.pussy].");
                        break;
                }
            } else {
                switch (owner.getPubicHair()) {
                    case ZERO_NONE:
                        descriptionSB.append(" There is no trace of any ").append(owner.getPubicHairType().getName(owner)).append(" around [npc.her] [npc.pussy].");
                        break;
                    case ONE_STUBBLE:
                        descriptionSB.append(" [npc.SheHasFull] a stubbly patch of ").append(owner.getPubicHairType().getFullDescription(owner, true)).append(" around [npc.her] [npc.pussy].");
                        break;
                    case TWO_MANICURED:
                        descriptionSB.append(" [npc.SheHasFull] a neat, manicured patch of ").append(owner.getPubicHairType().getFullDescription(owner, true)).append(" around [npc.her] [npc.pussy].");
                        break;
                    case THREE_TRIMMED:
                        descriptionSB.append(" [npc.SheHasFull] a trimmed patch of ").append(owner.getPubicHairType().getFullDescription(owner, true)).append(" around [npc.her] [npc.pussy].");
                        break;
                    case FOUR_NATURAL:
                        descriptionSB.append(" [npc.SheHasFull] a natural bush of ").append(owner.getPubicHairType().getFullDescription(owner, true)).append(" around [npc.her] [npc.pussy].");
                        break;
                    case FIVE_UNKEMPT:
                        descriptionSB.append(" [npc.SheHasFull] an unkempt bush of ").append(owner.getPubicHairType().getFullDescription(owner, true)).append(" around [npc.her] [npc.pussy].");
                        break;
                    case SIX_BUSHY:
                        descriptionSB.append(" [npc.SheHasFull] a thick, bushy mass of ").append(owner.getPubicHairType().getFullDescription(owner, true)).append(" around [npc.her] [npc.pussy].");
                        break;
                    case SEVEN_WILD:
                        descriptionSB.append(" [npc.SheHasFull] a wild, bushy mass of ").append(owner.getPubicHairType().getFullDescription(owner, true)).append(" around [npc.her] [npc.pussy].");
                        break;
                }
            }
        }
        ...
        if (isPlayer && !owner.isVaginaUrethraVirgin()) {
            for (SexAreaPenetration pt : SexAreaPenetration.values()) {
                if (owner.getVirginityLoss(new SexType(SexParticipantType.NORMAL, SexAreaOrifice.URETHRA_VAGINA, pt)) != null) {
                    descriptionSB.append(" <span style='color:").append(PresetColour.GENERIC_ARCANE.toWebHexString()).append(";'>").append(owner.getVirginityLossDescription(new SexType(SexParticipantType.NORMAL, SexAreaOrifice.URETHRA_VAGINA, pt))).append("</span>");
                    break;
                }
            }
        }
    ...
    public String getSpinneretDescription(GameCharacter owner) {
            ...
            for (SexAreaPenetration pt : SexAreaPenetration.values()) {
                if (pt.isTakesVirginity()) {
                    if (owner.getVirginityLoss(new SexType(SexParticipantType.NORMAL, SexAreaOrifice.SPINNERET, pt)) != null) {
                        descriptionSB.append(" [style.colourArcane(").append(owner.getVirginityLossDescription(new SexType(SexParticipantType.NORMAL, SexAreaOrifice.SPINNERET, pt))).append(")]");
                        virginityLossFound = true;
                        break;
                    }
                }
        ...
        descriptionSB.append(" [npc.Her] spinneret is ").append(Capacity.getCapacityFromValue(spinneret.getStretchedCapacity()).getDescriptor(true)).append(" and when lubricated can comfortably accommodate objects of up to").append(" [style.colourSex(").append(Units.size(Capacity.getMaximumComfortableDiameter(spinneret.getElasticity(), spinneret.getRawCapacityValue(), true))).append(")] in diameter.");

        if (Main.game.isPenetrationLimitationsEnabled()) {
            if (Objects.requireNonNull(owner.getSpinneretDepth()) == OrificeDepth.TWO_AVERAGE) {
            ...
            if (owner.getBodyMaterial().isOrificesLimitedDepth()) {
                if (owner.hasFetish(Fetish.FETISH_SIZE_QUEEN)) {
                    descriptionSB.append(" and as [npc.sheIsFull] a ").append(Fetish.FETISH_SIZE_QUEEN.getName(owner)).append(", [npc.she] can be pushed to [style.colourMinorGood(comfortably)] accommodate objects of a maximum length of ").append(" [style.colourSex(").append(Units.size(owner.getSpinneretMaximumPenetrationDepthUncomfortable())).append(")].");
                } else {
                    descriptionSB.append(" allowing [npc.herHim] to [style.colourMinorGood(comfortably)] accommodate [style.colourSex(").append(Units.size(owner.getSpinneretMaximumPenetrationDepthComfortable())).append(")] of a penetrative object,").append(" and [style.colourMinorBad(uncomfortably)] accommodate [style.colourSex(").append(Units.size(owner.getSpinneretMaximumPenetrationDepthUncomfortable())).append(")].");
                }
        ...
        descriptionSB.append(" and after being used, it ").append(spinneret.getPlasticity().getDescription()).append(".");
    ...
    private String getPregnancyDetails(GameCharacter owner) {
        ...
        if (owner.isVisiblyPregnant()) {
            ...
            if (father == null) {
                descriptionSB.append("<span style='color:").append(PresetColour.GENERIC_ARCANE.toWebHexString()).append(";'>From one of [npc.her] sexual encounters, [npc.name] has ended up getting impregnated.</span>");
            } else if (father.isPlayer()) {
                descriptionSB.append("<span style='color:").append(PresetColour.GENERIC_ARCANE.toWebHexString()).append(";'>From one of your sexual encounters, you've ended up impregnating [npc.name].</span>");
            } else {
                descriptionSB.append("<span style='color:").append(PresetColour.GENERIC_ARCANE.toWebHexString()).append(";'>From one of [npc.her] sexual encounters, [npc.name] has ended up getting impregnated by ").append(father.getName(true)).append(".</span>");
            }
        ...
        if (!owner.getLittersBirthed().isEmpty()) {
            ...
            descriptionSB.append("<span style='color:").append(PresetColour.GENERIC_ARCANE.toWebHexString()).append(";'>").append("[npc.Name] has given birth ").append(Util.intToString(owner.getLittersBirthed().size())).append(" ").append(owner.getLittersBirthed().size() == 1 ? "time" : "times").append(".</span>");

            for (Litter litter : owner.getLittersBirthed()) {
                if (litter.getFather() == null) {
                    descriptionSB.append("<br/>On ").append(Units.date(litter.getConceptionDate(), Units.DateType.LONG)).append(", [npc.she] was impregnated, and then on ").append(Units.date(litter.getBirthDate(), Units.DateType.LONG)).append(", [npc.she] gave birth to ");

                } else if (litter.getFather().isPlayer()) {
                    descriptionSB.append("<br/>On ").append(Units.date(litter.getConceptionDate(), Units.DateType.LONG)).append(", you impregnated [npc.herHim], and then on ").append(Units.date(litter.getBirthDate(), Units.DateType.LONG)).append(", [npc.she] gave birth to ");

                } else {
                    descriptionSB.append("<br/>On ").append(Units.date(litter.getConceptionDate(), Units.DateType.LONG)).append(", ").append(litter.getFather().getName(true)).append(" impregnated [npc.herHim], and then on ").append(Units.date(litter.getBirthDate(), Units.DateType.LONG)).append(", [npc.she] gave birth to ");
                }
        ...
        if (Main.game.getPlayer().isVisiblyPregnant()) {
            for (PregnancyPossibility pp : Main.game.getPlayer().getPotentialPartnersAsMother()) {
                if (pp.getFather() != null && pp.getFather().equals(owner)) {
                    ...
                    descriptionSB.append("<span style='color:").append(PresetColour.GENERIC_ARCANE.toWebHexString()).append(";'>From one of your sexual encounters, you've been impregnated, and it's possible that [npc.name] is the father.</span>");
                    descriptionSB.append("</p>");
                    sectionAdded = true;
                    break;
                }
        ...
        if (!Main.game.getPlayer().getLittersBirthed().isEmpty()) {
            ...
            if (fatheredLitters != 0) {
                ...
                descriptionSB.append("<span style='color:").append(PresetColour.GENERIC_ARCANE.toWebHexString()).append(";'>").append("[npc.Name] is the father of some of your children, and has, in total, impregnated you ").append(Util.intToString(fatheredLitters)).append(" ").append(fatheredLitters == 1 ? "time" : "times").append(".</span>");

                for (Litter litter : Main.game.getPlayer().getLittersBirthed()) {
                    if (litter.getFather() != null && litter.getFather().equals(owner)) {
                        descriptionSB.append("<br/>On ").append(Units.date(litter.getConceptionDate(), Units.DateType.LONG)).append(", [npc.she] impregnated you, and then on ").append(Units.date(litter.getBirthDate(), Units.DateType.LONG)).append(", you gave birth to ").append(litter.getBirthedDescription()).append(".");
                    }
                }
    ...
    private String getIncubationPregnancyDetails(GameCharacter owner) {
        ...
        if (!owner.getIncubatingLitters().isEmpty()) {
            ...
            for (Entry<SexAreaOrifice, Litter> entry : owner.getIncubatingLitters().entrySet()) {
                ...
                descriptionSB.append("<span style='color:").append(PresetColour.GENERIC_ARCANE.toWebHexString()).append(";'>");
                if (mother == null) {
                    descriptionSB.append("From one of [npc.her] sexual encounters, [npc.name] has [npc.her] ").append(areaEgged).append(" filled with eggs.");
                } else if (mother.isPlayer()) {
                    descriptionSB.append("From one of your sexual encounters, you've ended up filling [npc.namePos] ").append(areaEgged).append(" with eggs.");
                } else {
                    descriptionSB.append("From one of [npc.her] sexual encounters, [npc.name] has ended up having [npc.her] ").append(areaEgged).append(" filled with eggs by ").append(mother.getName(true)).append(".");
                }
        ...
        if (!owner.getLittersIncubated().isEmpty()) {
            ...
            sectionAdded = true;
            descriptionSB.append("<span style='color:").append(PresetColour.GENERIC_ARCANE.toWebHexString()).append(";'>").append("[npc.Name] has incubated and laid eggs ").append(Util.intToString(owner.getLittersIncubated().size())).append(" ").append(owner.getLittersIncubated().size() == 1 ? "time" : "times").append(".").append("</span>");
            ...
            for (Litter litter : owner.getLittersIncubated()) {
                if (litter.getMother() == null) {
                    descriptionSB.append("<br/>On ").append(Units.date(litter.getConceptionDate(), Units.DateType.LONG)).append(", [npc.she] was implanted with ").append(litter.getTotalLitterCount()).append(" eggs, and then on ").append(Units.date(litter.getBirthDate(), Units.DateType.LONG)).append(", [npc.she] laid and birthed ");

                } else if (litter.getMother().isPlayer()) {
                    descriptionSB.append("<br/>On ").append(Units.date(litter.getConceptionDate(), Units.DateType.LONG)).append(", you implanted [npc.herHim] with ").append(litter.getTotalLitterCount()).append(" eggs, and then on ").append(Units.date(litter.getBirthDate(), Units.DateType.LONG)).append(", [npc.she] laid and birthed ");

                } else {
                    descriptionSB.append("<br/>On ").append(Units.date(litter.getConceptionDate(), Units.DateType.LONG)).append(", ").append(litter.getMother().getName(true)).append(" implanted [npc.herHim] with ").append(litter.getTotalLitterCount()).append(" eggs, and then on ").append(Units.date(litter.getBirthDate(), Units.DateType.LONG)).append(", [npc.she] laid and birthed ");
                }
        ...
        if (!Main.game.getPlayer().getIncubatingLitters().isEmpty()) {
            ...
            if (!areasEgged.isEmpty()) {
                ...
                sectionAdded = true;
                descriptionSB.append("<span style='color:").append(PresetColour.GENERIC_ARCANE.toWebHexString()).append(";'>").append("[npc.NameIsFull] the one responsible for implanting the eggs that are currently in your ").append(Util.stringsToStringList(areasEgged, false)).append(".").append("</span>");
                descriptionSB.append("</p>");
            }
        }
        ...
        if (!Main.game.getPlayer().getLittersIncubated().isEmpty()) {
            ...
            if (incubatedLitters != 0) {
                ...
                descriptionSB.append("<span style='color:").append(PresetColour.GENERIC_ARCANE.toWebHexString()).append(";'>").append("[npc.Name] is responsible for previously having laid [npc.her] eggs in you, and in total, [npc.sheHasFull] done so ").append(Util.intToString(incubatedLitters)).append(" ").append(incubatedLitters == 1 ? "time" : "times").append(".</span>");

                for (Litter litter : Main.game.getPlayer().getLittersIncubated()) {
                    if (litter.getMother() != null && litter.getMother().equals(owner)) {
                        descriptionSB.append("<br/>On ").append(Units.date(litter.getIncubationStartDate(), Units.DateType.LONG)).append(", [npc.she] implanted a clutch of eggs in you, and then on ").append(Units.date(litter.getBirthDate(), Units.DateType.LONG)).append(", you laid and hatched ").append(litter.getBirthedDescription()).append(".");
                    }
                }
    ...
    /**
     * @param femininity Value to set femininity to.
    ...
    public void setFeral(GameCharacter target, AbstractSubspecies subspecies) {
        ...
        if (attributes == null) {
            System.getLogger("").log(System.Logger.Level.ERROR, "Error in Body.setFeral(): subspecies '" + Subspecies.getIdFromSubspecies(targetSubspecies) + "' does not support FeralAttributes!");
            return;
        }
