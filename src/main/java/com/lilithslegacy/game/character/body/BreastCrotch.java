...
public class BreastCrotch implements BodyPartInterface {
    ...
    protected final Nipples nipples;
    ...
    public String setType(GameCharacter owner, AbstractBreastType type) {
        ...
        sb.append(type.getTransformationCrotchDescription(owner)).append("</p>");
    ...
    public String setRows(GameCharacter owner, int rows) {
        ...
        if (this.getShape() == BreastShape.UDDERS) {
            if (rows == 0) {
                ...
                sb.append("[npc.Name] now [npc.has] [style.boldSex(").append(Util.intToString(rows)).append(" pair").append(rows > 1 ? "s" : "").append(" of udders)]!").append("</p>");
            }
        } else {
            sb.append("[npc.Name] now [npc.has] [style.boldSex(").append(Util.intToString(rows)).append(" pair").append(rows > 1 ? "s" : "").append(" of [npc.crotchBoobs])]!").append("</p>");
        }
