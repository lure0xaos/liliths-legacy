...
//		elementalStartingPerks.add(both1);
    ...
    public boolean isPerkEndOfTreeBranch(GameCharacter character, int row, AbstractPerk perk, boolean onlyCheckOwnedPerks) {
        ...
        if (perkEntry == null) {
            String message = "PerkManager.isPerkEndOfTreeBranch() cannot find an entry for: " + character.getId() + ", " + row + ", " + perk.getName(character);
    ...
    public int getPerkRow(GameCharacter character, AbstractPerk perk) {
        ...
        String message = "PerkManager.getPerkRow(): Could not find Perk in any row! " + perk.getName(character);
        System.getLogger("").log(System.Logger.Level.ERROR, message);
        return 0;
    }
}
