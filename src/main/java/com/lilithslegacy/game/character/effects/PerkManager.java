...
public enum PerkManager {
    ...
    private final StringBuilder treeSB = new StringBuilder();
    private final StringBuilder lineSB = new StringBuilder();
    private final StringBuilder entrySB = new StringBuilder();
    private final Map<Integer, Map<PerkCategory, List<TreeEntry<PerkCategory, AbstractPerk>>>> perkTree;
    private final Map<Integer, Map<PerkCategory, List<TreeEntry<PerkCategory, AbstractPerk>>>> elementalPerkTree;
    private final Map<Integer, Map<PerkCategory, List<TreeEntry<PerkCategory, AbstractPerk>>>> dollPerkTree;
    private final List<TreeEntry<PerkCategory, AbstractPerk>> elementalStartingPerks = new ArrayList<>();
    private final List<TreeEntry<PerkCategory, AbstractPerk>> standardStartingPerks = new ArrayList<>();
    private final List<TreeEntry<PerkCategory, AbstractPerk>> dollStartingPerks = new ArrayList<>();
...
//		elementalStartingPerks.add(both1);
    ...
    public String getPerkTreeDisplay(GameCharacter character, boolean includePointsRemaining) {
        ...
        treeSB.append("<div id='OCCUPATION_").append(Perk.getIdFromPerk(character.getHistory().getAssociatedPerk())).append("' class='square-button small' style='width:8%; display:inline-block; float:none; border:2px solid ").append(PresetColour.GENERIC_EXCELLENT.toWebHexString()).append(";'>").append("<div class='square-button-content'>").append(character.getHistory().getAssociatedPerk().getSVGString(character)).append("</div>").append("</div>");
        ...
        for (int i = 0; i < GameCharacter.MAX_TRAITS; i++) {
            ...
            if (p != null) {
                treeSB.append("<div id='TRAIT_").append(Perk.getIdFromPerk(p)).append("' class='square-button small' style='width:8%; display:inline-block; float:none; border:2px solid ").append(PresetColour.TRAIT.toWebHexString()).append(";'>").append("<div class='square-button-content'>").append(p.getSVGString(character)).append("</div>").append("</div>");

            } else {
                treeSB.append("<div id='TRAIT_").append(i).append("' class='square-button small' style='display:inline-block; float:none;'></div>");
        ...
        if (includePointsRemaining) {
            ...
            for (PerkCategory category : PerkCategory.values()) {
                ...
                if (points > 0) {
                    extraPerkPoints.append(" <span style='color:").append(category.getColour().toWebHexString()).append(";'>").append(points).append("</span>");
                }
            }
            if (!extraPerkPoints.isEmpty()) {
                treeSB.append("<br/>Category-specific points: ").append(extraPerkPoints);
            }
            treeSB.append("</h6>");
        }

        if (!character.getSpecialPerks().isEmpty()) {
            ...
            for (AbstractPerk hiddenPerk : Perk.getHiddenPerks()) {
                if (character.hasPerkAnywhereInTree(hiddenPerk)) {
                    if (Perk.getSubspeciesKnowledgePerks().contains(hiddenPerk)) {
                        ...
                        treeSB.append("<div id='HIDDEN_PERK_").append(Perk.getIdFromPerk(hiddenPerk)).append("' class='square-button round small'").append(" style='width:6%; display:inline-block; float:none; border-color:").append(PresetColour.GENERIC_EXCELLENT.toWebHexString()).append("; cursor:default;'>").append("<div class='square-button-content'>").append(hiddenPerk.getSVGString(character)).append("</div>").append("</div>");
                    }
                }
            }
            treeSB.append("</div>");
            ...
            for (AbstractPerk hiddenPerk : subspeciesKnowledgePerks) {
                treeSB.append("<div id='HIDDEN_PERK_").append(Perk.getIdFromPerk(hiddenPerk)).append("' class='square-button round small'").append(" style='width:6%; padding:0; left:12px; margin:0 0 0 -24px; display:inline-block; float:none; border-color:").append(hiddenPerk.getColour().toWebHexString()).append("; cursor:default;'>").append("<div class='square-button-content'>").append(hiddenPerk.getSVGString(character)).append("</div>").append("</div>");
            }
            treeSB.append("</div>");
        }
    ...
    private String getPerkEntry(GameCharacter character, TreeEntry<PerkCategory, AbstractPerk> perkEntry, int size) {
        ...
        entrySB.append("<div class='square-button").append(perkEntry.getEntry().isEquippableTrait() ? "" : " round").append(disabled ? " disabled" : "").append("' style='width:16%; margin:16px ").append(getMargin(size)).append("%; ").append(isPerkOwned(character, perkEntry)
                ...
                : "").append("' id='").append(perkEntry.getRow()).append("_").append(perkEntry.getCategory()).append("_").append(Perk.getIdFromPerk(perkEntry.getEntry())).append("'>").append("<div class='square-button-content'>").append(perkEntry.getEntry().getSVGString(character)).append("</div>").append(disabled
    ...
    public boolean isPerkEndOfTreeBranch(GameCharacter character, int row, AbstractPerk perk, boolean onlyCheckOwnedPerks) {
        ...
        if (perkEntry == null) {
            System.getLogger("").log(System.Logger.Level.ERROR, "PerkManager.isPerkEndOfTreeBranch() cannot find an entry for: " + character.getId() + ", " + row + ", " + perk.getName(character));
            return false;
        }
    ...
    public int getPerkRow(GameCharacter character, AbstractPerk perk) {
        ...
        System.getLogger("").log(System.Logger.Level.ERROR, "PerkManager.getPerkRow(): Could not find Perk in any row! " + perk.getName(character));
        return 0;
    }
}
