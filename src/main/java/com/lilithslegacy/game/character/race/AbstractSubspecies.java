...
import com.lilithslegacy.game.character.body.valueEnums.Affinity;
...
public abstract class AbstractSubspecies {
    ...
    public static final Map<LegConfiguration, String[]> demonLegConfigurationNames = Util.newHashMapOfValues(
            ...
            new Value<>(LegConfiguration.AVIAN,
                    new String[]{
                            ...
                            "incumoas",
                            "succumoas"}));
    protected static final Map<Integer, String> youkoIconMap;
    protected static final Map<Integer, String> youkoDesaturatedIconMap;
    protected static final Map<Integer, String> youkoHalfDemonIconMap;
    ...
    public AbstractSubspecies(
        ...
        this.bookIdFolderPath = Path.of(""); //FIXME
        ...
        this.pathName = ("/com/lilithslegacy/res/" + pathName);
        this.bookPathName = ("/com/lilithslegacy/res/" + pathName);
        this.backgroundPathName = ("/com/lilithslegacy/res/" + backgroundPathName);
        this.SVGString = null;
        this.iconSize = 80;
    }

    public AbstractSubspecies(Path XMLFile, String author, boolean mod) {
        if (Files.exists(XMLFile)) {
            try {
                ...
                if (coreElement.getOptionalFirstOf("personalityChances").isPresent()) {
                    for (Element e : coreElement.getMandatoryFirstOf("personalityChances").getAllOf("entry")) {
                        try {
                            ...
                            System.getLogger("").log(System.Logger.Level.ERROR, "AbstractSubspecies error: PersonalityTrait '" + e.getTextContent() + "' failed to load!");
                        }
                    }
                }
                ...
                Path path2 = XMLFile.getParent();
                this.pathName = path2.resolve(coreElement.getMandatoryFirstOf("iconName").getTextContent().trim()).toString();
                ...
                if (coreElement.getOptionalFirstOf("backgroundName").isPresent() && !coreElement.getMandatoryFirstOf("backgroundName").getTextContent().isEmpty()) {
                    ...
                    this.backgroundPathName = path.resolve(coreElement.getMandatoryFirstOf("backgroundName").getTextContent().trim()).toString();
                    this.externalFileBackground = true;
                } else {
                    this.backgroundPathName = ("/com/lilithslegacy/res/statusEffects/race/raceBackground");
                    this.externalFileBackground = false;
                }
                this.SVGString = null;

                Path path1 = XMLFile.getParent();
                this.bookPathName = path1.resolve(coreElement.getMandatoryFirstOf("bookIconName").getTextContent().trim()).toString();
...
//				System.out.println(bookIdFolderPath);
                ...
                this.anthroNames = new HashMap<>();
                if (coreElement.getOptionalFirstOf("nameAnthro").isEmpty()) { // Old version support
                ...
                if (coreElement.getOptionalFirstOf("nameHalfDemon").isPresent()) {
                    if (coreElement.getMandatoryFirstOf("nameHalfDemon").getOptionalFirstOf("namesDefault").isEmpty()) { // Old version support
                ...
                if (coreElement.getOptionalFirstOf("feralAttributes").isPresent()
                    ...
                    try {
                        ...
                        System.getLogger("").log(System.Logger.Level.ERROR, "Error in AbstractSubspecies loading: feralAttributes failed to initialise!<br/>" + ex.getMessage());
                    }
                }
                ...
                System.getLogger("").log(System.Logger.Level.ERROR, "AbstractSubspecies was unable to be loaded from file! (" + XMLFile.getFileName().toString() + ")\n" + ex);
            }
        }
    }
    ...
    public static AbstractSubspecies getMainSubspeciesOfRace(AbstractRace race) {
        ...
        System.getLogger("").log(System.Logger.Level.ERROR, "Warning: getMainSubspeciesOfRace(" + Race.getIdFromRace(race) + ") did not find a main subspecies!");
        return backup;
    }
    ...
    public static AbstractSubspecies getSubspeciesFromBody(Body body, AbstractRace race) {
        ...
        if (subspecies == null) {
            if (Main.game.isStarted()) { // Races get recalculated after the game starts in Game.handlePostGameInit(), so only show errors if the detection is still failing after that
                System.getLogger("").log(System.Logger.Level.ERROR, "Error: getSubspeciesFromBody() did not find a suitable Subspecies! (Race: " + (race == null ? "null" : race.getName(false)) + ")");
...
//	}
    ...
    public Map<PersonalityTrait, Float> getPersonalityTraitChances() {
        ...
        if (this.fromExternalFile && personalityChanceOverrides != null) {
            map.putAll(personalityChanceOverrides);
        }
...
//			System.getLogger("").log(System.Logger.Level.ERROR,"Warning: AbstractSubspecies is calling getAttributeItem() where attributeItemId does not exist!");
            return null;
        }
...
//			System.getLogger("").log(System.Logger.Level.ERROR,"Warning: AbstractSubspecies is calling getTransformativeItem() where transformativeItemId does not exist!");
            return null;
        }
    ...
    public List<String> getExtraEffects(GameCharacter character) {
        if (character != null) {
            ...
            if (character.isFeral()) {
                effectsModified.addAll(getFeralEffects());
            }
    ...
    private Path getXMLBookFile() {
        return FS.newPath(bookIdFolderPath + System.getProperty("file.separator") + "bookEntries.xml");
    }

    public String getBookAuthor() {
        Path bookFile = FS.newPath("/res/txt/characters/raceInfo.xml");
        if (this.isFromExternalFile()) {
        ...
        if (Files.exists(bookFile)) {
            try {
                DocumentBuilder fsDocumentBuilder = Main.getDocBuilder();
                Document doc = fsDocumentBuilder.parse(Files.newInputStream(bookFile));
                // Cast magic:
                doc.getDocumentElement().normalize();
...
//				return Affinity.TERRESTRIAL;
        ...
        if (character != null) {
            ...
            if (character.isFeral() || (character.isElemental() && ((Elemental) character).getSummoner() != null && !((Elemental) character).getSummoner().isElementalActive())) {
                try {
                    ...
                    InputStream is = FS.newResourceAsStream("/com/lilithslegacy/res/statusEffects/race/raceBackgroundFeral.svg");
                ...
                if (!backgroundPath.isEmpty()) {
                    try {
                        ...
                        InputStream is = FS.newResourceAsStream("/com/lilithslegacy/res/" + backgroundPath + ".svg");
    ...
    protected void initBookSVGString() {
        try {
            if (this.isFromExternalFile()) {
                List<String> lines = Files.readAllLines(FS.newPath(bookPathName + ".svg"));
                ...
                InputStream is = FS.newResourceAsStream(bookPathName + ".svg");
                if (is == null) {
                    System.getLogger("").log(System.Logger.Level.ERROR, "Error! Subspecies book icon file does not exist (Trying to read from '" + bookPathName + "')! (Code 1)");
                }
    ...
    protected void initSVGStrings() {
        if (getPathName() != null) {
            ...
            try {
                if (this.isFromExternalFile() || getPathName().startsWith("res")|| getPathName().startsWith("/res")) {
                    List<String> lines = Files.readAllLines(FS.newPath(getPathName() + ".svg"));
                    StringBuilder sb = new StringBuilder();
                    for (String line : lines) {
                        sb.append(line);
                    }
                    ...
                    InputStream is = FS.newResourceAsStream(getPathName() + ".svg");
                    if (is == null) {
                        System.getLogger("").log(System.Logger.Level.ERROR, "Error! Subspecies icon file does not exist (Trying to read from '" + getPathName() + "')! (Code 1)");
                    }
                ...
                if (this.externalFileBackground) {
                    List<String> lines = Files.readAllLines(FS.newPath(getBackgroundPathName() + ".svg"));
                    StringBuilder sb = new StringBuilder();
                    for (String line : lines) {
                        sb.append(line);
                    }
                    ...
                    if (!getBackgroundPathName().isEmpty()) {
                        InputStream is = FS.newResourceAsStream(getBackgroundPathName() + ".svg");
                        if (is == null) {
                            System.getLogger("").log(System.Logger.Level.ERROR, "Error! Subspecies background icon file does not exist (Trying to read from '" + getBackgroundPathName() + "')! (Code 1)");
                        }
    ...
    static {
        ...
        for (int i = 1; i <= 9; i++) {
            try {
                String SVGStringBackground = "";
                InputStream is = FS.newResourceAsStream("/com/lilithslegacy/res/statusEffects/race/raceBackground.svg");
                if (is == null) {
                    System.getLogger("").log(System.Logger.Level.ERROR, "Error! Subspecies background icon file does not exist (Trying to read from 'statusEffects/race/raceBackground')! (Code 1f)");
                }
        ...
        for (int i = 1; i <= 9; i++) {
            try {
                ...
                InputStream is = FS.newResourceAsStream("/com/lilithslegacy/res/statusEffects/race/raceBackground.svg");
                if (is == null) {
                    System.getLogger("").log(System.Logger.Level.ERROR, "Error! Subspecies background icon file does not exist (Trying to read from 'statusEffects/race/raceBackground')! (Code 2f)");
                }
                SVGStringBackground = "<div style='width:100%;height:100%;position:absolute;left:0;bottom:0;'>" + Util.inputStreamToString(is) + "</div>";
