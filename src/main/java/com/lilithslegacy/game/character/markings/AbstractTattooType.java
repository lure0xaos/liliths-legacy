...
import com.lilithslegacy.game.inventory.item.SvgInformation;
...
public class AbstractTattooType extends AbstractCoreType {
    ...
    public AbstractTattooType(
        ...
        defaultSecondaryColour = !this.availableSecondaryColours.isEmpty() ? this.availableSecondaryColours.get(0) : defaultPrimaryColour;
        defaultTertiaryColour = !this.availableTertiaryColours.isEmpty() ? this.availableTertiaryColours.get(0) : defaultPrimaryColour;
    ...
    public AbstractTattooType(Path tattooXMLFile) {

        if (Files.exists(tattooXMLFile)) {
            try {
                ...
                for (Element imagePathElement : coreAttributes.getAllOf("imageName")) {
                    ...
                    if (rawPath.contains("/res/")) {
                        pathName = tattooXMLFile.getFileSystem().getPath(rawPath).toString();
                    } else {
                        Path path = tattooXMLFile.getParent();
                        pathName = path.resolve(rawPath).toString();
                    }
                    int zLayer = 1;
                    int imageSize = 100;
                    int imageRotation = 0;
                ...
                try {
                    ...
                    System.getLogger("").log(System.Logger.Level.ERROR, "AbstractTattooType loading failed. Cause: 'primaryColours' element unable to be parsed. (" + tattooXMLFile.getFileName().toString() + ")\n" + ex);
                }
                ...
                try {
                    ...
                    System.getLogger("").log(System.Logger.Level.ERROR, "AbstractTattooType loading failed. Cause: 'secondaryColours' element unable to be parsed. (" + tattooXMLFile.getFileName().toString() + ")\n" + ex);
                }
                ...
                try {
                    ...
                    System.getLogger("").log(System.Logger.Level.ERROR, "AbstractTattooType loading failed. Cause: 'tertiaryColours' element unable to be parsed. (" + tattooXMLFile.getFileName().toString() + ")\n" + ex);
                }
                ...
                System.getLogger("").log(System.Logger.Level.ERROR, "TattooType was unable to be loaded from file! (" + tattooXMLFile.getFileName().toString() + ")\n" + ex);
            }
        }
    }
    ...
    public boolean isLimitedSlotAvailability() {
        return !new HashSet<>(slotAvailability).containsAll(standardInventorySlots);
    }
            ...
            try {
                ...
                if (isMod) {
                    ...
                    for (SvgInformation info : getSvgPathInformation()) {
                        List<String> lines = Files.readAllLines(FS.newPath(info.getPathName()));
                        StringBuilder sb = new StringBuilder();
                        for (String line : lines) {
                            sb.append(line);
                        }
                        int sizeOffset = (100 - info.getImageSize()) / 2;
                        svgBuilder.append("<div style='" + "width:").append(info.getImageSize()).append("%;").append("height:").append(info.getImageSize()).append("%;").append("transform:rotate(").append(info.getImageRotation()).append("deg);").append("position:absolute;").append("left:").append(sizeOffset).append("%;").append("bottom:").append(sizeOffset).append("%;").append("padding:0;").append("margin:0;'>");
                    ...
                    is = FS.newResourceAsStream("/com/lilithslegacy/res/tattoos/" + getPathName() + ".svg");
