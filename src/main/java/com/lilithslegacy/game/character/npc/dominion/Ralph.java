...
//		this.setAssPlasticity(OrificePlasticity.THREE_RESILIENT.getValue());
    ...
    public String getTraderDescription() {
        if (Main.game.getDialogueFlags().getSavedLong(RALPH_DISCOUNT_TIMER_ID) > 0) {
            ...
            descriptionSB.append("<p>" + "You look over at the counter to see Ralph smiling back at you. Sensing that you might need some assistance, he briskly walks over to where you're standing." + "</p>" + "<p>" + "Although the memory of you submissively pleasuring Ralph's huge horse-cock is still fresh in both of your minds, he treats you with the utmost respect and politely lists off the prices of the transformative consumables on display.").append(this.hasItemType(ItemType.getItemTypeFromId("innoxia_pills_sterility"))
                    ...
                    + "</p>"
                    + "<p>"
                    : "").append(" He reminds you that he's also interested in buying any transformative consumables that you're willing to sell.").append(" As you turn back to look at the goods, Ralph tells you that he refreshes his stock every day.").append("</p>");

            if (isDiscountActive()) {
                descriptionSB.append("<p>" + "<b>Ralph is giving you a</b> <b style='color:").append(PresetColour.GENERIC_GOOD.toWebHexString()).append(";'>").append(Main.game.getDialogueFlags().ralphDiscount).append("%</b> <b>discount!</b>").append("</p>");
            } else {
                descriptionSB.append("<p>" + "Before he turns to walk back to the counter, he leans in and whispers in your ear, ").append(UtilText.parseSpeech("If you're interested in earning a little discount again, just call me over.", Main.game.getNpc(Ralph.class))).append("</p>");
            }
