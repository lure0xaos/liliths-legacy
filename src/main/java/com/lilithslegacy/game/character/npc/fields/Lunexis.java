...
//		this.setBlusher(new Covering(BodyCoveringType.MAKEUP_BLUSHER, PresetColour.COVERING_BLACK));
    ...
    public String getRaidersClearedString() {
        ...
        if (raidersCells.size() > (totalCells / 4) * 3) { // 75%+ dangerous
            ...
            if (raidersCells.isEmpty()) {
                sb.append("You've defeated all of the looting raiders, and are now recognised, by Aurokaris at least, as the [style.colourExcellent(Hero").append(Main.game.getPlayer().isFeminine() ? "ine" : "").append(" of Themiscyra)]!");
            } else {
