package com.lilithslegacy.game.character.npc;
...
import com.lilithslegacy.game.inventory.item.AbstractPotion;
...
public abstract class NPC extends GameCharacter implements XMLSaving {
    ...
    public final Set<NPCFlagValue> NPCFlagValues;
    ...
    protected final Set<SexSlot> sexPositionPreferences;
    ...
    public String getPresentInTileDescription(boolean inHiding) {
        ...
        if (!this.isRaceConcealed()) {
            tileSB.append(this.isPlayerKnowsName()
                    ...
                    : "[npc.A_femininity(true)] [npc.raceStage(true)] [npc.race(true)]").append(" is ").append(inHiding ? "[style.boldBad(hiding)] in" : "prowling").append(" this area!");
        } else {
            tileSB.append("Someone, or something, is ").append(inHiding ? "[style.boldBad(hiding)] in" : "prowling").append(" this area!");
        }

        // Combat:
        if (this.getFoughtPlayerCount() > 0) {
            tileSB.append("<br/>");
            tileSB.append("You have [style.colourCombat(fought [npc.herHim] ").append(Util.intToCount(this.getFoughtPlayerCount())).append(")]");
            if (this.getFoughtPlayerCount() == this.getLostCombatCount()) {
                ...
                tileSB.append(", of which you've [style.colourGood(won)] [style.colourMinorGood(").append(Util.intToCount(this.getLostCombatCount())).append(")] ");
                tileSB.append(" and [style.colourBad(lost)] [style.colourMinorBad(").append(Util.intToCount(this.getWonCombatCount())).append(")].");
            }
        }
        ...
        if (this.getTotalTimesHadSex(Main.game.getPlayer()) > 0) {
            if (this.getSexAsDomCount(Main.game.getPlayer()) > 0) {
                tileSB.append("<br/>");
                tileSB.append("You have had <span style='color:").append(PresetColour.GENERIC_SEX.toWebHexString()).append(";'>submissive sex</span> with [npc.herHim] ");
                tileSB.append(Util.intToCount(this.getSexAsDomCount(Main.game.getPlayer()))).append(".");
            }

            if (this.getSexAsSubCount(Main.game.getPlayer()) > 0) {
                tileSB.append("<br/>");
                tileSB.append("You have had <span style='color:").append(PresetColour.GENERIC_SEX_AS_DOM.toWebHexString()).append(";'>dominant sex</span> with  [npc.herHim] ");
                tileSB.append(Util.intToCount(this.getSexAsSubCount(Main.game.getPlayer()))).append(".");
            }
        }
        ...
        if (this.getPlayerSurrenderCount() == 3) {
            tileSB.append("<br/>");
            tileSB.append("[npc.She] will demand that you <span style='color:").append(PresetColour.GENERIC_SEX.toWebHexString()).append(";'>submit and become [npc.her] bitch</span> the next time you see [npc.herHim]!");

        } else if (this.getPlayerSurrenderCount() > 3) {
            tileSB.append("<br/>");
            tileSB.append("You have <span style='color:").append(PresetColour.GENERIC_SEX.toWebHexString()).append(";'>submitted and become [npc.her] bitch</span>!");
        }
    ...
    public String getPlayerRelationStatusDescription() {
        ...
        if (this.isRelatedTo(Main.game.getPlayer())) {
            ...
            switch (al) {
                ...
                case POSITIVE_ONE_FRIENDLY:
                    if (this.isAttractedTo(Main.game.getPlayer()) && Main.game.isIncestEnabled()) {
                        sb.append("[npc.Name] is acting in a <i style='color:").append(al.getColour().toWebHexString()).append(";'>friendly</i>, [style.italicsSex(flirtatious)] manner towards you.");
                    } else {
                        sb.append("[npc.Name] is acting in a <i style='color:").append(al.getColour().toWebHexString()).append(";'>friendly</i> manner towards you.");
                    }
                    break;
                case POSITIVE_TWO_LIKE:
                    if (this.isAttractedTo(Main.game.getPlayer()) && Main.game.isIncestEnabled()) {
                        sb.append("[npc.Name] quite clearly <i style='color:").append(al.getColour().toWebHexString()).append(";'>likes you</i>, and sees you as [style.italicsSex(more than just [npc.her] [pc.relation(npc)])].");
                    } else {
                        sb.append("[npc.Name] quite clearly <i style='color:").append(al.getColour().toWebHexString()).append(";'>likes you</i>, and is happy to have you as [npc.her] [pc.relation(npc)].");
                    }
                    break;
                case POSITIVE_THREE_CARING:
                    if (this.isAttractedTo(Main.game.getPlayer()) && Main.game.isIncestEnabled()) {
                        sb.append("[npc.Name] quite clearly <i style='color:").append(al.getColour().toWebHexString()).append(";'>cares about you a lot</i>, and is [style.italicsSex(deeply attracted)] towards you.");
                    } else {
                        sb.append("[npc.Name] quite clearly <i style='color:").append(al.getColour().toWebHexString()).append(";'>cares about you a lot</i>, and considers you to be the best [pc.relation(npc)] [npc.she] could ask for.");
                    }
                    break;
                case POSITIVE_FOUR_LOVE:
                    if (this.isAttractedTo(Main.game.getPlayer()) && Main.game.isIncestEnabled()) {
                        sb.append("You can tell from the way that [npc.she] looks at you that [npc.name] <i style='color:").append(al.getColour().toWebHexString()).append(";'>loves you</i> in a [style.italicsSex(romantic)] manner.");
                    } else {
                        sb.append("You can tell that [npc.name] <i style='color:").append(al.getColour().toWebHexString()).append(";'>loves you</i> as only [npc.a_relation(pc)] can.");
                    }
                    break;
                case POSITIVE_FIVE_WORSHIP:
                    if (this.isAttractedTo(Main.game.getPlayer()) && Main.game.isIncestEnabled()) {
                        sb.append("[npc.Name] <i style='color:").append(al.getColour().toWebHexString()).append(";'>adores you</i>, and is [style.italicsSex(head-over-heels in love)] with you.");
                    } else {
                        sb.append("[npc.Name] <i style='color:").append(al.getColour().toWebHexString()).append(";'>adores you</i>, and would do almost anything you asked of [npc.herHim].");
                    }
                    break;
            }
            ...
            switch (al) {
                case NEGATIVE_FIVE_LOATHE:
                    if (this.isAttractedTo(Main.game.getPlayer())) {
                        sb.append("Even though [npc.name] seems to <i style='color:").append(al.getColour().toWebHexString()).append(";'>loathe</i> you, you can tell that [npc.sheIs] still attracted to you.");
                    } else {
                        sb.append("[npc.Name] seems to <i style='color:").append(al.getColour().toWebHexString()).append(";'>loathe</i> you.");
                    }
                    break;
                case NEGATIVE_FOUR_HATE:
                    if (this.isAttractedTo(Main.game.getPlayer())) {
                        sb.append("Even though [npc.name] seems to <i style='color:").append(al.getColour().toWebHexString()).append(";'>hate</i> you, you can tell that [npc.sheIs] still attracted to you.");
                    } else {
                        sb.append("[npc.Name] seems to <i style='color:").append(al.getColour().toWebHexString()).append(";'>hate</i> you.");
                    }
                    break;
                case NEGATIVE_THREE_STRONG_DISLIKE:
                    if (this.isAttractedTo(Main.game.getPlayer())) {
                        sb.append("Even though [npc.name] seems to <i style='color:").append(al.getColour().toWebHexString()).append(";'>strongly dislike</i> you, you can tell that [npc.sheIs] still attracted to you.");
                    } else {
                        sb.append("[npc.Name] seems to <i style='color:").append(al.getColour().toWebHexString()).append(";'>strongly dislike</i> you.");
                    }
                    break;
                case NEGATIVE_TWO_DISLIKE:
                    if (this.isAttractedTo(Main.game.getPlayer())) {
                        sb.append("Even though [npc.name] seems to <i style='color:").append(al.getColour().toWebHexString()).append(";'>dislike</i> you, you can tell that [npc.sheIs] still attracted to you.");
                    } else {
                        sb.append("[npc.Name] seems to <i style='color:").append(al.getColour().toWebHexString()).append(";'>dislike</i> you.");
                    }
                    break;
                case NEGATIVE_ONE_ANNOYED:
                    if (this.isAttractedTo(Main.game.getPlayer())) {
                        sb.append("Even though [npc.name] seems to be <i style='color:").append(al.getColour().toWebHexString()).append(";'>annoyed</i> with you, you can tell that [npc.sheIs] still attracted to you.");
                    } else {
                        sb.append("[npc.Name] seems to be <i style='color:").append(al.getColour().toWebHexString()).append(";'>annoyed</i> with you.");
                    }
                    break;
                case ZERO_NEUTRAL:
                    if (this.isAttractedTo(Main.game.getPlayer())) {
                        sb.append("[npc.Name] is acting in an <i style='color:").append(al.getColour().toWebHexString()).append(";'>amicable, flirtatious</i> manner towards you.");
                    } else {
                        sb.append("[npc.Name] is acting in an <i style='color:").append(al.getColour().toWebHexString()).append(";'>amicable</i> manner towards you.");
                    }
                    break;
                case POSITIVE_ONE_FRIENDLY:
                    if (this.isAttractedTo(Main.game.getPlayer())) {
                        sb.append("[npc.Name] is acting in a <i style='color:").append(al.getColour().toWebHexString()).append(";'>friendly, flirtatious</i> manner towards you.");
                    } else {
                        sb.append("[npc.Name] is acting in a <i style='color:").append(al.getColour().toWebHexString()).append(";'>friendly</i> manner towards you.");
                    }
                    break;
                case POSITIVE_TWO_LIKE:
                    if (this.isAttractedTo(Main.game.getPlayer())) {
                        sb.append("[npc.Name] quite clearly <i style='color:").append(al.getColour().toWebHexString()).append(";'>likes you</i>, and sees you as more than just a friend.");
                    } else {
                        sb.append("[npc.Name] quite clearly <i style='color:").append(al.getColour().toWebHexString()).append(";'>likes you</i>, and sees you as a close friend.");
                    }
                    break;
                case POSITIVE_THREE_CARING:
                    if (this.isAttractedTo(Main.game.getPlayer())) {
                        sb.append("[npc.Name] quite clearly <i style='color:").append(al.getColour().toWebHexString()).append(";'>cares about you a lot</i>, and is deeply attracted towards you.");
                    } else {
                        sb.append("[npc.Name] quite clearly <i style='color:").append(al.getColour().toWebHexString()).append(";'>cares about you a lot</i>, and considers you to be [npc.her] best friend.");
                    }
                    break;
                case POSITIVE_FOUR_LOVE:
                    if (this.isAttractedTo(Main.game.getPlayer())) {
                        sb.append("You can tell from the way that [npc.she] looks at you that [npc.name] <i style='color:").append(al.getColour().toWebHexString()).append(";'>loves you</i>.");
                    } else {
                        sb.append("You can tell that [npc.name] <i style='color:").append(al.getColour().toWebHexString()).append(";'>loves you</i> in a purely platonic manner.");
                    }
                    break;
                case POSITIVE_FIVE_WORSHIP:
                    if (this.isAttractedTo(Main.game.getPlayer())) {
                        sb.append("[npc.Name] <i style='color:").append(al.getColour().toWebHexString()).append(";'>adores you</i>, and is head-over-heels in love with you.");
                    } else {
                        sb.append("[npc.Name] <i style='color:").append(al.getColour().toWebHexString()).append(";'>adores you</i>, and would do almost anything you asked of [npc.herHim].");
                    }
                    break;
        ...
        if (Main.getProperties().getForcedTFPreference() != FurryPreference.HUMAN && !cannotTransformPreference) {
            ...
            if (Main.getProperties().getForcedTFPreference() != FurryPreference.MINIMUM) {
                if (possibleEffects.isEmpty()) {
                    ...
                    if (possibleEffects.size() >= numberOfTransformations) {
                        return new TransformativePotion(itemType, possibleEffects, body);
                    } // Apply arms & legs at the same time
                }
            }
            ...
            if (Main.getProperties().getForcedTFPreference() == FurryPreference.NORMAL || Main.getProperties().getForcedTFPreference() == FurryPreference.MAXIMUM) {
                if (possibleEffects.isEmpty()) {
                    ...
                    if (possibleEffects.size() >= numberOfTransformations) {
                        return new TransformativePotion(itemType, possibleEffects, body);
                    } // Apply face & skin at the same time
                }
            }
        }
...
//		System.out.println(count);
    ...
    public boolean isWantingToEquipCondom(GameCharacter partner) {
        ...
        return this.getFetishDesire(Fetish.FETISH_CUM_STUD).isNegative() || (partner.hasVagina() && !partner.isVisiblyPregnant() && this.getFetishDesire(Fetish.FETISH_IMPREGNATION).isNegative());
    }
