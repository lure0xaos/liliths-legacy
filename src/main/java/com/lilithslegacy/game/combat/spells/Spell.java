...
import com.lilithslegacy.game.character.effects.TreeEntry;
            ...
            if (isHit) {
                ...
                if (caster.hasSpellUpgrade(SpellUpgrade.FIREBALL_2)) {
                    ...
                    if (secondaryTarget.equals(target)) {
                        descriptionSB.append("<br/>" + "The second Fireball swerves around to hit ").append(UtilText.parse(target, "[npc.name]")).append(" for a second time!");

                        descriptionSB.append(getDamageDescription(caster, target, damage, isHit, isCritical));
                        descriptionSB.append(applyDamage(caster, target, damage));

                    } else {
                        descriptionSB.append("<br/>" + "The second Fireball swerves around to hit ").append(secondaryTarget.isPlayer() ? "you" : UtilText.parse(secondaryTarget, "[npc.name]")).append("!");
            ...
            if (isHit) {
                ...
                if (caster.hasSpellUpgrade(SpellUpgrade.FLASH_2)) {
                    ...
                    if (secondaryTarget.equals(target)) {
                        ...
                        descriptionSB.append("<br/>" + "The second Flash shoots out in front of ").append(UtilText.parse(secondaryTarget, "[npc.namePos] face, dazzling [npc.herHim] as well!"));
        ...
        public String applyEffect(GameCharacter caster, GameCharacter target, List<GameCharacter> enemies, List<GameCharacter> allies, boolean isHit, boolean isCritical) {
            ...
            if (isHit) {
                if (caster.hasSpellUpgrade(SpellUpgrade.SOOTHING_WATERS_3)) {
                    ...
                    descriptionSB.append("<br/>" + "The orb of water heals ").append(UtilText.parse(target, "[npc.name]")).append(" for a total of ").append((int) (target.getAttributeValue(Attribute.HEALTH_MAXIMUM) * 0.4f)).append(" ").append(Attribute.HEALTH_MAXIMUM.getColouredName("b")).append(" and ").append((int) (target.getAttributeValue(Attribute.MANA_MAXIMUM) * 0.2f)).append(" ").append(Attribute.MANA_MAXIMUM.getColouredName("b")).append("!");
...
//					target.incrementMana(target.getAttributeValue(Attribute.MANA_MAXIMUM)*0.3f);
                    ...
                    if (Main.game.isInCombat()) {
                        ...
                        for (GameCharacter combatant : alliesPlusCaster) {
                            descriptionSB.append("<br/>").append(UtilText.parse(combatant, "One of the small orbs flies towards [npc.name], healing [npc.herHim] for a total of "
                    ...
                    descriptionSB.append(getDamageDescription(caster, target, 0, isHit, isCritical));
                    descriptionSB.append("<br/>" + "The orb of water heals ").append(UtilText.parse(target, "[npc.name]")).append(" for a total of ").append((int) (target.getAttributeValue(Attribute.HEALTH_MAXIMUM) * 0.2f)).append(" ").append(Attribute.HEALTH_MAXIMUM.getColouredName("b")).append(" and ").append((int) (target.getAttributeValue(Attribute.MANA_MAXIMUM) * 0.2f)).append(" ").append(Attribute.MANA_MAXIMUM.getColouredName("b")).append("!");
                    descriptionSB.append(applyDamage(caster, target, -target.getAttributeValue(Attribute.HEALTH_MAXIMUM) * 0.2f));
                    target.incrementMana(target.getAttributeValue(Attribute.MANA_MAXIMUM) * 0.2f);

                } else {
                    descriptionSB.append(getDamageDescription(caster, target, 0, isHit, isCritical));
                    descriptionSB.append("<br/>" + "The orb of water heals ").append(UtilText.parse(target, "[npc.name]")).append(" for a total of ").append((int) (target.getAttributeValue(Attribute.HEALTH_MAXIMUM) * 0.2f)).append(" ").append(Attribute.HEALTH_MAXIMUM.getColouredName("b")).append("!");
                    descriptionSB.append(applyDamage(caster, target, -target.getAttributeValue(Attribute.HEALTH_MAXIMUM) * 0.2f));
                }
                if (caster.hasSpellUpgrade(SpellUpgrade.SOOTHING_WATERS_1_CLEAN)) {
            ...
            if (isHit) {
                ...
                if (caster.hasSpellUpgrade(SpellUpgrade.STEAL_3B)) {
                    ...
                    if (clothingToSteal != null && !clothingToSteal.isSealed()) {
                        ...
                        descriptionSB.append("<br/>").append(getCastDescription(caster, target,
                                null,
                                ...
                                "[npc2.Name] lets out an embarrassed cry as [npc1.name] steals the " + clothingToSteal.getName() + " that [npc2.sheIs] currently wearing, [npc2.speech(Y-You pervert!)]")).append("<br/>");
                ...
                if (clothingToSteal == null) {
                    ...
                    if (mainWeaponSteal && (rnd < 0.2 || (!offhandWeaponSteal && !stealItem && clothingToSteal == null))) {
                        ...
                        descriptionSB.append("<br/>").append(getCastDescription(caster, target,
                                ...
                                "[npc1.Name] stole [npc2.namePos] " + mainWeapon.getName() + " from out of [npc2.her] [npc2.hands]!")).append("<br/>").append(caster.addWeapon(mainWeapon, true));
                        ...
                        descriptionSB.append("<br/>").append(getCastDescription(caster, target,
                                ...
                                "[npc1.Name] stole [npc2.namePos] " + offhandWeapon.getName() + " from out of [npc2.her] [npc2.hands]!")).append("<br/>").append(caster.addWeapon(offhandWeapon, true));
                        ...
                        if (item != null && (itemStealRnd < 0.33 || (weapon == null && clothing == null))) {
                            ...
                            descriptionSB.append("<br/>").append(getCastDescription(caster, target,
                                    ...
                                    "[npc1.Name] stole [npc2.namePos] " + item.getName() + " from out of [npc2.her] inventory!")).append("<br/>").append(caster.addItem(item, false));
                            ...
                            descriptionSB.append("<br/>").append(getCastDescription(caster, target,
                                    ...
                                    "[npc1.Name] stole [npc2.namePos] " + weapon.getName() + " from out of [npc2.her] inventory!")).append("<br/>").append(caster.addWeapon(weapon, false));
                            ...
                            descriptionSB.append("<br/>").append(getCastDescription(caster, target,
                                    ...
                                    "[npc1.Name] stole [npc2.namePos] " + clothing.getName() + " from out of [npc2.her] inventory!")).append("<br/>").append(caster.addClothing(clothing, false));
                        }
                        ...
                        descriptionSB.append("<br/>").append(getCastDescription(caster, target,
                                ...
                                "[npc2.Name] lets out an embarrassed cry as [npc1.name] steals the " + clothingToSteal.getName() + " that [npc2.sheIs] currently wearing!")).append("<br/>");
                        clothingToSteal.setName(target.getNameIgnoresPlayerKnowledge() + "'s " + clothingToSteal.getName());
                        descriptionSB.append(caster.addClothing(clothingToSteal, true));
...
//			caster.incrementMana(-cost);
    ...
    protected static final Map<Integer, List<TreeEntry<SpellSchool, SpellUpgrade>>> spellStealUpgradeTree;
    protected static final Map<Integer, List<TreeEntry<SpellSchool, SpellUpgrade>>> soothingWatersUpgradeTree;
    ...
    protected final DamageVariance damageVariance;
    ...
    Spell(boolean forbiddenSpell,
        ...
        try {
            InputStream is = FS.newResourceAsStream("/com/lilithslegacy/res/combat/spell/" + pathName + ".svg");
            if (is == null) {
                System.getLogger("").log(System.Logger.Level.ERROR, "Error! Spell icon file does not exist (Trying to read from '" + pathName + "')!");
            }
    ...
    public static String getBasicStatusEffectApplication(GameCharacter target, boolean beneficial, Map<AbstractStatusEffect, Integer> statusEffects) {
        ...
        damageCostDescriptionSB.append("<br/>").append(UtilText.parse(target,
                (!target.isPlayer()
                        ? "[npc.She] is now "
                        : "You are now ")
                        + (beneficial
        ...
        for (Entry<AbstractStatusEffect, Integer> seEntry : statusEffects.entrySet()) {
            ...
            damageCostDescriptionSB.append("<b>").append(seEntry.getValue()).append("</b> turns of <b style='color:").append(seEntry.getKey().getColour().toWebHexString()).append(";'>").append(seEntry.getKey().getName(target)).append("</b>");
            i++;
        }
        damageCostDescriptionSB.append(".");
    ...
    public static String getSpellMiscTreeDisplay(GameCharacter character, GameCharacter target) {
        ...
        treeSB.append("<div class='container-full-width' style='width:100%; padding:0; margin:0;'>" + "<div class='container-full-width' style='text-align:center;'><h6 style='color:").append(PresetColour.DAMAGE_TYPE_SPELL.toWebHexString()).append(";'>Miscellaneous</h6></div>");

        for (Spell spell : Spell.values()) {
            if (!spell.isSpellBook()) { // Only append special spells obtained from weapons & other sources
                treeSB.append("<div class='container-full-width' style='border:1px solid ").append((character.hasSpell(spell, true) ? PresetColour.DAMAGE_TYPE_SPELL : PresetColour.BASE_GREY_DARK).toWebHexString()).append("; width:25%; padding:0; margin:0;'>");
    ...
    private static void appendSpellSchool(SpellSchool spellSchool, GameCharacter character, GameCharacter target) {
        treeSB.append("<div class='container-full-width' style='width:100%; padding:0; margin:0;'>" + "<div class='container-full-width' style='text-align:center;'><h6 style='color:").append(spellSchool.getColour().toWebHexString()).append(";'>").append(Util.capitaliseSentence(spellSchool.getName())).append("</h6>").append("<b style='color:").append(spellSchool.getColour().toWebHexString()).append(";'>").append(character.getSpellUpgradePoints(spellSchool)).append("</b> <b>Upgrade Points Available</b></div>");
        ...
        for (Spell spell : Spell.getSpellsFromSchoolMap().get(spellSchool)) {
            if (spell.isSpellBook()) { // Do not append spells obtained from weapons & other sources
                ...
                if (!spell.getSpellUpgradeTree().isEmpty()) {
                    treeSB.append("<div class='container-full-width' style='border:1px solid ").append((fullyUpgraded ? spell.getSpellSchool().getColour() : PresetColour.BASE_GREY_DARK).toWebHexString()).append("; width:25%; padding:0; margin:0;'>");
    ...
    private static String appendSpell(GameCharacter character, GameCharacter target, int row, Spell spell, boolean miscSpell) {
        ...
        if (row == -1) {
            ...
            spellSB.append("<div class='square-button ").append(!hasSpell ? " disabled" : "").append("' style='width:50%; margin:8px 25% 4px 25%; cursor: default; ").append(hasSpell
                    ? "border-color:" + spell.getSpellSchool().getColour().toWebHexString() + ";"
                    : "").append("' id='SPELL_TREE_").append(spell).append("'>").append("<div class='square-button-content' style='cursor: default;'>").append(spell.getSVGString()).append("</div>").append(!hasSpell
                    ? (forbidden
            ...
            spellSB.append("<div class='normal-button ").append(useDesc.getKey() ? "" : "disabled").append("' id='SPELL_TREE_CAST_").append(spell).append("' style='width:50%; margin:8px 25% ").append(miscSpell ? "8px" : "0").append(" 25%; text-align:center;'>");
            spellSB.append("Cast");
            spellSB.append("</div>");
    ...
    private static String getHorizontalLine(GameCharacter character, Spell spell, int row) {
        ...
        for (TreeEntry<SpellSchool, SpellUpgrade> entry : spell.getSpellUpgradeTree().get(row)) {
            ...
            for (TreeEntry<SpellSchool, SpellUpgrade> siblingEntry : entry.getSiblingLinks()) {
                ...
                lineSB.append("<div style='width:100%; height:100%; padding:0; margin:0; top:0; left:0; position:absolute; pointer-events:none;'>" + "<svg width='100%' height='100%'><line x1='").append(siblingX).append("%' y1='50%' x2='").append(entryX).append("%' y2='50%' stroke='").append(getPerkLineSiblingColour(character, spell, entry).toWebHexString()).append("' stroke-width='2px'/></svg></div>");
            }
            for (TreeEntry<SpellSchool, SpellUpgrade> parentEntry : entry.getParentLinks()) {
                ...
                if (Math.abs(entryX - parentX) > 0.01f) {
                    lineSB.append("<div style='width:100%; padding:0; margin:0; top:0; left:0; position:absolute; pointer-events: none;'>" + "<svg style='padding:0; margin:0;' width='100%'><line x1='").append(entryX).append("%' y1='0' x2='").append(parentX).append("%' y2='0' stroke='").append(colour).append("' stroke-width='4px'/></svg></div>");
                }
            }
        }
    ...
    private static String getUpgradeEntry(GameCharacter character, Spell spell, TreeEntry<SpellSchool, SpellUpgrade> perkEntry, int size) {
        ...
        if (!perkEntry.getParentLinks().isEmpty()) {
            entrySB.append("<div style='width:100%; height:100%; padding:0; margin:0; top:0; left:0; position:absolute; pointer-events:none;'>" + "<svg width='100%' height='100%'><line x1='").append(entryX).append("%' y1='0%' x2='").append(entryX).append("%' y2='50%' stroke='").append(getPerkLineParentColour(character, spell, perkEntry).toWebHexString()).append("' stroke-width='2px'/></svg></div>");
        }
        if (!perkEntry.getChildLinks().isEmpty()) {
            entrySB.append("<div style='width:100%; height:100%; padding:0; margin:0; top:0; left:0; position:absolute; pointer-events:none;'>" + "<svg width='100%' height='100%'><line x1='").append(entryX).append("%' y1='100%' x2='").append(entryX).append("%' y2='50%' stroke='").append(getPerkLineChildColour(character, spell, perkEntry).toWebHexString()).append("' stroke-width='2px'/></svg></div>");
        }

        entrySB.append("<div class='square-button round").append(!hasUpgrade && !isUpgradeAvailable ? " disabled" : "").append("' style='width:40%; margin:8px ").append(getMargin(size)).append("%; ").append(character.hasSpellUpgrade(perkEntry.getEntry())
                ...
                : "")).append("' id='SPELL_UPGRADE_").append(perkEntry.getEntry()).append("'>").append("<div class='square-button-content'>").append(perkEntry.getEntry().getSVGString()).append("</div>").append(!hasUpgrade && !isUpgradeAvailable
                ? (forbidden
    ...
    protected String getStatusEffectApplication(GameCharacter caster, GameCharacter target, boolean isHit, boolean isCritical) {
        ...
        if (this.getStatusEffects(caster, target, isCritical) != null && !this.getStatusEffects(caster, target, isCritical).isEmpty() && isHit) {
            ...
            for (Entry<AbstractStatusEffect, Integer> seEntry : this.getStatusEffects(caster, target, isCritical).entrySet()) {
                ...
                damageCostDescriptionSB.append("<b>").append(seEntry.getValue() * (caster.isPlayer() && caster.hasTrait(Perk.JOB_MUSICIAN, true) ? 2 : 1) * (isCritical ? 2 : 1)).append("</b> turns").append(caster.hasTrait(Perk.JOB_MUSICIAN, true)
                        ...
                        : "").append(" of <b style='color:").append(seEntry.getKey().getColour().toWebHexString()).append(";'>").append(seEntry.getKey().getName(target)).append("</b>");
                i++;
            }
            damageCostDescriptionSB.append(".");
        }
...
//									System.out.println(source.getName()+" | "+this.getName());
    ...
    public String getPrediction(int turnIndex, GameCharacter source, GameCharacter target, List<GameCharacter> enemies, List<GameCharacter> allies) {
        ...
        predictionSB.append(isCrit ? "[style.colourExcellent(Critical)]: " : "").append("<span style='color:").append(getSpellSchool().getColour().toWebHexString()).append(";'>Cast spell '").append(getName()).append("'</span>").append(" on [npc2.name].");

        if (getSpellSchool() == SpellSchool.FIRE && source.hasStatusEffect(StatusEffect.FIRE_MANA_BURN) && !Main.combat.getManaBurnStack().get(source).isEmpty() && Main.combat.getManaBurnStack().get(source).peek() < 0) {
            predictionSB.append("<br/>This will cost <b style='color:").append(PresetColour.ATTRIBUTE_HEALTH.toWebHexString()).append(";'>").append(Units.round((-Main.combat.getManaBurnStack().get(source).peek()), 1)).append(" ").append(Attribute.HEALTH_MAXIMUM.getName()).append("</b>").append(" ([style.colourFire(").append(StatusEffect.FIRE_MANA_BURN.getName(source)).append(")]).");
        } else {
            predictionSB.append("<br/>This will cost <b style='color:").append(PresetColour.ATTRIBUTE_MANA.toWebHexString()).append(";'>").append(this.getModifiedCost(source)).append(" aura</b>.");
        }

        predictionSB.append("<br/><i>").append(getBasicEffectsString(source, target, enemies, allies)).append("</i>");
