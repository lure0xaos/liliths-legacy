...
public enum SpellUpgrade {
    ...
    FIREBALL_2(2,
            ...
            null, Util.newArrayListOfValues(
            "Fires [style.colourExcellent(two)] Fireballs, doing <b>15</b> [style.colourFire(Fire Damage)] each")),
    FIREBALL_3(5,
            SpellSchool.FIRE,
    ...
    FLASH_3(3,
            ...
            null, Util.newArrayListOfValues(
            "[style.colourExcellent(Reduces)] base cost to [style.boldMana(25)] aura")),
    ...
    SpellUpgrade(boolean isAlwaysAvailable,
        ...
        try {
            if (!pathName.isEmpty()) {
                InputStream is = FS.newResourceAsStream("/com/lilithslegacy/res/combat/spell/upgrade/" + pathName + ".svg");
                if (is == null) {
                    System.getLogger("").log(System.Logger.Level.ERROR, "Error! SpellUpgrade icon file does not exist (Trying to read from '" + pathName + "')!");
                }
