...
public class CombatMove {
    ...
    public static final List<AbstractCombatMove> allCombatMoves = new ArrayList<>();
    ...
    public static final Map<CombatMoveCategory, List<AbstractCombatMove>> combatMoveCategoryMap = new HashMap<>();
    public static final Map<AbstractCombatMove, String> combatMoveToIdMap = new HashMap<>();
    public static final Map<String, AbstractCombatMove> idToCombatMoveMap = new HashMap<>();
    ...
    static {
        ...
        for (Spell spell : Spell.values()) {
            ...
            CombatMove.idToCombatMoveMap.put("SPELL_" + spell, newCombatMove);
            CombatMove.combatMoveToIdMap.put(newCombatMove, "SPELL_" + spell);
        }
	     ...
	     *              EXTERNAL FILES
	     =============================================*/
        // Modded Moves:
        Map<String, Map<String, Path>> moddedFilesMap = Util.getExternalModFilesById("/combatMove");
        for (Entry<String, Map<String, Path>> entry : moddedFilesMap.entrySet()) {
            for (Entry<String, Path> innerEntry : entry.getValue().entrySet()) {
                try {
                    ...
                    System.getLogger("").log(System.Logger.Level.ERROR, "Loading modded combat move failed at 'CombatMove'. File path: " + innerEntry.getValue());
        ...
        // External res Moves:
        Map<String, Map<String, Path>> filesMap = Util.getExternalFilesById("/res/combatMove");
        for (Entry<String, Map<String, Path>> entry : filesMap.entrySet()) {
            for (Entry<String, Path> innerEntry : entry.getValue().entrySet()) {
                try {
                    ...
                    System.getLogger("").log(System.Logger.Level.ERROR, "Loading combat move failed at 'CombatMove'. File path: " + innerEntry.getValue());
                    System.getLogger("").log(System.Logger.Level.ERROR, "Actual exception: ");
                    ex.printStackTrace(System.err);
                }
            }
