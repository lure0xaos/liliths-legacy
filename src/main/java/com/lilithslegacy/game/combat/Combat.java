...
import com.lilithslegacy.game.character.effects.AppliedStatusEffect;
...
//			}
        ...
        //at com.lilithslegacy.rendering.RenderingEngine.getInventoryIconsForPage(RenderingEngine.java:954)
        //at com.lilithslegacy.rendering.RenderingEngine.getInventoryDiv(RenderingEngine.java:797)
//		Main.mainController.openInventory();
    ...
    private String getCharactersTurnDiv(GameCharacter character, String title, List<String> descriptions) {
        ...
        sb.append("<div class='container-full-width' style='text-align:center; box-sizing: border-box; border:6px solid ").append((enemy ? PresetColour.GENERIC_MINOR_BAD : PresetColour.GENERIC_MINOR_GOOD).getShades()[0]).append("; border-radius:5px;'>");

        sb.append("<div class='container-full-width' style='margin:2px; padding:4px; width:100%; border-radius:5px; text-align:center;'>" + "<b style='color:").append(character.getFemininity().getColour().toWebHexString()).append(";'>").append(UtilText.parse(character, "[npc.Name]")).append("</b>").append("</div>");

        sb.append("<div class='container-full-width'>");
        for (String s : descriptions) {
            if (!s.isEmpty()) {
                sb.append("<div class='container-half-width' style='margin:2px; padding:4px; width:100%; border-radius:5px; background:").append(PresetColour.BACKGROUND.toWebHexString()).append(";'>").append(s).append("</div>");
            }
        }
        if (!effects.isEmpty()) {
            sb.append("<div class='container-half-width' style='margin:2px; padding:4px; width:100%; border-radius:5px; background:").append(PresetColour.BACKGROUND.toWebHexString()).append(";'>").append(effects).append("</div>");
        }
        sb.append("</div>");
    ...
    public void endCombat(boolean playerVictory) {
        ...
        if (playerVictory) {
            ...
            if (!itemsLooted.isEmpty()) {
                postCombatStringBuilder.append("<div class='container-full-width' style='text-align:center;'>You [style.boldGood(gained)] ").append(Util.stringsToStringList(itemsLooted, false)).append("!</div>");
            }
            // Apply essence drops:
            ...
            if (totalEssencesGained > 0) {
                postCombatStringBuilder.append("<div class='container-full-width' style='text-align:center;'>").append(Main.game.getPlayer().incrementEssenceCount(totalEssencesGained, true)).append("</div>").append("</br>");
            }
            ...
            if (moneyLoss != 0 && enemyLeader.isLootingPlayerAfterCombat()) {
                ...
                postCombatStringBuilder.append("<div class='container-full-width' style='text-align:center;'>You [style.boldBad(lost)] ").append(UtilText.formatAsMoney(Math.abs(Main.game.getPlayer().getMoney() == 0 ? money : moneyLoss))).append("!</div>");
            }
                    ...
                    for (int count = 0; count < entry.getValue(); count++) {
                        if (Math.random() * 100 <= weapon.getWeaponType().getOneShotChanceToRecoverAfterCombat()) {
                            ...
                            if (!anyWeaponsRecovered) {
                                ...
                                postCombatStringBuilder.append(", ").append(Util.capitaliseSentence(weapon.getDisplayName(true)));
                            }
                        }
                    }
                }
...
//        	resetWeaponsThrownDuringCombat(character); // This gets reset when combat is initialised anyway, and it might be useful for post-combat dialogue to reference thrown/one-shot weapons?
    ...
    private Response getMoveResponse(AbstractCombatMove move, List<GameCharacter> pcEnemies, List<GameCharacter> pcAllies) {
        ...
        if (move.getStatusEffects(Main.game.getPlayer(), moveTarget, isCrit) != null && !move.getStatusEffects(Main.game.getPlayer(), moveTarget, isCrit).isEmpty()) {
            for (Entry<AbstractStatusEffect, Integer> entry : move.getStatusEffects(Main.game.getPlayer(), moveTarget, isCrit).entrySet()) {
                moveStatblock.append("Applies <b style='color:").append(entry.getKey().getColour().toWebHexString()).append(";'>").append(Util.capitaliseSentence(entry.getKey().getName(moveTarget))).append("</b>").append(" for <b>").append(entry.getValue()).append(entry.getValue() == 1 ? " turn" : " turns").append("</b><br/>");
            }
        }
        ...
        public Response getResponse(int responseTab, int index) {
            ...
            if (index == 0) {
                ...
                return new Response("Reset",
                        Main.game.getPlayer().getSelectedMoves().isEmpty()
                ...
                for (Entry<AbstractWeapon, Integer> entry : weaponsThrownDuringTurn.get(character).get(InventorySlot.mainWeaponSlots[i]).entrySet()) {
                    ...
                    for (int count = 0; count < entry.getValue(); count++) {
                        if (Math.random() * 100 <= weapon.getWeaponType().getOneShotChanceToRecoverAfterTurn()) {
                            ...
                            if (thrownWeaponsRecoveredDescriptions.isEmpty()) {
    ...
    private String getTitleResources(GameCharacter character) {
        ...
        sb.append(" <b>(<span style='color:").append((apRemaining == 0 ? PresetColour.GENERIC_GOOD : PresetColour.GENERIC_BAD).toWebHexString()).append(";'>").append(apRemaining).append("</span>/").append(character.getMaxAP()).append(" AP)</b>");
        ...
        if (shields != 0) {
            shieldsFound = true;
            sb.append("<div style='display:inline-block; float:none; margin:auto; padding:0 2px; background-color:").append(PresetColour.BACKGROUND.toWebHexString()).append("; border-radius:5px; width:auto; position:relative;'>").append("<span style='color:").append(DamageType.HEALTH.getColour().toWebHexString()).append(";'>").append(UtilText.getShieldSymbol()).append("</span> ").append(shields < 0 ? "[style.colourDisabled(" + shields + ")]" : getShieldsDisplayValue(Attribute.ENERGY_SHIELDING, shields)).append("<div class='overlay' id='").append(character.getId()).append("_COMBAT_SHIELD_").append(DamageType.HEALTH).append("' style='cursor:default;'></div>").append("</div>");
        }
        ...
        for (DamageType dt : damageTypes) {
            ...
            if (shields != 0) {
                ...
                shieldsFound = true;
                sb.append("<div style='display:inline-block; float:none; margin:auto; padding:0 2px; background-color:").append(PresetColour.BACKGROUND.toWebHexString()).append("; border-radius:5px; width:auto; position:relative;'>").append("<span style='color:").append(dt.getColour().toWebHexString()).append(";'>").append(UtilText.getShieldSymbol()).append("</span> ").append(shields < 0 ? "[style.colourDisabled(" + shields + ")]" : getShieldsDisplayValue(dt.getResistAttribute(), shields)).append("<div class='overlay' id='").append(character.getId()).append("_COMBAT_SHIELD_").append(dt).append("' style='cursor:default;'></div>").append("</div>");
            }
        }
        ...
        if (shields != 0) {
            ...
            shieldsFound = true;
            sb.append("<div style='display:inline-block; float:none; margin:auto; padding:0 2px; background-color:").append(PresetColour.BACKGROUND.toWebHexString()).append("; border-radius:5px; width:auto; position:relative;'>").append("<span style='color:").append(DamageType.LUST.getColour().toWebHexString()).append(";'>").append(UtilText.getShieldSymbol()).append("</span> ").append(shields < 0 ? "[style.colourDisabled(" + shields + ")]" : getShieldsDisplayValue(DamageType.LUST.getResistAttribute(), shields)).append("<div class='overlay' id='").append(character.getId()).append("_COMBAT_SHIELD_").append(DamageType.LUST).append("' style='cursor:default;'></div>").append("</div>");
        }
    ...
    private String getCombatContent() {
        ...
        sb.append("<div class='container-full-width' style='text-align:center; box-sizing: border-box; border:6px solid ").append(PresetColour.BASE_WHITE.toWebHexString()).append("; border-radius:5px;'>");
        ...
        sb.append("[style.boldGood(You)]").append(getTitleResources(Main.game.getPlayer()));
        if (predictionContent.get(Main.game.getPlayer()).isEmpty()) {
            sb.append("<div class='container-half-width' style='margin:2px; padding:4px; width:100%; border-radius:5px; background:").append(PresetColour.BACKGROUND.toWebHexString()).append(";'>[style.colourDisabled(No moves selected...)]</div>");
        } else {
        ...
        for (GameCharacter ally : getAllies(Main.game.getPlayer())) {
            sb.append(UtilText.parse(ally, "</br>[style.boldMinorGood([npc.Name])]")).append(getTitleResources(ally));
        ...
        sb.append("<div class='container-half-width'>");
        for (GameCharacter enemy : getEnemies(Main.game.getPlayer())) {
            sb.append(UtilText.parse(enemy, (enemyLeader.equals(enemy) ? "[style.boldBad([npc.Name])]" : "</br>[style.boldMinorBad([npc.Name])]"))).append(getTitleResources(enemy));
        ...
        for (AppliedStatusEffect appliedSe : character.getAppliedStatusEffects()) {
            ...
            if (se.isCombatEffect()) {
                ...
                if (appliedSe.getEffect().getEffectInterval() <= 0 || ((turn - appliedSe.getLastTimeAppliedEffect()) > appliedSe.getEffect().getEffectInterval())) {
                    ...
                    if (!s.isEmpty()) {
                        endTurnStatusEffectText.append("<p><b style='color: ").append(se.getColour().toWebHexString()).append("'>").append(Util.capitaliseSentence(se.getName(character))).append(":</b> ").append(s).append("</p>");
                    }
                }
...
//				}
    ...
    public GameCharacter getRandomAlliedPartyMember(GameCharacter target) {
        ...
        possibleTargets.addAll(getAllies(target));
