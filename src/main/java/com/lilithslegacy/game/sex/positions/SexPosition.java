...
import com.lilithslegacy.game.sex.positions.slots.SexSlotAgainstWall;
import com.lilithslegacy.game.sex.positions.slots.SexSlotAllFours;
import com.lilithslegacy.game.sex.positions.slots.SexSlotBreedingStall;
import com.lilithslegacy.game.sex.positions.slots.SexSlotDesk;
import com.lilithslegacy.game.sex.positions.slots.SexSlotGeneric;
import com.lilithslegacy.game.sex.positions.slots.SexSlotLyingDown;
import com.lilithslegacy.game.sex.positions.slots.SexSlotMasturbation;
import com.lilithslegacy.game.sex.positions.slots.SexSlotMilkingStall;
...
import com.lilithslegacy.game.sex.positions.slots.SexSlotUnique;
import com.lilithslegacy.game.sex.sexActions.SexActionInterface;
import com.lilithslegacy.game.sex.sexActions.SexActionPresets;
...
import com.lilithslegacy.game.sex.sexActions.baseActionsMisc.PlayerTalk;
import com.lilithslegacy.game.sex.sexActions.dominion.MasturbationPanties;
import com.lilithslegacy.game.sex.sexActions.universal.ChairSex;
import com.lilithslegacy.game.sex.sexActions.universal.Cowgirl;
import com.lilithslegacy.game.sex.sexActions.universal.DoggyStyle;
import com.lilithslegacy.game.sex.sexActions.universal.FaceSitting;
import com.lilithslegacy.game.sex.sexActions.universal.GloryHole;
import com.lilithslegacy.game.sex.sexActions.universal.KneelingOral;
import com.lilithslegacy.game.sex.sexActions.universal.Masturbation;
import com.lilithslegacy.game.sex.sexActions.universal.MatingPress;
import com.lilithslegacy.game.sex.sexActions.universal.MilkingStall;
import com.lilithslegacy.game.sex.sexActions.universal.Missionary;
import com.lilithslegacy.game.sex.sexActions.universal.MissionaryDesk;
import com.lilithslegacy.game.sex.sexActions.universal.SixtyNine;
import com.lilithslegacy.game.sex.sexActions.universal.StocksSex;
...
//			List<String> sizeDifferenceAdditions = new ArrayList<>();
            ...
            if (totalDoms >= 2) {
                sb.append(Util.capitaliseSentence(Util.stringsToStringList(domNames, false))).append(" are standing side-by-side before ");
            } else {
                sb.append(Util.capitaliseSentence(Util.stringsToStringList(domNames, false))).append(UtilText.parse(mainDom, " [npc.is]")).append(" standing before ");
            }

            sb.append(Util.stringsToStringList(subNames, false)).append(", ready to have some fun with ").append(subNames.size() > 1
                    ? (playerInSubs ? (playerInSubs ? "the " + Util.intToString(subs.size()) + " of you" : "them") : "them")
                    : UtilText.parse(subs.keySet().iterator().next(), " [npc.herHim]")).append(". ");

            // Standing:
            if (subStandingNames.size() >= 2) {
                sb.append(Util.capitaliseSentence(Util.stringsToStringList(subStandingNames, false))).append(" are similarly standing beside one another in front of ").append(totalDoms == 1 ? UtilText.parse(mainDom, " [npc.herHim]") : (playerInDoms ? "the " + Util.intToString(totalDoms) + " of you" : "them")).append(". ");

            } else if (subStandingNames.size() == 1) {
                sb.append(Util.capitaliseSentence(Util.stringsToStringList(subStandingNames, false))).append(UtilText.parse(mainStandingSub, " [npc.is]")).append(" standing in front of ").append(totalDoms == 1 ? UtilText.parse(mainDom, " [npc.herHim]") : (playerInDoms ? "the " + Util.intToString(totalDoms) + " of you" : "them")).append(". ");
            }

            // Kneeling:
            if (subKneelingNames.size() >= 2) {
                sb.append(Util.capitaliseSentence(Util.stringsToStringList(subKneelingNames, false))).append(" are kneeling down beside one another");
                if (domTaurs.isEmpty()) {
                    sb.append(" before ").append(totalDoms == 1 ? UtilText.parse(mainDom, " [npc.herHim]") : (playerInDoms ? " you" : "the " + Util.intToString(totalDoms) + " of them")).append(", ready to perform oral.");
                } else if (doms.isEmpty()) {
                    sb.append(", and are ready to shuffle forwards beneath ").append(totalDoms == 1 ? UtilText.parse(mainDom, " [npc.her] animalistic body") : (playerInDoms ? "one of your animalistic bodies" : "one of their animalistic bodies")).append(" in order to perform oral. ");
                } else {
                    sb.append(", and are ready to shuffle forwards and start performing oral. ");
                }

            } else if (subKneelingNames.size() == 1) {
                sb.append(Util.capitaliseSentence(Util.stringsToStringList(subKneelingNames, false))).append(UtilText.parse(mainKneelingSub, " [npc.is]")).append(" kneeling down");
                if (domTaurs.isEmpty()) {
                    sb.append(" before ").append(totalDoms == 1 ? UtilText.parse(mainDom, " [npc.herHim]") : (playerInDoms ? " you" : "the " + Util.intToString(totalDoms) + " of them")).append(", ready to perform oral. ");
                } else if (doms.isEmpty()) {
                    sb.append(" and ").append(UtilText.parse(mainKneelingSub, " [npc.is]")).append(" ready to shuffle forwards beneath ").append(totalDoms == 1 ? UtilText.parse(mainDom, " [npc.her] animalistic body") : (playerInDoms ? "one of your animalistic bodies" : "one of their animalistic bodies")).append(" in order to perform oral. ");
                } else {
                    sb.append(" and ").append(UtilText.parse(mainKneelingSub, " [npc.is]")).append(" ready to shuffle forwards and start performing oral. ");
                }
            }
            // Kneeling size difference:
            if (subSizeDiffKneelingNames.size() >= 2) {
                sb.append(Util.capitaliseSentence(Util.stringsToStringList(subSizeDiffKneelingNames, false))).append(" are ready to perform oral, but due to ").append(playerInSubs ? "your" : "their").append(" short stature, ").append(playerInSubs ? "you" : "they").append(" do not need to kneel, and are instead standing before ").append(totalDoms == 1 ? UtilText.parse(mainDom, " [npc.herHim]") : (playerInDoms ? " you" : "the " + Util.intToString(totalDoms) + " of them")).append(".");

            } else if (subSizeDiffKneelingNames.size() == 1) {
                sb.append(Util.capitaliseSentence(Util.stringsToStringList(subSizeDiffKneelingNames, false))).append(UtilText.parse(mainKneelingSizeDiffSub, " [npc.is] ready to perform oral, but due to [npc.her] short stature, [npc.she] [npc.does] not need to kneel, and [npc.is] instead standing before ")).append(totalDoms == 1 ? UtilText.parse(mainDom, " [npc.herHim]") : (playerInDoms ? " you" : "the " + Util.intToString(totalDoms) + " of them")).append(".");
            }

            // Standing behind:
            if (subStandingBehindNames.size() >= 2) {
                sb.append(Util.capitaliseSentence(Util.stringsToStringList(subStandingBehindNames, false))).append(" are standing behind, next to one another.");

            } else if (subStandingBehindNames.size() == 1) {
                sb.append(Util.capitaliseSentence(Util.stringsToStringList(subStandingBehindNames, false))).append(UtilText.parse(mainStandingBehindSub, " [npc.is]")).append(" standing behind ").append(totalDoms == 1 ? UtilText.parse(mainDom, " [npc.name]") : (playerInDoms ? " you" : " them")).append(".");
            }

            // Kneeling behind:
            if (subKneelingBehindNames.size() >= 2) {
                sb.append(Util.capitaliseSentence(Util.stringsToStringList(subKneelingBehindNames, false))).append(" have knelt down beside one another");
                if (domTaurs.isEmpty()) {
                    sb.append(" behind ").append(totalDoms == 1 ? UtilText.parse(mainDom, " [npc.herHim]") : (playerInDoms ? " you" : " them")).append(", ready to perform anilingus.");
                } else if (doms.isEmpty()) {
                    sb.append(" behind ").append(totalDoms == 1
                    ...
                    sb.append(", and are ready to perform oral on ").append(totalDoms == 1 ? UtilText.parse(mainDom, " [npc.her] rear-end.") : (playerInDoms ? " your rear-ends." : " their rear-ends."));
                }

            } else if (subKneelingBehindNames.size() == 1) {
                sb.append(Util.capitaliseSentence(Util.stringsToStringList(subKneelingBehindNames, false))).append(UtilText.parse(mainBehindSub, " [npc.is]")).append(" kneeling down behind ").append(totalDoms == 1 ? UtilText.parse(mainDom, " [npc.herHim]") : (playerInDoms ? " you" : " them"));
                if (domTaurs.isEmpty()) {
                    ...
                    sb.append(", ready to perform oral on ").append(totalDoms == 1 ? UtilText.parse(mainDom, " [npc.her] rear-end.") : (playerInDoms ? " your rear-ends." : " their rear-ends."));
                }
            }
            // Kneeling behind size difference:
            if (subSizeDiffBehindNames.size() >= 2) {
                sb.append(Util.capitaliseSentence(Util.stringsToStringList(subSizeDiffBehindNames, false))).append(" are positioned behind ").append(totalDoms == 1 ? UtilText.parse(mainDom, "[npc.name]") : (playerInDoms ? "you" : "the " + Util.intToString(totalDoms) + " of them")).append(", ready to perform oral on ").append(totalDoms == 1 ? UtilText.parse(mainDom, " [npc.her] rear-end") : (playerInDoms ? " your rear-ends" : " their rear-ends")).append(", but due to ").append(playerInSubs ? "your" : "their").append(" short stature, ").append(playerInSubs ? "you" : "they").append(" do not need to kneel, and are instead standing upright.");

            } else if (subSizeDiffBehindNames.size() == 1) {
                sb.append(Util.capitaliseSentence(Util.stringsToStringList(subSizeDiffBehindNames, false))).append(UtilText.parse(mainBehindSizeDiffSub, " [npc.is] positioned behind ")).append(totalDoms == 1 ? UtilText.parse(mainDom, "[npc.name]") : (playerInDoms ? "you" : "the " + Util.intToString(totalDoms) + " of them")).append(", ready to perform oral on ").append(totalDoms == 1 ? UtilText.parse(mainDom, " [npc.her] rear-end") : (playerInDoms ? " your rear-ends" : " their rear-ends")).append(UtilText.parse(mainBehindSizeDiffSub, ", but due to [npc.her] short stature, [npc.she] [npc.does] not need to kneel, and [npc.is] instead standing upright."));
            }
...
//			}
    ...
    public static final AbstractSexPosition AGAINST_WALL = new AbstractSexPosition("Against [pc.wall]",
        ...
        public String getDescription(Map<GameCharacter, SexSlot> occupiedSlots) {
            ...
            if (facingWallCount >= 2) {
                sb.append(Util.capitaliseSentence(Util.stringsToStringList(facingWallNames, false))).append(" are standing side-by-side facing a nearby [pc.wall]");

            } else if (facingWallCount == 1) {
                sb.append(Util.capitaliseSentence(Util.stringsToStringList(facingWallNames, false))).append(UtilText.parse(mainFacingWall, " [npc.is] standing facing a nearby [pc.wall]"));
            }

            if (backToWallNames.size() >= 2) {
                if (facingWallCount > 0) {
                    sb.append(", while ").append(Util.stringsToStringList(backToWallNames, false)).append(" are standing beside ").append(facingWallCount == 1 ? UtilText.parse(mainFacingWall, "[npc.herHim]") : (playerBackToWall ? "you" : "them")).append(", with ").append(playerBackToWall ? "your" : "their").append(" backs to the [pc.wall] instead. ");
                } else {
                    sb.append(Util.capitaliseSentence(Util.stringsToStringList(backToWallNames, false))).append(" are standing side-by-side, with ").append(playerBackToWall ? "your" : "their").append(" rears pressed back against the [pc.wall] behind them. ");
                }

            } else if (backToWallNames.size() == 1) {
                if (facingWallCount > 0) {
                    sb.append(", while ").append(Util.stringsToStringList(backToWallNames, false)).append(UtilText.parse(mainBackToWall, " [npc.is] standing beside ")).append(facingWallCount == 1 ? UtilText.parse(mainFacingWall, "[npc.herHim]") : (playerBackToWall ? "you" : "them")).append(", with ").append(UtilText.parse(mainBackToWall, "[npc.her]")).append(" back to the [pc.wall] instead. ");
                } else {
                    sb.append(Util.capitaliseSentence(Util.stringsToStringList(backToWallNames, false))).append(UtilText.parse(mainBackToWall, " [npc.is] standing with [npc.her] rear pressed back against a [pc.wall] behind [npc.herHim]. "));
                }
            } else {
                if (facingWallCount > 1) {
                    sb.append(", with each of ").append(playerFacingWall ? "you" : "them").append(" throwing the occasional glance back over ").append(playerFacingWall ? "your" : "their").append(" shoulders. ");
                } else {
            ...
            if (standingCount >= 2) {
                sb.append(Util.capitaliseSentence(Util.stringsToStringList(standingNames, false))).append(" are positioned so as to be standing right up against ").append(totalAgainstWall > 1 ? (playerFacingWall || playerBackToWall ? "you" : "them") : UtilText.parse(mainWall, "[npc.name]"));

            } else if (standingCount == 1) {
                sb.append(Util.capitaliseSentence(Util.stringsToStringList(standingNames, false))).append(UtilText.parse(mainStanding, " [npc.is] positioned so as to be standing right up against ")).append(totalAgainstWall > 1 ? (playerFacingWall || playerBackToWall ? "you" : "them") : UtilText.parse(mainWall, "[npc.name]"));
            }

            if (performingOralNames.size() >= 2) {
                if (standingCount > 0) {
                    sb.append(", while ").append(Util.stringsToStringList(performingOralNames, false)).append(" are down on ").append(playerPerformingOral ? "your" : "their").append(" knees, ready to perform oral on ").append(totalAgainstWall == 1 ? UtilText.parse(mainWall, "[npc.name]") : (playerFacingWall || playerBackToWall ? "you" : "them")).append(".");
                } else {
                    sb.append(Util.capitaliseSentence(Util.stringsToStringList(performingOralNames, false))).append(" are positioned close behind and are down on ").append(playerPerformingOral ? "your" : "their").append(" knees, ready to perform oral on ").append(totalAgainstWall == 1 ? UtilText.parse(mainWall, "[npc.name]") : (playerFacingWall || playerBackToWall ? "you" : "them")).append(".");
                }

            } else if (performingOralNames.size() == 1) {
                if (standingCount > 0) {
                    sb.append(", while ").append(Util.stringsToStringList(performingOralNames, false)).append(UtilText.parse(mainPerformingOral, " [npc.is] down on [npc.her] knees, ready to perform oral on ")).append(totalAgainstWall == 1 ? UtilText.parse(mainWall, "[npc.name]") : (playerFacingWall || playerBackToWall ? "you" : "them")).append(".");
                } else {
                    sb.append(Util.capitaliseSentence(Util.stringsToStringList(performingOralNames, false))).append(UtilText.parse(mainPerformingOral, " [npc.is] positioned close behind and [npc.is] down on [npc.her] knees, ready to perform oral on ")).append(totalAgainstWall == 1 ? UtilText.parse(mainWall, "[npc.name]") : (playerFacingWall || playerBackToWall ? "you" : "them")).append(".");
                }

            } else {
                if (totalAgainstWall > 1) {
                    sb.append(", ready to have some fun with the ").append(Util.intToString(totalAgainstWall)).append(" of ").append(playerFacingWall ? "you" : "them").append(".");
                } else {
                    sb.append(", ready to have some fun with ").append(UtilText.parse(mainWall, "[npc.herHim]")).append(".");
                }
            }
        ...
        @Override
        public boolean isSelfOralAvailable(GameCharacter autoOralCharacter) {
            return !Main.sex.getSexPositionSlot(autoOralCharacter).hasTag(SexSlotTag.FACE_TO_WALL);
        }
    };
    ...
    public static final AbstractSexPosition OVER_DESK = new AbstractSexPosition("Over [pc.desk]",
        ...
        public String getDescription(Map<GameCharacter, SexSlot> occupiedSlots) {
            ...
            if (deskBackCount >= 2) {
                sb.append(Util.capitaliseSentence(Util.stringsToStringList(deskBackNames, false))).append(" are lying back over the [pc.desk]");

            } else if (deskBackCount == 1) {
                sb.append(Util.capitaliseSentence(Util.stringsToStringList(deskBackNames, false))).append(UtilText.parse(mainDeskBack, " [npc.is] lying back over the [pc.desk]"));
            }

            if (deskFrontNames.size() >= 2) {
                if (deskBackCount > 0) {
                    sb.append(", while ").append(Util.stringsToStringList(deskFrontNames, false)).append(" are lying down, face-first, beside ").append(deskBackCount == 1 ? UtilText.parse(mainDeskBack, "[npc.herHim]") : (playerDeskBack ? "you" : "them")).append(". ");
                } else {
                    sb.append(Util.capitaliseSentence(Util.stringsToStringList(deskFrontNames, false))).append(" are lying down, face-first, over the [pc.desk]. ");
                }

            } else if (deskFrontNames.size() == 1) {
                if (deskBackCount > 0) {
                    sb.append(", while ").append(Util.stringsToStringList(deskFrontNames, false)).append(UtilText.parse(mainDeskFront, " [npc.is] lying down, face-first, over the [pc.desk]. "));
                } else {
                    sb.append(Util.capitaliseSentence(Util.stringsToStringList(deskFrontNames, false))).append(UtilText.parse(mainDeskFront, " [npc.is] lying down, face-first, over the [pc.desk]. "));
                }
            ...
            if (deskFrontTaurNames.size() >= 2) {
                sb.append(Util.capitaliseSentence(Util.stringsToStringList(deskFrontTaurNames, false))).append(" have used their front legs to step up on top of the [pc.desk], before lying down and resting their animalistic lower bodies over it. ");

            } else if (deskFrontTaurNames.size() == 1) {
                sb.append(Util.capitaliseSentence(Util.stringsToStringList(deskFrontTaurNames, false))).append(UtilText.parse(mainDeskFrontTaur,
            ...
            GameCharacter mainDesk = mainDeskBack == null
                    ...
                    : mainDeskBack;
            if (standingCount >= 2) {
                sb.append(Util.capitaliseSentence(Util.stringsToStringList(standingNames, false))).append(" have moved forwards so as to be standing right up between ").append(totalDeskCount > 1 ? (playerDeskBack || playerDeskFront ? "your legs" : "their legs") : UtilText.parse(mainDesk, "[npc.namePos] [npc.legs]"));

            } else if (standingCount == 1) {
                sb.append(Util.capitaliseSentence(Util.stringsToStringList(standingNames, false))).append(UtilText.parse(mainStanding, " [npc.has] moved forwards so as to be standing right up between ")).append(totalDeskCount > 1 ? (playerDeskBack || playerDeskFront ? "your legs" : "their legs") : UtilText.parse(mainDesk, "[npc.namePos] [npc.legs]"));
            }
            if (standingCount > 0) {
                if (totalDeskCount > 1) {
                    sb.append(", ready to have some fun with the ").append(Util.intToString(totalDeskCount)).append(" of ").append(playerDeskBack || playerDeskFront ? "you" : "them").append(". ");
                } else {
                    sb.append(", ready to have some fun with ").append(UtilText.parse(mainDesk, "[npc.herHim]")).append(". ");
                }
            }

            int standingCountTaurs = standingNamesTaur.size();
            if (standingCountTaurs >= 2) {
                sb.append(Util.capitaliseSentence(Util.stringsToStringList(standingNamesTaur, false))).append(" have stepped up onto the [pc.desk] with their front legs, and are ready to use their feral lower bodies");

            } else if (standingCountTaurs == 1) {
                sb.append(Util.capitaliseSentence(Util.stringsToStringList(standingNamesTaur, false))).append(UtilText.parse(mainStandingTaur, " [npc.has] stepped up onto the [pc.desk] with [npc.her] front legs, and [npc.is] ready to use [npc.her] feral lower body"));
            }
            if (standingCountTaurs > 0) {
                if (totalDeskCount > 1) {
                    sb.append(" to mount the ").append(Util.intToString(totalDeskCount)).append(" of ").append(playerDeskBack || playerDeskFront ? "you" : "them").append(". ");
                } else {
                    sb.append(" to mount ").append(UtilText.parse(mainDesk, "[npc.herHim]")).append(". ");
                }
            }
            ...
            if (humpingCount > 0) {
                if (humpingCount >= 2) {
                    sb.append(Util.capitaliseSentence(Util.stringsToStringList(humpingNames, false))).append(" have eagerly clambered up onto the edge of the [pc.desk], and are ready to start humping their partners.");
                    ...
                    if (totalDeskCount > 1) {
                        ...
                        sb.append(UtilText.parse(mainDesk, " [npc.name]")).append(". ");
                    }
                }
            }
            ...
            if (performingOralNames.size() >= 2) {
                sb.append(Util.capitaliseSentence(Util.stringsToStringList(performingOralNames, false))).append(" are positioned so as to be right up between ").append(totalDeskCount > 1
                ...
                sb.append(Util.capitaliseSentence(Util.stringsToStringList(performingOralNames, false))).append(UtilText.parse(mainPerformingOral, " [npc.is] positioned so as to be right up between ")).append(totalDeskCount > 1
                        ? (playerDeskBack || playerDeskFront ? "your legs" : "their legs") + ", ready to perform oral on " + (playerDeskBack || playerDeskFront ? "you. " : "them. ")
                        : UtilText.parse(mainDesk, "[npc.namePos] [npc.legs], ready to perform oral on [npc.herHim]. "));
            }
            ...
            if (receivingOralNames.size() >= 2) {
                sb.append(Util.capitaliseSentence(Util.stringsToStringList(receivingOralNames, false))).append(additionalDoms ? ", meanwhile," : "").append(" are positioned around the other side of the [pc.desk], next to ").append(totalDeskCount > 1
                ...
                sb.append(Util.capitaliseSentence(Util.stringsToStringList(receivingOralNames, false))).append(additionalDoms ? ", meanwhile," : "").append(UtilText.parse(mainReceivingOral, " [npc.is] positioned around the other side of the [pc.desk], next to ")).append(totalDeskCount > 1
                        ? (playerDeskBack || playerDeskFront ? "your faces" : "their faces") + ", in order to receive oral from " + (playerDeskBack || playerDeskFront ? "you." : "them.")
                        : UtilText.parse(mainDesk, "[npc.namePos] [npc.face], in order to receive oral from [npc.herHim]."));
            }
            ...
            for (String s : sizeDifferenceAdditions) {
                sb.append("<br/>[style.italicsOrange(").append(s).append(")]");
            }
        ...
        @Override
        public boolean isSelfOralAvailable(GameCharacter autoOralCharacter) {
            return !Main.sex.getSexPositionSlot(autoOralCharacter).hasTag(SexSlotTag.OVER_DESK_FRONT);
        }
    };
    ...
    public static final AbstractSexPosition STOCKS = new AbstractSexPosition("Stocks",
        ...
        public String getDescription(Map<GameCharacter, SexSlot> occupiedSlots) {
            ...
            if (stocksNames.size() >= 2) {
                sb.append(Util.capitaliseSentence(Util.stringsToStringList(allStocksNames, false))).append(" are locked into ").append(Util.intToString(stocksNames.size())).append(" sets of stocks, and are completely at the mercy of anyone who wants to use ").append(playerStocks ? "you" : "them").append(". ");

            } else if (stocksTaurNames.isEmpty()) {
                sb.append(Util.capitaliseSentence(Util.stringsToStringList(allStocksNames, false))).append(UtilText.parse(soloStocks,
            ...
            if (stocksTaurNames.size() >= 2) {
                sb.append(" Having feral lower bodies, ").append(Util.stringsToStringList(stocksTaurNames, false)).append(" have been locked into special sets of stocks, which force their front halves down into a kneeling position, while their animalistic rear halves are held fully upright,").append(" allowing anyone to step up and use their mouths or genitals as they'd like. ");
            ...
            int standingCount = standingNames.size();
            if (standingCount >= 2) {
                sb.append(Util.capitaliseSentence(Util.stringsToStringList(standingNames, false))).append(" have moved forwards so as to be standing right up behind ").append(stocksCount > 1
                ...
                sb.append(Util.capitaliseSentence(Util.stringsToStringList(standingNames, false))).append(UtilText.parse(mainStanding, " [npc.has] moved forwards so as to be standing right up behind ")).append(stocksCount > 1
                        ? (playerStocks ? "you, ready to take advantage of your immobility. " : "them, ready to take advantage of their immobility. ")
                        : UtilText.parse(soloStocks, "[npc.name], ready to take advantage of [npc.her] immobility. "));
            }
            ...
            int standingCountTaurs = standingNamesTaur.size();
            if (standingCountTaurs >= 2) {
                sb.append(Util.capitaliseSentence(Util.stringsToStringList(standingNamesTaur, false))).append(" have reared up on their hind legs, before planting their front ").append(UtilText.parse(mainStandingTaur, "[npc.feet]")).append(" on the top of the stocks, allowing them to ").append(stocksCount > 1
                ...
                sb.append(Util.capitaliseSentence(Util.stringsToStringList(standingNamesTaur, false))).append(UtilText.parse(mainStanding, " [npc.has] reared up on [npc.her] hind legs, before planting [npc.her] front [npc.feet] on the top of the stocks, allowing [npc.herHim] to ")).append(stocksCount > 1
                        ? (playerStocks
            ...
            if (humpingNames.size() >= 2) {
                sb.append(Util.capitaliseSentence(Util.stringsToStringList(humpingNames, false))).append(" have clambered up onto ").append(stocksCount > 1
                ...
                sb.append(Util.capitaliseSentence(Util.stringsToStringList(humpingNames, false))).append(UtilText.parse(mainHumping, " [npc.has] clambered up onto ")).append(UtilText.parse(soloStocks, "[npc.namePos] lower back, ready to start humping [npc.herHim]. "));
            }

            if (performingOralNames.size() >= 2) {
                sb.append(Util.capitaliseSentence(Util.stringsToStringList(performingOralNames, false))).append(" are positioned so as to be right up between ").append(stocksCount > 1
                ...
                sb.append(Util.capitaliseSentence(Util.stringsToStringList(performingOralNames, false))).append(UtilText.parse(mainPerformingOral, " [npc.is] positioned so as to be right up between ")).append(stocksCount > 1
                        ? (playerStocks ? "your legs" : "their legs") + ", ready to perform oral on " + (playerStocks ? "you. " : "them. ")
                        : UtilText.parse(soloStocks, "[npc.namePos] [npc.legs], ready to perform oral on [npc.herHim]. "));
            }
            ...
            if (beneathNames.size() >= 2) {
                sb.append(Util.capitaliseSentence(Util.stringsToStringList(beneathNames, false))).append(" have dropped down beneath ").append(stocksCount > 1
                ...
                sb.append(Util.capitaliseSentence(Util.stringsToStringList(beneathNames, false))).append(UtilText.parse(mainBeneath, " [npc.has] dropped down beneath ")).append(stocksCount > 1
                        ? (playerStocks ? "you" : "them") + ", ready to get fucked. "
                        : UtilText.parse(soloStocks, "[npc.name], ready to get fucked by [npc.herHim]. "));
            }
            ...
            if (receivingOralNames.size() >= 2) {
                sb.append(Util.capitaliseSentence(Util.stringsToStringList(receivingOralNames, false))).append(additionalDoms ? ", meanwhile," : "").append(" are positioned around the other side of the stocks, next to ").append(stocksCount > 1
                ...
                sb.append(Util.capitaliseSentence(Util.stringsToStringList(receivingOralNames, false))).append(additionalDoms ? ", meanwhile," : "").append(UtilText.parse(mainReceivingOral, " [npc.is] positioned around the other side of the stocks, next to ")).append(stocksCount > 1
                        ? (playerStocks ? "your faces" : "their faces") + ", in order to receive oral from " + (playerStocks ? "you." : "them.")
                        : UtilText.parse(soloStocks, "[npc.namePos] [npc.face], in order to receive oral from [npc.herHim]."));
            }
        ...
        public boolean isActionBlocked(GameCharacter performer, GameCharacter target, SexActionInterface action) {
            if ((Main.sex.getSexPositionSlot(performer).hasTag(SexSlotTag.LOCKED_IN_STOCKS)
                    && action.getSexAreaInteractions().containsKey(SexAreaPenetration.FINGER)
        ...
        @Override
        public boolean isSelfOralAvailable(GameCharacter autoOralCharacter) {
            return !Main.sex.getSexPositionSlot(autoOralCharacter).hasTag(SexSlotTag.LOCKED_IN_STOCKS);
        }
    };
    ...
    public static final AbstractSexPosition MILKING_STALL = new AbstractSexPosition("Milking stall",
        ...
        public String getDescription(Map<GameCharacter, SexSlot> occupiedSlots) {
            ...
            GameCharacter soloStocks = mainStocks == null ? mainStocksTaur : mainStocks;
            if (stocksNames.size() >= 2) {
                sb.append(Util.capitaliseSentence(Util.stringsToStringList(allStocksNames, false))).append(" are locked into ").append(Util.intToString(stocksNames.size())).append(" sets of milking stalls, and are completely at the mercy of anyone who wants to use ").append(playerStocks ? "you" : "them").append(". ");
            ...
            if (stocksTaurNames.size() >= 2) {
                sb.append(" Having feral lower bodies, ").append(Util.stringsToStringList(stocksTaurNames, false)).append(" have been locked into specially-designed milking stalls, which force their front halves down into a kneeling position, while their animalistic rear halves are held fully upright,").append(" allowing anyone to step up and use their mouths or genitals as they'd like. ");
            ...
            int standingCountTaurs = standingNamesTaur.size();
            if (standingCountTaurs >= 2) {
                sb.append(Util.capitaliseSentence(Util.stringsToStringList(standingNamesTaur, false))).append(" have reared up on their hind legs, before planting their front ").append(UtilText.parse(mainStandingTaur, "[npc.feet]")).append(" on the top of the stall, allowing them to ").append(stocksCount > 1
                        ? (playerStocks ? "effectively mount the " + (Util.intToString(stocksCount)) + " of you. " : "effectively mount the " + (Util.intToString(stocksCount)) + " of them. ")
                        : UtilText.parse(soloStocks, "effectively mount [npc.name]. "));

            } else if (standingCountTaurs == 1) {
                sb.append(Util.capitaliseSentence(Util.stringsToStringList(standingNamesTaur, false))).append(UtilText.parse(mainStanding, " [npc.has] reared up on [npc.her] hind legs, before planting [npc.her] front [npc.feet] on the top of the stall, allowing [npc.herHim] to ")).append(stocksCount > 1
                        ? (playerStocks
                        ? UtilText.parse(mainStanding, "effectively mount whichever of the " + (Util.intToString(stocksCount)) + " of you [npc.she] [npc.verb(want)]. ")
                        : UtilText.parse(mainStanding, "effectively mount whichever of the " + (Util.intToString(stocksCount)) + " of them [npc.she] [npc.verb(want)]. "))
                        : UtilText.parse(soloStocks, "effectively mount [npc.name]. "));
            ...
            boolean additionalDoms = !standingNames.isEmpty() || !standingNamesTaur.isEmpty() || !performingOralNames.isEmpty();
            if (receivingOralNames.size() >= 2) {
                sb.append(Util.capitaliseSentence(Util.stringsToStringList(receivingOralNames, false))).append(additionalDoms ? ", meanwhile," : "").append(" are positioned around the other side of the ").append(stocksCount == 1 ? "stall" : "stalls").append(", next to ").append(stocksCount > 1
                        ? (playerStocks ? "your faces" : "their faces") + ", in order to receive oral from " + (playerStocks ? "you." : "them.")
                        : UtilText.parse(soloStocks, "[npc.namePos] [npc.face], in order to receive oral from [npc.herHim]."));

            } else if (receivingOralNames.size() == 1) {
                sb.append(Util.capitaliseSentence(Util.stringsToStringList(receivingOralNames, false))).append(additionalDoms ? ", meanwhile," : "").append(UtilText.parse(mainReceivingOral, " [npc.is] positioned around the other side of the " + (stocksCount == 1 ? "stall" : "stalls") + ", next to ")).append(stocksCount > 1
                        ? (playerStocks ? "your faces" : "their faces") + ", in order to receive oral from " + (playerStocks ? "you." : "them.")
                        : UtilText.parse(soloStocks, "[npc.namePos] [npc.face], in order to receive oral from [npc.herHim]."));
            }
...
//				}
//			}
            if ((!positioningSlots.containsValue(SexSlotAllFours.ALL_FOURS) || positioningSlots.get(characterToTakeSlot) == SexSlotAllFours.ALL_FOURS)
            ...
            if ((!positioningSlots.containsValue(SexSlotAllFours.ALL_FOURS_TWO) || positioningSlots.get(characterToTakeSlot) == SexSlotAllFours.ALL_FOURS_TWO)
            ...
            if ((!positioningSlots.containsValue(SexSlotAllFours.ALL_FOURS_THREE) || positioningSlots.get(characterToTakeSlot) == SexSlotAllFours.ALL_FOURS_THREE)
            ...
            if ((!positioningSlots.containsValue(SexSlotAllFours.ALL_FOURS_FOUR) || positioningSlots.get(characterToTakeSlot) == SexSlotAllFours.ALL_FOURS_FOUR)
        ...
        @Override
        public boolean isSelfOralAvailable(GameCharacter autoOralCharacter) {
            return !Main.sex.getSexPositionSlot(autoOralCharacter).hasTag(SexSlotTag.ALL_FOURS)
    ...
    public static final AbstractSexPosition LYING_DOWN = new AbstractSexPosition("Lying down",
        ...
        private final List<SexSlot> position1 = Util.newArrayListOfValues(
        ...
        private final List<SexSlot> position2 = Util.newArrayListOfValues(
        ...
        private final List<SexSlot> position3 = Util.newArrayListOfValues(
        ...
        private final List<SexSlot> position4 = Util.newArrayListOfValues(
        ...
        public Value<Boolean, String> isSlotUnlocked(GameCharacter characterToTakeSlot, SexSlot slot, Map<GameCharacter, SexSlot> positioningSlots) {
            ...
            if ((!positioningSlots.containsValue(SexSlotLyingDown.LYING_DOWN) || positioningSlots.get(characterToTakeSlot) == SexSlotLyingDown.LYING_DOWN)
            ...
            if ((!positioningSlots.containsValue(SexSlotLyingDown.LYING_DOWN_TWO) || positioningSlots.get(characterToTakeSlot) == SexSlotLyingDown.LYING_DOWN_TWO)
            ...
            if ((!positioningSlots.containsValue(SexSlotLyingDown.LYING_DOWN_THREE) || positioningSlots.get(characterToTakeSlot) == SexSlotLyingDown.LYING_DOWN_THREE)
            ...
            if ((!positioningSlots.containsValue(SexSlotLyingDown.LYING_DOWN_FOUR) || positioningSlots.get(characterToTakeSlot) == SexSlotLyingDown.LYING_DOWN_FOUR)
        ...
        public String getDescription(Map<GameCharacter, SexSlot> occupiedSlots) {
            ...
            if (besideNames.size() >= 2) {
                sb.append(Util.capitaliseSentence(Util.stringsToStringList(besideNames, false))).append(" are positioned off to the side, ready to receive oral or some non-penetrative sex. ");

            } else if (besideNames.size() == 1) {
                sb.append(Util.capitaliseSentence(Util.stringsToStringList(besideNames, false))).append(UtilText.parse(mainBeside,
...
//			}
        ...
        @Override
        public boolean isSelfOralAvailable(GameCharacter autoOralCharacter) {
            return !Main.sex.getSexPositionSlot(autoOralCharacter).hasTag(SexSlotTag.LAP_PILLOW)
...
//			}
        ...
        public String getDescription(Map<GameCharacter, SexSlot> occupiedSlots) {
            ...
            if (sittingNames.size() >= 2) {
                sb.append(Util.capitaliseSentence(Util.stringsToStringList(sittingNames, false))).append(" are sitting down next to one another, while ");
            ...
            if (sittingLapCount >= 2) {
                if (inLapNames.isEmpty()) {
                    sb.append(Util.stringsToStringList(inLapNamesTaur, false)).append(" have turned around and stepped backwards, before lowering their feral bodies down into ").append(sittingNames.size() > 1
                            ? (playerSitting ? "your laps. " : "their laps. ")
                            : UtilText.parse(mainSitting, "[npc.her] lap. "));

                } else {
                    sb.append(Util.stringsToStringList(allInLapNames, false)).append(" have stepped forwards so as to be sitting in ").append(sittingNames.size() > 1
                            ? (playerSitting ? "your laps. " : "their laps. ")
                            : UtilText.parse(mainSitting, "[npc.her] lap. "));
                    if (!inLapNamesTaur.isEmpty()) {
                        sb.append(" In order to do this, ").append(Util.capitaliseSentence(Util.stringsToStringList(inLapNamesTaur, false))).append(" have turned around and stepped backwards, allowing them to plant their feral bodies down against ").append(sittingNames.size() > 1
                ...
                sb.append(Util.stringsToStringList(inLapNames, false)).append(UtilText.parse(mainInLap, " [npc.has] stepped forwards so as to be sitting in ")).append(sittingNames.size() > 1
                        ? (playerSitting ? "your lap. " : "their lap. ")
                        : UtilText.parse(mainSitting, "[npc.her] lap. "));

            } else if (inLapNamesTaur.size() == 1) {
                sb.append(Util.stringsToStringList(inLapNamesTaur, false)).append(UtilText.parse(mainInLapTaur, " [npc.has] turned around and stepped backwards, before lowering [npc.her] feral [npc.legRace]'s body down into ")).append(sittingNames.size() > 1
                        ? (playerSitting ? "your lap. " : "their lap. ")
                        : UtilText.parse(mainSitting, "[npc.her] lap. "));
            } else {
            ...
            if (betweenLegsCount >= 2) {
                if (betweenLegsNames.isEmpty()) {
                    sb.append(sentenceContinuationFound ? Util.capitaliseSentence(Util.stringsToStringList(betweenLegsNamesTaur, false)) : Util.stringsToStringList(betweenLegsNamesTaur, false)).append(" have stepped forwards over the top of ").append(sittingNames.size() > 1
                            ? (playerSitting ? "you" : "them")
                            : UtilText.parse(mainSitting, "[npc.name]")).append(", bringing the groins of ").append(playerBetweenLegs ? "your" : "their").append(" animalistic lower bodies right up against ").append(sittingNames.size() > 1
                            ? (playerSitting ? "yours. " : "theirs. ")
                            : UtilText.parse(mainSitting, "[npc.hers]. "));

                } else {
                    sb.append(sentenceContinuationFound ? Util.capitaliseSentence(Util.stringsToStringList(betweenLegsNames, false)) : Util.stringsToStringList(betweenLegsNames, false)).append(" have stepped forwards so as to be standing in between ").append(sittingNames.size() > 1
                            ? (playerSitting ? "your legs" : "their legs")
                            : UtilText.parse(mainSitting, "[npc.namePos] [npc.legs]"));
                    if (!betweenLegsNamesTaur.isEmpty()) {
                        sb.append(", while ").append(Util.stringsToStringList(betweenLegsNamesTaur, false)).append(" ").append(betweenLegsNamesTaur.size() == 1 ? UtilText.parse(mainBetweenLegs, "[npc.has]") : "have").append(" stepped forwards over the top of ").append(sittingNames.size() > 1
                                ? (playerSitting ? "you" : "them")
                                : UtilText.parse(mainSitting, "[npc.name]")).append(betweenLegsNamesTaur.size() == 1
                                ? UtilText.parse(mainBetweenLegs, ", bringing the groin of [npc.her] animalistic [npc.legRace]'s body right up against ")
                                : ", bringing the groins of " + (playerBetweenLegs ? "your" : "their") + " animalistic lower bodies right up against").append(sittingNames.size() > 1
                                ? (playerSitting ? "yours. " : "theirs. ")
                                : UtilText.parse(mainSitting, "[npc.hers]. "));
                    } else {
                        sb.append(".");
                ...
                sb.append(sentenceContinuationFound ? Util.capitaliseSentence(Util.stringsToStringList(betweenLegsNames, false)) : Util.stringsToStringList(betweenLegsNames, false)).append(UtilText.parse(mainBetweenLegs, " [npc.has] stepped forwards so as to be standing in between ")).append(sittingNames.size() > 1
                        ? (playerSitting ? "your legs. " : "their legs. ")
                        : UtilText.parse(mainSitting, "[npc.her] [npc.legs]. "));
                sentenceContinuationFound = true;

            } else if (betweenLegsNamesTaur.size() == 1) {
                sb.append(sentenceContinuationFound ? Util.capitaliseSentence(Util.stringsToStringList(betweenLegsNamesTaur, false)) : Util.stringsToStringList(betweenLegsNamesTaur, false)).append(UtilText.parse(mainBetweenLegs, " [npc.has] stepped forwards over the top of ")).append(sittingNames.size() > 1
                        ? (playerSitting ? "you" : "them")
                        : UtilText.parse(mainSitting, "[npc.name]")).append(UtilText.parse(mainBetweenLegs, ", bringing the groin of [npc.her] animalistic [npc.legRace]'s body right up against ")).append(sittingNames.size() > 1
                        ? (playerSitting ? "yours. " : "theirs. ")
                        : UtilText.parse(mainSitting, "[npc.hers]. "));
                sentenceContinuationFound = true;
            }
            ...
            // Performing oral:
            if (performingOralNames.size() >= 2) {
                sb.append(sentenceContinuationFound ? Util.capitaliseSentence(Util.stringsToStringList(performingOralNames, false)) : Util.stringsToStringList(performingOralNames, false)).append(" are positioned so as to be down between ").append(sittingNames.size() > 1
                ...
                sb.append(sentenceContinuationFound ? Util.capitaliseSentence(Util.stringsToStringList(performingOralNames, false)) : Util.stringsToStringList(performingOralNames, false)).append(UtilText.parse(mainPerformingOral, " [npc.is] positioned so as to be right up between ")).append(sittingNames.size() > 1
                        ? (playerSitting ? "your legs" : "their legs") + ", ready to perform oral on " + (playerSitting ? "you. " : "them. ")
                        : UtilText.parse(mainSitting, "[npc.namePos] [npc.legs], ready to perform oral on [npc.herHim]. "));
                sentenceContinuationFound = true;
            }
            ...
            if (taurPresentingNames.size() >= 2) {
                sb.append(sentenceContinuationFound ? Util.capitaliseSentence(Util.stringsToStringList(taurPresentingNames, false)) : Util.stringsToStringList(taurPresentingNames, false)).append(" have turned around and stepped back in order to present their animalistic genitals to ").append(sittingNames.size() > 1
                ...
                sb.append(sentenceContinuationFound ? Util.capitaliseSentence(Util.stringsToStringList(taurPresentingNames, false)) : Util.stringsToStringList(taurPresentingNames, false)).append(UtilText.parse(mainTaurPresenting, " [npc.has] turned around and stepped back in order to present [npc.her] feral rear end to ")).append(sittingNames.size() > 1
                        ? (playerSitting ? "the " + Util.intToString(sittingNames.size()) + " of you." : "the " + Util.intToString(sittingNames.size()) + " of them.")
                        : UtilText.parse(mainSitting, "[npc.name]."));
            }
...
//				interactions.add(StandardSexActionInteractions.sittingBetweenLegs.getSexActionInteractions(SexSlotSitting.SITTING_FOUR, SexSlotSitting.SITTING_BETWEEN_LEGS_FOUR));
        ...
        @Override
        public boolean isSelfOralAvailable(GameCharacter autoOralCharacter) {
            return !Main.sex.getSexPositionSlot(autoOralCharacter).hasTag(SexSlotTag.SITTING_IN_LAP)
    ...
    public static final AbstractSexPosition BREEDING_STALL = new AbstractSexPosition("Breeding Stall",
        ...
        public boolean isActionBlocked(GameCharacter performer, GameCharacter target, SexActionInterface action) {
            ...
            if ((performerIsMother
                    ...
                    || ((action.getSexAreaInteractions().containsValue(SexAreaOrifice.VAGINA) || action.getSexAreaInteractions().containsValue(SexAreaPenetration.PENIS)) && action.getParticipantType() == SexParticipantType.SELF)))
                    || (Main.sex.getSexPositionSlot(performer) == SexSlotBreedingStall.BREEDING_STALL_FUCKING
                    && action.getSexAreaInteractions().containsValue(SexAreaPenetration.TAIL)
                    && action.getParticipantType() != SexParticipantType.SELF)) {
                return true;
            }
    ...
    public static final List<AbstractSexPosition> allSexPositions;

    public static final Map<AbstractSexPosition, String> sexPositionToIdMap = new HashMap<>();
    public static final Map<String, AbstractSexPosition> idToSexPositionMap = new HashMap<>();
