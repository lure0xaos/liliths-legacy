...
public class PositioningMenu {
    ...
    private static final StringBuilder positioningSB = new StringBuilder();
...
//			System.out.println(e.getKey().getName()+", "+e.getValue().getName(e.getKey()));
    ...
    private static String getDominantsDiv() {
        ...
        if (!Main.sex.getDominantParticipants(true).isEmpty()) {
    ...
    private static String getSubmissivesDiv() {
        ...
        sb.append("<div class='container-half-width'>");
        sb.append("<p style='text-align:center;'>");
        if (!Main.sex.getSubmissiveParticipants(true).isEmpty()) {
    ...
    private static String performAcceptableSlotsCheck() {
        ...
        for (Entry<GameCharacter, SexSlot> e : positioningSlots.entrySet()) {
            if (!e.getKey().equals(targetedCharacter) && !position.isSlotUnlocked(e.getKey(), e.getValue(), positioningSlots).getKey()) {
                ...
                sb.append("[style.colourBad(").append(UtilText.parse(e.getKey(),
                        "[npc.NamePos] selected position slot, '" + Util.capitaliseSentence(e.getValue().getDescription()) + "', is now unavailable, and as such, [npc.sheHasFull] been moved to a spectator slot!")).append(")]");
            }
        }
    ...
    public static final DialogueNode POSITIONING_MENU = new DialogueNode("Positioning Selection", "", true) {
        ...
        public String getContent() {
            ...
            if (!acceptablePosition.getKey()) {
                UtilText.nodeContentSB.append("<p style='text-align:center;'>" + "[style.boldBad(Invalid position)]<b>:</b><br/>" + "[style.italicsMinorBad(").append(acceptablePosition.getValue()).append(")]").append("</p>");

            } else {
                Value<Boolean, String> acceptableValue = isSlotsAcceptable();
                if (!acceptableValue.getKey()) {
                    UtilText.nodeContentSB.append("<p style='text-align:center;'>" + "[style.boldBad(Invalid position)]<b>:</b><br/>" + "[style.italicsMinorBad(").append(acceptableValue.getValue()).append(")]").append("</p>");
                } else {
                    UtilText.nodeContentSB.append("<p style='text-align:center;'>" + "<b>Projected outcome:</b><br/>" + "<i>").append(position.getDescription(positioningSlots)).append("</i>").append("</p>");
                }
            }
            ...
            if (index == 0) {
                ...
                return new ResponseEffectsOnly("Accept", "Move into the selected position.") {
                    ...
                    public void effects() {
                        ...
                        positioningSB.append("Wanting to switch into a new position, ").append(Main.sex.isMasturbation()
