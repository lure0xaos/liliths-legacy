...
import com.lilithslegacy.game.sex.managers.dominion.cultist.SMAltarMissionarySealed;
...
public class GenericActions {
    ...
    public static final SexAction HYPNOTIC_SUGGESTION_LUST_DECREASE = new SexAction(
        ...
        public String getDescription() {
            StringBuilder sb = new StringBuilder();
            sb.append("<p>" + "Wanting to take advantage of the fact that [npc2.nameIsFull] under the strong effect of a psychoactive substance, [npc.name] [npc.verb(lean)] towards [npc2.herHim] and [npc.moansVerb]," + " [npc.speech(You aren't really interested in having sex with me, are you?)]" + "</p>" + "<p>" + "[npc2.Name] can't help but agree with what [npc.sheIs] saying, and [npc2.name] haltingly [npc2.verb(answer)]," + " [npc2.speech(Yes... I... I don't know why I'm having sex with you...)]" + "</p>" + "<p>" + "Pushing a little further,").append(!Main.sex.getCharacterPerformingAction().isPlayer() ? " and driven on by [npc.her] fetish for having non-consensual sex," : "").append(" [npc.name] [npc.verb(continue)],").append(" [npc.speech(You'd rather I wasn't fucking you right now, isn't that right?)]").append("</p>");
        ...
        public String getDescription() {
            ...
            if (Main.sex.getCharacterOngoingSexArea(Main.sex.getCharacterTargetedForSexAction(this), SexAreaOrifice.ANUS) != null) {
                if (Main.sex.getCharacterOngoingSexArea(Main.sex.getCharacterTargetedForSexAction(this), SexAreaOrifice.ANUS).contains(Main.sex.getCharacterTargetedForSexAction(this))) {
                    if (!UtilText.nodeContentSB.isEmpty())
            ...
            UtilText.nodeContentSB.append("[npc.speech(I don't want to see you trying to get yourself off,)] you [npc.moanVerb] at [npc2.name].<br/><br/>" + "<i style='color:").append(PresetColour.GENERIC_ARCANE.toWebHexString()).append(";'>[npc2.Name] will no longer use any self-penetrative actions.</i>");
    ...
    public static final SexAction PLAYER_FORBID_PARTNER_CONTROL = new SexAction(
        ...
        public String getDescription() {
            ...
            UtilText.nodeContentSB.append("[npc.speech(You're not to do anything without being told,)] you order [npc2.name].<br/><br/>" + "<i style='color:").append(PresetColour.GENERIC_ARCANE.toWebHexString()).append(";'>[npc2.Name] now has a restricted level of control, and cannot initiate any non-self penetrative actions.</i>");
    ...
    public static final SexAction PLAYER_PERMIT_PARTNER_CONTROL = new SexAction(
        ...
        public String getDescription() {
            ...
            UtilText.nodeContentSB.append("[npc.speech(You can do what you like,)] you say to [npc2.name], freeing [npc2.herHim] to do as [npc2.she] pleases.<br/><br/>" + "<i style='color:").append(PresetColour.GENERIC_ARCANE.toWebHexString()).append(";'>[npc2.Name] now has an unrestricted level of control, and can initiate non-self penetrative actions at will.</i>");
    ...
    public static final SexAction PLAYER_FORBID_PARTNER_POSITIONING_SELF = new SexAction(
        ...
        public String getDescription() {
            ...
            UtilText.nodeContentSB.append("[npc.speech(I don't want to see you attempting to switch positions,)] you [npc.moanVerb] at [npc2.name].<br/><br/>" + "<i style='color:").append(PresetColour.GENERIC_ARCANE.toWebHexString()).append(";'>[npc2.Name] will no longer use any positioning actions.</i>");
...
//						isPenetratingSuitableOrifice = getCharacterToBeEgged().getIncubationLitter((SexAreaOrifice) areaContacted)==null;
        ...
        public void applyEffects() {
            ...
            Main.game.getTextEndStringBuilder().append("<p style='text-align:center;'>").append(UtilText.parse(getCharacterToBeEgged(),
                    "[style.italicsYellowLight([npc.Name] [npc.has] had " + Util.intToString(eggCount) + " egg" + (eggCount > 1 ? "s" : "") + " implanted in [npc.her] " + areaEgged + "!)]")).append("</p>");
...
//							+ "<b>[style.boldSexDom([npc.Name])] dominantly "+(Main.sex.isConsensual()?"having sex with":"fucking")+" [style.boldSexSub([npc2.name])]</b>"
                ...
                if (Main.sex.isInForeplay(dom)) {
                    ...
                    sb.append("[style.boldPurpleLight(Foreplay)] ([style.colourSexDom(").append(Util.capitaliseSentence(preference.getPerformingSexArea().getName(dom, true))).append(")]-[style.colourSexSub(").append(preference.getTargetedSexArea().getName(sub, true)).append(")]): ");
                ...
                for (int i = 0; i < orgamsNeeded; i++) {
                    ...
                    sb.append("<p style='margin:0; padding:0; text-align:center;'>");
                    sb.append("[style.boldPurple(Sex)] ([style.colourSexDom(").append(Util.capitaliseSentence(preference.getPerformingSexArea().getName(dom, true))).append(")]-[style.colourSexSub(").append(preference.getTargetedSexArea().getName(sub, true)).append(")]): ");
            ...
            if (characterOrgasming == target) {
                ...
                switch ((SexAreaOrifice) areaContacted) {
                    ...
                    case URETHRA_VAGINA:
                        break;
                    case ANUS:
                        sb.append(" in [npc2.her] own ass, [npc.name] [npc.verb(let)] out [npc.a_moan+] and [npc.verb(push)] [npc.her] ").append(penetrationAreaPlusText).append(" as deep as possible into [npc2.her] [npc2.asshole].");
                        sb.append(" Eagerly ").append(hipGrindText).append(", [npc.she] excitedly [npc.moansVerb] as [npc.she] [npc.verb(prepare)] to lay [npc.her] egg").append(eggCount != 1 ? "s" : "").append(".");
                        sb.append("</br>");
                        sb.append("With that, [npc2.name] suddenly [npc2.verb(feel)] the unmistakable bulbous lump of an egg travelling down the length of [npc.namePos] ").append(penetrationAreaText).append(",").append(" and [npc2.she] can't help but let out an alarmed [npc2.moan] as it squeezes out of the end").append(condomBreaks ? ", breaking [npc.namePos] condom in the process," : "").append(" and is safely laid deep in [npc2.her] stomach.");
                        if (eggCount > 1) {
                            sb.append(" Still with ").append(Util.intToString(eggCount - 1)).append(" egg").append(eggCount > 2 ? "s" : "").append(" left to lay, [npc.name] [npc.verb(keep)] [npc.her] ").append(penetrationAreaPlusText).append(" hilted deep in [npc2.her] own [npc2.asshole+]").append(" and [npc.verb(continue)] to let out a series of deeply satisfied [npc.moans] as [npc.she] [npc.verb(turn)] [npc2.her] stomach into an egg-incubation chamber.");
                        }
                        sb.append("</br>");
                        ...
                        break;
                    case MOUTH:
                        sb.append(" in [npc2.her] own stomach, [npc.name] [npc.verb(let)] out [npc.a_moan+] and [npc.verb(push)] [npc.her] ").append(penetrationAreaPlusText).append(" as deep as possible down [npc2.her] throat.");
                        sb.append(" Eagerly ").append(hipGrindText).append(", [npc.she] excitedly [npc.moansVerb] as [npc.she] [npc.verb(prepare)] to lay [npc.her] egg").append(eggCount != 1 ? "s" : "").append(".");
                        sb.append("</br>");
                        sb.append("With that, [npc2.name] suddenly [npc2.verb(feel)] the unmistakable bulbous lump of an egg travelling down the length of [npc.namePos] ").append(penetrationAreaText).append(",").append(" and [npc2.she] can't help but let out an alarmed, muffled [npc2.moan] as it squeezes out of the end").append(condomBreaks ? ", breaking [npc.namePos] condom in the process," : "").append(" and is safely laid deep in [npc2.her] stomach.");
                        if (eggCount > 1) {
                            sb.append(" Still with ").append(Util.intToString(eggCount - 1)).append(" egg").append(eggCount > 2 ? "s" : "").append(" left to lay, [npc.name] [npc.verb(keep)] [npc.her] ").append(penetrationAreaPlusText).append(" hilted deep down [npc2.her] own throat").append(" and [npc.verb(continue)] to let out a series of deeply satisfied [npc.moans] as [npc.she] [npc.verb(turn)] [npc2.her] stomach into an egg-incubation chamber.");
                        }
                        sb.append("</br>");
                        sb.append("With one last [npc.moan], [npc.name] [npc.verb(pull)] back, before lovingly rubbing [npc.her] [npc.hand] over [npc2.her] swollen belly and sighing,");
                        sb.append(" [npc.speech(That's better!)]");
                        break;
                    case NIPPLE:
                        sb.append(" in [npc2.her] own breasts, [npc.name] [npc.verb(let)] out [npc.a_moan+] and [npc.verb(push)] [npc.her] ").append(penetrationAreaPlusText).append(" as deep as possible into [npc2.her] [npc2.nipple(true)].");
                        sb.append(" Eagerly ").append(hipGrindText).append(", [npc.she] excitedly [npc.moansVerb] as [npc.she] [npc.verb(prepare)] to lay [npc.her] egg").append(eggCount != 1 ? "s" : "").append(".");
                        sb.append("</br>");
                        sb.append("With that, [npc2.name] suddenly [npc2.verb(feel)] the unmistakable bulbous lump of an egg travelling down the length of [npc.namePos] ").append(penetrationAreaText).append(",").append(" and [npc2.she] can't help but let out an alarmed [npc2.moan] as it squeezes out of the end").append(condomBreaks ? ", breaking [npc.namePos] condom in the process," : "").append(" and is safely laid deep within [npc2.her] fuckable breast.");
                        if (eggCount > 1) {
                            sb.append(" Still with ").append(Util.intToString(eggCount - 1)).append(" egg").append(eggCount > 2 ? "s" : "").append(" left to lay, [npc.name] [npc.verb(pull)] out and then immediately [npc.verb(penetrate)] [npc2.her] other [npc2.nipple(true)].").append(" Continuing to let out a series of deeply satisfied [npc.moans], [npc.she] quickly [npc.verb(turn)] [npc2.her] [npc2.breasts] into egg-incubation chambers.");
                        }
                        sb.append("</br>");
                        ...
                        break;
                    case NIPPLE_CROTCH:
                        sb.append(" in [npc2.her] own [npc2.crotchBoobs], [npc.name] [npc.verb(let)] out [npc.a_moan+] and [npc.verb(push)] [npc.her] ").append(penetrationAreaPlusText).append(" as deep as possible into [npc2.her] [npc2.nippleCrotch(true)].");
                        sb.append(" Eagerly ").append(hipGrindText).append(", [npc.she] excitedly [npc.moansVerb] as [npc.she] [npc.verb(prepare)] to lay [npc.her] egg").append(eggCount != 1 ? "s" : "").append(".");
                        sb.append("</br>");
                        sb.append("With that, [npc2.name] suddenly [npc2.verb(feel)] the unmistakable bulbous lump of an egg travelling down the length of [npc.namePos] ").append(penetrationAreaText).append(",").append(" and [npc2.she] can't help but let out an alarmed [npc2.moan] as it squeezes out of the end").append(condomBreaks ? ", breaking [npc.namePos] condom in the process," : "").append(" and is safely laid deep within [npc2.her] fuckable [npc2.crotchBoobs].");
                        if (eggCount > 1) {
                            sb.append(" Still with ").append(Util.intToString(eggCount - 1)).append(" egg").append(eggCount > 2 ? "s" : "").append(" left to lay, [npc.name] [npc.verb(pull)] out and then immediately [npc.verb(penetrate)] [npc2.her] other [npc2.nippleCrotch(true)].").append(" Continuing to let out a series of deeply satisfied [npc.moans], [npc.she] quickly [npc.verb(turn)] [npc2.her] [npc2.crotchBoobs] into egg-incubation chambers.");
                        }
                        sb.append("</br>");
                        ...
                        break;
                    case VAGINA:
                        sb.append(" in [npc2.her] own womb, [npc.name] [npc.verb(let)] out [npc.a_moan+] and [npc.verb(push)] [npc.her] ").append(penetrationAreaPlusText).append(" as deep as possible into [npc2.her] [npc2.pussy].");
                        sb.append(" Eagerly ").append(hipGrindText).append(", [npc.she] excitedly [npc.moansVerb] as [npc.she] [npc.verb(prepare)] to lay [npc.her] egg").append(eggCount != 1 ? "s" : "").append(".");
                        sb.append("</br>");
                        sb.append("With that, [npc2.name] suddenly [npc2.verb(feel)] the unmistakable bulbous lump of an egg travelling down the length of [npc.namePos] ").append(penetrationAreaText).append(",").append(" and [npc2.she] can't help but let out an alarmed [npc2.moan] as it squeezes out of the end").append(condomBreaks ? ", breaking [npc.namePos] condom in the process," : "").append(" and is safely laid deep in [npc2.her] womb.");
                        if (eggCount > 1) {
                            sb.append(" Still with ").append(Util.intToString(eggCount - 1)).append(" egg").append(eggCount > 2 ? "s" : "").append(" left to lay, [npc.name] [npc.verb(keep)] [npc.her] ").append(penetrationAreaPlusText).append(" hilted deep in [npc2.her] own [npc2.pussy+]").append(" and [npc.verb(continue)] to let out a series of deeply satisfied [npc.moans] as [npc.she] [npc.verb(turn)] [npc2.her] womb into an egg-incubation chamber.");
                        }
                        sb.append("</br>");
                ...
                switch ((SexAreaOrifice) areaContacted) {
                    ...
                    case URETHRA_VAGINA:
                        break;
                    case ANUS:
                        if (Objects.requireNonNull(Main.sex.getSexPace(characterOrgasming)) == SexPace.DOM_ROUGH) {
                        ...
                        sb.append("With that, [npc2.name] suddenly [npc2.verb(feel)] the unmistakable bulbous lump of an egg travelling down the length of [npc.namePos] ").append(penetrationAreaText).append(",").append(" and [npc2.she] can't help but let out an alarmed [npc2.moan] as it squeezes out of the end").append(condomBreaks ? ", breaking [npc.namePos] condom in the process," : "").append(" and is safely laid deep in [npc2.her] stomach.");
                        if (eggCount > 1) {
                            sb.append(" Still with ").append(Util.intToString(eggCount - 1)).append(" egg").append(eggCount > 2 ? "s" : "").append(" left to lay, [npc.name] [npc.verb(keep)] [npc.her] ").append(penetrationAreaPlusText).append(" hilted deep in [npc2.namePos] [npc2.asshole+]").append(" and [npc.verb(continue)] to let out a series of deeply satisfied [npc.moans] as [npc.she] [npc.verb(turn)] [npc2.her] stomach into an egg-incubation chamber.");
                        }
                        sb.append("</br>");
                        if (Objects.requireNonNull(Main.sex.getSexPace(characterOrgasming)) == SexPace.DOM_ROUGH) {
                    ...
                    case MOUTH:
                        ...
                        sb.append("With that, [npc2.name] suddenly [npc2.verb(feel)] the unmistakable bulbous lump of an egg travelling down the length of [npc.namePos] ").append(penetrationAreaText).append(",").append(" and [npc2.she] can't help but let out an alarmed, muffled [npc2.moan] as it squeezes out of the end").append(condomBreaks ? ", breaking [npc.namePos] condom in the process," : "").append(" and is safely laid deep in [npc2.her] stomach.");
                        if (eggCount > 1) {
                            sb.append(" Still with ").append(Util.intToString(eggCount - 1)).append(" egg").append(eggCount > 2 ? "s" : "").append(" left to lay, [npc.name] [npc.verb(keep)] [npc.her] ").append(penetrationAreaPlusText).append(" hilted deep down [npc2.namePos] throat").append(" and [npc.verb(continue)] to let out a series of deeply satisfied [npc.moans] as [npc.she] [npc.verb(turn)] [npc2.her] stomach into an egg-incubation chamber.");
                        }
                        sb.append("</br>");
                        if (Objects.requireNonNull(Main.sex.getSexPace(characterOrgasming)) == SexPace.DOM_ROUGH) {
                            sb.append("With one last [npc.moan], [npc.name] roughly [npc.verb(pull)] back, before dominantly rubbing [npc.her] [npc.hand] over [npc2.namePos] swollen belly and growling,");
                    ...
                    case NIPPLE:
                        ...
                        sb.append("With that, [npc2.name] suddenly [npc2.verb(feel)] the unmistakable bulbous lump of an egg travelling down the length of [npc.namePos] ").append(penetrationAreaText).append(",").append(" and [npc2.she] can't help but let out an alarmed [npc2.moan] as it squeezes out of the end").append(condomBreaks ? ", breaking [npc.namePos] condom in the process," : "").append(" and is safely laid deep within [npc2.her] fuckable breast.");
                        if (eggCount > 1) {
                            sb.append(" Still with ").append(Util.intToString(eggCount - 1)).append(" egg").append(eggCount > 2 ? "s" : "").append(" left to lay, [npc.name] [npc.verb(pull)] out and then immediately [npc.verb(penetrate)] [npc2.namePos] other [npc2.nipple(true)].").append(" Continuing to let out a series of deeply satisfied [npc.moans], [npc.she] quickly [npc.verb(turn)] [npc2.namePos] [npc2.breasts] into egg-incubation chambers.");
                        }
                        sb.append("</br>");
                        if (Objects.requireNonNull(Main.sex.getSexPace(characterOrgasming)) == SexPace.DOM_ROUGH) {
                    ...
                    case NIPPLE_CROTCH:
                        ...
                        sb.append("With that, [npc2.name] suddenly [npc2.verb(feel)] the unmistakable bulbous lump of an egg travelling down the length of [npc.namePos] ").append(penetrationAreaText).append(",").append(" and [npc2.she] can't help but let out an alarmed [npc2.moan] as it squeezes out of the end").append(condomBreaks ? ", breaking [npc.namePos] condom in the process," : "").append(" and is safely laid deep within [npc2.her] fuckable [npc2.crotchBoobs].");
                        if (eggCount > 1) {
                            sb.append(" Still with ").append(Util.intToString(eggCount - 1)).append(" egg").append(eggCount > 2 ? "s" : "").append(" left to lay, [npc.name] [npc.verb(pull)] out and then immediately [npc.verb(penetrate)] [npc2.namePos] other [npc2.nippleCrotch(true)].").append(" Continuing to let out a series of deeply satisfied [npc.moans], [npc.she] quickly [npc.verb(turn)] [npc2.namePos] [npc2.crotchBoobs] into egg-incubation chambers.");
                        }
                        sb.append("</br>");
                        if (Objects.requireNonNull(Main.sex.getSexPace(characterOrgasming)) == SexPace.DOM_ROUGH) {
                    ...
                    case VAGINA:
                        ...
                        sb.append("With that, [npc2.name] suddenly [npc2.verb(feel)] the unmistakable bulbous lump of an egg travelling down the length of [npc.namePos] ").append(penetrationAreaText).append(",").append(" and [npc2.she] can't help but let out an alarmed [npc2.moan] as it squeezes out of the end").append(condomBreaks ? ", breaking [npc.namePos] condom in the process," : "").append(" and is safely laid deep in [npc2.her] womb.");
                        if (eggCount > 1) {
                            sb.append(" Still with ").append(Util.intToString(eggCount - 1)).append(" egg").append(eggCount > 2 ? "s" : "").append(" left to lay, [npc.name] [npc.verb(keep)] [npc.her] ").append(penetrationAreaPlusText).append(" hilted deep in [npc2.namePos] [npc2.pussy+]").append(" and [npc.verb(continue)] to let out a series of deeply satisfied [npc.moans] as [npc.she] [npc.verb(turn)] [npc2.her] womb into an egg-incubation chamber.");
                        }
                        sb.append("</br>");
                        if (Objects.requireNonNull(Main.sex.getSexPace(characterOrgasming)) == SexPace.DOM_ROUGH) {
