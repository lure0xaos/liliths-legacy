...
import com.lilithslegacy.game.dialogue.utils.ParserTag;
...
import com.lilithslegacy.game.sex.managers.SexManagerInterface;
import com.lilithslegacy.game.sex.managers.SexManagerLoader;
import com.lilithslegacy.game.sex.positions.AbstractSexPosition;
import com.lilithslegacy.game.sex.positions.StandardSexActionInteractions;
...
import com.lilithslegacy.game.sex.sexActions.PositioningData;
...
import com.lilithslegacy.game.sex.sexActions.SexActionManager;
...
import com.lilithslegacy.game.sex.sexActions.baseActionsMisc.MiscActions;
...
public class Sex {
    ...
    private final SexManagerLoader sexManagerLoader;

    // Sex variables:
    private final SexActionManager sexActionManager;
    ...
    private final boolean displayOngoingPenetrationEffects = false;
    ...
    private static String getDirtyingAreasString(GameCharacter cumProvider, GameCharacter cumTarget, List<String> slotsDirtied, List<String> clothingDirtied) {
        ...
        if (!slotsDirtied.isEmpty() || !clothingDirtied.isEmpty()) {
            if (!slotsDirtied.isEmpty()) {
                dirtiedSlotsSB.append("[npc2.NamePos] ").append(Util.stringsToStringList(slotsDirtied, false));
                dirtiedSlotsSB.append(slotsDirtied.size() == 1 ? " is" : " are");
                dirtiedSlotsSB.append(" dirtied");
            }
            if (!clothingDirtied.isEmpty()) {
                if (!slotsDirtied.isEmpty()) {
                    dirtiedSlotsSB.append(", as ").append(clothingDirtied.size() == 1 ? "is" : "are").append(" [npc2.her] ");
                    ...
                    dirtiedSlotsSB.append(Util.stringsToStringList(clothingDirtied, false));
                    dirtiedSlotsSB.append(clothingDirtied.size() == 1 ? " is" : " are").append(" dirtied!");
                }
            } else {
                dirtiedSlotsSB.append("!");
            }
...
//							}
        ...
        sexSB.append("<p style='text-align:center;'><b>Starting Position:</b> <b style='color:").append(PresetColour.GENERIC_ARCANE.toWebHexString()).append(";'>").append(sexManager.getPosition().getName()).append("</b><br/>").append("<i><b>").append(sexManager.getPosition().getDescription(Main.sex.getAllOccupiedSlots(false))).append("</b></i></p>");
        ...
        for (GameCharacter character : Main.sex.getAllParticipants()) {
            if (sexManager.isCharacterStartNaked(character)
                ...
                if (anyClothingStripped) {
                    sexSB.append("<p style='text-align:center;'>").append(UtilText.parse(character, "[style.italicsSex(You quickly strip off all of [npc.namePos] clothes!)]")).append("</p>");
                }
            }
        }
        ...
        for (InitialSexActionInformation sexAction : startingSexActions) {
            if (sexAction.isConditionalMet()) {
                ...
                if (sexAction.isAppendDescription()) {
                    initialSexActionSB.append("<p>").append(sexAction.getSexAction().preDescriptionBaseEffects()).append(sexAction.getSexAction().getDescription()).append(sexAction.getSexAction().getFluidFlavourDescription(sexAction.getPerformer(), sexAction.getTarget())).append("</p>");
                }
...
//								+ "</p>");
                ...
                if (getHeavyLipstickUsedCharacter().contains(participant)) {
                    if (participant.hasItemType(ItemType.MAKEUP_SET)) {
                        endSexSB.append("<p style='text-align:center'><i>Your [style.italicsPinkDeep(heavy layer)] of lipstick has worn off, but you have ").append(ItemType.MAKEUP_SET.getName(true, false)).append(", so you take a few moments to [style.italicsGood(re-apply)] your [style.italicsPinkDeep(heavy layer)] of lipstick.</i></p>");
                    } else {
                ...
                if (getNumberOfOrgasms(participant) > 0
                    ...
                    if (participant.hasStatusEffect(StatusEffect.RECOVERING_AURA)) {
                        ...
                        endSexSB.append("<div class='container-full-width' style='text-align:center;'>" + "<i>Your arcane aura is still strengthened from a previous sexual encounter," + " so [style.italicsBad(you don't receive any essences)] from your [style.colourSex(").append(orgasmCount == 1 ? "single orgasm" : Util.intToString(orgasmCount) + " orgasms").append(")]!</i><br/>").append("</div>");
                        ...
                        if (!Main.game.getDialogueFlags().values.contains(DialogueFlagValue.essenceOrgasmDiscovered)) {
                            ...
                            if (!((PlayerCharacter) participant).isQuestCompleted(QuestLine.SIDE_ENCHANTMENT_DISCOVERY)) {
                                if (Main.sex.isMasturbation()) {
                                    endSexSB.append("<p><i>" + "As you stop masturbating, you suddenly become aware of a strange, shimmering pink glow that's started to materialise around your body," + " just like the one you saw in Lilaya's lab when she ran her tests on you the first time you met her." + " Quickly realising that you're somehow able to see your own arcane aura, you watch, fascinated, as it rapidly increases in luminosity." + " Just as you think that it can't get any brighter, your aura suddenly leaps back into your body, and you find yourself sharply inhaling as you feel it gaining strength." + "</i></p>" + "<p><i>" + "You think that it would probably be best to go and ask Lilaya about what just happened..." + "</i></p>").append(!((PlayerCharacter) participant).hasQuest(QuestLine.SIDE_ENCHANTMENT_DISCOVERY)
                                    ...
                                    endSexSB.append("<p><i>" + "As you stop watching the sex scene, you suddenly become aware of a strange, shimmering pink glow that's started to materialise around your body," + " just like the one you saw in Lilaya's lab when she ran her tests on you the first time you met her." + " Quickly realising that you're somehow able to see your own arcane aura, you watch, fascinated, as it rapidly increases in luminosity." + " Just as you think that it can't get any brighter, your aura suddenly leaps back into your body, and you find yourself sharply inhaling as you feel it gaining strength." + "</i></p>" + "<p><i>" + "You think that it would probably be best to go and ask Lilaya about what just happened..." + "</i></p>").append(!((PlayerCharacter) participant).hasQuest(QuestLine.SIDE_ENCHANTMENT_DISCOVERY)
                                            ? ((PlayerCharacter) participant).startQuest(QuestLine.SIDE_ENCHANTMENT_DISCOVERY)
                                            : "");
                                } else {
                                    endSexSB.append(
                        ...
                        endSexSB.append("<div class='container-full-width' style='text-align:center;'>" + "<i>Your aura absorbs the ").append(essences == 1 ? "arcane essence" : "arcane essences").append(" generated by your [style.colourSex(").append(orgasmCount == 1 ? "single orgasm" : Util.intToString(orgasmCount) + " orgasms").append(")]!</i><br/>").append(Main.game.getPlayer().incrementEssenceCount(essences, true)).append("</div>");
...
//								+ "</p>");
                ...
                if (getHeavyLipstickUsedCharacter().contains(participant)) {
                    if (participant.hasItemType(ItemType.MAKEUP_SET)) {
                        endSexSB.append("<p style='text-align:center'><i>[npc.NamePos] [style.italicsPinkDeep(heavy layer)] of lipstick has worn off, but [npc.she] [npc.has] ").append(ItemType.MAKEUP_SET.getName(true, false)).append(", so [npc.she] [npc.verb(take)] a few moments to [style.italicsGood(reapply)] [npc.her] [style.italicsPinkDeep(heavy layer)] of lipstick.</i></p>");
                    } else {
                        participant.removeHeavyMakeup(BodyCoveringType.MAKEUP_LIPSTICK);
                ...
                if (getNumberOfOrgasms(participant) > 0
                    ...
                    if (participant.hasStatusEffect(StatusEffect.RECOVERING_AURA)) {
                        ...
                        int orgasmCount = Main.sex.getNumberOfOrgasms(participant);
                        endSexSB.append("<div class='container-full-width' style='text-align:center;'>" + "<i>[npc.NamePos] arcane aura is still strengthened from a previous sexual encounter," + " so [style.italicsBad(you don't receive any essences)] from [npc.her] [style.colourSex(").append(orgasmCount == 1 ? "single orgasm" : Util.intToString(orgasmCount) + " orgasms").append(")]!</i><br/>").append("</div>");
                        ...
                        int orgasmCount = Main.sex.getNumberOfOrgasms(participant);
                        int essences = getEssenceGeneration(participant);
                        endSexSB.append("<div class='container-full-width' style='text-align:center;'>" + "<i>Your aura absorbs the ").append(essences == 1 ? "arcane essence" : "arcane essences").append(" generated by [npc.namePos] [style.colourSex(").append(orgasmCount == 1 ? "single orgasm" : Util.intToString(orgasmCount) + " orgasms").append(")]!</i><br/>").append(Main.game.getPlayer().incrementEssenceCount(essences, true)).append("</div>");
                    }
                    if (!participant.isDoll()) {
                        participant.addStatusEffect(StatusEffect.RECOVERING_AURA, (240 * 60) + (postSexDialogue.getSecondsPassed()));
                    }
        ...
        public Response getResponse(int responseTab, int index) {
            ...
            if (sexFinished) {
                ...
                if (responseTab == 0) {
                    ...
                    List<SexActionInterface> availableRepeatActionsPlayer = new LinkedList<>(repeatActionsPlayer);
...
//		System.out.println("startTurnPlayerArousal: "+startTurnPlayerArousal);
        ...
        sexSB.append(sexActionPlayer.preDescriptionBaseEffects()).append("<p>").append(sexActionPlayer.getDescription()).append("</p>").append(sexActionPlayer.getFluidFlavourDescription(Main.game.getPlayer(), Main.sex.getTargetedPartner(Main.game.getPlayer())));
                    ...
                    if (!character.isPlayer()) {
                        ...
                        if (sexActionPlayer.getActionType() != SexActionType.ORGASM && sexActionPlayer.getActionType() != SexActionType.ORGASM_DENIAL) {
                            ...
                            if (itemUseInformation == null || itemUseInformation.getKey().equals(character)) {
                                sexSB.append("<br/>" + "<p>" + "<span style='color:").append(PresetColour.TEXT_GREY.toWebHexString()).append(";'>&gt; ").append(UtilText.parse(character, "[npc.Name]")).append(": ").append(Util.capitaliseSentence(sexActionPartner.getActionTitle())).append("</span>").append("</br>").append(sexActionPartner.preDescriptionBaseEffects()).append(sexActionPartner.getDescription()).append(sexActionPartner.getFluidFlavourDescription(character, Main.sex.getTargetedPartner(character))).append("</p>");
        ...
        if (isReadyToOrgasm(Main.sex.getCharacterPerformingAction())) {
            ...
            if (SexFlags.playerPreparedForCharactersOrgasm.contains(Main.sex.getCharacterPerformingAction())
                ...
                for (SexActionInterface sexAction : Main.sex.getOrgasmActionsPartner(Main.sex.getCharacterPerformingAction(), targetedCharacter)) {
                    if (sexAction.isAddedToAvailableSexActions()) {
                        int weight = Main.sex.getCharacterPerformingAction().calculateSexTypeWeighting(sexAction.getAsSexType(), targetedCharacter, null);
        ...
        if (isReadyToOrgasm(Main.game.getPlayer()) && !Main.sex.isSpectator(Main.game.getPlayer())) { // Add orgasm reactions if ready to orgasm:
            ...
            for (SexActionInterface sexAction : Main.sex.getActionsAvailablePartner(Main.sex.getCharacterPerformingAction(), characterOrgasming)) { //TODO manually set target as characterOrgasming. Test more
                if (sexAction.getActionType() == SexActionType.PREPARE_FOR_PARTNER_ORGASM) {
                    if (sexAction.isAddedToAvailableSexActions()) {
                        int weight = Main.sex.getCharacterPerformingAction().calculateSexTypeWeighting(sexAction.getAsSexType(), characterOrgasming, null);
            ...
            if (actionsAvailableToPartner != null) {
                ...
                System.getLogger("").log(System.Logger.Level.ERROR, "Warning! Main.sex.getActionsAvailablePartner() is returning null for: "
        ...
        if (sexAction.getActionType() == SexActionType.ORGASM) {
                ...
                if (squirtAmount > 0) {
                    ...
                    if (vaginaClothing != null) {
                        if (vaginaClothing.isMilkingEquipment()
                            ...
                            stringBuilderForAppendingDescriptions.append("<p style='text-align:center; padding:0; margin:0;'>").append(UtilText.parse(Main.sex.getCharacterPerformingAction(), "[style.italicsGirlCum(" + Units.fluid(squirtAmount) + " of [npc.namePos] [npc.girlcum] is sucked down into the milking machine's storage vat!)]")).append("</p>");
                            ...
                            stringBuilderForAppendingDescriptions.append("[npc.NamePos] ").append(vaginaClothing.getName()).append(" ").append(vaginaClothing.getClothingType().isPlural() ? "are" : "is").append(" dirtied from [npc.her] squirting!");
            ...
            if ((Main.sex.getCharacterPerformingAction().hasPenis() && Main.sex.getCharacterPerformingAction().getPenisRawOrgasmCumQuantity() > 0)
                ...
                if ((Main.sex.getCharacterPerformingAction().hasPenis() && Main.sex.getCharacterPerformingAction().getPenisRawOrgasmCumQuantity() > 0)) {
                    ...
                    if (!dirtiedSlotsSB.isEmpty()) {
                        stringBuilderForAppendingDescriptions.append(dirtiedSlotsSB);
                    } else {
                        stringBuilderForAppendingDescriptions.append(")]</p>");
                    }
                }
...
//										}
        ...
        for (Entry<GameCharacter, SexPace> entry : initialPaces.entrySet()) {
            ...
            if (entry.getValue() != finalPace && !entry.getKey().isPlayer()) {
                stringBuilderForAppendingDescriptions.append("<p style='text-align:center;'>" + "<b style='color:").append(finalPace.getColour().toWebHexString()).append(";'>").append(Util.capitaliseSentence(finalPace.getName())).append(" Pace</b><br/>");
                switch (finalPace) {
                    case DOM_GENTLE:
                ...
                exposedSB.append(UtilText.parse(characterBeingExposed,
                        formatCoverableAreaBecomingExposed(
                                (atStartOfSex
                                        ...
                                        : "[npc.NamePos] [npc.asshole+] is now exposed!")))).append(sexManager.getAssRevealReaction(characterBeingExposed, charactersReacting, false)).append(formatCoverableAreaGettingWet(getLubricationDescription(characterBeingExposed, SexAreaOrifice.ANUS)));
                    ...
                    exposedSB.append(UtilText.parse(characterBeingExposed,
                            formatCoverableAreaBecomingExposed(
                                    (atStartOfSex
                                            ...
                                            : "[npc.NamePos] [npc.cock+] is now exposed!")))).append(sexManager.getPenisRevealReaction(characterBeingExposed, charactersReacting)).append(formatCoverableAreaGettingWet(getLubricationDescription(characterBeingExposed, SexAreaPenetration.PENIS)));
                }
                    ...
                    exposedSB.append(UtilText.parse(characterBeingExposed,
                            formatCoverableAreaBecomingExposed(
                                    (atStartOfSex
                                            ...
                                            : "[npc.NamePos] [npc.pussy+] is now exposed!")))).append(sexManager.getVaginaRevealReaction(characterBeingExposed, charactersReacting)).append(formatCoverableAreaGettingWet(getLubricationDescription(characterBeingExposed, SexAreaOrifice.VAGINA))).append(formatCoverableAreaGettingWet(getLubricationDescription(characterBeingExposed, SexAreaPenetration.CLIT)));
                    ...
                    exposedSB.append(UtilText.parse(characterBeingExposed,
                            formatCoverableAreaBecomingExposed(
                                    (atStartOfSex
                                            ...
                                            : "[npc.NamePos] doll-like mound is now exposed!")))).append(sexManager.getMoundRevealReaction(characterBeingExposed, charactersReacting));
                }
                ...
                exposedSB.append(UtilText.parse(characterBeingExposed,
                        formatCoverableAreaBecomingExposed(
                                (atStartOfSex
                                        ...
                                        : "[npc.NamePos] [npc.nipples+] are now exposed!")))).append(sexManager.getBreastsRevealReaction(characterBeingExposed, charactersReacting)).append(formatCoverableAreaGettingWet(getLubricationDescription(characterBeingExposed, SexAreaOrifice.NIPPLE)));
                ...
                exposedSB.append(UtilText.parse(characterBeingExposed,
                        formatCoverableAreaBecomingExposed(
                                (atStartOfSex
                                        ...
                                        : "[npc.NamePos] [npc.crotchBoobs+] are now exposed!")))).append(sexManager.getBreastsCrotchRevealReaction(characterBeingExposed, charactersReacting)).append(formatCoverableAreaGettingWet(getLubricationDescription(characterBeingExposed, SexAreaOrifice.NIPPLE_CROTCH)));
    ...
    private String getLubricationDescription(GameCharacter character, SexAreaInterface area) {
        ...
        return UtilText.parse(character, "[npc.NamePos] ") + area.getName(character) + " " + (area.isPlural() ? "are" : "is") + " lubricated with " + Util.stringsToStringList(lubes, false) + ".";
    }
    ...
    public void applyOngoingAction(GameCharacter characterPerformingAction, SexAreaInterface performerArea, GameCharacter characterTargeted, SexAreaInterface targetedArea, boolean freeUpSexAreas) {
        ...
        if (characterPerformingAction != null && characterTargeted != null) {
            ...
            System.getLogger("").log(System.Logger.Level.ERROR, "Warning! Main.sex.applyPenetration() is finding 'characterPenetrated' or 'characterPenetrating' to be null!!!");
        }
    }
                ...
                if (characterPenetrating == Main.sex.getCharacterPerformingAction()
                    ...
                    for (FluidStored fluid : fluids) {
                        try {
                            ...
                            penetrationSB.append("<p style='text-align:center; margin-bottom:0; padding-bottom:0; color:").append(PresetColour.BASE_ORANGE.toWebHexString()).append(";'><i>").append(UtilText.parse(characterPenetrating, characterPenetrated, "[npc.Name] [npc.verb(find)] [npc.herself] swallowing down some fluids previously deposited in [npc2.namePos] " + orifice.getName(characterPenetrated) + "!")).append("</i></p>").append(characterPenetrating.ingestFluid(
                                    fluidCharacter,
                                    fluid.getFluid(),
                                    SexAreaOrifice.MOUTH,
...
//					System.out.println(characterPenetrated.getName()+": "+characterPenetrated.getVaginaStretchedCapacity()+" | "+penisStretchSize);
    ...
    public SexActionInterface manageClothingToAccessCoverableArea(GameCharacter characterManagingClothing, GameCharacter targetForManagement, CoverableArea coverableArea) {
        ...
        if (clothingRemoval == null || clothingRemoval.getKey() == null) {
            ...
            System.getLogger("").log(System.Logger.Level.ERROR, "manageClothingToAccessCoverableArea() can't find clothing - CoverableArea." + coverableArea.toString());
            return SexActionUtility.CLOTHING_REMOVAL;
        }
    ...
    public void setTargetedPartner(GameCharacter targeter, GameCharacter targetedCharacter) {
        if (targetedCharacter != null && Main.sex.getSexPositionSlot(targetedCharacter) == SexSlotGeneric.MISC_WATCHING) {
            System.getLogger("").log(System.Logger.Level.ERROR, "Sex error: " + targeter.getName(true) + " attempted to target the spectator: " + targetedCharacter.getName(true));
...
//		System.out.println("Targeting: "+targeter.getName()+" "+targetedCharacter.getName());
    ...
    public boolean isCharacterEngagedInOngoingAction(GameCharacter character, GameCharacter target) {
        if (!Main.sex.getAllParticipants().contains(character)) {
            System.getLogger("").log(System.Logger.Level.ERROR, "isCharacterEngagedInOngoingAction(" + character.getId() + ") 1: This character is not in Sex!");
            return false;
        }
        if (!Main.sex.getAllParticipants().contains(target)) {
            System.getLogger("").log(System.Logger.Level.ERROR, "isCharacterEngagedInOngoingAction(" + target.getId() + ") 1: This character is not in Sex!");
            return false;
        }
    ...
    public boolean isCharacterEngagedInOngoingAction(GameCharacter character) {
        if (!Main.sex.getAllParticipants().contains(character)) {
            System.getLogger("").log(System.Logger.Level.ERROR, "isCharacterEngagedInOngoingAction(" + character.getId() + ") 2: This character is not in Sex!");
            return false;
        }
...
//				}
    ...
    public void setSexManager(SexManagerInterface sexManager, List<GameCharacter> dominantSpectators, List<GameCharacter> submissiveSpectators) {
        ...
        sexSB.append("<p style='text-align:center;'><b>New position:</b> <b style='color:").append(PresetColour.GENERIC_ARCANE.toWebHexString()).append(";'>").append(Main.sex.sexManager.getPosition().getName()).append("</b><br/>").append("<i><b>").append(Main.sex.sexManager.getPosition().getDescription(Main.sex.getAllOccupiedSlots(false))).append("</b></i></p>");
    }
    ...
    private void addSexAction(GameCharacter character, GameCharacter target, SexActionInteractions interactions, SexActionInterface action, boolean addedForCharacter, boolean addedForTarget) {
        if (!addedForCharacter) {
            if (action.getSexAreaInteractions().isEmpty()
                    || action.getSexAreaInteractions().containsKey(null)
                    || action.getSexAreaInteractions().containsValue(null)) {
...
//		Main.sex.resetAllOngoingActions(false);
    ...
    public boolean isDom(GameCharacter character) {
        try {
            ...
            return sexManager.getDominants().containsKey(character);
        }
    }
    ...
    public int getTimesCummedInside(GameCharacter character, GameCharacter target) {
        ...
        return cummedInsideMap.get(character).get(target).values().stream().mapToInt(Integer::intValue).sum();
    }
