...
import com.lilithslegacy.game.dialogue.companions.OccupantSortingMethod;
    ...
    static void fluidHandler(MilkingRoom room, FluidStored fluid) {
                ...
                if (canIngest) {
                    ((EventTarget) MainController.document.getElementById(id)).addEventListener("click", e -> {
                        ...
                        if (!ingestion.isEmpty()) {
                            Main.game.getTextEndStringBuilder().append("</p>" + "<p>").append(ingestion);
                        }
                        Main.game.getTextEndStringBuilder().append("</p>");
                        Main.game.getTextEndStringBuilder().append("<p style='text-align:center;'>" + "<i style='color:").append(PresetColour.GENERIC_MINOR_BAD.toWebHexString()).append(";'>").append(Units.fluid(milkAmount)).append(" of ").append(fluidOwnerName).append(" ").append(fluidName).append(" has been consumed!</i>").append("</p>");
        ...
        if (MainController.document.getElementById(id) != null) {
            ((EventTarget) MainController.document.getElementById(id)).addEventListener("click", e -> {
                ...
                Main.game.getTextEndStringBuilder().append("<p style='text-align:center;'>You sold the ").append(fluidName).append(" for ").append(UtilText.formatAsMoney(income, "span")).append("!</p>");
                Main.game.setContent(new Response("", "", Main.game.getCurrentDialogueNode()));
            }, false);
            MainController.addTooltipListeners(id,
