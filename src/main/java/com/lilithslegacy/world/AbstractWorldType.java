...
public abstract class AbstractWorldType {
    ...
    public AbstractWorldType(Path XMLFile, String author, boolean mod) {
        if (Files.exists(XMLFile)) {
            try {
                ...
                this.fileLocation = ("worldType.xml".equals(XMLFile.getFileName().toString()))? XMLFile.resolveSibling("map.png") : XMLFile;
                ...
                for (Element e : coreElement.getMandatoryFirstOf("places").getAllOf("place")) {
                    try {
                        ...
                        System.getLogger("").log(System.Logger.Level.ERROR, "WorldType loading error in '" + XMLFile.getFileName().toString() + "': PlaceType '" + e.getTextContent() + "' not recognised! (Not added)");
                    }
                }
    ...
    public boolean equals(Object o) { // Just placesMap and fileLocation should be enough to check for equality.
        if (super.equals(o)) {
            if (o instanceof AbstractWorldType) {
                return ((AbstractWorldType) o).getPlacesMap().equals(getPlacesMap())
...
//		throw new IllegalAccessError();
        System.getLogger("").log(System.Logger.Level.ERROR, "Warning: AbstractWorldType's toString() method is being called!");
